...
Once you have an `oci_image`, hand it to [`sonic_docker_archive`](/tools/bazel/oci/sonic_docker_archive.bzl) to get the `target/<name>.gz` archive the Make build consumes:
...
Call it once per image, including the debug image. See [Packaging images into `target/docker-sysmgr.gz`](/tools/bazel/docs/patterns-detail.md#packaging-images-into-targetdocker-sysmgrgz-for-make-interoperability) for what it generates.
...
Every `.bazelrc` that registers `@gcc_toolchains//:all` (the root of buildimage, and each first-party `src/` module's own `.bazelrc`) sets this as the default target platform:
...
In this section, we discuss patterns to adapt the OSS SONiC Bazel build to your internal environments.

### Airgapping Dependencies
...
See more details [here](/tools/bazel/docs/patterns-detail.md#airgapping-the-bazel-build).

### Airgapping Bazel Itself

In addition to the above, we may want to airgap Bazel itself.
It is fetched by bazelisk before Bazel starts, so the downloader config does not apply.
Instead, use `BAZELISK_BASE_URL` to configure where to fetch Bazel from.
Under normal operations, it is fetched from `releases.bazel.build/`, and from `github.com/bazelbuild/bazel/releases/` for forks and rolling releases.

### Remote Caching

Thanks to Bazel's hermeticity, we can safely share Bazel's output across developers in an organization.
This is called remote caching.

To take advantage of remote caching, the steps are:
- Choose and stand up a cache backend.
- Configure Bazel to read and write to that backend as required.

See more details [here](/tools/bazel/docs/patterns-detail.md#remote-caching).

### Patching SONiC Components
