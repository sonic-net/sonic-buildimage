...
#
# Secrets are read from stdin as KEY=VALUE lines rather than passed on argv. Both lines are required, in any order:
#   SED_DEFAULT_PW=<platform default SED password, read from TPM bank 3 by the caller>
#   SED_PSID=<PSID, used for the crypto erase>
#
# Exit codes:
#     0  success
#     1  aborted before erase (ramdisk load or pivot failed; syslog names the phase)
#     2  bad / missing args, or secrets absent from stdin
#     3  required tool or helper library missing (rsync, sedutil-cli, nvme, tpm2_*)
#     4  insufficient RAM (< TMPFS_MIN_AVAIL_GB available)
...
# pipefail only: each phase runs as `phase || exit $?`, a context where bash
# ignores errexit anyway, and -u would break the guards in sed_pw_utils.sh.
set -o pipefail

TMPFS_SIZE="${SSD_ERASE_TMPFS_SIZE:-4G}"
TMPFS_MIN_AVAIL_GB=4

# NVMe sanitize polling (see NVMe Base Spec §5.24).
...
usage() {
    echo "Usage: $0 -a <tpm_bank_a> -b <tpm_bank_b>"
    echo "       SED_DEFAULT_PW=<pw> and SED_PSID=<psid> are read from stdin, not argv."
    exit 2
}

tpm_reg=""
tpm_reg_2=""
# Filled by read_secrets() from stdin.
default_pw=""
psid_val=""
...
# Check utils script is available
if ! source /usr/local/bin/sed_pw_utils.sh; then
    echo "$0: cannot load /usr/local/bin/sed_pw_utils.sh" >&2
    exit 3
fi
...
# Consume the KEY=VALUE secret lines the caller piped in (see header), then detach stdin
# so nothing downstream can inherit the pipe or block on it.
read_secrets() {
    if [ -t 0 ]; then
        log_error "Secrets must be piped on stdin; refusing to read them from a terminal"
        return 2
    fi
    local line
    while IFS= read -r line || [ -n "$line" ]; do
        case "$line" in
            SED_DEFAULT_PW=*) default_pw=${line#SED_DEFAULT_PW=} ;;
            SED_PSID=*)       psid_val=${line#SED_PSID=} ;;
        esac
    done
    exec 0</dev/null
    if [ -z "$default_pw" ] || [ -z "$psid_val" ]; then
        log_error "stdin must supply both SED_DEFAULT_PW and SED_PSID"
        return 2
    fi
    log_info "Read SED secrets from stdin"
    return 0
}
...
pivot_to_ramfs() {
    ...
    for m in dev proc sys run; do
        ...
        mkdir -p "/$m"
        if [ -e "/oldroot/$m" ]; then
            mount --move "/oldroot/$m" "/$m" || { log_error "mount --move /oldroot/$m failed"; return 1; }
        fi
    done
    mkdir -p /home
    return 0
