#!/bin/bash
#
# sonie-discovery.sh - Emulate SONIE discovery behavior
#
...
readonly LOGFILE="/var/log/sonie-discovery.log"
readonly INSTALLER_PATH="/tmp/sonie-installer"
...
#######################################
# Import SONIE variables from /proc/cmdline.
# Globals:
#   SONIE_PLATFORM
#   SONIE_MACHINE
#   SONIE_ARCH
...
import_cmdline() {
  ...
  for x in $(cat /proc/cmdline); do
    case "$x" in
      ...
      boot_reason=*)
        SONIE_BOOT_REASON="${x#*=}"
        ;;
      sonie_dhcp_user_class=*)
        sonie_dhcp_user_class="${x#*=}"
        ;;
    esac
  done
  ...
  if [[ -f /host/machine.conf ]]; then
    ...
    . /host/machine.conf
    [[ -z "$SONIE_PLATFORM" ]] && SONIE_PLATFORM="$sonie_platform"
    [[ -z "$SONIE_MACHINE" ]] && SONIE_MACHINE="$sonie_machine"
    [[ -z "$SONIE_ARCH" ]] && SONIE_ARCH="$sonie_arch"
  fi
}
...
get_default_filenames() {
  local prefix="sonie-installer"
  if [[ "$SONIE_BOOT_REASON" == "update" ]]; then
    prefix="sonie-updater"
  fi

  # Default list based on SONIE logic
  # We keep "sonie-" prefix for compatibility with existing installer naming conventions
  echo "${prefix}-${SONIE_PLATFORM}"
  echo "${prefix}-${SONIE_ARCH}-${SONIE_MACHINE}"
  echo "${prefix}-${SONIE_MACHINE}"
  echo "${prefix}-${SONIE_ARCH}"
  echo "${prefix}"

  # Add .bin suffixes
  echo "${prefix}-${SONIE_PLATFORM}.bin"
  echo "${prefix}-${SONIE_ARCH}-${SONIE_MACHINE}.bin"
  echo "${prefix}-${SONIE_MACHINE}.bin"
  echo "${prefix}-${SONIE_ARCH}.bin"
  echo "${prefix}.bin"
}
...
#######################################
discover_static() {
  if [[ -n "$SONIE_INSTALL_URL" ]]; then
    url_run "$SONIE_INSTALL_URL" && return 0
  fi
  return 1
}
...
discover_dhcp() {
  ...
  local lease_file="/tmp/sonie-dhcp-lease.sh"
  rm -f "$lease_file"

  local sonie_iana_enterprise=42623
  # Default to SSONIE if not set
  local sonie_dhcp_user_class="${sonie_dhcp_user_class:-SONIE}"

  # Client ID: 00 + serial (Option 61)
  local client_id_hex="00$(str2hex "${sonie_serial_num:-0}")"

  # User Class: Option 77
  local user_class_hex
  user_class_hex="$(str2lenhex "$sonie_dhcp_user_class")"
  ...
  local vivso_arch
  local vivso_rev
  vivso_machine="$(make_str_opt 3 "${SONIE_MACHINE}")"
  vivso_arch="$(make_str_opt 4 "${SONIE_ARCH}")"
  vivso_rev="$(make_str_opt 5 "${sonie_machine_rev:-0}")"
  ...
  try_dhclient() {
      ...
      local cf="/etc/dhcp/dhclient_sonie.conf"
      mkdir -p /etc/dhcp
  ...
  try_dhclient6() {
      ...
      local cf="/etc/dhcp/dhclient6_sonie.conf"
      mkdir -p /etc/dhcp
# Prepare User Class Hex (RFC 8415: Len(2 bytes) + Data)
      local uc_hex_str
      uc_hex_str=$(str2lenhex16 "$sonie_dhcp_user_class" | sed 's/../&:/g;s/:$//')
...
request dhcp6.name-servers, dhcp6.domain-search, dhcp6.bootfile-url, dhcp6.ia-na;
...
EOF
      ...
      # Wait for the IPv6 address to become non-tentative (DAD to complete)
      if [[ $ret -eq 0 ]]; then
          log "Waiting for IPv6 address on $iface to graduate from 'tentative' state..."
          local wait_count=0
          while ip -6 addr show dev "$iface" | grep -q "tentative" && [[ $wait_count -lt 10 ]]; do
              sleep 1
              wait_count=$((wait_count + 1))
          done
          if [[ $wait_count -ge 10 ]]; then
              log "Warning: IPv6 address still tentative after ${wait_count}s"
          fi
      fi
      ...
      if [[ $ret -eq 0 ]]; then
         ...
         if ip -6 addr show dev "$iface" | grep -q "global"; then
             ...
             ip -6 addr show dev "$iface" > /dev/ttyS0
             ip -6 route show > /dev/ttyS0
...
discover_waterfall() {
  ...
  if [[ -f "/tmp/sonie-dhcp-lease.sh" ]]; then
    # shellcheck source=/dev/null
    . "/tmp/sonie-dhcp-lease.sh"
    local server="${DHCP_TFTP:-$DHCP_SERVER}"
    if [[ -n "$server" ]]; then
...
main() {
  ...
  {
      echo "DEBUG: sonie-discovery.sh STARTED"
  ...
  log "Starting SONIE discovery service..."

  import_cmdline

  # Check if we are in recovery mode
  if [[ "$SONIE_BOOT_REASON" == "recovery" ]]; then
    log "SONIE_BOOT_REASON is recovery. Exiting discovery service."
    exit 0
  fi

  # Only run if explicitly in SONIE mode or forced (optional)
  # SONIE usually runs this loop indefinitely until success
  while true; do
