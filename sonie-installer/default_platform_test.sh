...
test_install_grub_to_esp_onie() {
  ...
  # set install env to Onie.
  local install_env=onie
...
#   0 on success, exits on failure
#######################################
test_install_grub_to_esp_sonie() {
  echo "test_install_grub_to_esp..."
  ...
  # set install env to SONIE.
  local install_env=sonie
  ...
  # Verify grub-install called
  if ! echo "$output" | grep -q "MOCK_GRUB_INSTALL: --target=x86_64-efi --efi-directory=$mock_esp --boot-directory=$mock_os --bootloader-id=SONIE-TEST --recheck --modules=part_gpt part_msdos --disable-shim-lock /dev/sda"; then
    echo "FAIL: grub-install not called with expected args. Output: $output"
    rm -rf "$mock_fs_root"
    exit 1
  fi
...
main() {
  if [ -f "${SCRIPT_DIR}/utils.sh" ]; then
    ...
    echo "Required library utils.sh is missing."
    exit 1
  fi
