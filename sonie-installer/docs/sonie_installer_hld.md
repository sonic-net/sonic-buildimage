...
- **Bootloader**: The installer supports both GRUB and systemd-boot. For GRUB, `bootloader_state_machine.grub` is enhanced to recognize the `sonie` environment and skip aggressive cleanup. For systemd-boot, the `systemd-installer` utility configures loader entry specifications to boot the golden recovery image directly.
...
*   **Partition 1 (ESP)**: FAT32, type `EF00`. Stores GRUB or systemd-boot bootloader EFI binaries and configurations.
...
flowchart TD
    ...
    InstallBootloader -- "Yes (Not Build)" --> BootloaderChoice{Which Bootloader?}
    BootloaderChoice -- grub --> ESP_GRUB[Install GRUB to ESP]
    BootloaderChoice -- systemd-boot --> ESP_SDBOOT[Install systemd-boot to ESP]
...
- **Directory/File Conditions**:
    ...
    - `default_platform.conf`: Sourced for platform-specific functions (e.g., `install_systemd_to_esp`).
    - `bootloader_state_machine.grub`: Sourced and appended to `grub.cfg` (for GRUB).
    - `/loader/entries/00-golden-sonie.conf`: Configured to boot the golden recovery UKI (for systemd-boot).
...
### 7.5.2 Systemd-Boot Configuration and Failover
Unlike GRUB's dynamic shell scripting state machine, `systemd-boot` achieves failover and boot selection using static loader definitions and built-in entry counting:
- **Default Entry**: Configured in `/loader/loader.conf` via the `default` keyword. The `systemd-installer` python utility updates this dynamically during OS installation/update cycles.
- **Automatic Boot Counting**: Entries use file naming conventions like `<slot-entry>.conf+3` to specify remaining boot attempts. The `systemd-boot` bootloader automatically decrements the `+N` count on boot selection. If the count reaches `+0`, the entry is skipped, and it falls back to the next sorted entry (ultimately booting the golden recovery image `00-golden-sonie.conf`).
- **Unified Kernel Images (UKI)**: Payloads are placed in the ESP/XBOOTLDR structures, allowing systemd-boot to execute them directly as standalone signed EFI binaries.
