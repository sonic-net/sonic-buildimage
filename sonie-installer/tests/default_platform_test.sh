...
test_create_partition_sonie() {
  ...
  output=$(
    ...
    export -f find_device_from_partdev blkid create_sonie_uefi_partition mkfs.vfat mount
    ...
    # Mock ensure_esp_partition
    ensure_esp_partition() { echo "ensure_esp_partition called with $1"; }
    export -f ensure_esp_partition
  ...
  if [[ -d "/sys/firmware/efi/efivars" ]]; then
     if echo "$output" | grep -q "CALLED_create_sonie_uefi_partition"; then
        if echo "$output" | grep -q "ensure_esp_partition called with /dev/sda"; then
            echo "PASS"
        else
            echo "FAIL: ensure_esp_partition not called. Output: $output"
            exit 1
        fi
     else
       echo "FAIL: $output"
...
#######################################
# Verifies ensure_esp_partition logic.
# Case 1: ESP exists -> Do nothing.
# Case 2: ESP missing -> Create it.
# Globals:
#   DEFAULT_PLATFORM_CONF
# Arguments:
#   None
...
#   0 on success, exits on failure
#######################################
test_ensure_esp_partition() {
  echo "test_ensure_esp_partition..."
  ...
  output=$(
    ...
    ensure_esp_partition "/dev/sda"
  )
  if [[ "$output" != *"ESP found at partition 1"* ]]; then
    echo "FAIL: Case 1 (ESP exists) failed. Output: $output"
    exit 1
  fi
  ...
  output=$(
    ...
    sgdisk() {
        ...
        if [[ "$cmd" == "-p" ]]; then
             # No EF00
             echo "2 4096 8192 100MB 8300 Linux"
        elif [[ "$cmd" == "-n" ]]; then
             echo "Creating partition 1..."
        fi
    }
    partprobe_safe() { echo "partprobe $*"; }
    export -f sgdisk partprobe_safe make_partition_dev

    # Mock mkfs.vfat executable
    mkdir -p /tmp/mock_bin
    echo '#!/bin/sh' > /tmp/mock_bin/mkfs.vfat
    echo 'echo "mkfs.vfat $*"' >> /tmp/mock_bin/mkfs.vfat
    chmod +x /tmp/mock_bin/mkfs.vfat
  ...
  if [[ "$output" != *"ESP not found"* ]]; then echo "FAIL Case 2: Missed ESP not found check. Output: $output"; exit 1; fi
  if [[ "$output" != *"Creating partition 1..."* ]]; then echo "FAIL Case 2: Did not create partition. Output: $output"; exit 1; fi
  if [[ "$output" != *"mkfs.vfat -F 32 -n ESP /dev/sda1"* ]]; then echo "FAIL Case 2: Did not format ESP. Output: $output"; exit 1; fi
...
test_create_partition_sonic() {
  ...
  output=$(
    ...
    export -f blkid find_device_from_partdev create_sonie_uefi_partition onie-sysinfo
...
test_create_sonie_uefi_partition_cleanup() {
  ...
  output=$(
    ...
    export -f create_sonie_gpt_partition
  ...
  for entry in "0001:SONIE-OS" "0002:ACS-OS" "0003:SONiC-OS" "0005:ONIE"; do
    id="${entry%%:*}"
    label="${entry##*:}"
    if ! echo "$log_content" | grep -q "DELETING_BOOT_ENTRY $id"; then
      echo "FAIL: Missed deletion of Boot${id} (${label})"
      exit 1
    fi
  done

  # Check expected preservation
  if echo "$log_content" | grep -q "DELETING_BOOT_ENTRY 0004"; then
    echo "FAIL: Incorrectly deleted Boot0004 (Other-OS)"
    exit 1
  fi
