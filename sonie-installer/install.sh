...
detect_environment() {
    ...
    # Extract sonic_asic_platform from kernel cmdline if present
    if [ -z "${sonic_asic_platform:-}" ]; then
        sonic_asic_platform="$(cat /proc/cmdline | sed -n 's/^.*sonic_asic_platform=\(\S\+\).*$/\1/p')"
        if [ -n "${sonic_asic_platform}" ]; then
            log_info "Detected sonic_asic_platform from cmdline: ${sonic_asic_platform}"
...
main() {
    ...
    local sonie_efi="${demo_mnt}/sonie-${sonic_asic_platform:-$onie_platform}.efi"
    if [ ! -f "${sonie_efi}" ]; then
        # Try to find any .efi file if exact name differs
        local efi_found
        efi_found=$(find "${demo_mnt}" -maxdepth 1 -name "*.efi" | head -n 1)
        if [ -z "${efi_found}" ]; then
             log_error "SONIE payload verification failed: sonie-${sonic_asic_platform:-$onie_platform}.efi not found in ${demo_mnt}"
             ls -l "${demo_mnt}" >&2
        ...
        log_info "Verified payload: ${demo_mnt}/sonie-${sonic_asic_platform:-$onie_platform}.efi"
    fi
    ...
    # Ensure ESP is mounted at /efi for subsequent installers
    local esp_dev
    esp_dev="$(make_partition_dev "${blk_dev}" 1)"
    mkdir -p /efi
    if ! mountpoint -q /efi; then
        log_info "Mounting ${esp_dev} to /efi"
        mount "${esp_dev}" /efi || log_warn "Failed to mount ${esp_dev} to /efi"
    fi
