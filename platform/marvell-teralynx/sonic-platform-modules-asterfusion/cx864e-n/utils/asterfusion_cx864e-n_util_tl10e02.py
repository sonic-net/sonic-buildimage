...
def check_pca954x_bus():
    global I2C_SWITCH_LIST
    cpld = {}
    cmd = "i2cdetect -r -y {} | grep '70'"
    for bus in ['1', '2', '3', '4', '5']:
        result = subprocess.getoutput(cmd.format(bus)).split(':')[1]
        if '70' in result:
            cpld[1] = "i2c-" + bus
        elif '72' in result:
            cpld[2] = "i2c-" + bus
        elif '75' in result and '74' not in result:
            cpld[3] = "i2c-" + bus
    for pca954x in I2C_SWITCH_LIST.keys():
        if I2C_SWITCH_LIST[pca954x]["parent"] == "i2c-5":
            I2C_SWITCH_LIST[pca954x]["parent"] = cpld[1]
        elif I2C_SWITCH_LIST[pca954x]["parent"] == "i2c-3":
            I2C_SWITCH_LIST[pca954x]["parent"] = cpld[2]
        elif I2C_SWITCH_LIST[pca954x]["parent"] == "i2c-4":
            I2C_SWITCH_LIST[pca954x]["parent"] = cpld[3] 
...
def set_come_status():
    cpld = {}
    cmd = "i2cdetect -r -y {} | grep '40'"
    for bus in ['1', '2', '3', '4', '5']:
        result = subprocess.getoutput(cmd.format(bus)).split(':')[1]
        if '40' in result:
            subprocess.getoutput('i2cset -f -y {} 0x40 0x5 0x1'.format(bus))
            return
