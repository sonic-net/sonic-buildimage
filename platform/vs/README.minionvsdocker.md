HOWTO Use Minion Virtual Switch (Docker)
...
2. Start sonic minion virtual switch docker

```
$ docker run --privileged -v /sys/fs/cgroup:/sys/fs/cgroup:rw --network container:sw -d --name minion docker-sonic-minion-vs
```

3. Setup IP in the minion virtual switch docker
