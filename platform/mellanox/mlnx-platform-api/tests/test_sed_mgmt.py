...
class TestSedMgmt:
    ...
    @mock.patch('sonic_platform.sed_mgmt.utils.read_key_value_file')
    @mock.patch('sonic_platform_base.sed_mgmt_base._read_sed_config_value')
    def test_wipe_ssd_end_to_end(self, mock_cfg, mock_vpd, mock_run):
        ...
        assert call_args[call_args.index('-a') + 1] == '0x81010001'
        assert call_args[call_args.index('-b') + 1] == '0x81010002'
        # PSID and default password travel on stdin; argv is world-readable.
        assert 'MT_0000000123' not in call_args
        assert 'default_secret' not in call_args
        assert 'SED_PSID=MT_0000000123' in kwargs['input']
        assert 'SED_DEFAULT_PW=default_secret' in kwargs['input']
