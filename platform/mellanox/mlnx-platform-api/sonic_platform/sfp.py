...
try:
    ...
    from . import utils
    from .db_table_helper import db_table_helper
...
class SFP(NvidiaSFPCommon):
    ...
    def __init__(self, sfp_index, sfp_type=None, slot_id=0, linecard_port_count=0, lc_name=None, asic_id='asic0'):
        ...
        self.sn = None
        self.namespace = "" # Default namespace, should be updated when support multi-namespace
    ...
    def get_temperature_from_db(self):
        ...
        """
        present, value = self._get_data_from_db(self.namespace, db_table_helper.get_temperature_table, 'temperature')
        if not present:
            return 0
    ...
    def get_warning_threshold_from_db(self):
        present, value = self._get_data_from_db(self.namespace, db_table_helper.get_threshold_table, 'temphighwarning')
        # xcvrd returns N/A if threshold is not supported
        # xcvrd cannot tell read failure or not supppored,
    ...
    def get_critical_threshold_from_db(self):
        present, value = self._get_data_from_db(self.namespace, db_table_helper.get_threshold_table, 'temphighalarm')
        # xcvrd returns N/A if threshold is not supported
        # xcvrd cannot tell read failure or not supppored,
        # so we return 0 in both cases
        if not present or value == 'N/A':
    ...
    def get_vendor_name_from_db(self):
        present, value = self._get_data_from_db(self.namespace, db_table_helper.get_info_table, 'manufacturer')
        if not present:
            return ''
        return value.strip()

    def get_part_number_from_db(self):
        present, value = self._get_data_from_db(self.namespace, db_table_helper.get_info_table, 'model')
        if not present:
            return ''
        return value.strip()

    def _get_data_from_db(self, namespace, table_cb, key):
        logical_port = self.get_logical_port()
        if not logical_port:
            return False, None
        return table_cb(namespace).hget(logical_port, key)
