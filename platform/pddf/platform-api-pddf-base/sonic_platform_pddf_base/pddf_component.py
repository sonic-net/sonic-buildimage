...
class PddfComponent(ComponentBase, DeviceBase):
    ...
    def get_firmware_version(self):
        ...
        result = subprocess.run(version_cmd, shell=True, capture_output=True)
    ...
    def _install_firmware(self, image_path):
        ...
        result = subprocess.run(update_cmd.format(image_path), shell=True)
        if result.returncode != 0:
    ...
    def get_model(self):
        ...
        result = subprocess.run(model_cmd, shell=True, capture_output=True)
        if result.returncode != 0:
            return "Unknown"
    ...
    def get_serial(self):
        ...
        result = subprocess.run(serial_cmd, shell=True, capture_output=True)
        if result.returncode != 0:
            return "Unknown"
    ...
    def get_revision(self):
        ...
        result = subprocess.run(revision_cmd, shell=True, capture_output=True)
        if result.returncode != 0:
            return "Unknown"
