			...
			const char *buf, size_t count)
{
	...
	/* Deleting an adapter that was never added walks an uninitialised
	 * i2c_adapter into i2c_del_adapter(), which oopses in its debugfs
	 * teardown. The detach path guards the same call this way.
	 */
	if (!i2c_privdata->i2c_adapter_registered[index]) {
		pddf_dbg(MULTIFPGA,
			 KERN_INFO "[%s] bus index %d is not registered\n",
			 __FUNCTION__, index);
		return count;
	}
