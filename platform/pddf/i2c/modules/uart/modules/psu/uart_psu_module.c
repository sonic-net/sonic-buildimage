...
extern void *get_uart_psu_access_data(char *);
...
PDDF_DATA_ATTR(attr_cmd, S_IWUSR|S_IRUGO, show_pddf_data, store_pddf_data, PDDF_UINT32, sizeof(uint32_t), (void*)&psu_data.psu_attr.cmd, NULL);
PDDF_DATA_ATTR(attr_subcmd1, S_IWUSR|S_IRUGO, show_pddf_data, store_pddf_data, PDDF_UINT32, sizeof(uint32_t), (void*)&psu_data.psu_attr.subcmd1, NULL);
PDDF_DATA_ATTR(attr_subcmd2, S_IWUSR|S_IRUGO, show_pddf_data, store_pddf_data, PDDF_UINT32, sizeof(uint32_t), (void*)&psu_data.psu_attr.subcmd2, NULL);
PDDF_DATA_ATTR(attr_byte, S_IWUSR|S_IRUGO, show_pddf_data, store_pddf_data, PDDF_INT_DEC, sizeof(int), (void*)&psu_data.psu_attr.byte, NULL);
PDDF_DATA_ATTR(attr_calmode, S_IWUSR|S_IRUGO, show_pddf_data, store_pddf_data, PDDF_INT_DEC, sizeof(int), (void*)&psu_data.psu_attr.calmode, NULL);
...
static ssize_t do_attr_operation(struct device *dev, struct device_attribute *da, const char *buf, __kernel_size_t count)
{
    ...
    access_ptr = get_uart_psu_access_data(pdata->psu_attrs[pdata->len].aname);
...
struct i2c_board_info *i2c_get_psu_board_info(PSU_DATA *pdata, NEW_DEV_ATTR *cdata)
{
    ...
    if (strcmp(cdata->dev_type, "psu_uart")==0 || strcmp(cdata->dev_type, "psu_eeprom")==0 )
    {
...
static ssize_t do_device_operation(struct device *dev, struct device_attribute *da, const char *buf, __kernel_size_t count)
{
    ...
    if (strncmp(buf, "add", strlen(buf)-1)==0)
    {
        ...
        if(!IS_ERR(client_ptr))
        {
            ...
            pddf_dbg("UART_PSU", KERN_ERR "Created a %s client: 0x%p\n", cdata->i2c_name , (void *)client_ptr);
    ...
    else if (strncmp(buf, "delete", strlen(buf)-1)==0)
    {
        ...
        if (client_ptr)
        {
            pddf_dbg("UART_PSU", KERN_ERR "Removing %s client: 0x%p\n", cdata->i2c_name, (void *)client_ptr);
...
static struct kobject *uart_psu_kobj;
...
int __init pddf_data_init(void)
{
    ...
    pddf_dbg("UART_PSU", "PDDF_DATA MODULE.. init\n");

    device_kobj = get_device_i2c_kobj();
    if(!device_kobj) 
        return -ENOMEM;

    uart_psu_kobj = kobject_create_and_add("uart_psu", device_kobj);
    if(!uart_psu_kobj) 
        return -ENOMEM;
    psu_kobj = kobject_create_and_add("psu", uart_psu_kobj);
    if(!psu_kobj) 
        return -ENOMEM;
    
    ret = sysfs_create_group(psu_kobj, &pddf_clients_data_group);
    if (ret)
    {
        kobject_put(psu_kobj);
        kobject_put(uart_psu_kobj);
        return ret;
    }
    pddf_dbg("UART_PSU", "CREATED PSU I2C CLIENTS CREATION SYSFS GROUP\n");

    ret = sysfs_create_group(psu_kobj, &pddf_psu_client_data_group);
    if (ret)
    {
        sysfs_remove_group(psu_kobj, &pddf_clients_data_group);
        kobject_put(psu_kobj);
        kobject_put(uart_psu_kobj);
        return ret;
    }
    pddf_dbg("UART_PSU", "CREATED PDDF PSU DATA SYSFS GROUP\n");
...
void __exit pddf_data_exit(void)
{
    ...
    pddf_dbg("UART_PSU", "PDDF_DATA MODULE.. exit\n");
    sysfs_remove_group(psu_kobj, &pddf_psu_client_data_group);
    sysfs_remove_group(psu_kobj, &pddf_clients_data_group);
    kobject_put(psu_kobj);
    kobject_put(uart_psu_kobj);
    pddf_dbg("UART_PSU", KERN_ERR "%s: Removed the kobjects for 'uart' and 'psu'\n",__FUNCTION__);
