...
VERSION_UPDATE_TABLE_NAME = 'PLATFORM|INVENTORY'
...
REDIS_CHASSIS_SERVER_IP = '0.0.0.0'
CHASSIS_STATE_DB = 13
LOCALHOST = "127.0.0.1"
EVENT_PORT = 11360
PDS_PORT = 11357
...
class VersionUpdater():
        ...
        try:
            if self.db:
                ...
                if len(sonic_version_info_table) == 1:
                    ...
                    value = self.db.get(table).split('\n')
                    value_without_version = '\n'.join([i for i in value if not ('available' in i or 'current' in i or 'version' in i)])
                    self.db.set(table, value_without_version)
        except Exception as e:
    ...
    def _fetch_version_info(self):
        try:
            ...
            available_images = [img.split('-')[-1] for img in images]
            curimage = curimage.split('-')[-1]
            available_images.remove(curimage)
            available_image = available_images[0]
            ...
            version_info_list = [curimage, available_image]
    ...
    def _refresh_sonic_version_db(self, stat):
        try:
            ...
            [current, available, mainfwa_sw, mainfwa_uboot, goldfw_sw, goldfw_uboot] = stat
            value = self.db.get(self.table)
            inventory_value = ""
            if value != None:
                inventory_value = value
            inventory_value += f"\ndpu-sonic-version-current: {current}\ndpu-sonic-version-available: {available}"
            inventory_value += f"\nmainfwa-sw-version: {mainfwa_sw}\nmainfwa-uboot-version: {mainfwa_uboot}"
            inventory_value += f"\ngoldfw-sw-version: {goldfw_sw}\ngoldfw-uboot-version: {goldfw_uboot}"
...
def main():
    ...
    dpudbd.log_info("Shutting down DpuDBUtilDaemon")
