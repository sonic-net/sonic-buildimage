...
from public.platform_common_config import PLATFORM_DEBUG_INIT_PATH, HOST_MACHINE, CONFIG_FILE_PATH, ONIE_PLATFORM_KEY
...
def get_sonic_platform_info():
    ...
    if machine_info is not None:
        if ONIE_PLATFORM_KEY in machine_info:
            platform = machine_info[ONIE_PLATFORM_KEY].replace("-","_").lower()
            if len(platform) == 0:
...
boardid_configfile = (board_id + "_base_config")  # board_id
