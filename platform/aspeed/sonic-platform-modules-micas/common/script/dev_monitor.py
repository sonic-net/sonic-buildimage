...
from platform_util import AliasedGroup, CONTEXT_SETTINGS, get_value, set_value, setup_logger, BSP_COMMON_LOG_DIR, waitForDocker
...
class DevMonitor():
    ...
    def __init__(self):
        self.max_retry_time = DEV_MONITOR_PARAM.get("retry_time", 10)
    ...
    def removeDev(self, bus, loc):
        delete_device_path = "/sys/bus/i2c/devices/i2c-%d/delete_device" % bus
        devpath = "/sys/bus/i2c/devices/%d-%04x" % (bus, loc)
        if os.path.exists(devpath):
            with open(delete_device_path, "w") as fd:
                fd.write("0x%02x" % loc)

    def addDev(self, name, bus, loc):
        if name == "lm75":
            time.sleep(0.1)
        new_device_path = "/sys/bus/i2c/devices/i2c-%d/new_device" % bus
        devpath = "/sys/bus/i2c/devices/%d-%04x" % (bus, loc)
        if os.path.exists(devpath) is False:
            with open(new_device_path, "w") as fd:
                fd.write("%s 0x%02x" % (name, loc))
    ...
    def monitor(self, ret):
        ...
        for item in ret:
            try:
                name = item.get('name')
                driver_name = item.get('driver_name', name)
                ...
                for item_dev in devices:
                    ...
                    item_devattr_retry = '%s_retry' % item_devattr
                    val_t = getattr(DevMonitor, item_devattr, None)
                    val_retry = getattr(DevMonitor, item_devattr_retry, 0)
                    if val_t == 'OK':
                        continue
                    ...
                    if self.checkattr(bus, loc, attr) is False:
                        if val_retry >= self.max_retry_time:
                            deverror("%s exceeded maximum retries number %d, skip" % (item_devattr, self.max_retry_time))
                            continue
                        err_t -= 1
                        ...
                        if present is not None:
                            ...
                            if presentstatus.get('status') == 'PRESENT':
                                self.register_device_again(devname, bus, loc, driver_name, driver_type)
                                setattr(DevMonitor, item_devattr_retry, val_retry + 1)
                        else:
                            self.register_device_again(devname, bus, loc, driver_name, driver_type)
                            setattr(DevMonitor, item_devattr_retry, val_retry + 1)
                    else:
    ...
    def monitor_config_check(self):
        try:
            device_loc_list = []
            device_id_list = []
            device_name_list = []
            for name, config in DEV_MONITOR_PARAM.items():
                if not isinstance(config, list):
                    continue
                for item in config:
                    device_name = item.get('name')
                    if device_name in device_name_list:
                        deverror("monitor config: name %s duplicate " % device_name)
                        return False
                    else:
                    ...
                    devices = item.get('device')
                    for item_dev in devices:
                        device_id = '%s' % (item_dev['id'])
                        device_loc = '%s-%s' % (item_dev['bus'], item_dev['loc'])
                        if device_id in device_id_list:
                            deverror("monitor config: %s id duplicate " % item_dev)
                            return False
                        elif device_loc in device_loc_list:
                            deverror("monitor config: %s bus and loc duplicate " % item_dev)
                            return False
                        else:
                            device_id_list.append(device_id)
                            device_loc_list.append(device_loc)
        except Exception as e:
            deverror("monitor config check error")
            deverror(traceback.format_exc())
            return False
        devdebuglog("monitor config check pass")
        return True
...
def start():
    ...
    ret = devMonitor.monitor_config_check()
    if ret is False:
        deverror("dev_monitor config check fail!")
        time.sleep(5)
        sys.exit(1)
