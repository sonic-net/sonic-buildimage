...
import click
import logging
from platform_util import AliasedGroup, CONTEXT_SETTINGS, get_value, set_value, exec_os_cmd, exec_os_cmd_log, setup_logger, BSP_COMMON_LOG_DIR
from platform_config import WARM_UPGRADE_PARAM
from public.platform_common_config import FIRMWARE_UPGRADE_CMD
...
def subprocess_warm_upgrade(file, main_type, sub_type, slot, file_type, chain):
    command = FIRMWARE_UPGRADE_CMD % (file, main_type, sub_type, slot, file_type, chain)
...
class RefreshUpgradeBase(object):
    ...
    def save_value(self, cfg_list):
        for config in cfg_list:
            ...
            if ret:
                config["value"] = val
                warmupgradedebuglog("save_value, get_value success, config: %s" % config)
            else:
                warmupgradeerror("save_value, get_value failed, errmsg: %s " % val)
                return False, val
    ...
    def save_and_set_value(self, cfg_list):
        for config in cfg_list:
            ...
            if ret:
                ...
                warmupgradedebuglog("save_and_set_value, get_value success, config: %s" % config)
            else:
                warmupgradeerror("save_and_set_value, get_value failed, errmsg: %s " % val)
                return False, "get save value fail"
            set_val = config.get("set_value", None)
            if set_val is not None:
                config["value"] = set_val
            else:
                warmupgradeerror("save_and_set_value lack of set_val config: %s" % config)
                return False, "set value is not config"
            ret, log = set_value(config)
            if ret is False:
                warmupgradeerror("save_and_set_value, set_value failed, errmsg: %s" % log)
                return False, log
            warmupgradedebuglog("save_and_set_value, set_value success, config: %s" % config)
        return True, "save value success"
    ...
    def recover_value(self, cfg_list):
        ...
        for config in cfg_list:
            ...
            if ret is False:
                ...
                warmupgradedebuglog("recover_value, set_value success, config: %s" % config)
        if fail_flag != 0:
    ...
    def recover_save_value(self, cfg_list):
        ...
        for config in cfg_list:
            ...
            if val is None:
                warmupgradeerror("recover_save_value lack of save_value config: %s" % config)
                total_err -= 1
                continue
            config["value"] = val
            ret, log = set_value(config)
            if ret is False:
                total_err -= 1
                warmupgradeerror("recover_save_value, set_value failed, errmsg: %s" % log)
            else:
                warmupgradedebuglog("recover_save_value, set_value success, config: %s" % config)
        if total_err < 0:
        ...
        try:
            for init_cmd_config in init_cmd_list:
                ...
                if ret is False:
                    ...
                    return False, log
                warmupgradedebuglog("%s do init cmd: %s success" % (self.device_name, init_cmd_config))
    ...
    def do_fw_upg_finish_cmd(self, finish_cmd_list):
        ...
        for finish_cmd_config in finish_cmd_list:
            ...
            if ret_t is False:
                ...
                warmupgradedebuglog("%s do finish cmd: %s success" % (self.device_name, finish_cmd_config))
        if total_err < 0:
    ...
    def refresh_file_upgrade(self):
        try:
            warmupgradedebuglog("start %s warm upgrading, slot: %d, devtype: 0x%x, subtype: 0x%x, filetype: %s, chain: %d" %
                (self.device_name, self._slot_num, self._devtype, self._subtype, self.file_type, self.chain))
...
class WarmBasePlatform():
    ...
    def __init__(self):
        ...
        self.global_init_cmd = self.warm_upgrade_param.get("global_init_cmd", [])
        self.global_finish_cmd = self.warm_upgrade_param.get("global_finish_cmd", [])
    ...
    def do_warmupgrade(self, file, main_type, sub_type, slot, file_type, chain, specify_refresh_config=None):
        try:
            # get slot config
            ...
            self.__warm_upgrade_config_list = []
            if specify_refresh_config is None: # traverse all refresh config
                warmupgradedebuglog("specify_refresh_config is None, traverse all refresh config")
                for refresh_config in chain_list:
                    ...
                    if refresh_file_judge_flag == 1:
                        ...
                        if refresh_file is None or not os.path.isfile(refresh_file):
                ...
                warmupgradedebuglog("specify_refresh_config is not None")
                refresh_file_judge_flag = specify_refresh_config.get("refresh_file_judge_flag", 0)
                if refresh_file_judge_flag == 1:
                    refresh_file = specify_refresh_config.get("refresh_file", None)
                    if refresh_file is None or not os.path.isfile(refresh_file):
                        log = "%s not found" % refresh_file
                        return False, log
                # each refresh_config add as an instance of RefreshUpgrade Class
                refresh_instance = RefreshUpgrade(specify_refresh_config, slot, main_type, sub_type, file_type, chain)
                self.__warm_upgrade_config_list.append(refresh_instance)
