...
from platform_util import read_sysfs, allow_syslog
...
debuglevel = 0
STATUS_OK = 0
DEVICE_NAME = "/sys/bus/i2c/devices/%s/device_name"
...
XDPE_SUPPORTED_DEVICES = ["XDPE12284C", "XDPE132G5C", "XDPE132G5D", "XDPE1A2G5B"]
...
class Device():
    ...
    def dfx_info_check(self):
        ...
        try:
            ...
            device_name_loc = DEVICE_NAME % self.bus_addr
            ret, vr_device_name = read_sysfs(device_name_loc)
            if ret is False:
                monitor_syslog_error("dev: %s, loc:%s, get device_name failed." % (self.dev_name, device_name_loc))
                return
            if not vr_device_name:
                monitor_syslog_debug("dev: %s vr_device_name is empty, skip monitoring." % self.dev_name)
                return
            if vr_device_name not in XDPE_SUPPORTED_DEVICES:
                monitor_syslog_debug("dev: %s[%s] is not in supported list, skip monitoring." % (self.dev_name, vr_device_name))
                return
            ...
            for i in self.page_range:
                ...
                if self.last_status.get(i, []) != page_i_status:
                    ...
                    if self.last_status.get(i, []) == [STATUS_OK, STATUS_OK]:
                        monitor_syslog('dev: %s, page%d, loc:%s, status error detected. \n (byte status, word_status) are (%s).\nDFX INFO:\n%s' % \
                                       (self.dev_name, i, loc, ", ".join(str(hex(x)) for x in page_i_status), dfx_info))
                    # fail->ok.
                    elif page_i_status  == [STATUS_OK, STATUS_OK]:
                        monitor_syslog('dev: %s, page%d, loc:%s, status error recovered. \n (byte status, word_status) are (%s).\n%s' % \
                                       (self.dev_name, i, loc, ", ".join(str(hex(x)) for x in page_i_status), dfx_info))
                    # fail->fail. value changed.
                    else:
                        s = 'dev: %s, page%d, loc:%s, status error changed and value changed. \n (byte status, word_status) are (%s) changed to (%s). \n'
                        s += '\nDFX INFO Before:\n%s'
                        s += '\nDFX INFO After:\n%s'
                        
                        monitor_syslog(s % (self.dev_name, i, loc, ", ".join(str(hex(x)) for x in self.last_status.get(i, [])), ", ".join(str(hex(x)) for x in page_i_status), self.last_dfx_info[i], dfx_info))

                    self.last_status[i] = page_i_status
                    self.last_dfx_info[i] = dfx_info
                else:
                    monitor_syslog_debug('dev: %s, page%d, loc:%s, status Not changed. \n (byte status, word_status) are (%s).' % \
                                   (self.dev_name, i, loc, ", ".join(str(hex(x)) for x in page_i_status)))
