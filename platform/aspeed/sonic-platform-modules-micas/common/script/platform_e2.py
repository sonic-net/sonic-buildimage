...
from platform_util import AliasedGroup, CONTEXT_SETTINGS, byteTostr, dev_file_read, read_sysfs, exec_os_cmd, set_value
...
REFRESH_E2_CMD = "docker exec pmon supervisorctl restart syseepromd"
...
def fru_eeprom_show(e2_name, eeprom, e2_decode=None):
    try:
        ...
        print("=================chassis=================")
        print(ipmi_fru.chassisInfoArea)
        return True, ""
    except Exception as e:
        return False, str(e)
...
def generate_fru_value(fru_dict):
    ...
    try:
        ...
        chassisispresent = fru_dict.get("chassisInfoArea_ispresent", 0)
        if boradispresent == 0 and productispresent == 0 and chassisispresent == 0:
            print("fru config error, boradispresent == 0 and productispresent == 0 and chassisispresent == 0")
            return
        ...
        if chassisispresent != 0:
            chassisinfoarea_conf = fru_dict.get("chassisInfoArea")
            if chassisinfoarea_conf is None:
                print("fru config error, chassisinfoarea ispresent but chassisinfoarea_conf config is None")
                return

            cia = ChassisInfoArea(name="Chassis Info Area ", size=0)
            cia.isPresent = True
            cia.chassisType = int(chassisinfoarea_conf["Type"], 16)
            cia.chassisPartNumber = chassisinfoarea_conf["PartNumber"]
            cia.chassisSerialNumber = chassisinfoarea_conf["SerialNumber"]
...
def change_onie_tlv_dict_value(raw_dict, field, value):
    ...
    if field == onietlv.TLV_CODE_MAC_BASE:
        ret, mac_addr = check_mac_addr(value)
        if not ret:
            print(mac_addr)
            return False
        raw_dict[field] = mac_addr
    elif field == onietlv.TLV_CODE_MAC_SIZE:
        if value.isdigit():
            raw_dict[field] = int(value, 16)
        else:
            print("Version number is non-numeric, please check")
            return False
    elif field == onietlv.TLV_CODE_SERIAL_NUMBER:
        if value.isalnum() == False:
            print("Invalid serial number.please check carefully")
            return False
        elif len(value) < 13:
            print("Invalid serial number length.please check carefully")
            return False
        else:
            raw_dict[field] = value
    elif field == onietlv.TLV_CODE_PRODUCT_NAME:
        if len(value.strip()) == 0:
            print("Invalid product name.please check carefully")
            return False
        raw_dict[field] = value
    else:
        raw_dict[field] = value
...
def get_tlv_raw_dict(e2_path):
    onietlv = ot.onie_tlv()
    ret, binval = read_sysfs(e2_path)
    if ret == False:
        log = "eeprom read error, eeprom path: %s, msg: %s" % (e2_path, binval)
        return False, log

    tlv_decode_val = onietlv.decode(binval)
    tlv_raw_dict = {}
    for item in tlv_decode_val:
        if item["code"] == onietlv.TLV_CODE_CRC_32 or item["code"] == onietlv.TLV_CODE_WB_CARID:
            continue
        elif item["code"] == onietlv.TLV_CODE_MAC_SIZE:
            tlv_raw_dict[item["code"]] = int(item["value"])
        else:
            tlv_raw_dict[item["code"]] = item["value"]

    return True, tlv_raw_dict

def change_onie_tlv_e2_value(e2_path, field, value):
    onietlv = ot.onie_tlv()

    ret, tlv_raw_dict = get_tlv_raw_dict(e2_path)
    if ret is False:
        print(tlv_raw_dict)
        return False

    if int(field) == E2_FIELD.MAC_BASE.value:
        ret = change_onie_tlv_dict_value(tlv_raw_dict, onietlv.TLV_CODE_MAC_BASE, value)
        field_name = E2_FIELD.MAC_BASE.name
    elif int(field) == E2_FIELD.SERIAL_NUMBER.value:
        ret = change_onie_tlv_dict_value(tlv_raw_dict, onietlv.TLV_CODE_SERIAL_NUMBER, value)
        field_name = E2_FIELD.SERIAL_NUMBER.name
    elif int(field) == E2_FIELD.PRODUCT_NAME.value:
        ret = change_onie_tlv_dict_value(tlv_raw_dict, onietlv.TLV_CODE_PRODUCT_NAME, value)
        field_name = E2_FIELD.PRODUCT_NAME.name
    else:
        print("unsupport choice: %s" % field)
        return False

    if ret is False:
        print("change e2 field: %s fail" % field_name)
        return False

    changed_tlv_value, _ = onietlv.generate_value(tlv_raw_dict)
    write_rawdata_to_file(changed_tlv_value, e2_path)
    print("change tlv e2 success")
    return True

def set_syseeprom_value(choice, value, index):
    syseeprom_conf = PLATFORM_E2_CONF.get("syseeprom", [])
    syseeprom_num = len(syseeprom_conf)
    if syseeprom_num == 0:
        print("syseeprom number is 0, can't get syseeprom info")
    ...
    if not index.isdigit():
        print("index error, %s is not digital" % index)
        return

    syseeprom_index = int(index, 10) - 1
    if syseeprom_index < 0 or syseeprom_index >= syseeprom_num:
        print("index error, total syseeprom number: %d, syseeprom index: %d" % (syseeprom_num, syseeprom_index + 1))
        return

    e2_type = syseeprom_conf[syseeprom_index].get("e2_type")
    e2_path = syseeprom_conf[syseeprom_index].get("e2_path")

    if e2_type == "onie_tlv":
        ret = change_onie_tlv_e2_value(e2_path, choice, value)
    else:
        print("unsupport e2_type: %s" % e2_type)
        return

    if ret is True:
        print("refresh e2 cache, please wait")
        ret, log = exec_os_cmd(REFRESH_E2_CMD)
        if ret:
            print("refresh e2 fail, reason: %s" % log)
        else:
            print("refresh e2 cache success")

    return

def generate_extend(ext_dict):
    ret = ""
    iana = ext_dict.get("iana")
    if iana is not None:
        if isinstance(iana, str):
...
def set_tlv_e2_extend_field(e2_path, code, value):
    onietlv = ot.onie_tlv()
    code = int(code, 16)
    ...
    extend = tlv_raw_dict.get(onietlv.TLV_CODE_VENDOR_EXT)
    if extend is None:
        print("not found extend field")
        return False

    extend_decode_val = onietlv.decode_tlv(extend)
    extend_raw_dict = {}
    for item in extend_decode_val:
        extend_raw_dict[item["code"]] = item["value"]
        if item["code"] in tlv_raw_dict:
            del tlv_raw_dict[item["code"]]

    if onietlv.TLV_CODE_WB_CARID in extend_raw_dict:
        carid_value = extend_raw_dict[onietlv.TLV_CODE_WB_CARID]
        byte_data = bytes.fromhex(carid_value)
        data_str = byteTostr(byte_data)
        extend_raw_dict[onietlv.TLV_CODE_WB_CARID] = data_str

    if value == '':
        if code in extend_raw_dict:
            del extend_raw_dict[code]
    else:
        extend_raw_dict[code] = value

    status, ret= generate_extend(extend_raw_dict)
    if status is False:
        print("generate onie tlv eeprom extend rawdata failed, errmsg: %s" % ret)
        return False
    tlv_raw_dict[onietlv.TLV_CODE_VENDOR_EXT] = ret

    # Write the updated onie tlv e2 to the eeprom
    changed_tlv_value, _ = onietlv.generate_value(tlv_raw_dict)
    write_rawdata_to_file(changed_tlv_value, e2_path)
    return True

def get_eeprom_config(e2_alias, index):
    eeprom_conf = PLATFORM_E2_CONF.get(e2_alias, [])
    eeprom_num = len(eeprom_conf)
    if eeprom_num == 0:
        print("%s number is 0, can't get info", e2_alias)
        return None
    ...
    if index < 0 or index >= eeprom_num:
        print("index error, total syseeprom number: %d, syseeprom index: %d" % (eeprom_num, index + 1))
        return None

    return eeprom_conf[index]

def set_eeprom_write_protect(eeprom_conf, protect_flag):
    write_protect_conf = eeprom_conf.get("write_protect")
    if write_protect_conf is None:
        print("not found write_protect field")
        return

    if 'protect_on' not in write_protect_conf or 'protect_off' not in write_protect_conf:
        print("write_protect value error")
        return

    if protect_flag == True:
        write_protect_conf['value'] = write_protect_conf['protect_on']
    else:
        write_protect_conf['value'] = write_protect_conf['protect_off']

    ret, log = set_value(write_protect_conf)
    if ret is False:
        print("set write_protect failed, msg: %s" % log)
        return False

def set_eeprom_extend_field(e2_alias, code, value, index):
    eeprom_conf = get_eeprom_config(e2_alias, index)
    if eeprom_conf is None:
        return

    e2_type = eeprom_conf.get("e2_type")
    e2_path = eeprom_conf.get("e2_path")

    ret = set_eeprom_write_protect(eeprom_conf, False)
    if ret is False:
        print("turn off %s write protect fail." % e2_alias)
        return

    if e2_type == "onie_tlv":
        ret = set_tlv_e2_extend_field(e2_path, code, value)
    else:
        print("unsupport e2_type: %s" % e2_type)
        return
    ...
    ret = set_eeprom_write_protect(eeprom_conf, True)
    if ret is False:
        print("turn on %s write protect fail." % e2_alias)
        return
...
# syseeprom set
@main.command(name='syseeprom_set')
@click.argument('choice', required=True)
@click.argument('value', required=True)
@click.argument('syseeprom_index', required=False, default="1")
def syseeprom_set(choice, value, syseeprom_index):
    '''change syseeprom field value, syseeprom_index(1, 2, 3...)

    CHOICE : 1: mac_addr, 2: serial_number, 3: product_name.

    VALUE : value to set.

    SYSEEPROM_INDEX: syseeprom_index.
    '''
    set_syseeprom_value(choice, value, syseeprom_index)

# extend set
@main.command(name='extend_set')
@click.argument('e2_alias', required=True)
@click.argument('code', required=True)
@click.argument('value', required=True)
@click.argument('index', required=False, default="1")
def eeprom_extend_set(e2_alias, code, value, index):
    '''change eeprom extend field value, index(1, 2, 3...)

    E2_ALIAS : eeprom alias.
    ...
    INDEX: eeprom index.
    '''
    set_eeprom_extend_field(e2_alias, code, value, index)
