...
class devicebase(object):
    ...
    def set_value(self, config, val):
        ...
        if way == "devfile":
            ...
            if path and loc:
                msg = "conf path and loc is not allowed existed at the same time"
                return False, msg

            if not path and not loc:
                msg = "conf path and loc is none at the same time"
                return False, msg
            ...
            if mask:
                ret, val_list = self.devfile_read(path, offset, length)
                if ret is True:
            ...
            ret, log = self.devfile_write(path, offset, write_value)
            if ret is True:
