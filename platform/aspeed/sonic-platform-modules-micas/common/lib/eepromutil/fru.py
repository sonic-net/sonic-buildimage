...
class ChassisInfoArea(BaseArea):
    ...
    def __str__(self):
        ...
        tmpstr = formatstr % (ord(self.chassisversion), ord(self.chassisType),
                              self.chassisSerialNumber, self.chassisPartNumber)

        for i in range(1, 11):
            valtmp = "chassisextra%d" % i
            if hasattr(self, valtmp):
                valtmpval = getattr(self, valtmp)
                tmpstr += "chassisextra%d         : %s \n" % (i, valtmpval)
            else:
                break
    ...
    def todict(self):
        ...
        dic["chassisversion"] = ord(self.chassisversion)
        dic["chassisType"] = ord(self.chassisType)
        dic["chassisSerialNumber"] = self.chassisSerialNumber
        dic["chassisPartNumber"] = self.chassisPartNumber

        return dic

    def decodedata(self):
        index = 0
        self.chassisversion = self.data[index]
        index += 1
        d_print("decode length :%d class size:%d" %
                ((ord(self.data[index]) * 8), self.size))
        index += 1
        
        self.chassisType = self.data[index]
        index += 1

        templen = FruUtil.decodeLength(self.data[index])
        self.chassisSerialNumber = self.data[index + 1: index + templen + 1]
        index += templen + 1
        d_print("decode chassisSerialNumber:%s" % self.chassisSerialNumber)

        templen = FruUtil.decodeLength(self.data[index])
        self.chassisPartNumber = self.data[index + 1: index + templen + 1]
        index += templen + 1
        d_print("decode chassisPartNumber:%s" % self.chassisPartNumber)
        ...
        for i in range(1, 11):
            ...
            if self.data[index] != chr(0xc1):
                ...
                index += templen + 1
                d_print("decode chassisextra%d:%s" % (i, tmpval))
            else:
                break
        pass

    def recalcute(self):
        self.chassisversion = self.COMMON_HEAD_VERSION
        d_print("chassisInfoArea version:%x" % ord(self.chassisversion))
        d_print("chassisInfoArea length:%d" % self.size)

        self.data = chr(ord(self.chassisversion)) + chr(self.size // 8) + chr(ord(self.chassisType))

        d_print("chassisInfoArea chassisSerialNumber:%s" % self.chassisSerialNumber)
        self.data += chr(FruUtil.getTypeLength(self.chassisSerialNumber))
        self.data += self.chassisSerialNumber

        d_print("chassisInfoArea chassisPartNumber:%s" % self.chassisPartNumber)
        self.data += chr(FruUtil.getTypeLength(self.chassisPartNumber))
        self.data += self.chassisPartNumber

        for i in range(1, 11):
            valtmp = "chassisextra%d" % i
            if hasattr(self, valtmp):
                valtmpval = getattr(self, valtmp)
                d_print("chassisInfoArea chassisextra%d:%s" % (i, valtmpval))
        ...
        d_print("chassis info checksum:%x" % checksum)
        self.data += chr(checksum)

    @property
    def chassisversion(self):
        return self._chassisversion

    @chassisversion.setter
    def chassisversion(self, val):
        self._chassisversion = val

    @property
    def chassisPartNumber(self):
        return self._chassisPartNumber

    @chassisPartNumber.setter
    def chassisPartNumber(self, val):
        self._chassisPartNumber = val

    @property
    def chassisSerialNumber(self):
        return self._chassisSerialNumber

    @chassisSerialNumber.setter
    def chassisSerialNumber(self, val):
        self._chassisSerialNumber = val
    ...
    @chassisType.setter
    def chassisType(self, val):
        self._chassisType = val
class BoardInfoArea(BaseArea):
    ...
    def decodedata(self):
        index = 0
        self.boardversion = self.data[index]
        index += 1
        d_print("decode length :%d class size:%d" %
                ((ord(self.data[index]) * 8), self.size))
        index += 2
    ...
    def recalcute(self):
        self.boardversion = self.COMMON_HEAD_VERSION
    ...
    @boardversion.setter
    def boardversion(self, val):
        self._boardversion = val
...
class ProductInfoArea(BaseArea):
    ...
    def todict(self):
        ...
        try:
            ...
            dic["productVersion"] = self.productVersion
    ...
    def recalcute(self):
        self.areaversion = self.COMMON_HEAD_VERSION
...
class ipmifru(BaseArea):
    ...
    def decodeBin(self, eeprom):
        ...
        if ord(commonHead[2]) != ord(self.INITVALUE):
            ...
            self.chassisInfoArea.size = ord(
                eeprom[self.chassicInfoAreaOffset * 8 + 1]) * 8
            self.chassisInfoArea.data = eeprom[self.chassicInfoAreaOffset * 8: (
                self.chassicInfoAreaOffset * 8 + self.chassisInfoArea.size)]
            if FruUtil.checksum(self.chassisInfoArea.data[:-1]) != ord(self.chassisInfoArea.data[-1:]):
                strtmp = "check chassisInfoArea checksum error[cal:%02x data:%02x]" %  \
                    (FruUtil.checksum(
                        self.chassisInfoArea.data[:-1]), ord(self.chassisInfoArea.data[-1:]))
                raise FruException(strtmp, -3)
            self.chassisInfoArea.decodedata()
    ...
    def recalcutebin(self):
        ...
        if self.chassisInfoArea is not None and self.chassisInfoArea.isPresent:
            ...
            self.chassisInfoArea.recalcute()
