...
#include "wb_peci_hwmon.h"

#define DEFAULT_CHANNEL_NUMS	7
...
struct temp_group {
	...
	struct peci_sensor_data		tmargin;
...
static int get_tmargin(struct peci_cputemp *priv)
{
	s32 t_margin;
	u8  pkg_cfg[4];
	int ret;
	if (!peci_sensor_need_update(&priv->temp.tmargin))
		return 0;
	...
	t_margin = le16_to_cpup((__le16 *)pkg_cfg);
	 ...
	 *   0x8003-0x81ff: Reserved
	 */
	if (t_margin >= 0x8000 && t_margin <= 0x81ff)
		return -EIO;
	t_margin = ten_dot_six_to_millidegree(t_margin);

	/* Note that the tcontrol should be available before calling it */
	priv->temp.tmargin.value = t_margin;
	peci_sensor_mark_updated(&priv->temp.tmargin);
	...
	case hwmon_temp_input:
		...
		case channel_tjmax:
			*val = priv->temp.tjmax.value;
			break;
		case channel_tmargin:
			ret = get_tmargin(priv);
			if (ret)
				break;
			*val = priv->temp.tmargin.value;
...
static int peci_cputemp_probe(struct platform_device *pdev)
{
	...
	priv->temp_config[priv->config_idx++] = config_table[channel_tmargin];
