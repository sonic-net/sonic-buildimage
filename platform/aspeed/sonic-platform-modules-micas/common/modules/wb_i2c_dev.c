...
typedef struct i2c_dev_info {
    ...
    uint32_t cs_index;
    unsigned long cs_enable_intf_addr;
    unsigned long cs_disable_intf_addr;
...
static int wb_cs_enable(wb_i2c_dev_t *wb_i2c_dev)
{
    wb_cs_func pfunc;

    if (wb_i2c_dev->cs_index) {
        pfunc = (wb_cs_func)wb_i2c_dev->cs_enable_intf_addr;
        return pfunc(wb_i2c_dev->cs_index);
    } else {
        return 0;
    }
...
static int wb_cs_disable(wb_i2c_dev_t *wb_i2c_dev)
{
    wb_cs_func pfunc;

    if (wb_i2c_dev->cs_index) {
        pfunc = (wb_cs_func)wb_i2c_dev->cs_disable_intf_addr;
        return pfunc(wb_i2c_dev->cs_index);
    } else {
        return 0;
    }
...
static int minor_to_dev(int minor, struct i2c_dev_info **i2c_dev)
{
    int i;
    ...
    for (i = 0; i < MAX_DEV_NUM; i++) {
        ...
        if (i2c_dev_arry[i]->misc.minor == minor) {
...
static int add_dev_to_g_dev_list(struct i2c_dev_info *i2c_dev)
{
    int i;
    unsigned long flags;
...
static int i2c_dev_open(struct inode *inode, struct file *file)
{
    ...
    ret = minor_to_dev(minor, &i2c_dev);
    if (ret) {
        return ret;
    }
...
static int device_read_nocs(struct i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    int i, ret;
    u8 val[MAX_RW_LEN + MAX_DATA_WIDTH];
    u32 start_offset, tmp_offset, delta_offset, total_rd_len, remain_rd_len, rd_count;
    ...
    start_offset = ALIGN_DOWN(offset, i2c_dev->data_bus_width);
    delta_offset = offset - start_offset;
    total_rd_len  = ALIGN(count + delta_offset, i2c_dev->data_bus_width);

    mem_clear(val, sizeof(val));
    rd_count = total_rd_len / i2c_dev->per_rd_len;
    for (i = 0; i < rd_count; i++) {
        tmp_offset = i * i2c_dev->per_rd_len;
        ret = transfer_read(i2c_dev->client, val + tmp_offset, start_offset + tmp_offset, i2c_dev->per_rd_len);
        if (ret < 0) {
    ...
    remain_rd_len = total_rd_len % i2c_dev->per_rd_len;
    if (remain_rd_len != 0) {
        tmp_offset = rd_count * i2c_dev->per_rd_len;
        ret = transfer_read(i2c_dev->client, val + tmp_offset, start_offset + tmp_offset, remain_rd_len);
        if (ret < 0) {
            DEBUG_ERROR("read error.read offset = %u\n", (start_offset + tmp_offset));
            return -EFAULT;
        }
    ...
    /* do big endian order to cpu order  */
    if ((i2c_dev->data_bus_width != WIDTH_1Byte) && (!i2c_dev->is_little_endian)) {
        if (i2c_dev->data_bus_width == WIDTH_4Byte) {
            for (i = 0; i < total_rd_len / i2c_dev->data_bus_width; i++) {
                __be32_to_cpus((u32*)(val + (i * i2c_dev->data_bus_width)));
            }
        } else {
            for (i = 0; i < total_rd_len / i2c_dev->data_bus_width; i++) {
                __be16_to_cpus((u16*)(val + (i * i2c_dev->data_bus_width)));
            }
        }
    }

    memcpy(buf, val + delta_offset, count);
...
static int device_read(struct i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    ...
    ret = wb_cs_enable(i2c_dev);
    if (ret < 0) {
        DEBUG_ERROR("wb_cs_enable failed.\n");
        return ret;
    }

    len = device_read_nocs(i2c_dev, offset, buf, count);
    if (len < 0) {
        DEBUG_ERROR("device_read_nocs failed, ret = %d.\n", len);
        wb_cs_disable(i2c_dev);
        return len;
    }


    ret = wb_cs_disable(i2c_dev);
    if (ret < 0) {
        DEBUG_ERROR("wb_cs_disable failed.\n");
        return ret;
    }
...
static int device_write_nocs(struct i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    int i, j, ret;
    u8 tmp_offset;
    u32 width;
...
static int device_write(struct i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    ...
    len = device_write_nocs(i2c_dev, offset, buf, count);
    if (len < 0) {
        DEBUG_ERROR("device_write_nocs failed, ret = %d.\n", len);
        wb_cs_disable(i2c_dev);
        return len;
    }
...
static ssize_t status_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    type = i2c_dev->status_check.status_check_type_bmp;
    ...
    if (time_before(jiffies, i2c_dev->status_check.last_jiffies + msecs_to_jiffies(status_cache_ms))) {
        ...
        DEBUG_VERBOSE("time before last time %d ms return last status: %d\n",
            status_cache_ms, i2c_dev->status_check.dev_status);
        return sprintf(buf, "%u\n", i2c_dev->status_check.dev_status);
    }

    i2c_dev->status_check.last_jiffies = jiffies;
    ...
    ret = wb_logic_dev_get_status(&i2c_dev->status_check, len, buf, PAGE_SIZE);
    if (ret < 0) {
...
static ssize_t status_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    i2c_dev = container_of(kobj, wb_i2c_dev_t, kobj);
    if (!i2c_dev) {
        DEBUG_ERROR("status_store i2c dev is null.\n");
        return -ENODEV;
    }
    ...
    mutex_lock(&i2c_dev->update_lock);
    ret = wb_logic_clear_status(&i2c_dev->status_check, len, val);
    if (ret < 0) {
...
static ssize_t status_show_with_type(struct kobject *kobj, struct kobj_attribute *attr, char *buf, uint32_t type)
{
    ...
    ret = wb_logic_get_cache_status_with_type(&i2c_dev->status_check, type, status_cache_ms, buf, PAGE_SIZE);
    if (ret > 0) {
        /* use cache status */
    ...
    mutex_lock(&i2c_dev->update_lock);
    ret = wb_logic_dev_get_status_with_type(&i2c_dev->status_check, len, buf, PAGE_SIZE, type);
    if (ret < 0) {
        DEBUG_ERROR("wb_logic_dev_get_status fail. (ret %d)\n", ret);
    }
    mutex_unlock(&i2c_dev->update_lock);
...
static int of_i2c_dev_config_init(struct i2c_client *client, struct i2c_dev_info *i2c_dev)
{
    ...
    if(!of_property_read_u32(client->dev.of_node, "cs_index", &i2c_dev->cs_index)) {
        if (i2c_dev->cs_index) {
            ret = find_cs_intf_addr(&i2c_dev->cs_enable_intf_addr, &i2c_dev->cs_disable_intf_addr);
            if (ret != 0) {
                dev_err(&client->dev, "find_cs_intf_addr fail cs_index: %u\n", i2c_dev->cs_index);
                return -EINVAL;
            }
        }
    } else {
    ...
    i2c_dev->is_little_endian = of_property_read_bool(client->dev.of_node, "is_little_endian");

    if (of_property_read_string(client->dev.of_node, "i2c_alias", &i2c_dev->alias)) {
        i2c_dev->alias = i2c_dev->name;
    }

    ret = of_dev_status_check_config_init(&client->dev, &i2c_dev->status_check, i2c_dev->data_bus_width,
                                            i2c_device_func_read, i2c_device_func_write, i2c_dev->name);
    if (ret != 0) {
        dev_err(&client->dev, "status_check_config_init fail: %d\n", ret);
        return -EINVAL;
    }
    ...
    dev_info(&client->dev, "i2c_name: %s, i2c_alias: %s, data_bus_width: %d, addr_bus_width: %d, per_rd_len: %d, per_wr_len: %d, i2c_len: 0x%x, cs_index: %d\n",
        i2c_dev->name, i2c_dev->alias, i2c_dev->data_bus_width, i2c_dev->addr_bus_width, i2c_dev->per_rd_len,
        i2c_dev->per_wr_len, i2c_dev->i2c_len, i2c_dev->cs_index);
    return 0;
}
...
static int i2c_dev_config_init(struct i2c_client *client, struct i2c_dev_info *i2c_dev)
{
    ...
    i2c_dev->is_little_endian = i2c_dev_device->is_little_endian;
    if (i2c_dev->data_bus_width == 0) {
        dev_err(&client->dev, "Invalid data_bus_width: %u\n", i2c_dev->data_bus_width);
        return -EINVAL;
    }
    ...
    i2c_dev->cs_index = i2c_dev_device->cs_index;
    if (i2c_dev->cs_index) {
        ret = find_cs_intf_addr(&i2c_dev->cs_enable_intf_addr, &i2c_dev->cs_disable_intf_addr);
        if (ret != 0) {
            dev_err(&client->dev, "find_cs_intf_addr fail cs_index: %u\n", i2c_dev->cs_index);
    ...
    ret = platform_dev_status_check_config_init(&client->dev, &i2c_dev->status_check, i2c_dev->data_bus_width, &i2c_dev_device->status_check,
                                            i2c_device_func_read, i2c_device_func_write, i2c_dev->name);
    if (ret != 0) {
        dev_err(&client->dev, "platform status_check_config_init fail: %d\n", ret);
        return -EINVAL;
    }
...
#endif
{
    ...
    if ((i2c_dev->data_bus_width != WIDTH_1Byte) && (i2c_dev->data_bus_width != WIDTH_2Byte) && 
        (i2c_dev->data_bus_width != WIDTH_4Byte)) {
        dev_err(&client->dev, "Invalid data bus_width %d.\n", i2c_dev->data_bus_width);
        return -EINVAL;
    }

    if ((i2c_dev->per_rd_len == 0) || (i2c_dev->per_rd_len & (i2c_dev->data_bus_width - 1)) ||
        (i2c_dev->per_wr_len == 0) || (i2c_dev->per_wr_len & (i2c_dev->data_bus_width - 1))) {
    ...
    ret = add_dev_to_g_dev_list(i2c_dev);
    if (ret) {
    ...
    ret = wb_logic_check_status_hw_init(&i2c_dev->status_check, i2c_dev->data_bus_width);
    if (ret != 0) {
        dev_err(&client->dev, "Failed to wb_logic_check_status_hw_init, ret: %d\n", ret);
    }
