...
#define WB_XDPE_MAX_MISC_DEV_NUM         (64)
...
static int minor_to_dev(int minor, struct xdpe_data **xdpe_dev)
{
    int i;

    for (i = 0; i < WB_XDPE_MAX_MISC_DEV_NUM; i++) {
        if (xdpe132g5c_dev_arry[i] == NULL) {
            continue;
        }
        if (xdpe132g5c_dev_arry[i]->misc.minor == minor) {
            *xdpe_dev = xdpe132g5c_dev_arry[i];
            return 0;
        }
    }
    return -ENODEV;
...
static int add_dev_to_g_dev_list(struct xdpe_data *xdpe_dev)
{
    int i;
    unsigned long flags;
    ...
    for (i = 0; i < WB_XDPE_MAX_MISC_DEV_NUM; i++) {
        if (xdpe132g5c_dev_arry[i] == NULL) {
            xdpe132g5c_dev_arry[i] = xdpe_dev;
...
#endif
{
    ...
    ret = add_dev_to_g_dev_list(data);
    if (ret) {
