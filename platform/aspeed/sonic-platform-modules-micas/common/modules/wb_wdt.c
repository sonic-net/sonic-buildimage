...
typedef struct wb_wdt_priv_s {
    ...
    uint32_t is_need_stop;                  /* 0: need stop ; 1: not need stop */
    bool no_device;
} wb_wdt_priv_t;

static int wb_wdt_maybe_defer_probe(struct device *dev, const char *dev_name,
...
static int wb_wdt_maybe_defer_probe_io(struct device *dev, const char *dev_name, int ret)
{
...
static int thread_timer_cfg(wb_wdt_priv_t *priv, wb_wdt_device_t *wb_wdt_device)
{
    ...
    if (priv->timer_accuracy_reg_flag != 0) {
        ...
        if (ret < 0) {
            ...
            return wb_wdt_maybe_defer_probe_io(priv->dev, priv->config_dev_name, ret);
        }
    }
    ...
    if (ret < 0) {
        dev_err(dev, "set wdt thread timer reg error, ret: %d.\n", ret);
        return wb_wdt_maybe_defer_probe_io(priv->dev, priv->config_dev_name, ret);
    }
...
static int watchdog_device_cfg(wb_wdt_priv_t *priv)
{
    ...
    ret = wb_wdt_write(priv, priv->config_dev_name,
                priv->timeout_cfg_reg, PARENT_WDT, &set_time_val, ONE_BYTE);
    if (ret < 0) {
        dev_err(priv->dev, "set wdt time reg error, ret: %d.\n", ret);
        return wb_wdt_maybe_defer_probe_io(priv->dev, priv->config_dev_name, ret);
    }
...
static int logic_wdt_init(wb_wdt_priv_t *priv, wb_wdt_device_t *wb_wdt_device)
{
    ...
    if (ret) {
        ...
        return wb_wdt_maybe_defer_probe(dev, logic_wdt->feed_dev_name, logic_wdt->logic_func_mode, ret);
    }
    ...
    if (!logic_wdt->write_intf_addr || !logic_wdt->read_intf_addr) {
        ...
        return wb_wdt_maybe_defer_probe(dev, logic_wdt->feed_dev_name, logic_wdt->logic_func_mode, -ENOSYS);
    }
...
static int of_gpio_wdt_init(wb_wdt_priv_t *priv, wb_wdt_device_t *wb_wdt_device)
{
    struct device *dev;
    gpio_wdt_info_t *gpio_wdt;
    ...
    gpiod = devm_gpiod_get(dev, NULL, gflags);
    if (IS_ERR(gpiod)) {
    ...
    DEBUG_VERBOSE("gpio number: %d\n", gpio_wdt->gpio);

    gpio_wdt->active_low = 0;
...
static int dev_gpio_wdt_init(wb_wdt_priv_t *priv, wb_wdt_device_t *wb_wdt_device)
{
    struct device *dev;
    gpio_wdt_info_t *gpio_wdt;
...
static int gpio_wdt_init(wb_wdt_priv_t *priv, wb_wdt_device_t *wb_wdt_device)
{
    ...
    if (dev->of_node) {
        ret = of_gpio_wdt_init(priv, wb_wdt_device);
    } else {
        ret = dev_gpio_wdt_init(priv, wb_wdt_device);
    }
...
static ssize_t wb_show_wdt_timeout_value(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    wdd = &priv->wdd;
    if (wdd == NULL) {
        DEBUG_ERROR("watchdog device is null.\n");
        return -EINVAL;
    }

    return snprintf(buf, PAGE_SIZE, "%us\n", wdd->timeout);
...
static int wb_wdt_probe(struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node) {
        ...
        priv->no_device = of_property_read_bool(pdev->dev.of_node, "no_device");
        ...
        dev_info(&pdev->dev, "enable_val:%u, disable_val:%u, enable_mask:%u, hw_margin:%u, feed_wdt_type:%u, no_device:%u\n",
            priv->enable_val, priv->disable_val, priv->enable_mask,   priv->hw_margin, priv->feed_wdt_type, priv->no_device);
    } else {
    ...
    if (ret) {
        ...
        return wb_wdt_maybe_defer_probe(&pdev->dev, priv->config_dev_name, priv->priv_func_mode, ret);
    }

    if (!priv->write_intf_addr || !priv->read_intf_addr) {
        dev_err(&pdev->dev, "Fail: func mode %u rw symbol undefined.\n", priv->priv_func_mode);
        return wb_wdt_maybe_defer_probe(&pdev->dev, priv->config_dev_name, priv->priv_func_mode, -ENOSYS);
    }
