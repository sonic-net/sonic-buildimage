...
static int wb_fpga_i2c_retry_time_max = LOGIC_DEV_RETRY_TIME;
module_param(wb_fpga_i2c_retry_time_max, int, S_IRUGO | S_IWUSR);

#define RETRY_TIME_MAX            (wb_logic_get_retry_time(wb_fpga_i2c_retry_time_max))
static int debug = 0;
module_param(debug, int, S_IRUGO | S_IWUSR);

static DEFINE_RATELIMIT_STATE(fpga_print_ratelimit, I2C_BUSY_DUMP_INTERVAL * HZ, I2C_BUSY_DUMP_TIMES);

static void fpga_i2c_dump_regs(fpga_i2c_dev_t *fpga_i2c);

static int fpga_i2c_maybe_defer_probe(struct device *dev, const char *dev_name,
    uint32_t func_mode, int ret)
{
    if (ret != -ENOSYS) {
        return ret;
    }

    dev_info(dev, "defer probe until func mode %u interface for %s is ready\n",
        func_mode, dev_name ? dev_name : "<unknown>");
    return -EPROBE_DEFER;
}

static int fpga_i2c_maybe_defer_probe_io(struct device *dev, const char *dev_name, int ret)
{
    if (!dev_name || strncmp(dev_name, "/dev/", 5) != 0) {
        return ret;
    }

    if (ret != -EINVAL && ret != -ENODEV && ret != -ENXIO) {
        return ret;
    }

    dev_info(dev, "defer probe until backing device %s is registered and ready, ret: %d\n",
        dev_name, ret);
    return -EPROBE_DEFER;
}
...
static inline void fpga_i2c_update_retry_config(struct i2c_adapter *adap, fpga_i2c_dev_t *fpga_i2c)
{
    int retries;

    retries = RETRY_TIME_MAX;
    adap->retries = retries;
    adap->timeout = msecs_to_jiffies(retries * fpga_i2c->i2c_timeout);
}
...
static void fpga_i2c_dump_buf_seg(struct device *dev, uint32_t base, const uint8_t *buf, size_t bytes)
{
    int i, j, pos;
...
static void logic_dump_reg(fpga_i2c_dev_t *fpga_i2c)
{
    if (!__ratelimit(&fpga_print_ratelimit)) {
        return;
    }

    fpga_i2c_dump_regs(fpga_i2c);
    logic_dev_dump_regs(&fpga_i2c->logic_dev_dump, fpga_i2c->dev, "fpga_i2c");
}
...
static void fpga_i2c_dump_regs(fpga_i2c_dev_t *fpga_i2c)
{
    ...
    config_addr_base = fpga_i2c->reg.i2c_scale;
    data_addr_base = fpga_i2c->reg.i2c_data_buf;
    read_len = fpga_i2c->reg.i2c_data_buf_len;
    ...
    fpga_data_read(fpga_i2c, config_addr_base, config_buf, config_bytes);
    fpga_data_read(fpga_i2c, data_addr_base, data_buf, data_bytes);

    dev_info(fpga_i2c->dev, "[fpga_i2c] :dump config and data buf:\n");
    fpga_i2c_dump_buf_seg(fpga_i2c->dev, config_addr_base, config_buf, config_bytes);
    dev_info(fpga_i2c->dev, "===== FPGA I2C: End dump from config buf =====\n");
    fpga_i2c_dump_buf_seg(fpga_i2c->dev, data_addr_base, data_buf, data_bytes);
    dev_info(fpga_i2c->dev, "===== FPGA I2C: End dump from data buf =====\n");
    return;
}
...
static int fpga_i2c_adapter_init(fpga_i2c_dev_t *fpga_i2c)
{
    ...
    val = fpga_i2c->i2c_scale_value;
    ret = fpga_device_write(fpga_i2c, reg->i2c_scale, &val, sizeof(val));
    if (ret < 0) {
        DEBUG_ERROR_I2C_ADAPTER(&(fpga_i2c->adap),
            "fpga_i2c_init failed to set i2c_scale, reg: 0x%x, ret: %d.\n",
            reg->i2c_scale, ret);
        return fpga_i2c_maybe_defer_probe_io(fpga_i2c->dev, fpga_i2c->dev_name, ret);
    }

    val = fpga_i2c->i2c_filter_value;
    ret = fpga_device_write(fpga_i2c, reg->i2c_filter, &val, sizeof(val));
    if (ret < 0) {
        DEBUG_ERROR_I2C_ADAPTER(&(fpga_i2c->adap),
            "fpga_i2c_init failed to set i2c_filter, reg: 0x%x, ret: %d.\n",
            reg->i2c_filter, ret);
        return fpga_i2c_maybe_defer_probe_io(fpga_i2c->dev, fpga_i2c->dev_name, ret);
    }

    val = fpga_i2c->i2c_stretch_value;
    ret = fpga_device_write(fpga_i2c, reg->i2c_stretch, &val, sizeof(val));
    if (ret < 0) {
        DEBUG_ERROR_I2C_ADAPTER(&(fpga_i2c->adap),
            "fpga_i2c_init failed to set i2c_stretch, reg: 0x%x, ret: %d.\n",
            reg->i2c_stretch, ret);
        return fpga_i2c_maybe_defer_probe_io(fpga_i2c->dev, fpga_i2c->dev_name, ret);
    }
        ...
        struct i2c_msg *msgs, int num)
{
    ...
    fpga_i2c_update_retry_config(adap, fpga_i2c);
...
static int fpga_i2c_config_init(fpga_i2c_dev_t *fpga_i2c)
{
    ...
    if (dev->of_node) {
        ...
        if (ret) {
            ...
            return fpga_i2c_maybe_defer_probe(dev, fpga_i2c->dev_name, fpga_i2c->i2c_func_mode, ret);
        }
        ...
        if (!fpga_i2c->write_intf_addr || !fpga_i2c->read_intf_addr) {
            ...
            return fpga_i2c_maybe_defer_probe(dev, fpga_i2c->dev_name, fpga_i2c->i2c_func_mode, -ENOSYS);
        }
        ...
        ret = logic_dev_dump_of_node_init(&fpga_i2c->logic_dev_dump, fpga_i2c->dev);
        if (ret) {
            dev_err(dev, "logic dev dump of_node init failed, ret: %d\n", ret);
            return ret;
        }
    } else {
        ...
        ret = logic_dev_dump_cfg_info_init(&fpga_i2c->logic_dev_dump, fpga_i2c->dev,
            &fpga_i2c_bus_device->logic_dev_dump_cfg);
        if (ret) {
            dev_err(dev, "logic dev dump cfg init failed, ret: %d\n", ret);
            return ret;
        }
    }
...
static int fpga_i2c_probe(struct platform_device *pdev)
{
    ...
    fpga_i2c_update_retry_config(&fpga_i2c->adap, fpga_i2c);
...
MODULE_SOFTDEP("pre: wb_logic_dev_common");
