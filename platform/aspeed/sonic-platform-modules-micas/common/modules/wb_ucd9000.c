...
#define UCD9000_FAULT_REG_LENG              (16)
...
#define FAULT_DAY_MASK                      (0x7FFFFF)
...
#define UCD9000_RECORD_ANALY                (1)
#define UCD9000_RECORD_NO_ANALY             (0)

static char *fail_type_string[] = {
    "VOUT_OV Fault",
    "VOUT_UV Fault",
    "TON_MAX Fault",
    "IOUT_OC Fault",
    "IOUT_UC Fault",
    "TEMPERATURE_OT Fault",
    "Sequence On Timeout",
...
static ssize_t get_fault_flag(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (ret <= 0) {
        dev_info(&client->dev, "Failed to read LOGGED_FAULT_DETAIL_INDEX, ret = %d\n", ret);
    ...
    if (fault_num > 0) {
        err = 1;
        DEBUG_VERBOSE("%d-%04x: ucd9000 has err record, ret: %d, fault_num = %d\n",
            client->adapter->nr, client->addr, ret, fault_num);
        mutex_unlock(&data->update_lock);
        return scnprintf(buf, FAULT_RECORD_SIZE, "%d\n", err);
    } else {
        err = 0;
        DEBUG_VERBOSE("%d-%04x: ucd9000 has not err record, ret: %d, fault_num = %d\n",
            client->adapter->nr, client->addr, ret, fault_num);
        mutex_unlock(&data->update_lock);
        return scnprintf(buf, FAULT_RECORD_SIZE, "%d\n", err);
    }
...
static ssize_t get_fault_record(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    index = to_sensor_dev_attr(da)->index;
    size = 0;
    ...
    if (fault_num > 0) {
        for (i = 0; i < fault_num; i++) {
            ...
            if (tmp_data & PAGE_FAULT) {
                ...
                if (index == UCD9000_RECORD_ANALY) {
                    size += snprintf(buf + size, PAGE_SIZE - size, "fault id %d:\n", i);
                    ms_data = (u32)(block_buffer[1] << 24 | block_buffer[2] << 16 | block_buffer[3] << 8 | block_buffer[4]);
                    convert_to_tm_using_time64(tmp_data & FAULT_DAY_MASK, ms_data, &tm);
                    size += snprintf(buf + size, PAGE_SIZE - size, "page number %d\n", fault_page);
                    size += snprintf(buf + size, PAGE_SIZE - size, "time:%ld-%02d-%02d %02d:%02d:%02d\n",
                        tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    if (fault_type < ARRAY_SIZE(fail_type_string)) {
                        size += snprintf(buf + size, PAGE_SIZE - size, "Fault type:%s\n", fail_type_string[fault_type]);
                    } else {
                        size += snprintf(buf + size, PAGE_SIZE - size, "Fault type:unknown\n");
                    }

                    size += snprintf(buf + size, PAGE_SIZE - size, "Fault value = %d.%dv\n", value / 1000, value % 1000);
                }
            } else {  /* not page fault */
                DEBUG_VERBOSE("bus %d addr 0x%x, not page fault\n", client->adapter->nr, client->addr);
                if (index == UCD9000_RECORD_ANALY) {
                    size += snprintf(buf + size, PAGE_SIZE - size, "fault id %d:\n", i + 1);
                    size += snprintf(buf + size, PAGE_SIZE - size, "not page fault\n");
                }
            }

            if (index == UCD9000_RECORD_ANALY) {
                size += snprintf(buf + size, PAGE_SIZE - size, "regs value:\n");
                for (j = 0; j < UCD9000_FAULT_REG_LENG; j++) {
                    size += snprintf(buf + size, PAGE_SIZE - size, "0x%x ", block_buffer[j]);
                }

                size += snprintf(buf + size, PAGE_SIZE - size, "\n");
            } else {
                memcpy(buf + i * UCD9000_FAULT_REG_LENG, &block_buffer[0], UCD9000_FAULT_REG_LENG);
            }
        }
...
static int get_current_time(uint32_t *day, uint32_t *milliseconds)
{
    ...
    mem_clear(&ts, sizeof(struct timespec64));
...
static SENSOR_DEVICE_ATTR(fault_flag, S_IRUGO , get_fault_flag, NULL, 0);
static SENSOR_DEVICE_ATTR(fault_record, S_IRUGO | S_IWUSR, get_fault_record, clear_fault_record,
        UCD9000_RECORD_NO_ANALY);
static SENSOR_DEVICE_ATTR(fault_record_analy, S_IRUGO | S_IWUSR , get_fault_record,
        clear_fault_record, UCD9000_RECORD_ANALY);
static SENSOR_DEVICE_ATTR(fault_date, S_IRUGO | S_IWUSR, get_fault_date, set_fault_date_and_time,
        0);

static struct attribute *ucd90160_attrs[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_fault_flag.dev_attr.attr,
    &sensor_dev_attr_fault_record.dev_attr.attr,
    &sensor_dev_attr_fault_record_analy.dev_attr.attr,
