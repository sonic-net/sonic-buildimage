...
static bool kfile_filldir_hook_wrapper(struct dir_context *ctx, const char *name, int len,
                                       loff_t pos, u64 ino, unsigned int d_type)
{
    int ret;

    ret = kfile_filldir_one(ctx, name, len, pos, ino, d_type);
    if (ret < 0) {
        return false;
    }
    return true;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,0,0)
        .ctx.actor = kfile_filldir_hook_wrapper,
#else
