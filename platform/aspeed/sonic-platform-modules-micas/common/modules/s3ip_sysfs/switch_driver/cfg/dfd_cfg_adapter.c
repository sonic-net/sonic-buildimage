...
static int dfd_ko_i2c_retry_times = LOGIC_DEV_RETRY_TIME;
module_param(dfd_ko_i2c_retry_times, int, S_IRUGO | S_IWUSR);

#define DFD_KO_I2C_RETRY_TIME wb_logic_get_retry_time(dfd_ko_i2c_retry_times)
...
static int32_t dfd_ko_i2c_smbus_word_transfer(int read_write, int bus, int addr, int offset, uint16_t *buf)
{
    int rv;
    struct i2c_adapter *i2c_adap;
    union i2c_smbus_data data;
    ...
    i2c_adap = i2c_get_adapter(bus);
    if (i2c_adap == NULL) {
        DBG_DEBUG(DBG_ERROR, "Failed to get I2C bus[%d] adapter\n", bus);
        return -DFD_RV_DEV_FAIL;
    }
    ...
    rv = i2c_smbus_xfer(i2c_adap, addr, 0, read_write, offset, I2C_SMBUS_WORD_DATA, &data);
    if (rv < 0) {
        DBG_DEBUG(DBG_ERROR, "I2C transfer failed [bus=%d addr=0x%x offset=0x%x rw=%d], rv=%d\n",
                  bus, addr, offset, read_write, rv);
        rv = -DFD_RV_DEV_FAIL;
    } else {
        DBG_DEBUG(DBG_VERBOSE, "I2C transfer success [bus=%d addr=0x%x offset=0x%x rw=%d]\n",
                  bus, addr, offset, read_write);
        rv = DFD_RV_OK;
    }
...
{
    int i, rv;
    for (i = 0; i < DFD_KO_I2C_RETRY_TIME; i++) {
...
int32_t dfd_ko_i2c_dev_smbus_read(int bus, int addr, int offset, uint8_t *buf, uint32_t size)
{
    ...
    if (size <= 0) {
        DBG_DEBUG(DBG_ERROR, "Invalid size for I2C read: %d\n", size);
        return -DFD_RV_DEV_FAIL;
    }
    ...
    switch (size) {
    case 1:
        ret = dfd_ko_i2c_read(bus, addr, offset, buf, size, NULL);
        break;

    case 2:
        ret = dfd_ko_i2c_smbus_word_transfer(I2C_SMBUS_READ, bus, addr, offset, &word_val);
        buf[0] = word_val & 0xff;
        buf[1] = word_val >> 8;
        break;
        
    default: 
        ret = dfd_ko_i2c_read_bulk_data(bus, addr, offset, buf, size);
        break;
    }

    if (ret < 0) {
        DBG_DEBUG(DBG_ERROR, "I2C read failed [bus=%d addr=0x%x offset=0x%x size=%d], rv=%d\n",
                  bus, addr, offset, size, ret);
        return -DFD_RV_DEV_FAIL;
    }
...
{
    char block_value[DFD_SMBUS_BLOCK_MAX_LEN];

    if (size <= 0 || buf == NULL || offset < 0 || block_length < 0) {
        DBG_DEBUG(DBG_ERROR, "Invalid size for I2C SMBus read, size %d, buf == NULL: %d, offset: %d, block_length: %d\n",
            size, buf == NULL, offset, block_length);
        return -DFD_RV_DEV_FAIL;
    }

    if (block_length > DFD_SMBUS_BLOCK_MAX_LEN || offset + size > block_length) {
        DBG_DEBUG(DBG_ERROR, "Invalid size for I2C SMBus read, offset + size > block_length, size: %d, offset: %d block_length: %d\n",
            size, offset, block_length);
        return -DFD_RV_DEV_FAIL;
    }
    
    mem_clear(block_value, sizeof(block_value));

    if (dfd_ko_i2c_read_bulk_data(bus, addr, command_code, block_value, block_length) < 0) {
        DBG_DEBUG(DBG_ERROR, "I2C SMBus failed to read, bus %d, addr 0x%x, command_code 0x%x, block_length %d, offset %d, size %d\n",
            bus, addr, command_code, block_length, offset, size);
        return -DFD_RV_DEV_FAIL;
    }

    memcpy(buf, block_value + offset, size);
...
/* *
 * access_file_exist - check if file exist
 * @filepath: file path
 *
...
bool access_file_exist(const char *filepath)
{
    ...
    if (IS_ERR(filp)) {
        DBG_DEBUG(DBG_ERROR, "file %s does not exist\n", filepath);
        return false;
    }
    filp_close(filp, NULL);
    return true;
...
/**
 * dfd_ko_extract_i2c_bus_number - i2c extract
 * @fpath: file path
 *
 * @returns: <0 Failure, other bus num
 */
