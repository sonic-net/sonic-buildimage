...
/* system status decode conversion level list header */
LIST_HEAD(dfd_lib_cfg_system_status_decode_conv_lst);
 ...
 * @return : void
 */
static void val_convert_node_lst_free(struct list_head *root)
{
                ...
                int index1, int index2)
{
    ...
    list_for_each_entry(tmp_val_convert, root, lst) {
        if ((tmp_val_convert->index1 == index1) && (tmp_val_convert->index2 == index2)) {
            if (str != NULL) {
                strscpy(tmp_val_convert->str_val, str, sizeof(tmp_val_convert->str_val));
            }
...
/* Get an index value from an integer value */
static int dfd_ko_cfg_get_1st_index2_by_index1(struct list_head *root, int index1,
               int *index2)
{
    val_convert_node_t *val_convert;
                ...
                int index1, int index2)
{
    ...
    } else if (cfg_item_id == DFD_CFG_ITEM_SYSTEM_STATUS_DECODE) {
        dfd_ko_cfg_regval_conv_lst_add(&dfd_lib_cfg_system_status_decode_conv_lst, val, str, index1, index2);
...
int dfd_ko_cfg_is_system_status_decode_exist(int index1, int *index2)
{
    ...
    rv = dfd_ko_cfg_get_1st_index2_by_index1(&dfd_lib_cfg_system_status_decode_conv_lst, index1, index2);
    if (rv < 0) {
        DBG_DEBUG(DBG_ERROR, "get system status decode by index1[0x%x] fail\n", index1);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
...
static int dfd_ko_cfg_add_int_item(uint64_t key, int value, int line_num)
{
    ...
    if (int_cfg == NULL) {
        ...
        int_cfg = (int *)kmalloc(data_len, GFP_KERNEL);
        if (int_cfg == NULL) {
        ...
        /* Add to linked list */
        *int_cfg = value;
        rv = lnode_insert_node(&dfd_ko_cfg_list_root, key, int_cfg, data_len);
        if (rv == 0) {
...
static int dfd_ko_cfg_add_str_item(uint64_t key, char *str, int line_num)
{
    ...
    data_len = DFD_CFG_STR_MAX_LEN;
    str_cfg = lnode_find_node(&dfd_ko_cfg_list_root, key);
    if (str_cfg == NULL) {
        /* kmalloc new node */
        str_cfg = (char *)kmalloc(data_len, GFP_KERNEL);
        if (str_cfg == NULL) {
            DBG_DEBUG(DBG_ERROR, "line%d: kmalloc str[%zu] fail\n", line_num, strlen(str));
            return -1;
        }
        mem_clear(str_cfg, data_len);
        strscpy(str_cfg, str, data_len);

        /* Add to linked list */
        rv = lnode_insert_node(&dfd_ko_cfg_list_root, key, str_cfg, data_len);
        if (rv == 0) {
...
static int dfd_ko_cfg_add_i2c_dev_item(uint64_t key, dfd_i2c_dev_mem_t member, int value, int line_num)
{
    ...
    data_len = sizeof(dfd_i2c_dev_t);
    i2c_dev_cfg = lnode_find_node(&dfd_ko_cfg_list_root, key);
    if (i2c_dev_cfg == NULL) {
        /* Node does not exist,kmalloc new node */
        i2c_dev_cfg = (dfd_i2c_dev_t *)kmalloc(data_len, GFP_KERNEL);
        if (i2c_dev_cfg == NULL) {
            DBG_DEBUG(DBG_ERROR, "line%d: kmalloc i2c_dev fail\n", line_num);
            return -1;
        }
        mem_clear(i2c_dev_cfg, data_len);

        /* Add to linked list */
        dfd_ko_cfg_set_i2c_dev_mem_value(i2c_dev_cfg, member, value);
        rv = lnode_insert_node(&dfd_ko_cfg_list_root, key, i2c_dev_cfg, data_len);
        if (rv == 0) {
...
static int dfd_cfg_query_item_type(uint64_t key)
{
    int item_id;
    ...
    if (DFD_CFG_ITEM_IS_INT(item_id)) {
        return CFG_ITEM_INT;
    } else if (DFD_CFG_ITEM_IS_STRING(item_id)) {
        return CFG_ITEM_STR;
    } else if (DFD_CFG_ITEM_IS_I2C_DEV(item_id)) {
        return CFG_ITEM_I2C_DEV;
    } else if (DFD_CFG_ITEM_IS_INFO_CTRL(item_id)) {
        return CFG_ITEM_INFO_CTRL;
    } else {
        DBG_DEBUG(DBG_ERROR, "dfd_cfg_query_item_type fail, can't query key=0x%08llx.\n", key);
        return -1;
    }
}
...
int dfd_ko_check_cfg_node_debug_mode(void)
{
    ...
    debug_mode = lnode_find_node_debug_mode(&dfd_ko_cfg_list_root);
    return debug_mode;
}

static int dfd_ko_set_debug_data(void *debug_value, uint64_t key, int data_type)
{
    int debug_mode, cfg_item_type, ret;
    unsigned int data_len;
    int *int_value;
    char *str_value;
    ...
    if (debug_value == NULL) {
        DBG_DEBUG(DBG_ERROR, "set debug data fail, debug_value is null  key=0x%08llx\n", key);
    ...
    if (debug_mode != NODE_DEBUG_ON) {
        DBG_DEBUG(DBG_ERROR, "set debug data fail, make sure s3ip debug threshold reset is 0.\n");
        return -EPERM;
    }

    cfg_item_type = dfd_cfg_query_item_type(key);

    debug_data = lnode_find_node(&dfd_ko_cfg_list_root, key);
    if (debug_data == NULL) {
        DBG_DEBUG(DBG_VERBOSE, "failed to set debug data, can't find node key=0x%08llx, we insert a new node.\n", key);
        if (cfg_item_type != CFG_ITEM_INFO_CTRL || data_type != CFG_STR_DATA) {
            DBG_DEBUG(DBG_ERROR, "only info ctrl item can insert new node, but cfg_item_type=%d, data_type=%d.\n", cfg_item_type, data_type);
            return -DFD_RV_TYPE_ERR;
        }
        rv = dfd_ko_cfg_add_info_ctrl_item(key, INFO_CTRL_MEM_MODE, "str_constant", 0);
        if (rv < 0) {
            DBG_DEBUG(DBG_ERROR, "fail to add new node fail, key=0x%08llx, info_ctrl_t member: %d.\n", key, INFO_CTRL_MEM_MODE);
            return -EINVAL;
        }
        debug_data = lnode_find_node(&dfd_ko_cfg_list_root, key);
        if (debug_data == NULL) {
            DBG_DEBUG(DBG_ERROR, "added new node but still can't find it, key=0x%08llx.\n", key);
            return -DFD_RV_NO_NODE;
        }
    }
    ...
    case CFG_ITEM_INT:
        int_value = (int *)debug_data;
        ret = kstrtoint(debug_value, 0, int_value);
        if (ret != 0) {
            DBG_DEBUG(DBG_ERROR,"invaild value ret: %d, buf: %s.\n", ret, (char *)debug_value);
            return -EINVAL;
        }
        break;
    case CFG_ITEM_STR:
        str_value = (char *)debug_data;
        mem_clear(str_value, data_len);
        strscpy(str_value, (char *)debug_value, data_len);
        break;
    case CFG_ITEM_INFO_CTRL:
        info_ctrl_cfg = (info_ctrl_t *)debug_data;
        if (data_type == CFG_INT_DATA) {
            info_ctrl_cfg->mode = INFO_CTRL_MODE_CONS;
            ret = kstrtoint(debug_value, 0, &info_ctrl_cfg->int_cons);
            if (ret != 0) {
                DBG_DEBUG(DBG_ERROR,"info ctrl invaild value ret: %d, buf: %s.\n", ret, (char *)debug_value);
                return -EINVAL;
            }
        } else if (data_type == CFG_STR_DATA) {
            info_ctrl_cfg->mode = INFO_CTRL_MODE_SRT_CONS;
            mem_clear(info_ctrl_cfg->str_cons, data_len);
            strscpy(info_ctrl_cfg->str_cons, (char *)debug_value, data_len);
        } else{
            DBG_DEBUG(DBG_ERROR, "unsupport  info ctrl type %d.\n", data_type);
            return -DFD_RV_TYPE_ERR;
        }
        break;
    case CFG_ITEM_I2C_DEV:
    default:
        DBG_DEBUG(DBG_ERROR, "set debug data fail, key=0x%08llx cfg_item_type=%d\n", key, cfg_item_type);
        return -DFD_RV_TYPE_ERR;
    }
...
static int dfd_ko_cfg_init(bool init_root_list)
{
    int rv;
    int card_type;
    char file_name[32] = {0};
...
/**
 * dfd_ko_cfg_set_debug_and_reset - Module initialization
 *
 * @returns: <0 Fail, or succeed
 */
static int dfd_ko_cfg_set_debug_and_reset(unsigned int debug_mode)
{
    int ret;

    if (debug_mode != NODE_DEBUG_ON && debug_mode != NODE_DEBUG_OFF) {
        DBG_DEBUG(DBG_ERROR, "debug mode not support, debug_mode=%u\n", debug_mode);
        return -1;
    }
    ret = lnode_set_debug_mode_and_reset(&dfd_ko_cfg_list_root, debug_mode);
    if (ret < 0) {
        DBG_DEBUG(DBG_ERROR, "lnode_set_debug_mode_and_reset fail, ret=%d\n", ret);
        return -1;
    }
    /* todo Configure data validity check */
    return ret;
...
/**
 * dfd_dev_set_debug_and_reset - Module debug mode reset
 *
 * @returns: <0 Fail, or succeed
 */
int dfd_dev_set_debug_and_reset(unsigned int debug_mode)
{
    return dfd_ko_cfg_set_debug_and_reset(debug_mode);
}
...
{
    return dfd_ko_set_debug_data(debug_value, key, data_type);
}
/**
 ...
 * @returns: <0 Fail, or succeed
 */
int32_t dfd_dev_cfg_init(bool init_root_list)
{
    return dfd_ko_cfg_init(init_root_list);
}
...
void dfd_dev_cfg_exit(void)
{
    ...
    val_convert_node_lst_free(&dfd_lib_cfg_system_status_decode_conv_lst);
