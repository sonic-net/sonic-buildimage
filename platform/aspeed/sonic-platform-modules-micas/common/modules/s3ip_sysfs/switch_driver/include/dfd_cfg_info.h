...
typedef enum info_src_e {
    ...
    INFO_SRC_FILE_EXIST,    /* judge file exist or not */
    INFO_SRC_GPIO,          /* gpio chip label + offset */
    INFO_SRC_END
} info_src_t;
...
typedef struct {
    unsigned int key;
    const char *desc;
} dfd_status_desc_map_t;

typedef struct {
    unsigned int status;
    unsigned int normal_status;
    const dfd_status_desc_map_t *map;
    size_t map_num;
    bool bit_mode;
} dfd_status_detail_t;

#define DFD_DETAIL_INFO_BUF_MAX_LEN     (256)

#define DFD_STATUS_DETAIL_CFG_SET(cfg, status_val, normal_status_val, map_val, bit_mode_val) \
    do { \
        (cfg).status = (unsigned int)(status_val); \
        (cfg).normal_status = (unsigned int)(normal_status_val); \
        (cfg).map = (map_val); \
        (cfg).map_num = ARRAY_SIZE(map_val); \
        (cfg).bit_mode = (bit_mode_val); \
    } while (0)

#define DFD_STATUS_DETAIL_INFO_SET(map, status, normal_status, bit_mode, \
            buf, buf_len, extra_info, extra_info_len) \
({ \
    int __ret = 0; \
    char __detail[DFD_DETAIL_INFO_BUF_MAX_LEN]; \
    int __detail_len; \
    dfd_status_detail_t __detail_cfg; \
    if ((map) == NULL || (buf) == NULL || (buf_len) == 0) { \
        __ret = -EINVAL; \
    } else { \
        mem_clear(buf, buf_len); \
        mem_clear(__detail, sizeof(__detail)); \
        DFD_STATUS_DETAIL_CFG_SET(__detail_cfg, (status), (normal_status), (map), (bit_mode)); \
        __detail_len = dfd_status_get_detail(&__detail_cfg, __detail, sizeof(__detail)); \
        if (__detail_len > 0) { \
            if ((extra_info) != NULL && (extra_info_len) > 0) { \
                __ret = snprintf((buf), (buf_len), "%d %s%.*s\n", (status), __detail, \
                                (int)(extra_info_len), (char *)(extra_info)); \
            } else { \
                __ret = snprintf((buf), (buf_len), "%d %s\n", (status), __detail); \
            } \
        } else { \
            __ret = snprintf((buf), (buf_len), "%d\n", (status)); \
        } \
        if (__ret >= (buf_len)) { \
            __ret = -ENOSPC; \
        } \
...
size_t dfd_status_get_detail(const dfd_status_detail_t *detail_cfg,
