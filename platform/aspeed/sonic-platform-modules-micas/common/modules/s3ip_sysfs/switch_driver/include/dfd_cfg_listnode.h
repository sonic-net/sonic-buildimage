...
#define LNODE_RV_NOMEM          (-3)    /* Node memory allocation failed */
...
typedef struct lnode_node_s {
    ...
    int debug_on;               /* use debug data flag */
    void *data;                 /* The actual data pointer */
    void *debug_data;           /* The debug data pointer */
    void *tmp_debug_data;       /* The tmp debug data pointer */
 ...
 * reset debug node
 * @root: Root node pointer
 * @debug_on: 1:enable all debug, 0  disable all debug_on
 */
