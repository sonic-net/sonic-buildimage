...
#define NVME_SENSOR_INDEX_OFFSET      (16)
#define INFO_INT_MAX_LEN              (32)
#define SENSOR_THRESHOLD_CNT_DEFALUT    1
...
typedef enum switch_rv_s {
    WB_SYSFS_RV_OK          = 0,
    WB_SYSFS_RV_ABSENT      = 2,
    WB_SYSFS_RV_UNSUPPORT   = 999,
} switch_rv_t;

typedef enum temp_status_e {
    TEMP_STATUS_OK                  = 0x00,
    TEMP_STATUS_MAJOR_WARNING       = 0x01,
    TEMP_STATUS_FATAL_WARNING       = 0x02,
    TEMP_STATUS_FETCH_ERROR         = 0xff,
} temp_status_t;

typedef enum vol_status_e {
    VOL_STATUS_OK                  = 0x00,
    VOL_STATUS_ABNORMAL            = 0x01,
} vol_status_t;

typedef enum pcie_status_e {
    PCIE_STATUS_OK                  = 0x00,
    PCIE_STATUS_ABNORMAL            = 0x01,
} pcie_status_t;

typedef enum disk_status_e {
    DISK_STATUS_OK                  = 0x00,
    DISK_STATUS_LINKDOWN            = 0x01,
    DISK_STATUS_SLOWDOWN            = 0x02,
    DISK_STATUS_CE                  = 0x04,
    DISK_STATUS_UE                  = 0x08,
} disk_status_t;
...
typedef enum avs_bus_status_e {
    AVS_BUS_STATUS_OK                  = 0x00,
    AVS_BUS_STATUS_ABNORMAL            = 0x01,
} avs_bus_status_t;

typedef enum avs_circuit_status_e {
    AVS_CIRCUIT_STATUS_OK                  = 0x00,
    AVS_CIRCUIT_STATUS_ABNORMAL            = 0x01,
} avs_circuit_status_t;
