...
static const dfd_status_desc_map_t dfd_cpld_selftest_status_map[] = {
    {1, "Selftest_Error"},
};
#endif

#define CPLD_BOOT_VIEW_STATUS_PRIMARY_INTERNAL_FLASH    (0x01)
#define CPLD_BOOT_VIEW_STATUS_BACKUP_EXTERNAL_FLASH     (0x02)
#define CPLD_BOOT_VIEW_STATUS_PRIMARY_EXTERNAL_FLASH    (0x11)
#define CPLD_BOOT_VIEW_STATUS_BACKUP_INTERNAL_FLASH     (0x12)
#define CPLD_BOOT_VIEW_PRIMARY                         (0)
#define CPLD_BOOT_VIEW_BACKUP                          (1)
...
dfd_sysfs_func_map_t cpld_func_table[DFD_CPLD_MAX_E] = {
    ...
    [DFD_CPLD_SELTEST_STATUS_E] = {dfd_get_cpld_selftest_status, NULL},
    [DFD_CPLD_BOOT_VIEW_E] = {dfd_get_cpld_boot_view, NULL},
};
...
/**
 * dfd_get_cpld_selftest_status - Obtain the CPLD selftest status
 * @main_dev_id: Motherboard :0 Subcard :5
 * @index:The number of the CPLD starts from 0
 * @buf: Receive buf
 * @count: Accept the buf length
 ...
 *       : Failed: A negative value is returned
 */
ssize_t dfd_get_cpld_selftest_status(unsigned int main_dev_id, unsigned int cpld_index, char *buf, size_t count)
{
    uint64_t key;
    int selftest_status;
    int ret;
    ...
    mem_clear(buf, count);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_CPLD_SELFTEST_STATUS, main_dev_id, cpld_index);

    ret = dfd_info_get_int(key, &selftest_status, NULL);
    if (ret < 0) {
        DBG_CPLD_DEBUG(DBG_ERROR, "failed to get selftest_status, main_dev_id: %u, cpld_index %u, key_name: %s, ret: %d\n",
            main_dev_id, cpld_index, key_to_name(DFD_CFG_ITEM_CPLD_SELFTEST_STATUS), ret);
        return -DFD_RV_DEV_NOTSUPPORT;
    }

#if PRODUCT_STRATEGY == PRODUCT_STRATEGY_2
    mem_clear(detail, sizeof(detail));
    DFD_STATUS_DETAIL_CFG_SET(detail_cfg, selftest_status, 0, dfd_cpld_selftest_status_map, false);
...
/**
 * dfd_get_cpld_boot_view - Obtain the CPLD boot view
 * @main_dev_id: Motherboard :0 Subcard :5
 * @cpld_index:The number of the CPLD starts from 0
 * @buf: Receive buf
 * @count: Accept the buf length
 ...
 *       : Failed: A negative value is returned
 */
ssize_t dfd_get_cpld_boot_view(unsigned int main_dev_id, unsigned int cpld_index, char *buf, size_t count)
{
    uint64_t key;
    int ret;
    int boot_status;
    ...
    mem_clear(buf, count);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_CPLD_BOOT_VIEW, main_dev_id, cpld_index);
    ret = dfd_info_get_int(key, &boot_status, NULL);
    if (ret < 0) {
        DBG_CPLD_DEBUG(DBG_ERROR, "failed to get cpld boot view status, main_dev_id: %u, cpld_index %u, key_name: %s, ret: %d\n",
            main_dev_id, cpld_index, key_to_name(DFD_CFG_ITEM_CPLD_BOOT_VIEW), ret);
        return ret;
    }

    switch (boot_status) {
    case CPLD_BOOT_VIEW_STATUS_PRIMARY_INTERNAL_FLASH:
    case CPLD_BOOT_VIEW_STATUS_PRIMARY_EXTERNAL_FLASH:
        boot_view = CPLD_BOOT_VIEW_PRIMARY;
        break;
    case CPLD_BOOT_VIEW_STATUS_BACKUP_EXTERNAL_FLASH:
    case CPLD_BOOT_VIEW_STATUS_BACKUP_INTERNAL_FLASH:
        boot_view = CPLD_BOOT_VIEW_BACKUP;
        break;
    default:
        DBG_CPLD_DEBUG(DBG_ERROR, "invalid cpld boot view status, main_dev_id: %u, cpld_index %u, status: 0x%x\n",
            main_dev_id, cpld_index, boot_status);
        return -DFD_RV_INVALID_VALUE;
    }

    DBG_CPLD_DEBUG(DBG_VERBOSE, "main_dev_id: %u, cpld%u boot status: 0x%x, boot_view: %d\n",
        main_dev_id, cpld_index, boot_status, boot_view);
    return (ssize_t)snprintf(buf, count, "%d\n", boot_view);
}
