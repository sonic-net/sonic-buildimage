...
#define PSU_PMBUS_BASE_PATH           "/sys/bus/i2c/devices/%u-%04x/"
#define PSU_MFR_REVISION_PATH         "/sys/bus/i2c/devices/%u-%04x/mfr_revision"
#define PSU_PMBUS_INFO_PATH           "/sys/bus/i2c/devices/%u-%04x/pmbus_info"
#define PSU_CLEAR_BLACKBOX_PATH       "/sys/bus/i2c/devices/%u-%04x/clear_blackbox"
#define PSU_DFX_INFO_PATH             "/proc/psu/%u-%04x/dfx_info"
#define PSU_BLACKBOX_INFO_PATH        "/proc/psu/%u-%04x/blackbox_info"
...
typedef enum psu_fault_e {
    PSU_FAULT_NO_ERR        = 0x0,      /* PSU no fault */
    PSU_FAULT_IN_ABSENT     = 0x1,      /* PSU present but no input */
    PSU_FAULT_OVER_TEMP     = 0x2,      /* PSU over temperature */
    PSU_FAULT_FAN_ERR       = 0x4,      /* PSU fan error */
    PSU_FAULT_VI_ERR        = 0x8,      /* PSU voltage/current error */
    PSU_FAULT_I2C_ERR       = 0x10,     /* PSU I2C fault */
    PSU_FAULT_OUT_ABSENT    = 0x20,     /* PSU present but no output */
} psu_fault_t;

#if PRODUCT_STRATEGY == PRODUCT_STRATEGY_2
static const dfd_status_desc_map_t dfd_psu_fault_status_map[] = {
    {PSU_FAULT_I2C_ERR,     "I2c_Error"},
    {PSU_FAULT_IN_ABSENT,   "Input_Error"},
    {PSU_FAULT_OUT_ABSENT,  "Output_Error"},
    {PSU_FAULT_OVER_TEMP,   "OverTemp_Error"},
    {PSU_FAULT_FAN_ERR,     "Fan_Error"},
    {PSU_FAULT_VI_ERR,      "VI_Error"},
};
#endif
...
dfd_debug_data_key_map_t psu_status_dbg_key_table[DFD_PSU_STATUS_END] = {
    [DFD_PSU_PRESENT_STATUS] = {DFD_CFG_ITEM_PSU_STATUS, CFG_2INDEXES_2, CFG_INT_DATA},
};

dfd_debug_data_key_map_t psu_sensor_dbg_key_table[PSU_SENSOR_TYPE_END] = {
    [0 ... PSU_SENSOR_TYPE_END - 1] = {DFD_CFG_ITEM_HWMON_PSU, CFG_2INDEXES_2, CFG_INT_DATA}
};
...
static ssize_t dfd_get_psu_pmbus_path(unsigned int psu_index, char *sysfs_path, int size)
{
    ...
    if (psu_pmbus_addr == NULL) {
        DFD_PSU_DEBUG(DBG_ERROR, "get psu pmbus addr error, key: 0x%llx\n", key);
    ...
    snprintf(sysfs_path, size, PSU_PMBUS_BASE_PATH, (unsigned int)bus, (unsigned int)addr);
    return 0;
}
...
static int dfd_get_psu_hw_status_value(unsigned int psu_index)
{
    int ret;
    int status_word;
    int status;
...
ssize_t dfd_get_psu_hw_status_str(unsigned int psu_index, char *buf, size_t count)
{
    ...
    status = dfd_get_psu_hw_status_value(psu_index);
    if (status < 0) {
        DFD_PSU_DEBUG(DBG_ERROR, "get psu hw status error, status: %d, psu_index: %u\n", status, psu_index);
        return status;
    }
...
#if PRODUCT_STRATEGY == PRODUCT_STRATEGY_2
static ssize_t dfd_psu_format_status(char *buf, size_t count, int status)
{
    char detail[128];
    size_t len;
    ...
    DFD_STATUS_DETAIL_CFG_SET(detail_cfg, status, PSU_FAULT_NO_ERR, dfd_psu_fault_status_map, true);
    len = dfd_status_get_detail(&detail_cfg, detail, sizeof(detail));
...
ssize_t dfd_get_psu_status_str(unsigned int psu_index, char *buf, size_t count)
{
    ...
    int status;
    int input_vol;
    char input_vol_buf[INFO_BUF_MAX_LEN];
    ...
    if (ret < 0) {
        ...
        status |= PSU_FAULT_I2C_ERR;
        return dfd_psu_format_status(buf, count, status);
    }
    ...
    if (status_word < 0) {
        ...
        if (status_word & PSU_INPUT_FAULT) {
            mem_clear(input_vol_buf, sizeof(input_vol_buf));
            ret = dfd_get_psu_threshold_str(psu_index, PSU_IN_VOL, input_vol_buf, sizeof(input_vol_buf));
            if (ret < 0) {
                DFD_PSU_DEBUG(DBG_ERROR, "get psu%u input vol failed, ret: %d\n", psu_index, ret);
                return ret;
            }
            ret = kstrtoint(input_vol_buf, 0, &input_vol);
            if (ret < 0) {
                DFD_PSU_DEBUG(DBG_ERROR, "parse psu%u input vol failed, buf: %s, ret: %d\n",
                    psu_index, input_vol_buf, ret);
                return ret;
            }
            if (input_vol != 0) {
                status |= PSU_FAULT_VI_ERR;
            } else {
                status |= PSU_FAULT_IN_ABSENT;
            }
        }
        status = (status_word & PSU_TEMP_FAULT) ? (status | PSU_FAULT_OVER_TEMP) : status;
        status = (status_word & PSU_FAN_FAULT) ? (status | PSU_FAULT_FAN_ERR) : status;
        status = ((status_word & PSU_VOUT_FAULT) || (status_word & PSU_IOUT_FAULT)) ? (status | PSU_FAULT_VI_ERR) : status;
        status = (status_word & PSU_PG_FAULT) ? (status | PSU_FAULT_OUT_ABSENT) : status;
    }
    return dfd_psu_format_status(buf, count, status);
}
#else
...
static int dfd_psu_max_output_power(int power_type, char *psu_buf, int buf_len)
{
    ...
    mem_clear(psu_buf, buf_len);
    strscpy(psu_buf, p_power_max_output_power, buf_len);
    DFD_PSU_DEBUG(DBG_VERBOSE, "psu name %s match max output power %s\n", psu_buf, p_power_max_output_power);
    return DFD_RV_OK;
}
...
static ssize_t dfd_get_psu_e2_bus_addr(unsigned int psu_index, uint8_t cmd, char *buf, size_t count)
{
    uint64_t key;
...
static int dfd_get_psu_pmbus_bus_and_addr(unsigned int psu_index, uint16_t *bus, uint16_t *addr)
{
    uint64_t key;
    uint32_t *psu_pmbus_addr;

    if ((bus == NULL) || (addr == NULL)) {
        DFD_PSU_DEBUG(DBG_ERROR, "psu%u bus or addr is NULL, %p, %p\n", psu_index, bus, addr);
        return -EINVAL;
    }
...
static ssize_t dfd_get_psu_pmbus_bus_addr(unsigned int psu_index, uint8_t cmd, char *buf, size_t count)
{
    uint16_t bus, addr;
    int status;
    ...
    bus  = 0;
    addr = 0;
    status = dfd_get_psu_pmbus_bus_and_addr(psu_index, &bus, &addr);
    if (status) {
       return status;
    }

    if (cmd == DFD_DEV_INFO_I2C_PMBUS_BUS) {
        snprintf(buf, count, "%u\n", bus);
    } else {
        snprintf(buf, count, "0x%x\n", addr);
    }
...
static ssize_t dfd_get_psu_firmware_version(unsigned int psu_index, char *buf, size_t count)
{
    int status;
    uint16_t bus, addr;
    char path[DFD_SYSFS_PATH_MAX_LEN];
    ...
    mem_clear(path, sizeof(path));
    (void)snprintf(path, sizeof(path), PSU_MFR_REVISION_PATH, (unsigned int)bus, (unsigned int)addr);
    SWITCH_DEBUG(DBG_VERBOSE, "%s\n", path);

    return dfd_ko_read_file(path, 0, buf, count);
}

ssize_t dfd_get_psu_dfx_info(unsigned int psu_index, char *buf, size_t count)
{
    int status;
    uint16_t bus, addr;
    char path[DFD_SYSFS_PATH_MAX_LEN];
    ...
    if (buf == NULL) {
        DFD_PSU_DEBUG(DBG_ERROR, "params error.psu_index: %u.\n", psu_index);
        return -EINVAL;
    }
    if (count <= 0) {
        DFD_PSU_DEBUG(DBG_ERROR, "buf size error, count: %zu, psu index: %u\n",
    ...
    mem_clear(path, sizeof(path));
    snprintf(path, sizeof(path), PSU_DFX_INFO_PATH, (unsigned int)bus, (unsigned int)addr);
    DFD_PSU_DEBUG(DBG_VERBOSE, "psu_index: %u, dfx_info path: %s\n", psu_index, path);
...
ssize_t dfd_get_psu_info(unsigned int psu_index, uint8_t cmd, char *buf, size_t count)
{
    ...
    if (cmd == DFD_DEV_INFO_I2C_BUS || cmd == DFD_DEV_INFO_I2C_ADDR) {
        rv = dfd_get_psu_e2_bus_addr(psu_index, cmd, buf, count);
        return rv;
    }

    if (cmd == DFD_DEV_INFO_I2C_PMBUS_BUS || cmd == DFD_DEV_INFO_I2C_PMBUS_ADDR) {
        rv = dfd_get_psu_pmbus_bus_addr(psu_index, cmd, buf, count);
        return rv;
    }

    if (cmd == DFD_DEV_INFO_PSU_FW_VER) {
        rv = dfd_get_psu_firmware_version(psu_index, buf, count);
        return rv;
    }

    if (cmd == DFD_DEV_INFO_PSU_DFX_INFO) {
        rv = dfd_get_psu_dfx_info(psu_index, buf, count);
        return rv;
    }
    ...
    if (cmd == DFD_DEV_INFO_TYPE_PART_NAME) {
    ...
    } else if (cmd == DFD_DEV_INFO_PSU_MATCH_CHECK) {
        rv = dfd_get_psu_type(psu_index, i2c_dev, &power_type, sysfs_name, fru_mode);
        if (rv == (-DFD_RV_NO_NODE)) {
            snprintf(psu_buf, sizeof(psu_buf), PSU_NOT_MATCH_STR);
            goto out;
        } else if (rv < 0) {
            DFD_PSU_DEBUG(DBG_ERROR, "psu get type error, rv: %d\n", rv);
            return rv;
        ...
        if (rv == (-DFD_RV_DEV_NOTSUPPORT)) {
            snprintf(psu_buf, sizeof(psu_buf), PSU_NOT_MATCH_STR);
        } else if (rv == DFD_RV_OK) {
            snprintf(psu_buf, sizeof(psu_buf), PSU_MATCH_STR);
        } else {
            DFD_PSU_DEBUG(DBG_ERROR, "psu name decode error, power_type[0x%x] rv: %d\n",
                power_type, rv);
            return rv;
...
ssize_t dfd_get_psu_blackbox(unsigned int psu_index, char *buf, size_t count)
{
    int status;
    uint16_t bus, addr;
    char blackbox_path[DFD_SYSFS_PATH_MAX_LEN];
    ...
    mem_clear(blackbox_path, sizeof(blackbox_path));
    snprintf(blackbox_path, sizeof(blackbox_path), PSU_BLACKBOX_INFO_PATH, (unsigned int)bus, (unsigned int)addr);
...
ssize_t dfd_get_psu_pmbus(unsigned int psu_index, char *buf, size_t count)
{
    ...
    ssize_t rd_len;
    uint16_t bus, addr;
    char pmbus_info_path[DFD_SYSFS_PATH_MAX_LEN];
    ...
    mem_clear(pmbus_info_path, sizeof(pmbus_info_path));
    snprintf(pmbus_info_path, sizeof(pmbus_info_path), PSU_PMBUS_INFO_PATH, (unsigned int)bus, (unsigned int)addr);
...
int dfd_clear_psu_blackbox(unsigned int psu_index, uint8_t value)
{
    ...
    int ret;
    uint16_t bus, addr;
    char clear_blackbox_info_path[DFD_SYSFS_PATH_MAX_LEN];
    ...
    mem_clear(clear_blackbox_info_path, sizeof(clear_blackbox_info_path));
    snprintf(clear_blackbox_info_path, sizeof(clear_blackbox_info_path), PSU_CLEAR_BLACKBOX_PATH, (unsigned int)bus, (unsigned int)addr);
...
int dfd_set_psu_off_func(unsigned int psu_index, uint8_t value)
{
    uint64_t key;
    int ret;
    char *psu_off_info_path;
    uint8_t wr_buf[INFO_INT_MAX_LEN];

    /* get current step cfg */
    key = DFD_CFG_KEY(DFD_CFG_ITEM_PSU_OFF, psu_index, 0);
    psu_off_info_path = dfd_ko_cfg_get_item(key);
    if (psu_off_info_path == NULL) {
        DFD_PSU_DEBUG(DBG_ERROR, "get psu_off_info_path fail, key_name=%s, psu_index=0x%x\n",
            key_to_name(DFD_CFG_ITEM_PSU_OFF), psu_index);
        return -DFD_RV_DEV_NOTSUPPORT;
    }

    DFD_PSU_DEBUG(DBG_VERBOSE, "psu_index: %u, psu off path: %s, write value: %u\n",
        psu_index, psu_off_info_path, value);

    mem_clear(wr_buf, sizeof(wr_buf));
    snprintf(wr_buf, sizeof(wr_buf), "%u", value);
    ret = dfd_ko_write_file(psu_off_info_path, 0, wr_buf, strlen(wr_buf));
    if (ret < 0) {
        DFD_PSU_DEBUG(DBG_ERROR, "set_psu_off_func value error, key_name=%s, psu%u, value=%d, ret:%d\n",
            key_to_name(DFD_CFG_ITEM_PSU_OFF), psu_index, value, ret);
        return ret;
    }

    DFD_PSU_DEBUG(DBG_VERBOSE, "psu_index: %u, psu off success\n", psu_index);
