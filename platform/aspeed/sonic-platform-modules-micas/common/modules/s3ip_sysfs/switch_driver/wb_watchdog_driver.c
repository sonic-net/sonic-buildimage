...
ssize_t dfd_watchdog_set_timeout(int value)
{
    ...
    watchdog_id = dfd_get_watchdog_id(0);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_WATCHDOG_DEV, watchdog_id, WB_WDT_TYPE_TIMEOUT);
    ret = dfd_info_set_int(key, value);
    if (ret < 0) {
        DFD_WDT_DEBUG(DBG_ERROR, "set watchdog timeout status, key: 0x%x\n", key);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
    DFD_WDT_DEBUG(DBG_VERBOSE, "set watchdog timeout status ok, watchdog index:%u, enable:0x%x.\n",
        watchdog_id, value);
    return 0;
}
...
ssize_t dfd_watchdog_set_reset(int value)
{
    ...
    watchdog_id = dfd_get_watchdog_id(0);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_WATCHDOG_DEV, watchdog_id, WB_WDT_TYPE_RESET);
    ret = dfd_info_set_int(key, value);
    if (ret < 0) {
        DFD_WDT_DEBUG(DBG_ERROR, "set watchdog reset status, key: 0x%x\n", key);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
    DFD_WDT_DEBUG(DBG_VERBOSE, "set watchdog reset status ok, watchdog index:%u, reset:0x%x.\n",
        watchdog_id, value);
    return 0;
}
