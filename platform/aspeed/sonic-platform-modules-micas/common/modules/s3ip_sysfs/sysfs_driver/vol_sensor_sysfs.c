...
static ssize_t vol_attr_threshold_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    ...
    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->set_main_board_debug_vol_attr);
    
    vol_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);

    ret = g_vol_sensor_drv->set_main_board_debug_vol_attr(tmp_attr->type, vol_index, buf);
    if (ret < 0) {
        VOL_SENSOR_ERR("vol index: %u , type: %d , debug_value : %s, set fail.\n", vol_index, tmp_attr->type, buf);
        return -EIO;
    }
    VOL_SENSOR_DBG("vol index: %u , type: %d , debug_value : %s, set success.\n", vol_index, tmp_attr->type, buf);
    return count;
}
...
static ssize_t vol_attr_status_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int vol_index;
    struct switch_device_attribute  *tmp_attr;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->get_main_board_vol_status);

    vol_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);
    return g_vol_sensor_drv->get_main_board_vol_status(vol_index, buf, PAGE_SIZE);
}
...
static struct switch_attribute vol_status_attr = __ATTR(status, S_IRUGO, vol_attr_status_show, NULL);
static SWITCH_DEVICE_ATTR(max, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_MAX);
static SWITCH_DEVICE_ATTR(high, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_HIGH);
static SWITCH_DEVICE_ATTR(notice_high, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(notice_low, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(low, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_LOW);
static SWITCH_DEVICE_ATTR(min, S_IRUGO | S_IWUSR, vol_attr_threshold_show, vol_attr_threshold_dbg_store, WB_SENSOR_MIN);
...
static struct attribute *vol_sensor_attrs[] = {
    ...
    &vol_status_attr.attr,
    &switch_dev_attr_max.switch_attr.attr,
    &switch_dev_attr_high.switch_attr.attr,
    &switch_dev_attr_notice_high.switch_attr.attr,
    &switch_dev_attr_notice_low.switch_attr.attr,
