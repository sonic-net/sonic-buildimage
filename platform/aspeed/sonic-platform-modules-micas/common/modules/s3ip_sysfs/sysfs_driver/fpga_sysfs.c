...
static SWITCH_DEVICE_ATTR(selftest_status, S_IRUGO, fpga_attr_show, NULL, DFD_FPGA_SELTEST_STATUS_E);
static SWITCH_DEVICE_ATTR(boot_view, S_IRUGO, fpga_attr_show, NULL, DFD_FPGA_BOOT_VIEW_E);
...
static struct attribute *fpga_attrs[] = {
    ...
    &switch_dev_attr_selftest_status.switch_attr.attr,
    &switch_dev_attr_boot_view.switch_attr.attr,
    NULL,
};
...
/*******************************cram dir and attrs*******************************************/
static SWITCH_DEVICE_ATTR(cram_status, S_IRUGO, fpga_attr_show, NULL, DFD_FPGA_CRAM_STATUS_E);

static struct attribute *cram_attrs[] = {
    &switch_dev_attr_cram_status.switch_attr.attr,
    NULL,
};

static struct attribute_group cram_attr_group = {
    .attrs = cram_attrs,
};

/* remove single cram directory and attributes */
static void fpga_cram_remove_kobj_and_attrs(struct fpga_obj_s *curr_fpga)
{
    if (curr_fpga->cram.obj) {
        sysfs_remove_group(&curr_fpga->cram.obj->kobj, &cram_attr_group);
        switch_kobject_delete(&curr_fpga->cram.obj);
        FPGA_DBG("delete fpga%u cram dir and attrs success.\n", curr_fpga->obj->index);
    }
    return;
}

/* create single cram directory and attributes */
static int fpga_cram_create_kobj_and_attrs(struct fpga_obj_s *curr_fpga)
{
    /* create cram directory */
    curr_fpga->cram.obj = switch_kobject_create("cram", &curr_fpga->obj->kobj);
    if (!curr_fpga->cram.obj) {
        FPGA_ERR("create fpga%u cram object error!\n", curr_fpga->obj->index);
        return -ENOMEM;
    }

    curr_fpga->cram.obj->index = curr_fpga->obj->index;
    /* create attributes in cram directory */
    if (sysfs_create_group(&curr_fpga->cram.obj->kobj, &cram_attr_group) != 0) {
        FPGA_ERR("create fpga%u cram attrs error.\n", curr_fpga->obj->index);
        switch_kobject_delete(&curr_fpga->cram.obj);
        return -EBADRQC;
    }
    
    FPGA_DBG("create fpga%u cram dir and attrs success.\n", curr_fpga->obj->index);
    return 0;
}

/* create cram directory and attributes for all fpga devices */
static int fpga_cram_create(void)
{
    int fpga_num;
    unsigned int fpga_index, i;
    struct fpga_obj_s *curr_fpga;

    fpga_num = g_fpga.fpga_number;
    if (fpga_num <= 0) {
        FPGA_DBG("fpga number: %d, skip to create cram dir and attrs.\n", fpga_num);
        return 0;
    }

    for (fpga_index = 1; fpga_index <= fpga_num; fpga_index++) {
        curr_fpga = &g_fpga.fpga[fpga_index - 1];
        if (fpga_cram_create_kobj_and_attrs(curr_fpga) != 0) {
            goto error;
        }
    }
    return 0;
...
error:
    for (i = fpga_index - 1; i > 0; i--) {
        curr_fpga = &g_fpga.fpga[i - 1];
        fpga_cram_remove_kobj_and_attrs(curr_fpga);
    }
    return -EBADRQC;
}

/* delete cram directory and attributes for all fpga devices */
static void fpga_cram_remove(void)
{
    unsigned int fpga_index;
    struct fpga_obj_s *curr_fpga;
...
int s3ip_sysfs_fpga_drivers_register(struct s3ip_sysfs_fpga_drivers_s *drv)
{
    ...
    ret = fpga_cram_create();
    if (ret < 0) {
        FPGA_ERR("create fpga cram dir and attrs failed, ret: %d\n", ret);
        fpga_sub_remove();
        fpga_root_remove();
        g_fpga_drv = NULL;
        return ret;
