...
static ssize_t mem_led_status_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    check_p(g_sysled_drv);
    check_p(g_sysled_drv->get_mem_led_status);

    return g_sysled_drv->get_mem_led_status(buf, PAGE_SIZE);
}

static ssize_t mem_led_status_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char *buf, size_t count)
{
    int ret, value;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->set_mem_led_status);

    value = 0;
    ret = kstrtoint(buf, 0, &value);
    if (ret != 0) {
        SYSLED_ERR("invaild mem led status ret: %d, can't set mem led status\n", ret);
        SYSLED_ERR("invaild mem led status buf: %s\n", buf);
        return -EINVAL;
    }

    ret = g_sysled_drv->set_mem_led_status(value);
    if (ret < 0) {
        SYSLED_ERR("set mem led status %d faield, ret: %d\n", value, ret);
        return ret;
    }

    SYSLED_DBG("set mem led status %d success\n", value);
    return count;
}

static ssize_t sys_hot_led_status_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    check_p(g_sysled_drv);
    check_p(g_sysled_drv->get_sys_hot_led_status);

    return g_sysled_drv->get_sys_hot_led_status(buf, PAGE_SIZE);
}

static ssize_t sys_hot_led_status_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char *buf, size_t count)
{
    int ret, value;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->set_sys_hot_led_status);

    value = 0;
    ret = kstrtoint(buf, 0, &value);
    if (ret != 0) {
        SYSLED_ERR("invaild sys hot led status ret: %d, can't set sys hot led status\n", ret);
        SYSLED_ERR("invaild sys hot led status buf: %s\n", buf);
        return -EINVAL;
    }

    ret = g_sysled_drv->set_sys_hot_led_status(value);
    if (ret < 0) {
        SYSLED_ERR("set sys hot led status %d faield, ret: %d\n", value, ret);
        return ret;
    }

    SYSLED_DBG("set sys hot led status %d success\n", value);
    return count;
}
...
static struct switch_attribute mem_led_attr = __ATTR(mem_led_status, S_IRUGO | S_IWUSR, mem_led_status_show, mem_led_status_store);
static struct switch_attribute sys_hot_led_attr = __ATTR(sys_hot_led_status, S_IRUGO | S_IWUSR, sys_hot_led_status_show, sys_hot_led_status_store);
