...
static int psu_redundancy_num = -1;

#define PSU_REDUNDANCY_NUM_MAX          (64)
#define PSU_RESET_ENABLE_VALUE          (1)
#define PSU_OFF_ENABLE_VALUE            (1)
#define MAX_PSU_NUMBER  (32)  /* max psu number in device */
static DECLARE_BITMAP(g_psu_present_debug_bd, MAX_PSU_NUMBER);
/* simulate psu present condition. 0: pull out, 1: plug in*/
...
static ssize_t psu_temp_number_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    if(test_bit(index - 1, g_psu_present_debug_bd) == 0) {
        PSU_DBG("psu:%d open present debug, return not present.\n", index);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SWITCH_DEV_NO_SUPPORT);
    }

    temp_num = g_psu_drv->get_psu_temp_number(index);
    if (temp_num < 0) {
        if(temp_num == -WB_SYSFS_RV_ABSENT) {
            PSU_DBG("psu:%d get temp number absent.\n", index);
            return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SWITCH_DEV_NO_SUPPORT);
        }
        PSU_ERR("psu:%d get temp number failed, return value: %d.\n", index, temp_num);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SWITCH_DEV_ERROR);
    }
...
static ssize_t psu_alias_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    return (ssize_t)snprintf(buf, PAGE_SIZE, "psu%u\n", obj->index);
}
...
static ssize_t psu_present_status_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    if(test_bit(psu_index - 1, g_psu_present_debug_bd) == 0) {
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%d\n", PSU_STATUS_ABSENT);
    }
...
ssize_t psu_debug_present_get(char *buf)
{
    unsigned int i;

    memset(buf, 0, PAGE_SIZE);
    for (i = 0; i < g_psu.psu_number; i++)
    {
        buf[i] = test_bit(i, g_psu_present_debug_bd) ? '1' : '0';
    }
    buf[i] = '\n';
    PSU_DBG("psu get debug present.output=%s", buf);
    return (ssize_t)strlen(buf);
}

ssize_t psu_debug_present_set(const char* buf, size_t count)
{
    unsigned int i;

    for (i = 0; (i < count - 1) && i < g_psu.psu_number; i++) {
        if (buf[i] != '0' && buf[i] != '1') {
            PSU_ERR("param invalid, can not set debug present: %d.\n", buf[i]);
            return -EINVAL;
        }

        if (buf[i] - '0') {
            set_bit(i, g_psu_present_debug_bd);
        } else {
            clear_bit(i, g_psu_present_debug_bd);
        }
    }
...
static ssize_t psu_fan_ratio_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
                               ...
                               char *buf, loff_t offset, size_t count)
{
    ...
    PSU_DBG("start write psu%u eeprom data\n", psu_index);

    mem_clear(buf,  count);
    ret = g_psu_drv->read_psu_eeprom_data(psu_index, buf, offset, count);
    if (ret < 0) {
        if (ret == -WB_SYSFS_RV_ABSENT) {
            PSU_DBG("read psu%u eeprom data absent.\n", psu_index);
            return -ENOENT;
        }
        PSU_ERR("read psu%u eeprom data error, offset: 0x%llx, read len: %zu, ret: %zd.\n",
            psu_index, offset, count, ret);
        return -EIO;
    }
...
static ssize_t psu_eeprom_write(struct file *filp, struct kobject *kobj, struct bin_attribute *attr,
                   char *buf, loff_t offset, size_t count)
{
    struct switch_obj *psu_obj;
    ssize_t ret;
    ...
    check_p(g_psu_drv);
    check_p(g_psu_drv->write_psu_eeprom_data);
    ...
    PSU_DBG("start read psu%u eeprom data\n", psu_index);

    ret = g_psu_drv->write_psu_eeprom_data(psu_index, buf, offset, count);
    if (ret < 0) {
        if (ret == -WB_SYSFS_RV_ABSENT) {
            PSU_DBG("write psu%u eeprom data absent.\n", psu_index);
            return -ENOENT;
        }
        PSU_ERR("write psu%u eeprom data error, offset: 0x%llx, write len: %zu, ret: %zd.\n",
            psu_index, offset, count, ret);
        return -EIO;
    }

    PSU_DBG("write psu%u eeprom data success, offset:0x%llx, write len:%zu, really write len:%zd.\n",
        psu_index, offset, count, ret);
...
static ssize_t psu_off_store(struct switch_obj *obj, struct switch_attribute *attr,
                                  const char* buf, size_t count)
{
    int ret, value;
    unsigned int psu_index;
    ...
    if (value != PSU_OFF_ENABLE_VALUE) {
        PSU_ERR("Invaild value: %u, only support write 1 to off psu\n", value);
        return -EINVAL;
    }
    PSU_DBG("psu index: %u, off psu\n", psu_index);
    ret = g_psu_drv->set_psu_off(psu_index, 1);
    if (ret < 0) {
        PSU_ERR("clear psu%u blackbox information failed, ret: %d\n",
            psu_index, ret);
        return ret;
...
static ssize_t psu_status_attr_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;
    unsigned int psu_index;
    ...
    check_p(g_psu_drv);
    check_p(g_psu_drv->set_main_board_debug_psu_status_attr);
    
    psu_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);

    ret = g_psu_drv->set_main_board_debug_psu_status_attr(tmp_attr->type, psu_index, buf);
    if (ret < 0) {
        PSU_ERR("psu index: %u , type: %d , debug_value : %s, set fail.\n", psu_index, tmp_attr->type, buf);
        return -EIO;
    }
    PSU_DBG("psu index: %u , type: %d , debug_value : %s, set success.\n", psu_index, tmp_attr->type, buf);
    return count;
}

static ssize_t psu_sensor_attr_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;
    unsigned int psu_index;
    ...
    check_p(g_psu_drv);
    check_p(g_psu_drv->set_main_board_debug_psu_sensor_value_attr);

    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);

    psu_index = obj->index;
    ret = g_psu_drv->set_main_board_debug_psu_sensor_value_attr(tmp_attr->type, psu_index, buf);
    if (ret < 0) {
        PSU_ERR("psu index: %u , type: %d , debug_value : %s, set fail.\n", psu_index, tmp_attr->type, buf);
        return -EIO;
    }
...
static ssize_t psu_attr_threshold_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;
    unsigned int psu_index;
...
static ssize_t psu_redundancy_num_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    if (psu_redundancy_num == -1) {
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SWITCH_DEV_NO_SUPPORT);
    }
...
static ssize_t psu_redundancy_num_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;
    int val;
    ...
    if (val < 0 || val > PSU_REDUNDANCY_NUM_MAX) {
        PSU_ERR("param invalid, can not set psu redundancy num: %d.\n", val);
        return -EINVAL;
    }

    psu_redundancy_num = val;
    PSU_DBG("set psu_redundancy_num success. psu_redundancy_num = %d\n", psu_redundancy_num);
...
static struct switch_attribute redundancy_num_att = __ATTR(redundancy_num, S_IRUGO | S_IWUSR, psu_redundancy_num_show, psu_redundancy_num_store);
...
static struct switch_attribute psu_alias_attr = __ATTR(alias, S_IRUGO, psu_alias_show, NULL);
...
static struct switch_attribute psu_off_attr = __ATTR(psu_off, S_IWUSR, NULL, psu_off_store);

static SWITCH_DEVICE_ATTR(present, S_IRUGO | S_IWUSR, psu_present_status_show, psu_status_attr_dbg_store, DFD_PSU_PRESENT_STATUS);
...
static SWITCH_DEVICE_ATTR(e2_bus, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_I2C_BUS);
static SWITCH_DEVICE_ATTR(e2_addr, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_I2C_ADDR);
static SWITCH_DEVICE_ATTR(pmbus_bus, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_I2C_PMBUS_BUS);
static SWITCH_DEVICE_ATTR(pmbus_addr, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_I2C_PMBUS_ADDR);
static SWITCH_DEVICE_ATTR(firmware_version, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_PSU_FW_VER);
static SWITCH_DEVICE_ATTR(match_check, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_PSU_MATCH_CHECK);
static SWITCH_DEVICE_ATTR(dfx_info_path, S_IRUGO, psu_attr_show, NULL, DFD_DEV_INFO_PSU_DFX_INFO);

static SWITCH_DEVICE_ATTR(in_curr, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_IN_CURR);
static SWITCH_DEVICE_ATTR(in_vol, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_IN_VOL);
static SWITCH_DEVICE_ATTR(in_power, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_IN_POWER);
static SWITCH_DEVICE_ATTR(out_curr, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_OUT_CURR);
static SWITCH_DEVICE_ATTR(out_vol, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_OUT_VOL);
static SWITCH_DEVICE_ATTR(out_power, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_OUT_POWER);
static SWITCH_DEVICE_ATTR(fan_speed, S_IRUGO | S_IWUSR, psu_sensor_attr_show, psu_sensor_attr_dbg_store, PSU_FAN_SPEED);

static SWITCH_DEVICE_ATTR(in_vol_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_MAX);
static SWITCH_DEVICE_ATTR(in_vol_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_HIGH);
static SWITCH_DEVICE_ATTR(in_vol_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(in_vol_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(in_vol_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_LOW);
static SWITCH_DEVICE_ATTR(in_vol_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_VOL_MIN);
static SWITCH_DEVICE_ATTR(in_curr_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_MAX);
static SWITCH_DEVICE_ATTR(in_curr_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_HIGH);
static SWITCH_DEVICE_ATTR(in_curr_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(in_curr_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(in_curr_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_LOW);
static SWITCH_DEVICE_ATTR(in_curr_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_CURR_MIN);
static SWITCH_DEVICE_ATTR(in_power_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_MAX);
static SWITCH_DEVICE_ATTR(in_power_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_HIGH);
static SWITCH_DEVICE_ATTR(in_power_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(in_power_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(in_power_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_LOW);
static SWITCH_DEVICE_ATTR(in_power_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_IN_POWER_MIN);
static SWITCH_DEVICE_ATTR(out_vol_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_MAX);
static SWITCH_DEVICE_ATTR(out_vol_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_HIGH);
static SWITCH_DEVICE_ATTR(out_vol_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(out_vol_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(out_vol_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_LOW);
static SWITCH_DEVICE_ATTR(out_vol_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_VOL_MIN);
static SWITCH_DEVICE_ATTR(out_curr_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_MAX);
static SWITCH_DEVICE_ATTR(out_curr_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_HIGH);
static SWITCH_DEVICE_ATTR(out_curr_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(out_curr_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(out_curr_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_LOW);
static SWITCH_DEVICE_ATTR(out_curr_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_CURR_MIN);
static SWITCH_DEVICE_ATTR(out_power_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_MAX);
static SWITCH_DEVICE_ATTR(out_power_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_HIGH);
static SWITCH_DEVICE_ATTR(out_power_notice_high, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(out_power_notice_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(out_power_low, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_LOW);
static SWITCH_DEVICE_ATTR(out_power_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_OUT_POWER_MIN);
static SWITCH_DEVICE_ATTR(fan_speed_max, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_FAN_SPEED_MAX);
static SWITCH_DEVICE_ATTR(fan_speed_min, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_attr_threshold_dbg_store, PSU_FAN_SPEED_MIN);

static SWITCH_DEVICE_ATTR(in_curr_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_IN_CURR_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(in_vol_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_IN_VOL_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(in_power_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_IN_POWER_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(out_curr_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_OUT_CURR_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(out_vol_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_OUT_VOL_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(out_power_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_OUT_POWER_THRESHOLD_CNT);
static SWITCH_DEVICE_ATTR(fan_speed_threshold_cnt, S_IRUGO | S_IWUSR, psu_attr_threshold_show, psu_sensor_attr_dbg_store, PSU_FAN_SPEED_THRESHOLD_CNT);
...
static struct attribute *psu_attrs[] = {
    ...
    &switch_dev_attr_e2_bus.switch_attr.attr,
    &switch_dev_attr_e2_addr.switch_attr.attr,
    &switch_dev_attr_pmbus_bus.switch_attr.attr,
    &switch_dev_attr_pmbus_addr.switch_attr.attr,
    &switch_dev_attr_firmware_version.switch_attr.attr,
    &switch_dev_attr_match_check.switch_attr.attr,
    &switch_dev_attr_dfx_info_path.switch_attr.attr,
    ...
    &switch_dev_attr_in_vol_notice_high.switch_attr.attr,
    &switch_dev_attr_in_vol_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_in_curr_notice_high.switch_attr.attr,
    &switch_dev_attr_in_curr_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_in_power_notice_high.switch_attr.attr,
    &switch_dev_attr_in_power_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_out_vol_notice_high.switch_attr.attr,
    &switch_dev_attr_out_vol_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_out_curr_notice_high.switch_attr.attr,
    &switch_dev_attr_out_curr_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_out_power_notice_high.switch_attr.attr,
    &switch_dev_attr_out_power_notice_low.switch_attr.attr,
    ...
    &switch_dev_attr_in_curr_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_in_vol_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_in_power_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_out_curr_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_out_vol_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_out_power_threshold_cnt.switch_attr.attr,
    &switch_dev_attr_fan_speed_threshold_cnt.switch_attr.attr,
...
static SWITCH_DEVICE_ATTR(threshold_cnt, S_IRUGO, psu_temp_attr_show, NULL, WB_SENSOR_THRESHOLD_CNT);
...
static struct attribute *psu_temp_attrs[] = {
    ...
    &switch_dev_attr_threshold_cnt.switch_attr.attr,
    NULL,
};
...
static int psu_sub_single_create_eeprom_attrs(unsigned int index)
{
    ...
    curr_psu->bin.attr.mode = 0644;
    curr_psu->bin.read = psu_eeprom_read;
    curr_psu->bin.write = psu_eeprom_write;
...
int s3ip_sysfs_psu_drivers_register(struct s3ip_sysfs_psu_drivers_s *drv)
{
    ...
    bitmap_fill(g_psu_present_debug_bd, MAX_PSU_NUMBER);
...
EXPORT_SYMBOL(psu_debug_present_get);
EXPORT_SYMBOL(psu_debug_present_set);
