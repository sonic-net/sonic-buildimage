...
static ssize_t bsp_version_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    return (ssize_t)snprintf(buf, PAGE_SIZE, "%d\n", BSP_VERSION);
}

static struct switch_attribute bsp_version_att = __ATTR(bsp_version, S_IRUGO, bsp_version_show, NULL);

static struct attribute *bsp_version_attrs[] = {
    &bsp_version_att.attr,
    NULL,
};

static struct attribute_group bsp_version_attr_group = {
    .attrs = bsp_version_attrs,
};
...
static int __init switch_init(void)
{
    ...
    switch_kset = kset_create_and_add(S3IP_SYSFS_NAME, NULL, NULL);
    if (!switch_kset) {
    ...
    ret = sysfs_create_group(&switch_kset->kobj, &bsp_version_attr_group);
    if (ret != 0) {
        s3ip_debug_root_remove();
        kset_unregister(switch_kset);
        SWITCH_ERR("create bsp version error, ret: %d. \n", ret);
        return ret;
    }
...
{
    if (switch_kset) {
        sysfs_remove_group(&switch_kset->kobj, &bsp_version_attr_group);
        kset_unregister(switch_kset);
    }
    s3ip_debug_root_remove();
