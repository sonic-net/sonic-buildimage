...
static ssize_t temp_attr_threshold_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    ...
    check_p(g_temp_sensor_drv);
    check_p(g_temp_sensor_drv->set_main_board_debug_temp_attr);
    
    temp_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);

    ret = g_temp_sensor_drv->set_main_board_debug_temp_attr(tmp_attr->type, temp_index, buf);
    if (ret < 0) {
        TEMP_SENSOR_ERR("temp index: %u , type: %d , debug_value : %s, set fail.\n", temp_index, tmp_attr->type, buf);
        return -EIO;
    }
    TEMP_SENSOR_DBG("temp index: %u , type: %d , debug_value : %s, set success.\n", temp_index, tmp_attr->type, buf);
    return count;
}
...
static ssize_t temp_attr_status_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int temp_index;
    struct switch_device_attribute  *tmp_attr;

    check_p(g_temp_sensor_drv);
    check_p(g_temp_sensor_drv->get_main_board_temp_status);

    temp_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);
    return g_temp_sensor_drv->get_main_board_temp_status(temp_index, buf, PAGE_SIZE);
}
...
static struct switch_attribute temp_status_attr = __ATTR(status, S_IRUGO, temp_attr_status_show, NULL);
...
static SWITCH_DEVICE_ATTR(max, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_MAX);
static SWITCH_DEVICE_ATTR(high, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_HIGH);
static SWITCH_DEVICE_ATTR(notice_high, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(notice_low, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(low, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_LOW);
static SWITCH_DEVICE_ATTR(min, S_IRUGO | S_IWUSR, temp_attr_show, temp_attr_threshold_dbg_store, WB_SENSOR_MIN);

static struct attribute *temp_sensor_attrs[] = {
    &temp_monitor_flag_attr.attr,
    &temp_status_attr.attr,
