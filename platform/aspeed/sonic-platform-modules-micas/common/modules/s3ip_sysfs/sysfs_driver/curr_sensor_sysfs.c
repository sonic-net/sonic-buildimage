...
static ssize_t curr_attr_threshold_dbg_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    ...
    check_p(g_curr_sensor_drv->set_main_board_debug_curr_attr);
    
    curr_index = obj->index;
    tmp_attr = to_switch_device_attr(attr);
    check_p(tmp_attr);

    ret = g_curr_sensor_drv->set_main_board_debug_curr_attr(tmp_attr->type, curr_index, buf);
    if (ret < 0) {
        CURR_SENSOR_ERR("curr index: %u , type: %d , debug_value : %s, set fail.\n", curr_index, tmp_attr->type, buf);
        return -EIO;
    }
    CURR_SENSOR_DBG("curr index: %u , type: %d , debug_value : %s, set success.\n", curr_index, tmp_attr->type, buf);
    return count;
}
...
static SWITCH_DEVICE_ATTR(max, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_MAX);
static SWITCH_DEVICE_ATTR(high, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_HIGH);
static SWITCH_DEVICE_ATTR(notice_high, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_NOTICE_HIGH);
static SWITCH_DEVICE_ATTR(notice_low, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_NOTICE_LOW);
static SWITCH_DEVICE_ATTR(low, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_LOW);
static SWITCH_DEVICE_ATTR(min, S_IRUGO | S_IWUSR, curr_attr_threshold_show, curr_attr_threshold_dbg_store, WB_SENSOR_MIN);
