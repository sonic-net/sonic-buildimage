...
#define REBOOT_TYPE_PATH            "/tmp/sonic/.reboot/.reboot_type"
#define REBOOT_CAUSE_NAME_LEN       (8)
...
static ssize_t cpld_reboot_cause_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    ret = cpld_file_read(REBOOT_TYPE_PATH, reboot_cause_buf, sizeof(reboot_cause_buf) - 1);
    if (ret < 0) {
        CPLD_ERR("read file %s error, ret:%d\n", REBOOT_TYPE_PATH, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SWITCH_DEV_ERROR);
    }

    CPLD_DBG("read reboot cause:%s", reboot_cause_buf);

    return (ssize_t)snprintf(buf, PAGE_SIZE, "%s", reboot_cause_buf);
}
...
static SWITCH_DEVICE_ATTR(selftest_status, S_IRUGO, cpld_attr_show, NULL, DFD_CPLD_SELTEST_STATUS_E);
static SWITCH_DEVICE_ATTR(boot_view, S_IRUGO, cpld_attr_show, NULL, DFD_CPLD_BOOT_VIEW_E);
