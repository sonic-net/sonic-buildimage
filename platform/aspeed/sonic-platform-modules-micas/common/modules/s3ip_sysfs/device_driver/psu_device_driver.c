...
/********************************************psu**********************************************/
static ssize_t wb_set_main_board_debug_psu_status_attr(unsigned int type, unsigned int psu_index, const char *value)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_main_board_debug_psu_status_attr);

    ret = g_drv->set_main_board_debug_psu_status_attr(type, psu_index, value);
    return ret;
}

static ssize_t wb_set_main_board_debug_psu_sensor_value_attr(unsigned int type, unsigned int psu_index, const char *value)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_main_board_debug_psu_sensor_value_attr);

    ret = g_drv->set_main_board_debug_psu_sensor_value_attr(type, psu_index, value);
    return ret;
}
...
/*
 * wb_set_psu_off - Used to off psu
 * @psu_index: start with 1
 * @value: 1
 *
 * This function returns 0 on success,
 * otherwise it returns a negative value on failed.
 */
static int wb_set_psu_off(unsigned int psu_index, uint8_t value)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->set_psu_off);

    ret = g_drv->set_psu_off(psu_index, value);
    return ret;
}

/*
 * wb_write_psu_eeprom_data - Used to write eeprom data
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_write_psu_eeprom_data(unsigned int psu_index, char *buf, loff_t offset,
                   size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->write_psu_eeprom_data);

    ret = g_drv->write_psu_eeprom_data(psu_index, buf, offset, count);
    return ret;
}
/****************************************end of psu*******************************************/
...
static struct s3ip_sysfs_psu_drivers_s drivers = {
    ...
    .set_psu_off = wb_set_psu_off,
    .write_psu_eeprom_data = wb_write_psu_eeprom_data,
    .set_main_board_debug_psu_status_attr = wb_set_main_board_debug_psu_status_attr,
    .set_main_board_debug_psu_sensor_value_attr = wb_set_main_board_debug_psu_sensor_value_attr,
};
