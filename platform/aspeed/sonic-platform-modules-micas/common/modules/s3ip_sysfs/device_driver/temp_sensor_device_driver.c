...
/*
 * wb_set_main_board_debug_temp_attr - Used to set the debug attr of temperature sensor
 * @type: threshold type
 * @temp_index: start with 1
 * @value: the value to be set, the value is integer with millidegree Celsius
 *
 * This function returns 0 on success, otherwise it returns a negative value on failed.
 */
static ssize_t wb_set_main_board_debug_temp_attr(unsigned int type, unsigned int temp_index, const char *value)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_main_board_debug_temp_attr);

    ret = g_drv->set_main_board_debug_temp_attr(type, temp_index, value);
    return ret;
}
...
/*
 * wb_get_main_board_temp_status - Used to get status of the sensor
 * filled the value to buf, the value is hex with 0x0/0x1/0x2/0xff/NA
 * @temp_index: start with 1
 * @buf: Data receiving buffer
 * @count: length of buf
 *
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_get_main_board_temp_status(unsigned int temp_index, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_main_board_temp_status);

    ret = g_drv->get_main_board_temp_status(temp_index, buf, count);
    return ret;
}
...
static struct s3ip_sysfs_temp_sensor_drivers_s drivers = {
    ...
    .get_main_board_temp_status = wb_get_main_board_temp_status,
    .set_main_board_debug_temp_attr = wb_set_main_board_debug_temp_attr,
};
