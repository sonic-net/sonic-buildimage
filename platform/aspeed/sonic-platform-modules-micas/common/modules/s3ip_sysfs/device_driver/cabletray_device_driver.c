...
/*
 * wb_get_cabletray_attr - Used to get cabletray attr,
 ...
 */
static ssize_t wb_get_cabletray_attr(unsigned int cabletray_index, unsigned int type, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_cabletray_attr);

    ret = g_drv->get_cabletray_attr(cabletray_index, type, buf, count);
    return ret;
}
...
static struct s3ip_sysfs_cabletray_drivers_s drivers = {
    ...
    .get_cabletray_attr = wb_get_cabletray_attr,
