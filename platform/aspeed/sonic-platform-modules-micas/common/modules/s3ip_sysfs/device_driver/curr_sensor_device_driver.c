...
static ssize_t wb_set_main_board_debug_curr_attr(unsigned int type, unsigned int curr_index, const char *value)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_main_board_debug_curr_attr);

    ret = g_drv->set_main_board_debug_curr_attr(type, curr_index, value);
    return ret;
}
...
static struct s3ip_sysfs_curr_sensor_drivers_s drivers = {
    ...
    .set_main_board_debug_curr_attr = wb_set_main_board_debug_curr_attr,
};
