...
/*
 * wb_get_slot_card_type - Used to get slot card type,
 * @slot_index: start with 1
 * @buf: Data receiving buffer
 * @count: length of buf
 *
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_get_slot_card_type(unsigned int slot_index, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_slot_card_type);

    ret = g_drv->get_slot_card_type(slot_index, buf, count);
    return ret;
}

/*
 * wb_get_slot_present - Used to get slot present,
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_get_slot_present(unsigned int slot_index, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_slot_present);

    ret = g_drv->get_slot_present(slot_index, buf, count);
    return ret;
}
...
/*
 * wb_set_slot_power_ctrl_status - Used to set slot power control status,
 * filled the value to buf, slot status define as below:
 * 0: OFF
 * 1: ON
 *
 ...
 * otherwise it returns a negative value on failed.
 */
static int wb_set_slot_power_ctrl_status(unsigned int slot_index, int status)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_slot_power_ctrl_status);

    ret = g_drv->set_slot_power_ctrl_status(slot_index, status);
    return ret;
}

/*
 * wb_get_slot_power_ctrl_status - Used to get slot power control status,
 * filled the value to buf, slot status define as below:
 * 0: OFF
 * 1: ON
 *
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_get_slot_power_ctrl_status(unsigned int slot_index, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_slot_power_ctrl_status);

    ret = g_drv->get_slot_power_ctrl_status(slot_index, buf, count);
    return ret;
}
...
static struct s3ip_sysfs_slot_drivers_s drivers = {
    ...
    .get_slot_card_type = wb_get_slot_card_type,
    .get_slot_present = wb_get_slot_present,
    .get_slot_status = wb_get_slot_status,
    .get_slot_led_status = wb_get_slot_led_status,
    .set_slot_led_status = wb_set_slot_led_status,
    .get_slot_power_ctrl_status = wb_get_slot_power_ctrl_status,
    .set_slot_power_ctrl_status = wb_set_slot_power_ctrl_status,
