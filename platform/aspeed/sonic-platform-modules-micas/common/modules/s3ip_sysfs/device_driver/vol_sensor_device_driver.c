...
static ssize_t wb_set_main_board_debug_vol_attr(unsigned int type, unsigned int vol_index, const char *value)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_main_board_debug_vol_attr);

    ret = g_drv->set_main_board_debug_vol_attr(type, vol_index, value);
    return ret;
}
...
/*
 * wb_get_main_board_vol_status - Used to get status of the sensor
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t wb_get_main_board_vol_status(unsigned int vol_index, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_main_board_vol_status);

    ret = g_drv->get_main_board_vol_status(vol_index, buf, count);
    return ret;
}
...
static struct s3ip_sysfs_vol_sensor_drivers_s drivers = {
    ...
    .set_main_board_debug_vol_attr = wb_set_main_board_debug_vol_attr,
    .get_main_board_vol_status = wb_get_main_board_vol_status,
};
