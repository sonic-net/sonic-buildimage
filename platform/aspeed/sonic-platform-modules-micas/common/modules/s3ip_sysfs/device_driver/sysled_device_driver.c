...
static int wb_set_sys_led_status(int status)
{
    ...
    /* call cust logic here */
    ret = cust_set_sys_led(status);
#else
...
/* Similar to wb_get_sys_led_status */
static ssize_t wb_get_mem_led_status(char *buf, size_t count)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->get_mem_led_status);

    ret = g_drv->get_mem_led_status(buf, count);
    return ret;
}

/* Similar to wb_set_sys_led_status */
static int wb_set_mem_led_status(int status)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_mem_led_status);

    ret = g_drv->set_mem_led_status(status);
    return ret;
}

/* Similar to wb_get_sys_led_status */
static ssize_t wb_get_sys_hot_led_status(char *buf, size_t count)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->get_sys_hot_led_status);

    ret = g_drv->get_sys_hot_led_status(buf, count);
    return ret;
}

/* Similar to wb_set_sys_led_status */
static int wb_set_sys_hot_led_status(int status)
{
    int ret;

    check_p(g_drv);
    check_p(g_drv->set_sys_hot_led_status);

    ret = g_drv->set_sys_hot_led_status(status);
    return ret;
}
...
static struct s3ip_sysfs_sysled_drivers_s drivers = {
    ...
    .get_mem_led_status = wb_get_mem_led_status,
    .set_mem_led_status = wb_set_mem_led_status,
    .get_sys_hot_led_status = wb_get_sys_hot_led_status,
    .set_sys_hot_led_status = wb_set_sys_hot_led_status,
