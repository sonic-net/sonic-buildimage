...
static ssize_t wb_get_system_decode_string_value(unsigned int type, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_system_value_decode_string);

    ret = g_drv->get_system_value_decode_string(type, buf, count);
    return ret;
}
...
static ssize_t  wb_get_bmc_status(int bmc_sw_status, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_bmc_status);

    ret = g_drv->get_bmc_status(bmc_sw_status, buf, count);
    return ret;
}
...
static ssize_t wb_get_system_bmc_dualboot_wdt_status(char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_bmc_dualboot_wdt_status);

    ret = g_drv->get_bmc_dualboot_wdt_status(buf, count);
    return ret;
}

static ssize_t wb_set_system_bmc_dualboot_wdt(const char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->set_bmc_dualboot_wdt);

    ret = g_drv->set_bmc_dualboot_wdt(buf, count);
    return ret;
}
...
static ssize_t wb_set_mem_isolation_value(unsigned int type,  int value)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->set_mem_isolation_value);

    ret = g_drv->set_mem_isolation_value(type, value);
    return ret;
}

static ssize_t wb_get_mem_isolation_value(unsigned int type, char *buf, size_t count)
{
    ssize_t ret;

    check_p(g_drv);
    check_p(g_drv->get_mem_isolation_value);

    ret = g_drv->get_mem_isolation_value(type, buf, count);
    return ret;
}
...
static struct s3ip_sysfs_system_drivers_s drivers = {
    ...
    .get_system_value_decode_string = wb_get_system_decode_string_value,
    ...
    .get_bmc_dualboot_wdt_status = wb_get_system_bmc_dualboot_wdt_status,
    .set_bmc_dualboot_wdt = wb_set_system_bmc_dualboot_wdt,
    .get_my_slot_id = wb_get_my_slot_id,
    .get_bmc_status =  wb_get_bmc_status,
    .get_system_serial_number = wb_get_system_serial_number,
    .get_mem_isolation_value = wb_get_mem_isolation_value,
    .set_mem_isolation_value = wb_set_mem_isolation_value,
};
