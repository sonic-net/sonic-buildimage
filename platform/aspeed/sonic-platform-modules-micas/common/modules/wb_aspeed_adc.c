...
static void adjust_adc_raw_data(struct aspeed_adc_data *data, struct iio_chan_spec const *chan, int *val) 
{
    char vol_ratio[64];
    u32 vol_ratio_adc_chx;
    u64 vref_mv, dividend, divisor;
    u32 src_adc_val;
    int ret;

    /* get adc chx ratio, default 1000 */
    memset(vol_ratio, 0, sizeof(vol_ratio));
    snprintf(vol_ratio, sizeof(vol_ratio), "vol-ratio-adc-ch%d", chan->channel);
    ret = of_property_read_u32(data->dev->of_node, vol_ratio, &vol_ratio_adc_chx);
    if (ret != 0) {
        vol_ratio_adc_chx = 1000;
    }

    /* compute adc vol = vref * (src_adc_val + 1) / 2^adc_bits * (vol_ratio / 1000) */
    vref_mv = (u64)data->vref_mv;
    src_adc_val = *val + 1;
    dividend = vref_mv * src_adc_val * vol_ratio_adc_chx;
    divisor = 1000 * (1 << ASPEED_RESOLUTION_BITS);
    *val = div64_u64(dividend, divisor);

    dev_dbg(data->dev, "vref_mv:%llu, src_adc_val:%u, %s:%u, vol value: %d\n", 
             vref_mv, src_adc_val, vol_ratio, vol_ratio_adc_chx, *val);
    return;
}
			       ...
			       int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		adjust_adc_raw_data(data, chan, val);
		return IIO_VAL_INT;
