...
static int of_firmware_upgrade_config_init(struct device *dev, firmware_sysfs_t *sysfs_info)
{
    ...
    if (strcmp(sysfs_info->type, FIRMWARE_SYSFS_TYPE_SPI_LOGIC) == 0) {
        ret = of_property_read_u32(dev->of_node, "logic_dev_type", &sysfs_info->info.spi_logic_info.logic_dev_type);
        if (ret != 0) {
            FIRMWARE_DRIVER_DEBUG_VERBOSE("dts config logic_dev_type error, default xilinx FPGA.\n");
            sysfs_info->info.spi_logic_info.logic_dev_type = FIRMWARE_XILINX_FPGA_LOGIC_DEV;
        }
...
static int firmware_upgrade_config_init(struct device *dev, firmware_sysfs_t *sysfs_info)
{
    ...
    if (strcmp(sysfs_info->type, FIRMWARE_SYSFS_TYPE_SPI_LOGIC) == 0) {
        ...
        sysfs_info->info.spi_logic_info.logic_dev_type = sysfs_upg_device.logic_dev_type;
    } else if (strcmp(sysfs_info->type, FIRMWARE_SYSFS_TYPE_SYSFS) == 0) {
...
static void firmware_sysfs_remove(struct platform_device *pdev)
#else
static int firmware_sysfs_remove(struct platform_device *pdev)
#endif
{
    firmware_device_t *frm_dev;
