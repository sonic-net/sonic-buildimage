...
static long jbi_atol(char *buffer)
{
    long result = 0L;
    int index = 0;
...
static void jbi_ltoa(char *buffer, long number)
{
    int index = 0;
    int rev_index = 0;
    char reverse[32];
...
static void jbi_make_dword(unsigned char *buf, unsigned long num)
{
...
static unsigned long jbi_get_dword(unsigned char *buf)
{
    return
        (((unsigned long) buf[0]) |
