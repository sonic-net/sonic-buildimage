...
#include "firmware_app_up_bios.h"
...
typedef struct name_info_s {
    ...
    void *priv_data;                                 /* private data for firmware upgrade */
} name_info_t;
...
typedef enum firmware_app_cmd_type_s {
    FIRMWARE_APP_CMD_LEGACY = 0,
    FIRMWARE_APP_CMD_UPGRADE_BIOS,
    FIRMWARE_APP_CMD_UPGRADE_TEST = 0xff,
} firmware_app_cmd_type_t;

typedef struct firmware_app_cmd_info {
    firmware_app_cmd_type_t cmd_type;
    const char *cmd_name;
    int test_mode; /* 0: normal mode, 1: test mode */
    int argc;
    char **argv;
    firmware_app_cmd_bios_t bios;
} firmware_app_cmd_t;

typedef struct firmware_cmd_name_info_s {
    char firmware_app_cmd_name[FIRMWARE_NAME_LEN];
    int firmware_app_cmd_type;
} firmware_cmd_name_t;

typedef int  (*firmware_cmd_parse_fn)(firmware_app_cmd_t *cmd);
typedef int  (*firmware_cmd_exec_fn)(firmware_app_cmd_t *cmd);
typedef int  (*firmware_cmd_finish_fn)(firmware_app_cmd_t *cmd);
typedef void (*firmware_cmd_usage_fn)(void);

typedef struct firmware_cmd_entry {
    const char *name;
    firmware_app_cmd_type_t type;
    firmware_cmd_parse_fn parse;
    firmware_cmd_exec_fn exec;
    firmware_cmd_finish_fn finish;
    firmware_cmd_usage_fn usage;

} firmware_cmd_entry_t;
...
extern int firmware_upgrade_legacy(int argc, char *argv[]);

/* fw_up_bios cmd handlers */
extern int firmware_cmd_parse_fw_up_bios(firmware_app_cmd_t *cmd);
extern int firmware_cmd_exec_fw_up_bios(firmware_app_cmd_t *cmd);
extern int firmware_cmd_finish_fw_up_bios(firmware_app_cmd_t *cmd);
extern void firmware_cmd_usage_fw_up_bios(void);
