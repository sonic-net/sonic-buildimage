...
{
    if (info == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_error_type.\n");
        return ERR_FW_UPGRADE;
    }

    if((info->type <= FIRMWARE_UNDEF_TYPE) || (info->type > FIRMWARE_OTHER)) {
        dbg_print(is_debug_on, "Error: Invalid firmware type for firmware_error_type.\n");
        return ERR_FW_UPGRADE;
    }
...
int firmware_check_file_info(name_info_t *info, int main_type, int sub_type, int slot)
{
    ...
    if (info == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_check_file_info.\n");
        return ERR_FW_UPGRADE;
    }
...
static int firmware_get_dev_file_name(name_info_t *info, char *file_name, int len)
{
    ...
    if (info == NULL || file_name == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_get_dev_file_name.\n");
        return ERR_FW_UPGRADE;
    }
...
static int firmware_check_chip_verison(int fd, name_info_t *info)
{
    ...
    if (fd < 0 || info == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_check_chip_verison.\n");
        return ERR_FW_UPGRADE;
    }
...
static int firmware_get_file_size(char *file_name, uint32_t *size)
{
    ...
    if (file_name == NULL || size == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_get_file_size.\n");
...
static int firmware_get_file_info(char *file_name, uint8_t *buf, uint32_t size)
{
    ...
    if (file_name == NULL || buf == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_get_file_info.\n");
        return FIRMWARE_FAILED;
    }
...
int firmware_upgrade(char *file_name, name_info_t *info)
{
    ...
    if (file_name == NULL || info == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_upgrade.\n");
        return ERR_FW_UPGRADE;
    }
    ...
    /*TODO: 未来需要考虑分片设计，固件最大一个64MB, BMC内存冗余情况需要考究
     *方案建议：数据先分片读出计算crc,确保文件没顺坏，然后把文件名或者句柄传递给底层，
     *底层升级的时候再把数据分片读出写入，避免一次性读入内存占用过大
     */
...
int firmware_upgrade_test(char *file_name, name_info_t *info)
{
    ...
    if (file_name == NULL || info == NULL) {
        dbg_print(is_debug_on, "Error: Invalid parameter for firmware_upgrade_test.\n");
        return ERR_FW_UPGRADE;
    }
...
firmware_file_type_t firmware_upgrade_file_type_map(char *type_str)
{
    ...
    if (type_str == NULL) {
        dbg_print(is_debug_on, "Error: failed to get the file type string.\n");
...
static int firmware_upgrade_parse_kv(const char *key, const char *value, name_info_t *info)
{
    ...
    if (info == NULL) {
        dbg_print(is_debug_on, "Error: failed to get the upgrade file information.\n");
        return FIRMWARE_FAILED;
    }
...
int firmware_upgrade_read_header(char *file_name, name_info_t *info)
{
    ...
    if (file_name == NULL || info == NULL) {
        dbg_print(is_debug_on, "Error: failed to get the file name or upgrade file information.\n");
        return ERR_FW_UPGRADE;
    }
