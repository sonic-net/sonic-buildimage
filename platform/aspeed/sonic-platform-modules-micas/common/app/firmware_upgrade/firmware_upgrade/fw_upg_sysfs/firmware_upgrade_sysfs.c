            ...
            uint8_t *page, uint8_t *addr, uint8_t *val, uint8_t *mask)
{
    ...
    result = strtol(tmp_info, &endptr, 16);
    if (endptr == tmp_info || *endptr != '\0' || result < 0 || result > 255) {
        dbg_print(is_debug_on, "Invalid PAGE conversion: %s\n", tmp_info);
        return FIRMWARE_FAILED;
    }
    *page = (uint8_t)result;
    ...
    result = strtol(tmp_info, &endptr, 16);
    if (endptr == tmp_info || *endptr != '\0' || result < 0 || result > 255) {
        dbg_print(is_debug_on, "Invalid ADDR conversion: %s\n", tmp_info);
        return FIRMWARE_FAILED;
    }
    *addr = (uint8_t)result;
    ...
    result = strtol(tmp_info, &endptr, 16);
    if (endptr == tmp_info || *endptr != '\0' || result < 0 || result > 255) {
        dbg_print(is_debug_on, "Invalid VAL conversion: %s\n", tmp_info);
        return FIRMWARE_FAILED;
    }
    *val = (uint8_t)result;
    ...
    result = strtol(tmp_info, &endptr, 16);
    if (endptr == tmp_info || *endptr != '\0' || result < 0 || result > 255) {
        dbg_print(is_debug_on, "Invalid MASK conversion: %s\n", tmp_info);
        return FIRMWARE_FAILED;
    }
    *mask = (uint8_t)result;
