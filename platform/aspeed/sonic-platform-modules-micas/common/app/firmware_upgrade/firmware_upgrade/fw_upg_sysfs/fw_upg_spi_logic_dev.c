...
static int firmware_upgrade_get_spi_logic_info(int fd, firmware_spi_logic_upg_t *current_upg_priv)
{
    ...
    memcpy(current_upg_priv->dev_path, syfs_info.dev_name, FIRMWARE_LOGIC_DEV_NAME_LEN - 1);
    ...
    current_upg_priv->logic_info          = syfs_info;
...
int firmware_fpga_read_word(char *dev_name, uint32_t addr, uint32_t *val)
{
    int ret;
    uint32_t retry;
...
int firmware_fpga_read_buf(char *dev_name, uint32_t addr, uint8_t *buf, uint32_t rd_len)
{
    int ret;
    uint32_t retry;
...
int firmware_fpga_write_word(char *dev_name, uint32_t addr, uint32_t val)
{
    int ret;
    uint32_t retry, tmp;
...
int firmware_fpga_write_buf(char *dev_name, uint32_t addr, uint8_t *buf, uint32_t wr_len)
{
    int ret;
    uint32_t retry;
...
static int firmware_upgrade_exec(firmware_spi_logic_upg_t *current_upg_priv,
        unsigned char *buf, uint32_t size)
{
    int ret;

    /* Upgrade logic device */
    if (current_upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_LOGIC_DEV) {   /* Intel FPGA logic dev upgrade */
        ret = firmware_upgrade_do_spi_intel_logic(&current_upg_priv->logic_info, buf, size);
    } else if (current_upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_MAILBOX) {
        ret = firmware_upgrade_do_mailbox_func(&current_upg_priv->logic_info, buf, size);
    } else {
...
int firmware_upgrade_spi_logic_dev(int fd, uint8_t *buf, uint32_t size, name_info_t *info)
{
    ...
    /* Upgrade logic device */
    ret = firmware_upgrade_exec(&current_upg_priv, buf, size);
    if (ret < 0) {
...
int firmware_fpga_exec_test(firmware_spi_logic_upg_t *current_upg_priv)
{
    int ret;

    if (current_upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_LOGIC_DEV) {   /* Intel FPGA logic dev upgrade */
        ret = firmware_intel_fpga_upgrade_test(&current_upg_priv->logic_info);
    } else if (current_upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_MAILBOX) {
        ret = firmware_fpga_mailbox_upgrade_test_func(&current_upg_priv->logic_info);
    } else {
...
int firmware_upgrade_spi_logic_dev_test(int fd, name_info_t *info)
{
    ...
    /* Upgrade logic device */
    ret = firmware_fpga_exec_test(&current_upg_priv);
    if (ret < 0) {
        dbg_print(is_debug_on, "Error:firmware_upgrade_do_spi_logic failed ret %d.\n", ret);
        goto fail;
    }
...
static int firmware_upgreade_dump(firmware_spi_logic_upg_t *upg_priv,
        uint32_t offset, uint8_t *buf, uint32_t size)
{
    int ret;

    if (upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_LOGIC_DEV) {
        ret = firmware_upgreade_spi_intel_logic_dump(&upg_priv->logic_info, offset, buf, size);
    } else if (upg_priv->logic_info.logic_dev_type == FIRMWARE_INTEL_FPGA_MAILBOX) {
        ret = firmware_upgreade_mailbox_dump(&upg_priv->logic_info, offset, buf, size);
    } else {
        ret = firmware_upgreade_spi_logic_dump(upg_priv, offset, buf, size);
    }
...
static int firmware_fpga_dump_read(int fd, uint32_t offset, uint8_t *buf, uint32_t len)
{
    ...
    /* read logic device */
    ret = firmware_upgreade_dump(&current_upg_priv, offset, buf, len);
    if (ret < 0) {
        dbg_print(is_debug_on, "Error:firmware_upgrade_do_spi_logic failed ret %d.\n", ret);
        goto fail;
    }
...
int firmware_upgrade_spi_logic_dev_get_flash_id(char *dev_name, uint32_t *flash_id)
{
    ...
    if (current_upg_priv.logic_info.logic_dev_type != FIRMWARE_XILINX_FPGA_LOGIC_DEV) {
        dbg_print(is_debug_on, "Error:dev type %d unsupport get flash id\n", current_upg_priv.logic_info.logic_dev_type);
        goto free_fd;
    }
