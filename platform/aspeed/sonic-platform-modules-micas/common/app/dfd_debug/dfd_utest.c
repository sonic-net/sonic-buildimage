...
#define PECI_COMPLETION_CODE              (0x40)
#define PECI_COMPLETION_CODE_LEN          (1)

#define PECI_PKGCFG_RD_DATA_LEN           (4)
#define PECI_PKGCFG_RD_BUF_LEN            (PECI_PKGCFG_RD_DATA_LEN + PECI_COMPLETION_CODE_LEN)

#define PECI_PCICFGLOCAL_RD_DATA_LEN      (4)
#define PECI_PCICFGLOCAL_RD_BUF_LEN       (PECI_PCICFGLOCAL_RD_DATA_LEN + PECI_COMPLETION_CODE_LEN)

#define PECI_PCICONFIG_RD_DATA_LEN        (4)
#define PECI_PCICONFIG_RD_BUF_LEN         (PECI_PCICONFIG_RD_DATA_LEN + PECI_COMPLETION_CODE_LEN)

#define PECI_IAMSR_RD_DATA_LEN            (8)
#define PECI_IAMSR_RD_BUF_LEN             (PECI_IAMSR_RD_DATA_LEN + PECI_COMPLETION_CODE_LEN)
...
struct i2c_rdwr_ioctl_data
{
 struct i2c_msg *msgs;
 int nmsgs;
 /* nmsgs This number determines how many start signals there are, and for "single start timing", take 1 */
};

#define DFD_I2C_SHORT_ADDR_TYPE 0
#define DFD_I2C_RETRY_SLEEP_TIME        (10000)   /* 10ms */
#define DFD_I2C_RETRY_TIME              (50000 / DFD_I2C_RETRY_SLEEP_TIME)  /* 50ms attempt */
 ...
 * @i2c_name: indicates the name of i2c device
 * @dev_addr: indicates the device address
 * @offset_addr: indicates the offset address
 * @recv_buf: read buffer
 * @size: indicates the size of the read buf
 * @addr_type: 0: indicates single-byte offset access and 1:2byte offset access
 *
 * return: Returns 0 or positive on success and negative on failure
 */
...
/**
 * dfd_write_port_i2c_one_time - Write i2c
 * @i2c_name: indicates the name of i2c device
 * @dev_addr: indicates the device address
 * @offset_addr: indicates the offset address
                                    ...
                                    uint8_t *write_buf, int32_t size,int addr_type)
{
    ...
    /* To open a character device, open a channel */
    rv = 0;
...
static int dfd_process_mem(char *dev_name, char is_wr, char width, off_t offset, uint8_t *buf, int32_t size)
{
    ...
    /* Only 1/2/4 bytes can be read and written */
    if (size & (width - 1)) {
    ...
    if (base == MAP_FAILED) {
        printf("mmap offset 0x%" PRIx64 " failed error(%s).\n", (uint64_t)map_offset, strerror(errno));
        close(mfd);
        return -1;
    }
    printf("width %d map_offset 0x%" PRIx64 ", offset 0x%" PRIx64 ", mmap base %p, g_sys_page_size %d\n",
        width, (uint64_t)map_offset, (uint64_t)offset, base, g_sys_page_size);
                ...
                uint32_t offset_addr, uint8_t *recv_buf, int32_t rd_len)
{
    ...
    i = 0;
    /* The number of register address bits is determined by the address bit width */
...
/* The buf size is 5, guaranteed by the caller */
static int dfd_peci_pkgconfig_rd(u8 index, u16 parameter, u8 *buf)
{
    int ret;
    int fd;
    struct xfer_msg msg;

    if ((fd = open(PECI_DEVICE, O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO)) < 0) {
        printf("Error: Could not open file /dev/aspeed-peci: %s\n", strerror(errno));
        return -1;
    }
    ...
    msg.tx_len = 0x05;
    msg.rx_len = PECI_PKGCFG_RD_BUF_LEN;
    msg.tx_buf[0] = 0xa1; /* cmd */
    msg.tx_buf[1] = 0x0; /* Host ID[7:1] & Retry[0] */
    msg.tx_buf[2] = index;
    msg.tx_buf[3] = (u8)(parameter & 0xff);
    msg.tx_buf[4] = (u8)((parameter & 0xff00) >> 8);

    ret = ioctl(fd, AST_PECI_IOCXFER, &msg);
    if (ret < 0) {
        printf("Error: ioctl peci read error, ret = %d\n", ret);
        close(fd);
        return -1;
    }

    memcpy(buf, msg.rx_buf, PECI_PKGCFG_RD_BUF_LEN);
    close(fd);
    return 0;
}

int dfd_utest_peci_pkgcfg_rd(int argc, char* argv[])
{
    int ret;
    char *stopstring;
    u8 index;
    u16 parameter;
    u8 recv_buf[PECI_PKGCFG_RD_BUF_LEN];
    u8 data_buf[PECI_PKGCFG_RD_DATA_LEN];

    if (argc != 4) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_PKGCFG_RD);
        goto exit;
    }

    index = strtol(argv[2], &stopstring, 0);
    parameter = strtol(argv[3], &stopstring, 16);
    memset(recv_buf, 0, sizeof(recv_buf));
    dfd_utest_print_cmd(argc, argv);
    printf(":\n");

    /* get recv_buf. recv_buf include 'completion code + data'  */
    ret = dfd_peci_pkgconfig_rd(index, parameter, recv_buf);
    if (ret) {
        printf("failed ret %d\n", ret);
        goto exit;
    }

    /* completion code check. judge whether communicate normally */
    if (recv_buf[0] != PECI_COMPLETION_CODE) {
        printf("read failed. recv code:0x%x not equal to compiletion code\n", recv_buf[0]);
        goto exit;
    }

    /* print data buf */
    memcpy(data_buf, recv_buf + PECI_COMPLETION_CODE_LEN, PECI_PKGCFG_RD_DATA_LEN);
    dfd_utest_printf_reg(data_buf, PECI_PKGCFG_RD_DATA_LEN, 0);

exit:
    return DFD_RV_MODE_NOTSUPPORT;
}

static int dfd_peci_pkgconfig_wr(u8 index, u16 parameter, u32 wbuf, u8 *rbuf)
{
    int ret;
    int fd;
    struct xfer_msg msg;
    ...
    msg.tx_len = 0x0a;
    msg.rx_len = 0x01;
    msg.tx_buf[0] = 0xa5; /* cmd */
    msg.tx_buf[1] = 0x0; /* Host ID[7:1] & Retry[0] */
    msg.tx_buf[2] = index;
    msg.tx_buf[3] = (u8)(parameter & 0xff);
    msg.tx_buf[4] = (u8)((parameter & 0xff00) >> 8);
    msg.tx_buf[5] = (u8)(wbuf & 0xff);
    msg.tx_buf[6] = (u8)((wbuf & 0xff00) >> 8);
    msg.tx_buf[7] = (u8)((wbuf & 0xff0000) >> 16);
    msg.tx_buf[8] = (u8)((wbuf & 0xff000000) >> 24);
    msg.tx_buf[9] = 0x01; /* aw fcs */
    msg.fcs_en = 0x01;

    ret = ioctl(fd, AST_PECI_IOCXFER, &msg);
    if (ret < 0) {
        printf("Error: ioctl peci error, ret = %d\n", ret);
        close(fd);
        return -1;
    }

    memcpy(rbuf, msg.rx_buf, PECI_COMPLETION_CODE_LEN);
    close(fd);
    return 0;
}

int dfd_utest_peci_pkgcfg_wr(int argc, char* argv[])
{
    int ret;
    char *stopstring;
    u8 index;
    u16 parameter;
    u8 recv_buf[PECI_COMPLETION_CODE_LEN];
    u32 wr_buf;

    if (argc != 5) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_PKGCFG_WR);
        goto exit;
    }

    index = strtol(argv[2], &stopstring, 0);
    parameter = strtol(argv[3], &stopstring, 16);
    wr_buf = strtol(argv[4], &stopstring, 16);

    memset(recv_buf, 0, sizeof(recv_buf));
    dfd_utest_print_cmd(argc, argv);
    printf(":\n");

    /* write wr_buf and recv completion code */
    ret = dfd_peci_pkgconfig_wr(index, parameter, wr_buf, recv_buf);
    if (ret) {
        printf("failed ret %d\n", ret);
        goto exit;
    }
...
static int dfd_peci_pciconfiglocal_rd(u32 pci_cfg_address, u8 rd_len, u8 *buf)
{
    int ret;
    int fd;
    struct xfer_msg msg;
    ...
    msg.client_addr = 0x30;
    msg.tx_len = 0x05;
    msg.rx_len = rd_len + PECI_COMPLETION_CODE_LEN;
    msg.tx_buf[0] = 0xe1; /* cmd */
    msg.tx_buf[1] = 0x0; /* Host ID[7:1] & Retry[0] */
    msg.tx_buf[2] = (pci_cfg_address & 0xff);
    msg.tx_buf[3] = (pci_cfg_address & 0xff00) >> 8;
    msg.tx_buf[4] = (pci_cfg_address & 0xff0000) >> 16;
    ...
    memcpy(buf, msg.rx_buf, rd_len + PECI_COMPLETION_CODE_LEN);
    close(fd);
    return 0;
}

int dfd_utest_peci_pcicfglocal_rd(int argc, char* argv[])
{
    int ret;
    char *stopstring;
    u8 bus, function, rd_len;
    u16 device, reg;
    u8 recv_buf[PECI_PCICFGLOCAL_RD_BUF_LEN];
    u8 data_buf[PECI_PCICFGLOCAL_RD_DATA_LEN];
    u32 pci_cfg_address = 0;

    if (argc != 7) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_PCICFGLOCAL_RD);
        goto exit;
    }

    bus = strtol(argv[2], &stopstring, 0);
    device = strtol(argv[3], &stopstring, 0);
    function = strtol(argv[4], &stopstring, 16);
    reg = strtol(argv[5], &stopstring, 16);
    rd_len = strtol(argv[6], &stopstring, 10);

    if (bus > 0xf || device > 0x1f || function > 0x7 || reg > 0xfff) {
        printf("Param input invalid. bus support 4 bit, device support 5 bit, function support 3 bit, reg support 12 bit\n");
        goto exit;
    }

    if (rd_len != 1 && rd_len != 2 && rd_len != 4) {
        printf("rd_len:%u param input invalid. only support 1, 2, 4 byte\n", rd_len);
        goto exit;
    }

    /* PCI Configuration Address : bus:[23:20] device:[19:15] function:[14:12] register:[11:0] */
    pci_cfg_address = ((bus & 0xf) << 20) | ((device & 0x1f) << 15) | ((function & 0x7) << 12) | (reg & 0xfff);
    ...
    /* get completion code  */
    ret = dfd_peci_pciconfiglocal_rd(pci_cfg_address, rd_len, recv_buf);
    if (ret) {
        printf("failed read pci_config_local. ret:%d\n", ret);
        goto exit;
    }
    ...
    /* print data buf */
    memcpy(data_buf, recv_buf + PECI_COMPLETION_CODE_LEN, rd_len);
    dfd_utest_printf_reg(data_buf, rd_len, reg);

exit:
    return DFD_RV_MODE_NOTSUPPORT;
}

static int dfd_peci_pciconfig_rd(u32 pci_cfg_address, u8 rd_len, u8 *buf)
{
    int ret;
    int fd;
    struct xfer_msg msg;
    ...
    msg.tx_len = 0x06;
    msg.rx_len = rd_len + PECI_COMPLETION_CODE_LEN;
    msg.tx_buf[0] = 0x61; /* cmd */
    msg.tx_buf[1] = 0x0; /* Host ID[7:1] & Retry[0] */
    msg.tx_buf[2] = (pci_cfg_address & 0xff);
    msg.tx_buf[3] = (pci_cfg_address & 0xff00) >> 8;
    msg.tx_buf[4] = (pci_cfg_address & 0xff0000) >> 16;
    msg.tx_buf[5] = (pci_cfg_address & 0xff000000) >> 24;
...
int dfd_utest_peci_pciconfig_rd(int argc, char* argv[])
{
    int ret;
    char *stopstring;
    u8 function, rd_len;
    u16 device, reg, bus;
    u8 recv_buf[PECI_PCICONFIG_RD_BUF_LEN];
    u8 data_buf[PECI_PCICONFIG_RD_DATA_LEN];
    u32 pci_cfg_address = 0;

    if (argc != 7) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_PCICONFIG_RD);
        goto exit;
    }
    ...
    if (bus > 0xff || device > 0x1f || function > 0x7 || reg > 0xfff) {
        printf("Param input invalid. bus support 16 bit, device support 5 bit, function support 3 bit, reg support 12 bit\n");
        goto exit;
    }

    if (rd_len != 4) {
        printf("rd_len:%u param input invalid. only support read 4 byte\n", rd_len);
        goto exit;
    }

    /* PCI Configuration Address : bus:[27:20] device:[19:15] function:[14:12] register:[11:0] */
    pci_cfg_address = (bus & 0xff) << 20 | (device & 0x1f) << 15 | (function & 0x7) << 12 | (reg & 0xfff);
    ...
    /* get completion code  */
    ret = dfd_peci_pciconfig_rd(pci_cfg_address, rd_len, recv_buf);
    if (ret) {
        printf("failed read pci_config_local. ret:%d\n", ret);
        goto exit;
    }
...
int dfd_utest_peci_tool(int argc, char *argv[])
{
    return do_peci_cmd(argc, argv);
}

int dfd_utest_peci_pkgcfg_temp(int argc, char* argv[])
{
    int ret;
    u8 index;
    u16 parameter;
    ...
    if (argc != 2) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_PKGCFG_TEMP);
        goto exit;
    }
    ...
    tmp = ((((~(recv_buf[2])) & 0x7F) << 2) | ((~(recv_buf[1]) & 0xC0) >> 6)) + 1;
    if ((recv_buf[2]) & 0x80){
        temp = TMAX - tmp;
    } else {
        temp = TMAX + tmp;
...
static int dfd_peci_iamsr_rd(u8 processor_id, u16 parameter, u8 *buf)
{
    int ret;
    int fd;
    struct xfer_msg msg;
    ...
    msg.client_addr = 0x30;
    msg.tx_len = 0x05;
    msg.rx_len = PECI_IAMSR_RD_BUF_LEN;
    msg.tx_buf[0] = 0xb1; /* cmd */
    msg.tx_buf[1] = 0x0; /* Host ID[7:1] & Retry[0] */
    msg.tx_buf[2] = processor_id;
    msg.tx_buf[3] = (u8)(parameter & 0xff);
    msg.tx_buf[4] = (u8)((parameter & 0xff00) >> 8);
    ...
    memcpy(buf, msg.rx_buf, PECI_IAMSR_RD_BUF_LEN);
    close(fd);
    return 0;
}

int dfd_utest_peci_iamsr_rd(int argc, char* argv[])
{
    int ret;
    char *stopstring;
    u8 processor_id;
    u16 parameter;
    u8 recv_buf[PECI_IAMSR_RD_BUF_LEN];
    u8 data_buf[PECI_IAMSR_RD_DATA_LEN];

    if (argc != 4) {
        printf("Input invalid.\n");
        dfd_utest_printf_single_help(DFD_UTEST_ITEM_PECI_IAMSR_RD);
        goto exit;
    }

    processor_id = strtol(argv[2], &stopstring, 16);
    parameter = strtol(argv[3], &stopstring, 16);
    memset(recv_buf, 0, sizeof(recv_buf));
    dfd_utest_print_cmd(argc, argv);
    printf(":\n");
    ...
    /* get recv_buf. recv_buf include 'completion code + data'  */
    ret = dfd_peci_iamsr_rd(processor_id, parameter, recv_buf);
    if (ret) {
        printf("failed read iamsr, ret %d\n", ret);
        goto exit;
    }
    ...
    /* print data buf */
    memcpy(data_buf, recv_buf + PECI_COMPLETION_CODE_LEN, PECI_IAMSR_RD_DATA_LEN);
    dfd_utest_printf_reg(data_buf, PECI_IAMSR_RD_DATA_LEN, 0);
...
int dfd_utest_msr_rd(int argc, char* argv[])
{
    ...
    if (pread(fd, &data, sizeof(data), offset) != sizeof(data)) {
        fprintf(stderr, "rdmsr failed: CPU:%u offset:0x%" PRIx64 " read failed\n", cpu_index, offset);
        goto fail;
    }
