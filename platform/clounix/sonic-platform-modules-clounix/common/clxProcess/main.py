...
def process_adm1166_fault():
    ...
    for dev in adm1166_dev:
        ...
        adm1166_fault = _read_text(fault_position)

        if os.path.exists(fault_history):
            adm1166_history_fault = _read_text(fault_history)
            if adm1166_fault[0:2] != adm1166_history_fault[0:2]:
                fault_indx = indx
        
        _write_text(fault_history, adm1166_fault)
        indx = indx+1
		
        if fault_indx != 0:
            current_time = datetime.now().strftime("%a %b %d %H:%M:%S %Z %Y").strip()
            usr = "adm1166_" + str(fault_indx)
            msg = ADM1166_CAUSE_MSG.format(usr, current_time)
