...
void parse_cli(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        {
            case 's':
                snprintf(sysfs_path, sizeof(sysfs_path), "%s", optarg);
                break;
            case 'f':
...
static void parse_platform_info()
{
    ...
    if(count > 0)
    {
        size_t copy_len = (size_t)count - 1;
        if (copy_len >= sizeof(g_platform))
            copy_len = sizeof(g_platform) - 1;
        memcpy(g_platform, buffer, copy_len);
        g_platform[copy_len] = '\0';
    }
    pclose(fp); 
    return;
}
...
int parse_fpga_resource()
{
    ...
    char bar_attr[3][20];
    char resource[sizeof(sysfs_path) + sizeof("resource")];
    if (snprintf(resource, sizeof(resource), "%sresource", sysfs_path) >= sizeof(resource)) {
        printf("resource path too long: %s\n", sysfs_path);
        return -1;
    }
...
void stress_handler()
{
    ...
    do {
        ...
        char *endptr = NULL;
        loops_ul = strtoul(input, &endptr, 10);
        if (endptr == input) {
            ret = 0;
            continue;
        }
        loops = (uint32_t)loops_ul;
        ret = 1;
    } while (1 != ret);
    if(DEV_FPGA == bus){
