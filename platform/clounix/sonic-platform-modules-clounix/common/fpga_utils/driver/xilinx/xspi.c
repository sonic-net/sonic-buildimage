/******************************************************************************
* Copyright (C) 2001 - 2020 Clxinx, Inc.  All rights reserved.
...
int XSpi_Start(XSpi *InstancePtr)
{
	...
	Clx_AssertNonvoid(InstancePtr != NULL);
	Clx_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);
...
void XSpi_Reset(XSpi *InstancePtr)
{
	...
	Clx_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);
		  ...
		  u8 *RecvBufPtr, unsigned int ByteCount)
{
	 ...
	 */
	Clx_AssertNonvoid(InstancePtr != NULL);
	Clx_AssertNonvoid(SendBufPtr != NULL);
	Clx_AssertNonvoid(ByteCount > 0);
	Clx_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);
...
int read_write_flash(XSpi *SpiInstancePtr, u16 SpiDeviceId)
{
	 ...
	 */
	Status = XSpi_SetSlaveSelect(SpiInstancePtr, SpiInstancePtr->SlaveSelectMask);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	...
	//Status = SpiFlashGetID(SpiInstancePtr);
	if (Status != XST_SUCCESS) {
		xil_printf("Flash ID Failed\r\n");
	}
