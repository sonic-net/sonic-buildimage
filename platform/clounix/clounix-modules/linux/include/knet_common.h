...
typedef enum {
    NETIF_ADMIN_STATE_DOWN,
    NETIF_ADMIN_STATE_UP,
} clx_netif_admin_e;
...
struct clx_ioctl_port_map_cookie {
    uint32_t unit;
    uint32_t port; /* only support unit port and local port */
    uint32_t slice;
    uint32_t slice_port;
};
...
typedef enum { ACTION_NETDEV = 1, ACTION_NETLINK, ACTION_SDK, ACTION_DROP } clx_rx_action_e;
