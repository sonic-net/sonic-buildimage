...
dma_rx_tasklet_func(unsigned long data)
{
    ...
    uint32_t port = 0, reason, pop_idx, work_idx;
    struct dma_rx_packet *rx_packet = NULL;
    ...
    while (desc_processed++ < clx_dma_drv(unit)->ring_size) {
        ...
        ret = clx_netif_drv(unit)->get_pkt_dst(unit, rx_packet, &port, &reason);
        if (0 != ret) {
            kfree(rx_packet);
            continue;
        }

        rule = clx_netif_match_profile(unit, port, reason, rx_packet);
        if (!rule) {
        ...
        switch (rx_action) {
            case ACTION_NETDEV:
                ret = clx_netif_netdev_receive_skb(unit, rx_packet, port);
                if (0 != ret) {
                    dma_free_rx_packet(unit, rx_packet, true);
                }
                break;
            case ACTION_NETLINK:
                clx_netif_drv(unit)->parse_netlink_info(unit, rx_packet, &netlink_cookie);
                netlink_cookie.nl = &rule->netlink;
                netif_netlink_reveive_skb(unit, rx_packet, port, &netlink_cookie);
                break;
            case ACTION_SDK:
