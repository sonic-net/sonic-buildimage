...
namchabar_top_irq_mask(uint32_t unit)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CTL_CHAIN_INTR_MSK, &intr_mask,
                                              sizeof(uint32_t));
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFG_PDMA2PCIE_INTR_MASK_ALL, &intr_mask,
                                              sizeof(uint32_t));
...
namchabar_get_dma_irq_channel(uint32_t unit, uint32_t *channel_bmp)
{
    uint32_t intr_mask = 0x0;
    uint32_t channel = 0;
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, STA_PDMA_NORMAL_INTR, channel_bmp,
                                             sizeof(uint32_t));
...
nb_clear_dma_channel_irq(uint32_t unit, uint32_t channel)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFW_PDMA2PCIE_INTR_CLR, &intr_clr,
                                              sizeof(uint32_t));
    return 0;
}
...
nb_dma_channel_enable(uint32_t unit, uint32_t channel)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, CFG_PDMA_CH_ENABLE, &enable, sizeof(uint32_t));
    NB_SET_BITMAP(enable, 1 << channel);
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFG_PDMA_CH_ENABLE, &enable, sizeof(uint32_t));
    return 0;
}
...
nb_dma_channel_ring_base_set(uint32_t unit, uint32_t channel, clx_addr_t ring_base)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFG_PDMA_CHx_RING_BASE(channel),
...
{
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, CFG_PDMA_CHx_RING_BASE(channel),
                                             (uint32_t *)ring_base, sizeof(clx_addr_t));
    return 0;
}
...
nb_dma_channel_ring_size_set(uint32_t unit, uint32_t channel, uint32_t ring_size)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFG_PDMA_CHx_RING_SIZE(channel), &ring_size,
                                              sizeof(uint32_t));
    return 0;
}
...
{
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, CFG_PDMA_CHx_RING_SIZE(channel), ring_size,
                                             sizeof(uint32_t));
    return 0;
}
...
nb_dma_channel_work_idx_set(uint32_t unit, uint32_t channel, uint32_t work_idx)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFG_PDMA_CHx_DESC_WORK_IDX(channel), &work_idx,
                                              sizeof(uint32_t));
    return 0;
}
...
{
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, CFG_PDMA_CHx_DESC_WORK_IDX(channel), work_idx,
                                             sizeof(uint32_t));
    return 0;
}
...
{
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, STA_PDMA_CHx_DESC_POP_IDX(channel), pop_idx,
                                             sizeof(uint32_t));
    return 0;
}
...
{
    uint32_t reset;
    clx_misc_dev->clx_pci_dev[unit]->read_cb(unit, CFW_PDMA_CH_RESET, &reset, sizeof(uint32_t));
    NB_CLR_BITMAP(reset, 1 << channel);
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFW_PDMA_CH_RESET, &reset, sizeof(uint32_t));
...
nb_dma_channel_restart(uint32_t unit, uint32_t channel)
{
    ...
    clx_misc_dev->clx_pci_dev[unit]->write_cb(unit, CFW_PDMA_CH_RESTART, &restart,
                                              sizeof(uint32_t));
...
static int
nb_pph_prepare(uint32_t unit, uint32_t port, void *pph)
{
    ...
    ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
    ...
    ptr_pph->tc = ptr_priv->tc & 0x7; /* tc */
    ptr_pph->src_idx = clx_netif_drv(unit)->cpu_port;

    nb_pkt_pph_set_dst_idx(ptr_pph, port);
               ...
               uint32_t *reason)
{
    ...
    *port = ptr_pph->src_idx;
    if (*port > clx_netif_drv(unit)->ports_num_unit) {
        *port = CLX_NETIF_GET_PORT_DI(unit, ptr_pph->slice_id, ptr_pph->port_num);
        if (-1 == *port) {
            dbg_print(DBG_RX, "port is invlaid!!!,slice_id:%u port_num:%u\n", ptr_pph->slice_id,
                      ptr_pph->port_num);
            return -1;
        }
        dbg_print(DBG_RX, "get port:%u slice_id:%u port_num:%u\n", *port, ptr_pph->slice_id,
                  ptr_pph->port_num);
    }
    ...
    *reason = ptr_pph->cpu_reason;
    ...
    clx_netif_drv(unit)->cpu_reason_cnt[*reason].byte_cnts += rx_packet->packet_len - clx_dma_drv(unit)->dma_hdr_size;
...
nb_parse_netlink_info(const uint32_t unit, struct dma_rx_packet *rx_packet, void *ptr_cookie)
{
    ...
    if (-1 == port) {
        ...
        ptr_igr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
        if (!ptr_igr_net_dev) {
            return -EFAULT;
        }
        netlink_cookie->pkt.iifindex = ptr_igr_net_dev->ifindex;
        netlink_cookie->pkt.igr_port_si = port;
    }
    
    // obtain the egress netdev ifindex 
    port = nb_pkt_pph_get_dst_idx(ptr_pph);
    if (port >= CLX_NETIF_MAX_NUM) {
        netlink_cookie->pkt.eifindex = netlink_cookie->pkt.iifindex;
    } else {
        ptr_egr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
        if (!ptr_egr_net_dev) {
            netlink_cookie->pkt.eifindex = netlink_cookie->pkt.iifindex;
            return -EFAULT;
        }
    ...
    dbg_print(DBG_RX, "unit:%d, netlink_type:%d, psample_dir:%d, "
