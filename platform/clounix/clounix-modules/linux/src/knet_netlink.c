...
static int
netlink_get_header_len(struct netif_netlink *ptr_netlink,
                       ...
                       struct netlink_rx_cookie *ptr_cookies)
{
    if (ptr_cookies->netlink_type == NETLINK_RX_TYPE_SFLOW) {
        ...
        *msg_hdr_len = nla_total_size(sizeof(uint16_t)); /* MOD_ATTR_IGR_PORT*/
    } else {
        *msg_hdr_len = 0;
    }
    return 0;
...
static int
netlink_get_data_len(struct sk_buff *ptr_ori_skb, uint32_t *data_len)
{
    *data_len = nla_total_size(ptr_ori_skb->len);

    if ((*data_len + nla_total_size(ptr_ori_skb->len)) > NETIF_NL_PKT_LEN_MAX) {
        *data_len = NETIF_NL_PKT_LEN_MAX - *data_len - NLA_HDRLEN - NLA_ALIGNTO;
    } else {
                        ...
                        struct sk_buff *ptr_nl_skb)
{
    ...
    uint32_t data_len;
    uint16_t igr_port_si = 0;
    int rc = 0;

    rc = netlink_get_data_len(ptr_ori_skb, &data_len);
    if (0 != rc) {
        return rc;
    }
    ...
    nla_put_u16(ptr_nl_skb, NETIF_NL_ATTR_MOD_IGR_PORT, (uint16_t)igr_port_si);
                      ...
                      struct netlink_rx_cookie *ptr_cookies)
{
    ...
    rc = netlink_get_header_len(ptr_netlink, &msg_hdr_len, ptr_cookies);
    if (0 != rc) {
        return NULL;
    }
