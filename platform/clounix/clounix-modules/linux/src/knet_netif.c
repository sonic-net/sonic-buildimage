...
static const char *action_str[] = {"UNKNOWN", "ACTION_NETDEV", "ACTION_NETLINK", "ACTION_SDK",
                                   "ACTION_DROP"};
...
clx_netif_net_dev_tx(struct sk_buff *ptr_skb, struct net_device *ptr_net_dev)
{
    ...
    clx_dma_drv(unit)->prepare_pph(unit, ptr_priv->port, ptr_virt_addr);

    dbg_print(DBG_TX, "netdev:%s bind port:%d, data_len:%d len:%d pkt_len:%d, new_skb->data:%lx\n",
              ptr_priv->ptr_net_dev->name, ptr_priv->port, new_skb->data_len, new_skb->len, pkt_len,
              (unsigned long)new_skb->data);
...
#if 0
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
static int clx_netif_net_dev_ethtool_get(struct net_device *ptr_dev, struct ethtool_cmd *ptr_cmd) {
...
static struct ethtool_ops clx_netif_net_dev_ethtool_ops = {
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
    .get_settings = clx_netif_net_dev_ethtool_get,
#endif
...
clx_netif_net_dev_create(uint32_t unit, unsigned long arg)
{
    ...
    if (netif_cookie.port >= CLX_NETIF_MAX_NUM) {
        dbg_print(DBG_INTF, "Invalid port%d\n", netif_cookie.port);
        return -EINVAL;
    }
    ...
    ptr_priv->port = netif_cookie.port;
    ptr_priv->id = netif_cookie.port;
    ptr_priv->unit = unit;
    ...
    netif_cookie.id = netif_cookie.port;
    if (copy_to_user((void __user *)arg, &netif_cookie, sizeof(struct clx_netif_ioctl_intf))) {
        return -EFAULT;
    }

    clx_netif_drv(unit)->port_db[netif_cookie.port].intf.id = netif_cookie.id;
    clx_netif_drv(unit)->port_db[netif_cookie.port].ptr_net_dev = ptr_net_dev;
    dbg_print(DBG_INTF, "Network device %s created\n", ptr_net_dev->name);
...
clx_netif_net_dev_destroy(uint32_t unit, unsigned long arg)
{
    ...
    for (port = 0; port < CLX_NETIF_MAX_NUM; port++) {
        if ((clx_netif_drv(unit)->port_db[port].intf.id == intf_id) &&
            (clx_netif_drv(unit)->port_db[port].ptr_net_dev != NULL)) {
            dbg_print(DBG_INTF, "Delete network device %s.\n",
                      clx_netif_drv(unit)->port_db[port].ptr_net_dev->name);
...
clx_netif_net_dev_set(uint32_t unit, unsigned long arg)
{
    ...
    for (port = 0; port < CLX_NETIF_MAX_NUM; port++) {
        if ((clx_netif_drv(unit)->port_db[port].intf.id == netif_cookie.id) &&
            (clx_netif_drv(unit)->port_db[port].ptr_net_dev != NULL)) {
            ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
            ptr_priv = netdev_priv(ptr_net_dev);

            ether_addr_copy((u8 *)ptr_net_dev->dev_addr, netif_cookie.mac);
            if (netif_cookie.port != ptr_priv->port) {
                clx_netif_drv(unit)->port_db[port].ptr_net_dev = NULL;
                clx_netif_drv(unit)->port_db[netif_cookie.port].ptr_net_dev = ptr_net_dev;
            }
            ptr_priv->port = netif_cookie.port;
...
clx_netif_get_netdev(uint32_t unit, unsigned long arg)
{
    ...
    for (port = 0; port < CLX_NETIF_MAX_NUM; port++) {
        if ((clx_netif_drv(unit)->port_db[port].intf.id == netif_cookie.id) &&
            ...
            netif_cookie.port = ptr_priv->port;
            found = true;
            break;
        }
    }
    if (found) {
        if (copy_to_user((void __user *)arg, &netif_cookie, sizeof(struct clx_netif_ioctl_intf)))
            return -EFAULT;
        dbg_print(DBG_INTF, "Found net device %s success. port:%u\n", ptr_net_dev->name,
                  ptr_priv->port);
    } else {
...
clx_netif_get_netdev_cnt(uint32_t unit, unsigned long arg)
{
    ...
    for (port = 0; port < CLX_NETIF_MAX_NUM; port++) {
        if ((clx_netif_drv(unit)->port_db[port].intf.id == cnt_cookie.intf_id) &&
            (clx_netif_drv(unit)->port_db[port].ptr_net_dev != NULL)) {
            ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
            ptr_priv = netdev_priv(ptr_net_dev);
...
clx_netif_net_dev_destroy_all(uint32_t unit)
{
    ...
    for (port = 0; port < CLX_NETIF_MAX_NUM; port++) {
        if (clx_netif_drv(unit)->port_db[port].ptr_net_dev != NULL) {
            ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
            ptr_priv = netdev_priv(ptr_net_dev);
            /* stop DMA */
...
clx_netif_set_intf_attr(uint32_t unit, unsigned long arg)
{
    ...
    ptr_net_dev = clx_netif_drv(unit)->port_db[netif_cookie.port].ptr_net_dev;
    if (NULL == ptr_net_dev) {
        dbg_print(DBG_INFO, "ptr_net_dev is null pointer,port=%d\n", netif_cookie.port);
        return -EINVAL;
    }

    rtnl_lock();
    if (NETIF_ADMIN_STATE_UP == netif_cookie.admin_state) {
...
clx_netif_get_intf_attr(uint32_t unit, unsigned long arg)
{
    ...
    if (!status) {
        netif_cookie.admin_state = NETIF_ADMIN_STATE_DOWN;
    } else {
        netif_cookie.admin_state = NETIF_ADMIN_STATE_UP;
    }
...
netif_construct_skb_from_rx_packet(uint32_t unit, uint32_t port, struct dma_rx_packet *rx_packet)
{
    ...
    if (port >= CLX_NETIF_MAX_NUM) {
        dbg_print(DBG_INFO, "invalid port,port=%d\n", port);
        return NULL;
    }

    ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
    if (!ptr_net_dev) {
        dbg_print(DBG_RX, "ptr_net_dev is null pointer,port=%d\n", port);
        return NULL;
    }
                                 ...
                                 struct sk_buff *ptr_merge_skb)
{
    ...
    ptr_net_dev = clx_netif_drv(unit)->port_db[port].ptr_net_dev;
    if (!ptr_net_dev) {
        dbg_print(DBG_ERR, "ptr_net_dev is null pointer,port=%d\n", port);
        return;
    }
...
clx_netif_netdev_receive_skb(uint32_t unit, struct dma_rx_packet *rx_packet, uint32_t port)
{
    ...
    ptr_skb = netif_construct_skb_from_rx_packet(unit, port, rx_packet);
    if (!ptr_skb) {
        dbg_print(DBG_RX, "Failed to construct skb.\n");
        return -EFAULT;
    }

    netif_process_skb_from_rx_packet(unit, port, rx_packet, ptr_skb);
