...
class FwMgrUtil(object):
    ...
    def get_cpld_version(self):
        ...
        for i in range(1, int(output)+1):
            cmd = "cat /sys_switch/cpld/cpld{}/firmware_version".format(i)
            status, output = run_command(cmd)
            cpld_version["CPLD{}".format(i)] = output
    ...
    def cpld3_upgrade(self, fw_path):
        ret = False
        cmd = self.clx_update_tool + fw_path + " -b 5 -r 3"
        try:
            status, retstr = run_command(cmd)
            if status != 0:
                return ret
    ...
    def cpld4_upgrade(self, fw_path):
        ret = False
        cmd = self.clx_update_tool + fw_path + " -b 5 -r 4"
        try:
            status, retstr = run_command(cmd)
            if status != 0:
                return ret
    ...
    def firmware_upgrade(self, fw_type, fw_path, fw_extr=None, platform_path=None):
        ...
        if fw_type == "FPGA":
            ...
            ret = self.cpld3_upgrade(fw_path)
        elif fw_type == "CPLD-4":
            ret = self.cpld4_upgrade(fw_path)
        else:
            pass
