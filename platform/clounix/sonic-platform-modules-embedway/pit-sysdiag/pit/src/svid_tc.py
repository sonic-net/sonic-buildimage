    ...
    def svid_level_test(self, device, volset_level, also_print_console=False):
        ...
        if status != 0 or len(out) < 0:
            ...
            if get_status != 0 or len(get_log) <= 0:
                ...
                max_err = self.svid_info_dict["{}".format(device)]["{}_voltage".format(format(volset_level))]
                min_err = self.svid_info_dict["{}".format(device)]["{}_voltage".format(format(volset_level))]
                reasonable_err = self.svid_info_dict["{}".format(device)]["normal_voltage"]/100
                max_err = max_err + reasonable_err
                min_err = min_err - reasonable_err
                if switch_vol_test_readback > max_err or switch_vol_test_readback < min_err:
                    if volset_level == "high":
                        ret = E.ESVID33001
                    elif volset_level == "low":
                        ret = E.ESVID33002
