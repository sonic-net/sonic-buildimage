...
class WATCHDOGTC(TestCaseCommon):
    ...
    def get_watchdog_node_property(self, node):
        node_path = os.path.join(WTD_PATH, node)
    ...
    def test_watchdog_timeleft(self):
        timeout_val = 128
        os.system("docker exec pmon supervisorctl stop pmon_daemon:watchdogd")
        ...
        os.system("docker exec pmon supervisorctl start pmon_daemon:watchdogd")
    ...
    def run_test(self, *argv):
        ...
        timeout_org = self.get_watchdog_node_property("timeout")
        ...
        self.set_watchdog_node_property("timeout", str(timeout_org))
