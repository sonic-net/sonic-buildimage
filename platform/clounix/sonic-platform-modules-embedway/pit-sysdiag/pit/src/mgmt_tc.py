...
class MGMTTC(TestCaseCommon):
    ...
    def mgmt_ping_test(self, also_print_console=True):
        ...
        get_netgate = "arp -n | grep eth0 | head -n1 | awk '{print $1}'"
        status, result = run_command(get_netgate)
        if status != 0:
            self.logger.log_err("can not find net neighbor, use config serv ip")
        else:
            self.server_ip = result
