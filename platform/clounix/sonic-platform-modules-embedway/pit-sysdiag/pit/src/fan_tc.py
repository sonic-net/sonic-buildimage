...
class FANTC(TestCaseCommon):
    ...
    def speed_check(self, fanid, motorid):
        ...
        speed_per_ratio = int(speed_max/100) 
    ...
    '''
        ...
        for num in self.fan_info_dict["fan_list"]:
            ...
            if model_name == part_number or part_number == serial_number or model_name == serial_number:
                self.logger.log_info("FAIL fan {n} fru has same value".format(n=num), True)
    ...
    def start_fancontrol(self):
        os.system("docker exec pmon supervisorctl start fancontrol")
        os.system("docker exec pmon supervisorctl start pmon_daemon:thermalctld")


    def stop_fancontrol(self):
        os.system("docker exec pmon supervisorctl stop pmon_daemon:thermalctld")
        os.system("docker exec pmon supervisorctl stop fancontrol")
