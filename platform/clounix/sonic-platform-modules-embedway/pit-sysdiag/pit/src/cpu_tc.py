        ...
        if status != 0 or len(log) <= 0:
            ...
            for line in lines:
                ...
                if expected_cpu_model and cols[0].strip() == "Model name":
                    for cpu_name in expected_cpu_model:
                        if cpu_name == cols[1].strip():
                            break
                        type_index += 1
            ...
            for line in lines:
                ...
                if expected_cpu_model and cols[0].strip() == "Model name":
                    if cols[1].strip() != expected_cpu_model[type_index]:
                ...
                if expected_cpu_num and cols[0].strip() == 'CPU(s)':
                    if cols[1].strip() != expected_cpu_num[type_index]:
                        reason = "Failed, CPU number {}(expected {})".format( \
                                 cols[0], expected_cpu_num[type_index])
                        self.fail_reason.append(reason)
                        ret = E.ECPU3001

                if expected_cpu_mhz and cols[0].strip() == 'CPU max MHz':
                    read_cpu_mhz = float(cols[1].strip())
                    conf_cpu_mhz = float(expected_cpu_mhz[type_index])
                    if read_cpu_mhz <= (conf_cpu_mhz * 0.99) or \
                        read_cpu_mhz >= (conf_cpu_mhz * 1.01):
                        reason = "Failed, CPU MHz {}(expected {})".format( \
                                 cols[0], expected_cpu_mhz[type_index])
                        self.fail_reason.append(reason)
                        ret = E.ECPU3001
