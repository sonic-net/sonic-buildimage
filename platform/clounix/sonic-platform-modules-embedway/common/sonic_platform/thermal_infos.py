...
class FanInfo(ThermalPolicyInfoBase):
    ...
    def __init__(self):
        ...
        self.fans = {}
        self._sufficient_speed_fans = set()
        self._insufficient_cnt = 0
    ...
    def _collect_fans(self, fans):
        ...
        for fan in fans:
            ...
            try:
                ...
                speed_target = 0
                speed_tolerance = 1
            if ((speed + speed_tolerance) >= speed_target):
                if fan not in self._sufficient_speed_fans:
                    self._sufficient_speed_fans.add(fan)
            else:
                if fan in self._sufficient_speed_fans:
                    self._sufficient_speed_fans.remove(fan)
    ...
    def is_fan_all_insufficient(self):
        """
        speed is limited by the physical characteristics of the fan, and the value update may be temporarily later than speed_target,
        If speed-insufficient is detected twice in a consecutive time, then assert insufficient
        :return: True if all fan insufficient else False
        """
        if len(self._sufficient_speed_fans) == 0:
            self._insufficient_cnt = self._insufficient_cnt + 1
        else:
            self._insufficient_cnt = 0

        if self._insufficient_cnt > 1:
            #reuse THERMAL_OVERLOAD_POSITION_FILE to record fan insufficient
            try:
                fd = open(THERMAL_OVERLOAD_POSITION_FILE, 'wb',buffering=0)
            except Exception as e:
                print(e)
                return self._sufficient_speed_fans
            fd.write(('temperature below critical threshold but all fan speed insufficient\r\n').encode("ascii"))
            fd.flush()
            fd.close()
            return True
        else:
