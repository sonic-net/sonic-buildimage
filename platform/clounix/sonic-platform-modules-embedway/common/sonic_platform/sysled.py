...
class SYSLED(DeviceBase):
    ...
    DEVICE_NAME = ['sys_led_status', 'bmc_led_status', 'fan_led_status', 'psu_led_status', 'id_led_status', 'mode']
    ...
    def get_led_mode(self):
        """
        Get led mode(only for sys-led)

        Returns:
            0: Auto mode  1: Manual mode
        """        
        return self._get_attr_val('mode',0)

    def set_led_mode(self, mode):
        """
        Set led mode(only for sys-led)

        Args: 0: Auto mode  1: Manual mode

        Returns:
            bool: True if led mode is set successfully
        """
        if mode not in [0,1]:
            return False
        return self._set_attr_val('mode', mode)