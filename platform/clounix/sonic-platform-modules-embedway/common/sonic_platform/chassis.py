...
class Chassis(PddfChassis):
    ...
    """
    # Possible reboot causes addition
    ...
    def __init__(self, pddf_data=None, pddf_plugin_data=None):
        ...
        # VRM THERMALs(for temp sensor xpde122)
        ...
        # VRM THERMALs(for temp sensor raa228)
        num = self.platform_inventory['num_pmbus_raa'] if 'num_pmbus_raa' in self.platform_inventory else 0
        self.num_temp_per_raa = 2
        for i in range(num):
            for j in range(self.num_temp_per_raa):
                thermal = Thermal(0, self.pddf_obj, self.plugin_data)
                thermal.thermal_index = j + 1
                thermal.thermal_obj_name = "PMBUS-RAA_{}".format(i + 1)
                thermal.thermal_obj = self.pddf_obj.data[thermal.thermal_obj_name]
                thermal.is_vc_sensor_thermal = True
                self._thermal_list.append(thermal)
    ...
    def get_reboot_cause(self):
        ...
        #reboot_cause reg parse 
        status,cause_pwr_loss = self.__api_helper.get_cause_reg('cause_pwr_loss')
        if status and '1' in cause_pwr_loss:
            reboot_cause = (self.REBOOT_CAUSE_POWER_LOSS, "Power Loss")
            self.__api_helper.set_cause_reg('cause_pwr_loss','0')

        status,cause_hw_other = self.__api_helper.get_cause_reg('cause_hw_other')
        if status and '1' in cause_hw_other:
            reboot_cause = (self.REBOOT_CAUSE_HARDWARE_OTHER, "Hardware - Other")
            self.__api_helper.set_cause_reg('cause_hw_other','0')

        status,cause_cold_reset = self.__api_helper.get_cause_reg('cause_cold_reset')
        if status and '1' in cause_cold_reset:
            reboot_cause = (self.REBOOT_CAUSE_CPU_COLD_RESET, "CPU Cold Reset")
            self.__api_helper.set_cause_reg('cause_cold_reset','0')

        status,cause_bios_reset = self.__api_helper.get_cause_reg('cause_bios_reset')
        if status and '1' in cause_bios_reset:
            reboot_cause = (self.REBOOT_CAUSE_BIOS_RESET, "BIOS Reset")
            self.__api_helper.set_cause_reg('cause_bios_reset','0')

        status,cause_psu_shutdown = self.__api_helper.get_cause_reg('cause_psu_shutdown')
        if status and '1' in cause_psu_shutdown:
            reboot_cause = (self.REBOOT_CAUSE_PSU_SHUTDOWN, "PSU Shutdown")
            self.__api_helper.set_cause_reg('cause_psu_shutdown','0')

        status,cause_bmc_shutdown = self.__api_helper.get_cause_reg('cause_bmc_shutdown')
        if status and '1' in cause_bmc_shutdown:
            reboot_cause = (self.REBOOT_CAUSE_BMC_SHUTDOWN, "BMC Shutdown")
            self.__api_helper.set_cause_reg('cause_bmc_shutdown','0')
        ...
        if os.path.exists(wdt_indicator):
            ...
            fd.close()
            if '1' in val:
                reboot_cause = (self.REBOOT_CAUSE_WATCHDOG, "CPLD-CB Watchdog")
        ...
        if os.path.isfile(THERMAL_OVERLOAD_POSITION_FILE):
            ...
            if thermal_overload_pos != "Unknown":
                ...
                elif str.find('insufficient') >= 0:
                    reboot_cause = (
                        self.REBOOT_CAUSE_INSUFFICIENT_FAN_SPEED, thermal_overload_pos)
                else:
                    reboot_cause = (
        ...
        '''
        # no adm1166 fault recording for REBOOT_CAUSE_HARDWARE_OTHER
         #ADM1166 cause
