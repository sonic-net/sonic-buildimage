...
PLATFORM_CFG_MODULE = "embedway_platform"
...
def deviceInit():
    ...
    SYSLED_MODE_PATH = "/sys_switch/sysled/mode"
    ...
    # Set SYSLED to manual mode
    result = common.writeFile(SYSLED_MODE_PATH, "0x1")
    

    return

def fix_s3ip_sysfs_permission():

    """
        fantray led, bmc led, fan led, psu led permission fix: remove write permission
    """

    # fix fantray led status permission
    fan_path = "/sys_switch/fan"
    fan_numbers = common.readFile(fan_path + "/number")
    for i in range(1, int(fan_numbers) + 1):
        path = fan_path + "/fan" + str(i) + "/led_status"
        if os.path.exists(path):
            common.doBash("chmod 444 " + path)

    # fix front panel bmc led status permission
    bmc_led_path = "/sys_switch/sysled/bmc_led_status"
    if os.path.exists(bmc_led_path):
        common.doBash("chmod 444 " + bmc_led_path)

    # fix front panel fan led status permission
    fan_led_path = "/sys_switch/sysled/fan_led_status"
    if os.path.exists(fan_led_path):
        common.doBash("chmod 444 " + fan_led_path)

    # fix front panel psu led status permission
    psu_led_path = "/sys_switch/sysled/psu_led_status"
    if os.path.exists(psu_led_path):
        common.doBash("chmod 444 " + psu_led_path)
...
def main():
    ...
    if args[0] == 'install':
        ...
        deviceInit()
        fix_s3ip_sysfs_permission()
        # No fault recording(like adm1166) for REBOOT_CAUSE_HARDWARE_OTHER
        #process_adm1166_fault()
        #process_adm1166_factors()

        self_test.firmware_inspection()
