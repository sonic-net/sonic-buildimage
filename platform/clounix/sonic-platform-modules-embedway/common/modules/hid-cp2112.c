...
static const int XFER_STATUS_RETRIES = 20;//set 20 as we changed clock to 50k
...
static int cp2112_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	config.clock_speed = cpu_to_be32(50000);        /* 50KHZ */

	hid_info(hdev, "clock_speed: %d Hz \n", cpu_to_be32(config.clock_speed));
	hid_info(hdev, "auto_send_read: %d \n", config.auto_send_read);
	hid_info(hdev, "write_timeout: %d ms \n", cpu_to_be16(config.write_timeout));
	hid_info(hdev, "read_timeout: %d ms \n", cpu_to_be16(config.read_timeout));
	hid_info(hdev, "scl_low_timeout: %d \n", config.scl_low_timeout);
	hid_info(hdev, "retry_time: %d \n", cpu_to_be16(config.retry_time));
