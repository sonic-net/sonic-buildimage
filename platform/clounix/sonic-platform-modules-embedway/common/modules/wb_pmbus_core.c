...
static int g_wb_pmbus_debug = 0;
module_param(g_wb_pmbus_debug, int, S_IRUGO | S_IWUSR);
#define WB_PMBUS_VERBOSE(fmt, args...) do {                                        \
    if (g_wb_pmbus_debug) { \
        printk(KERN_INFO "[WB_PMBUS][VER][func:%s line:%d]\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
                 ...
                 const struct pmbus_sensor_attr *attr)
{
    ...
    WB_PMBUS_VERBOSE("nlimit:%d\n",nlimit);
    for (i = 0; i < nlimit; i++) {
        WB_PMBUS_VERBOSE("reg:0x%x\n",l->reg);
                      ...
                      bool paged)
{
    ...
    if (attr->sfunc && phase == 0xff) {
        WB_PMBUS_VERBOSE("name:%s index=%d\n",name,index);
...
static const struct pmbus_limit_attr vin_limit_attrs[] = {
    ...
    }, /* 以下reg的规范较新，xdpe122并没有支持，且全部check_word_register一遍会触发cp2112 timeout
    {
...
int wb_pmbus_do_probe(struct i2c_client *client, struct pmbus_driver_info *info)
{
    ...
    WB_PMBUS_VERBOSE("wb_pmbus_do_probe\n");
    if (!info)
        return -ENODEV;
