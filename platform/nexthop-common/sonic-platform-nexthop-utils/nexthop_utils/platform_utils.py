...
class CommandError(subprocess.CalledProcessError):
    """CalledProcessError whose str() also carries the captured stderr/stdout.

    The base class reports only the exit status; the actionable error is on
    the streams `run_cmd` captured, which would otherwise be discarded.
    """

    # Enough to keep every error line of a chatty child while bounding the
    # message when it floods stderr.
    TAIL_CHARS = 4000

    def __str__(self):
        message = super().__str__()
        for label, text in (("stderr", self.stderr), ("stdout", self.output)):
            if isinstance(text, str) and text.strip():
                message += f"; {label}: {text.strip()[-self.TAIL_CHARS:]}"
        return message
...
def run_cmd(cmd, check_exit=True):
    ...
    Raises:
      CommandError: If check_exit is True and command fails
    ...
    try:
        ...
        raise CommandError(e.returncode, cmd, output=e.stdout, stderr=e.stderr) from e
