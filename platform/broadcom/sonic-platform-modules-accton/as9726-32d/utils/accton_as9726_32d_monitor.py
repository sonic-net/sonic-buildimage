                    ...
                    if fan_policy_alarm==0:
                        if current_temp >= fan_thermal_spec["max_to_yellow_alarm"][i][1]:
                            if send_yellow_alarm==0:
                                if fan_thermal_spec["max_to_yellow_alarm"][i][0] == TYPE_SENSOR:
                                    name = thermal.get_thermal_name(thermal_idx)
                                elif fan_thermal_spec["max_to_yellow_alarm"][i][0] == TYPE_TRANSCEIVER:
                                    name = "port {}".format(sfp.port_num)
                                logging.warning('Monitor %s, temperature is %.1f. Temperature is over the error threshold(%.1f) of thermal policy.',
                                                name,
                                                current_temp/1000,
                                                fan_thermal_spec["max_to_yellow_alarm"][i][1]/1000)
                        ...
                        if current_temp >= fan_thermal_spec["yellow_to_red_alarm"][i][1]:
                            if send_red_alarm==0:
                                ...
                                logging.warning('Monitor %s, temperature is %.1f. Temperature is over the critical threshold(%.1f) of thermal policy.',
                                                name,
                                                current_temp/1000,
                                                fan_thermal_spec["yellow_to_red_alarm"][i][1]/1000)
                        ...
                        if current_temp >= fan_thermal_spec["max_to_yellow_alarm"][i][1]:
                            if send_yellow_alarm==0:
                                if fan_thermal_spec["max_to_yellow_alarm"][i][0] == TYPE_SENSOR:
                                    ...
                                    name = "port {}".format(sfp.port_num)
                                logging.warning('Monitor %s, temperature is %.1f. Temperature is over the low-critical threshold(%.1f) of thermal policy.',
                                                name,
                                                current_temp/1000,
                                                fan_thermal_spec["max_to_yellow_alarm"][i][1]/1000)
                                fan_policy_alarm=LEVEL_FAN_YELLOW_ALARM
                        ...
                        if current_temp >= fan_thermal_spec["yellow_to_red_alarm"][i][1]:
                            if send_red_alarm==0:
                                if fan_thermal_spec["yellow_to_red_alarm"][i][0] == TYPE_SENSOR:
                                    ...
                                    name = "port {}".format(sfp.port_num)
                                logging.warning('Monitor %s, temperature is %.1f. Temperature is over the high-critical threshold(%.1f) of thermal policy.',
                                                name,
                                                current_temp/1000,
                                                fan_thermal_spec["yellow_to_red_alarm"][i][1]/1000)
                                fan_policy_alarm=LEVEL_FAN_RED_ALARM
