...
#define DECLARE_FAN_FAULT_SENSOR_DEV_ATTR(index, index2) \
    static SENSOR_DEVICE_ATTR(fan##index##_fault, S_IRUGO, fan_show_value, NULL, FAN##index##_FRONT_FAULT);\
    static SENSOR_DEVICE_ATTR(fan##index2##_fault, S_IRUGO, fan_show_value, NULL, FAN##index##_REAR_FAULT)
...
static u8 is_fan_fault(struct as9736_64d_fan_data *data, enum fan_id id)
{
    ...
    u8 present = 1;
    int fan_index;
    int fan_speed_index = id + 11; /*front_fan=11~14, rear_fan=15~18*/

    fan_index = (id > 4) ? (id - 4) : id;
    present = reg_val_to_is_present(data->reg_val[FAN_PRESENT_REG], fan_index); /*4 FanTray, index= 0~3*/

    if( !present ) { /*Fan unpresent*/
        return 0;
    }

    if (reg_val_to_speed_rpm(data->reg_val[fan_speed_index], data->reg_val[FAN_TACH_SPEED_REG]) ) {
        ret = 0;
    }
