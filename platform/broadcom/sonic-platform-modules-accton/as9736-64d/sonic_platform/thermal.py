...
class Thermal(ThermalBase):
    ...
    def __init__(self, thermal_index=0, is_psu=False, psu_index=0):
        ...
        self.default_threshold = {
            ...
            THERMAL_NAME_LIST[2] : {
                self.conf.HIGH_THRESHOLD_FIELD : '97.0',
                self.conf.LOW_THRESHOLD_FIELD : '3.0',
                self.conf.HIGH_CRIT_THRESHOLD_FIELD : '103.0',
            ...
            THERMAL_NAME_LIST[5] : {
                ...
                self.conf.HIGH_CRIT_THRESHOLD_FIELD : '62.0',
            ...
            THERMAL_NAME_LIST[11] : {
                ...
                self.conf.HIGH_CRIT_THRESHOLD_FIELD : '99.0',
        ...
        i2c_path = {
            ...
            2: {"hwmon_path":"14-004c/hwmon/hwmon*/", "ss_index":2},
            ...
            10: {"hwmon_path":"66-004d/hwmon/hwmon*/", "ss_index":2},
    ...
    def __refresh_psu_threshold(self):
        high_threshold_field = self.conf.HIGH_THRESHOLD_FIELD
        low_threshold_field = self.conf.LOW_THRESHOLD_FIELD

        default_value = self.default_threshold[self.get_name()][high_threshold_field]
        if default_value == self.conf.NOT_AVAILABLE or float(default_value) in [0.0, 80.0]:
            max_temp = self.__get_temp(self.psu_hwmon_path + "psu_temp1_max")
            self.default_threshold[self.get_name()][high_threshold_field] = str(float(max_temp))

        default_value = self.default_threshold[self.get_name()][low_threshold_field]
        if default_value == self.conf.NOT_AVAILABLE or float(default_value) in [0.0, 80.0]:
            min_temp = self.__get_temp(self.psu_hwmon_path + "psu_temp1_min")
            self.default_threshold[self.get_name()][low_threshold_field] = str(float(min_temp))
    ...
    def get_temperature(self):
        ...
        # When the PSU is powered on, try to update the PSU threshold value
        if self.is_psu and current not in [0.0, 80.0]:
            self.__refresh_psu_threshold()
