...
CONFIG_PORT_PREFIX = "PORT|"
STATE_PORT_PREFIX = "PORT_TABLE|"
...
class NexthopLedControlBase(LedControlBase):
    ...
    def _get_port_status(self, port):
        admin_status = self.db.get(
            self.db.CONFIG_DB, CONFIG_PORT_PREFIX + port, "admin_status"
        )
        if admin_status is None:
            admin_status = "down"
        oper_status = self.db.get(
            self.db.STATE_DB, STATE_PORT_PREFIX + port, "netdev_oper_status"
        )
        return admin_status, oper_status

    def _get_xcvr_presence(self, port):
        xcvr_info = self.db.get_all(self.db.STATE_DB, STATE_XCVR_PREFIX + port)
        if xcvr_info:
            return True
        return False
    ...
    def _get_port_num(self, interface):
        return int(self.db.get(self.db.CONFIG_DB, CONFIG_PORT_PREFIX + interface, "index"))
    ...
    def port_link_state_change(self, port):
        ...
        if all_oper_down:
            if all_admin_disabled:
                ...
                if not self._get_xcvr_presence(port):
                    color = "off"
        else:
            if not self._get_xcvr_presence(port):
                color = "yellow"
