 ...
 *   - Prepend nh_ to exported function names
 *   - Remove CONFIG_DEBUG_FS related code (which caused crash)
 *
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_clear_cache, PMBUS);
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_set_update, PMBUS);
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_lock, PMBUS);
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_lock_interruptible, PMBUS);
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_unlock, PMBUS);
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_set_page, PMBUS);

int nh_pmbus_write_byte(struct i2c_client *client, int page, u8 value)
{
	int rv;

	rv = nh_pmbus_set_page(client, page, 0xff);
	if (rv < 0)
		return rv;

	return i2c_smbus_write_byte(client, value);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_write_byte, PMBUS);
...
static int _pmbus_write_byte(struct i2c_client *client, int page, u8 value)
{
	...
	return nh_pmbus_write_byte(client, page, value);
}

int nh_pmbus_write_word_data(struct i2c_client *client, int page, u8 reg,
			  u16 word)
{
	int rv;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_write_word_data, PMBUS);
				...
				u16 word)
{
	...
	case PMBUS_VIRT_FAN_TARGET_1 ... PMBUS_VIRT_FAN_TARGET_4:
		...
		rv = nh_pmbus_update_fan(client, page, id, bit, bit, word);
		break;
	default:
		rv = -ENXIO;
		break;
				  ...
				  u16 word)
{
	...
	return nh_pmbus_write_word_data(client, page, reg, word);
}
...
static int _pmbus_write_byte_data(struct i2c_client *client, int page, int reg, u8 value)
{
	...
	return nh_pmbus_write_byte_data(client, page, reg, value);
}
...
static int _pmbus_read_byte_data(struct i2c_client *client, int page, int reg)
{
	...
	return nh_pmbus_read_byte_data(client, page, reg);
}

int nh_pmbus_update_fan(struct i2c_client *client, int page, int id,
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_update_fan, PMBUS);

int nh_pmbus_read_word_data(struct i2c_client *client, int page, int phase, u8 reg)
{
	int rv;

	rv = nh_pmbus_set_page(client, page, phase);
	if (rv < 0)
		return rv;

	return i2c_smbus_read_word_data(client, reg);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_read_word_data, PMBUS);
...
static int pmbus_read_virt_reg(struct i2c_client *client, int page, int reg)
{
	...
	case PMBUS_VIRT_FAN_TARGET_1 ... PMBUS_VIRT_FAN_TARGET_4:
		id = reg - PMBUS_VIRT_FAN_TARGET_1;
		rv = nh_pmbus_get_fan_rate_device(client, page, id, rpm);
		break;
	default:
		rv = -ENXIO;
		break;
				 ...
				 int phase, int reg)
{
	...
	return nh_pmbus_read_word_data(client, page, phase, reg);
}
...
int nh_pmbus_read_byte_data(struct i2c_client *client, int page, u8 reg)
{
	int rv;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_read_byte_data, PMBUS);

int nh_pmbus_write_byte_data(struct i2c_client *client, int page, u8 reg, u8 value)
{
	int rv;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_write_byte_data, PMBUS);

int nh_pmbus_update_byte_data(struct i2c_client *client, int page, u8 reg,
			   u8 mask, u8 value)
{
	unsigned int tmp;
	int rv;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_update_byte_data, PMBUS);
			      ...
			      bool from_cache)
{
	...
	if (want_rpm == have_rpm)
		return nh_pmbus_read_word_data(client, page, 0xff,
...
int nh_pmbus_get_fan_rate_device(struct i2c_client *client, int page, int id,
			      enum pmbus_fan_mode mode)
{
	return pmbus_get_fan_rate(client, page, id, mode, false);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_get_fan_rate_device, PMBUS);

int nh_pmbus_get_fan_rate_cached(struct i2c_client *client, int page, int id,
			      enum pmbus_fan_mode mode)
{
	return pmbus_get_fan_rate(client, page, id, mode, true);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_get_fan_rate_cached, PMBUS);
...
void nh_pmbus_clear_faults(struct i2c_client *client)
{
	struct pmbus_data *data = i2c_get_clientdata(client);
	int i;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_clear_faults, PMBUS);
...
bool nh_pmbus_check_byte_register(struct i2c_client *client, int page, int reg)
{
	return pmbus_check_register(client, _pmbus_read_byte_data, page, reg);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_check_byte_register, PMBUS);

bool nh_pmbus_check_word_register(struct i2c_client *client, int page, int reg)
{
	return pmbus_check_register(client, __pmbus_read_word_data, page, reg);
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_check_word_register, PMBUS);
...
const struct pmbus_driver_info *nh_pmbus_get_driver_info(struct i2c_client *client)
{
	struct pmbus_data *data = i2c_get_clientdata(client);

	return data->info;
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_get_driver_info, PMBUS);
				 ...
				 const struct pmbus_sensor_attr *attr)
{
	...
	for (i = 0; i < nlimit; i++) {
		if (nh_pmbus_check_word_register(client, page, l->reg)) {
				    ...
				    struct pmbus_data *data)
{
	...
	for (page = 0; page < info->pages; page++) {
		...
		for (f = 0; f < ARRAY_SIZE(pmbus_fan_registers); f++) {
			...
			if (!nh_pmbus_check_word_register(client, page,
			...
			if ((info->func[page] & pmbus_fan_status_flags[f]) &&
			    nh_pmbus_check_byte_register(client,
					...
					struct pmbus_data *data)
{
	...
	for (s = 0; s < ARRAY_SIZE(pmbus_samples_registers); s++) {
		...
		if (!nh_pmbus_check_word_register(client, 0, attr->reg))
			continue;
				 ...
				 struct pmbus_data *data, int page)
{
	...
	if (nh_pmbus_check_byte_register(client, page, PMBUS_VOUT_MODE))
...
static int _pmbus_regulator_on_off(struct regulator_dev *rdev, bool enable)
{
	...
	mutex_lock(&data->update_lock);
	ret = nh_pmbus_update_byte_data(client, page, PMBUS_OPERATION,
...
static int pmbus_regulator_get_low_margin(struct i2c_client *client, int page)
{
	...
	if (data->vout_low[page] < 0) {
		if (nh_pmbus_check_word_register(client, page, PMBUS_MFR_VOUT_MIN))
...
static int pmbus_regulator_get_high_margin(struct i2c_client *client, int page)
{
	...
	if (data->vout_high[page] < 0) {
		if (nh_pmbus_check_word_register(client, page, PMBUS_MFR_VOUT_MAX))
			s.data = _pmbus_read_word_data(client, page, 0xff,
						       PMBUS_MFR_VOUT_MAX);
		if (s.data < 0) {
			s.data = _pmbus_read_word_data(client, page, 0xff,
...
const struct regulator_ops nh_pmbus_regulator_ops = {
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_regulator_ops, PMBUS);
...
int nh_pmbus_do_probe(struct i2c_client *client, struct pmbus_driver_info *info)
{
	struct device *dev = &client->dev;
	const struct pmbus_platform_data *pdata = dev_get_platdata(dev);
	struct pmbus_data *data;
...
EXPORT_SYMBOL_NS_GPL(nh_pmbus_do_probe, PMBUS);

struct dentry *nh_pmbus_get_debugfs_dir(struct i2c_client *client)
{
	struct pmbus_data *data = i2c_get_clientdata(client);

	return data->debugfs;
}
EXPORT_SYMBOL_NS_GPL(nh_pmbus_get_debugfs_dir, PMBUS);
