...
CONFIG_ADMIN_STATUS_KEY = "admin_status"
STATE_OPER_STATUS_KEY = "netdev_oper_status"
...
class PortLedd(daemon_base.DaemonBase):
    ...
    def __init__(self):
        ...
        # Subscribe to CONFIG_DB and STATE_DB events
        self.config_db = daemon_base.db_connect("CONFIG_DB")
        self.state_db = daemon_base.db_connect("STATE_DB")

        self.config_port_tbl = swsscommon.SubscriberStateTable(
            self.config_db, swsscommon.CFG_PORT_TABLE_NAME
        )
        self.sel.addSelectable(self.config_port_tbl)

        self.state_port_tbl = swsscommon.SubscriberStateTable(
            self.state_db, swsscommon.STATE_PORT_TABLE_NAME
        )
        self.sel.addSelectable(self.state_port_tbl)
    ...
    def handle_admin_status_event(self):
        key, op, fvp = self.config_port_tbl.pop()
        ...
        if op == "DEL":
            ...
            fvp_dict = dict(fvp)
            if CONFIG_ADMIN_STATUS_KEY in fvp_dict:
                self.led_control.port_link_state_change(key)

        return

    def handle_oper_status_event(self):
        key, op, fvp = self.state_port_tbl.pop()
        ...
        if fvp:
            fvp_dict = dict(fvp)
            if op == "SET" and STATE_OPER_STATUS_KEY in fvp_dict:
                self.led_control.port_link_state_change(key)
    ...
    def run(self):
        ...
        self.handle_admin_status_event()
        self.handle_oper_status_event()
