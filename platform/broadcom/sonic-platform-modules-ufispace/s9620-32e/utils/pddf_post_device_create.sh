...
function is_tr45_pass {
    local cmd="ipmitool i2c bus=3 0xa0 0x1 0x5 0x0"
    local raw_out
    
    # Execute ipmitool and discard stderr (simulating os.devnull)
    if ! raw_out=$($cmd 2>/dev/null); then
        echo "Failed to read EEPROM (ipmitool returned error)."
        return 1 # Fail
    fi

    # Check if output is empty
    if [ -z "$raw_out" ]; then
        echo "Failed to read EEPROM data at offset 0x500 for tr45 result."
        return 1 # Fail
    fi

    # Parse output: get the first line and remove spaces/carriage returns
    local hex_val=$(echo "$raw_out" | head -n 1 | tr -d ' ' | tr -d '\r')

    # Convert hex string to integer
    local int_val
    if int_val=$((16#$hex_val)) 2>/dev/null; then
        if [ "$int_val" -eq 1 ]; then
            echo "TR45 Test Value: $int_val (pass)"
            return 0 # Pass
        else
            echo "TR45 Test Value: $int_val (fail or not configured)"
            return 1 # Fail
        fi
    else
        echo "Failed to convert EEPROM data '$hex_val' to integer."
        return 1 # Fail
    fi
}
...
function set_mac_rov {
    ...
    # PMBus Registers
    local PMBUS_PAGE_REG="0x00"
    local PMBUS_OPERATION_REG="0x01"
    ...
    for ((i = 0; i < ${#rov_sysfs_paths[@]}; i++)); do
        ...
        # Check TR45 status per MAC iteration
        local tr45_pass=0
        if is_tr45_pass; then
            tr45_pass=1
        fi

        # Determine AVS value based on TR45 status
        if [ "$tr45_pass" -eq 1 ]; then
            # TR45 Pass: Force fixed VDDC 715mV (0x90)
            avs=$((0x90))
            echo "TR45 Test Pass, Set MAC $i to fixed VDDC 715mV, reg_val=0x90"
        else
            ...
            avs=$(( rov_reg ))
            echo "TR45 Test Fail or not configured, Set MAC $i to runtime ROV"
            echo "${sysfs_path}=0x$(printf "%02X" $avs)"
        fi
        ...
        for ((j = 0; j < ${#avs_array[@]}; j++)); do
            if [ $avs -eq $((${avs_array[j]})) ]; then
                ...
                i2cset -f -y $rov_i2c_bus $addr $PMBUS_PAGE_REG 0x0
                i2cset -f -y $rov_i2c_bus $addr $PMBUS_OPERATION_REG 0x80
                i2cset -f -y $rov_i2c_bus $addr $rov_config_reg $val w
                
                touch "$done_flag"
                echo "MAC $i configured successfully"
                break
            fi
        done
    done
