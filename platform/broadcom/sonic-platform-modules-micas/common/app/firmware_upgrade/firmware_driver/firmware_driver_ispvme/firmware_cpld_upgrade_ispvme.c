...
static int firmware_file_read(const char *path, uint32_t addr, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        if (filp->f_op->read) {
            ret = filp->f_op->read(filp, val, size, &pos);
        }
    }
    if (ret != size) {
...
static int firmware_file_write(const char *path, uint32_t addr, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        if (filp->f_op->write) {
            ret = filp->f_op->write(filp, val, size, &pos);
        }
    }
    if (ret < 0) {
...
static int firmware_upgrade_en(int flag)
{
    ...
    for (i = 0; i < current_fmw_cpld->gpio_en_info_num; i++) {
        if (flag) {
            if (!current_fmw_cpld->gpio_en_info[i].en_desc) {
                current_fmw_cpld->gpio_en_info[i].en_desc = gpio_to_desc(current_fmw_cpld->gpio_en_info[i].en_gpio);
                if (!current_fmw_cpld->gpio_en_info[i].en_desc) {
            ...
            ret = gpiod_direction_output(current_fmw_cpld->gpio_en_info[i].en_desc, current_fmw_cpld->gpio_en_info[i].en_level);
            if (ret) {
                FIRMWARE_DRIVER_DEBUG_ERROR("Set cpld_ispvme_upgrade EN[%d] GPIO direction failed!\n", i);
                goto free_gpio;
            }
            current_fmw_cpld->gpio_en_info[i].flag = 1;
        } else {
            gpiod_set_value(current_fmw_cpld->gpio_en_info[i].en_desc, !current_fmw_cpld->gpio_en_info[i].en_level);
...
static int init_cpld(void)
{
    ...
    current_fmw_cpld->tdi_desc = gpio_to_desc(current_fmw_cpld->tdi);
    if (!current_fmw_cpld->tdi_desc) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Requesting cpld_ispvme_upgrade TDI GPIO %d failed!\n", current_fmw_cpld->tdi);
        return -1;
    }
    ret = gpiod_direction_output(current_fmw_cpld->tdi_desc, 1);
    if (ret) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Set cpld_ispvme_upgrade TDI GPIO direction failed!\n");
        return ret;
    }

    current_fmw_cpld->tck_desc = gpio_to_desc(current_fmw_cpld->tck);
    if (!current_fmw_cpld->tck_desc) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Requesting cpld_ispvme_upgrade TCK GPIO %d failed!\n", current_fmw_cpld->tck);
        goto free_tdi;
    }
    ret = gpiod_direction_output(current_fmw_cpld->tck_desc, 1);
    if (ret) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Set cpld_ispvme_upgrade TCK GPIO direction failed!\n");
        goto free_tdi;
    }

    current_fmw_cpld->tms_desc = gpio_to_desc(current_fmw_cpld->tms);
    if (!current_fmw_cpld->tms_desc) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Requesting cpld_ispvme_upgrade TMS GPIO %d failed!\n", current_fmw_cpld->tms);
        goto free_tck;
    }
    ret = gpiod_direction_output(current_fmw_cpld->tms_desc, 1);
    if (ret) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Set cpld_ispvme_upgrade TMS GPIO direction failed!\n");
        goto free_tck;
    }

    current_fmw_cpld->tdo_desc = gpio_to_desc(current_fmw_cpld->tdo);
    if (!current_fmw_cpld->tdo_desc) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Requesting cpld_ispvme_upgrade TDO GPIO %d failed!\n", current_fmw_cpld->tdo);
        goto free_tms;
    }
    ret = gpiod_direction_input(current_fmw_cpld->tdo_desc);
    if (ret) {
        FIRMWARE_DRIVER_DEBUG_ERROR("Set cpld_ispvme_upgrade TDO GPIO direction failed!\n");
        goto free_tms;
    }
...
/* Loop waiting for */
static int pull_wait(struct gpio_desc *desc, int gpio, int value) {
    int i, j;
    /* Timeout time is two seconds */
    for (i = 0; i < 20; i++) {
        for (j = 0; j < 100; j++) {
            if (!!gpiod_get_value(desc) == !!value ) {
                return 0;
            }
...
static int pull_tdi_up(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tdi_desc, 1);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tdi_desc, current_fmw_cpld->tdi, 1);
}
...
static int pull_tdi_down(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tdi_desc, 0);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tdi_desc, current_fmw_cpld->tdi, 0);
}
...
static int pull_tck_up(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tck_desc, 1);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tck_desc, current_fmw_cpld->tck, 1);
}
...
static int pull_tck_down(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tck_desc, 0);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tck_desc, current_fmw_cpld->tck, 0);
}
...
static int pull_tms_up(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tms_desc, 1);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tms_desc, current_fmw_cpld->tms, 1);
}
...
static int pull_tms_down(void)
{
    ...
    gpiod_set_value(current_fmw_cpld->tms_desc, 0);

    /* Wait for the GPIO value to be set successfully */
    return pull_wait(current_fmw_cpld->tms_desc, current_fmw_cpld->tms, 0);
}
...
static int read_tdo(void)
{
    ...
    return gpiod_get_value(current_fmw_cpld->tdo_desc);
}
