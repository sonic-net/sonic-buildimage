...
#define SAP_CREDO_CFG_FILE    "sap_credo_55321"
...
#define CREDO_55321_CHIP_ID_ADDR        (0x70C2)
#define CREDO_55321_CHIP_ID             (0x2161)
...
static sap_phy_fec_map fec_map[] = {
    {SAP_FEC_NONE,        CR_FEC_NONE       },
    {SAP_FEC_RSFEC,       CR_FEC_RS_528     },
    {SAP_FEC_RS544,       CR_FEC_RS_544     },
};
...
static int sap_credo_get_profile_info(sap_slice_info_t *slice_info, int port_id, CredoPortConfig_t *conf);
static int sap_credo_query_port(sap_credo_port_info_t *port_info, int if_side, int tx_rx, CredoPortConfig_t *port_config);
static int sap_credo_prbs_operation_set(int tx_rx, sap_slice_info_t *slice_info, int if_side, 
    const CredoPortConfig_t *port_config, int lane, CredoPrbsPattern_t cr_poly, int admin);
static int sap_credo_prbs_operation_get(sap_slice_info_t *slice_info, int if_side, 
    const CredoPortConfig_t *port_config, int lane, sap_prbs_status_t *prbs_info);
...
static int sap_credo_shell_cmd_run(int unit, int port, char *cmdline);
static int sap_credo_port_fec_get(int unit, int port, int if_side, sap_fec_e *fec_type);
...
sap_apis_t sap_extphy_credo_55321_apis = {
    .type                           = "CREDO_55321",
    .sap_platform_init              = sap_credo_platform_init,
    .sap_port_support               = sap_credo_port_support,
    .sap_shell_cmd_run              = sap_credo_shell_cmd_run,
    .sap_port_fec_get               = sap_credo_port_fec_get,
...
static int sap_credo_chip_id_get(sap_slice_info_t *slice_info, uint32_t *chip_id)
{
    int rv;
    uint32_t data;

    *chip_id = 0xFFFF;
    rv = slice_info->mdio_info->mdio_read(NULL, slice_info->mdio_id, slice_info->phy_addr, 
            slice_info->mdio_info->phy_reg_offset | CREDO_55321_CHIP_ID_ADDR, &data);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_CR_LOG_ERR("mdio_read fail rv: %d\n", rv);
        return rv;
    }
    SAP_CR_LOG_DBG("mdio_read chip id addr:0x%x date:0x%x\n", CREDO_55321_CHIP_ID_ADDR, data);
    *chip_id = data;
...
static int sap_credo_config_parse_json(int unit)
{
    ...
    if (global_dev_info == NULL) {
        SAP_COMMON_MEM_FREE(global_mdio_info);
        return SAP_STATUS_NO_MEMORY;
    }
    ...
    if (global_slice_info == NULL) {
        SAP_COMMON_MEM_FREE(global_mdio_info);
        SAP_COMMON_MEM_FREE(global_dev_info);
        return SAP_STATUS_NO_MEMORY;
    }
    ...
    for (i = 0; i < global_slice_num; i++) {
        ...
        /* check chip id */
        chip_id = 0xFFFF;
        rv = sap_credo_chip_id_get(slice_info, &chip_id);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_CR_LOG_ERR("PHY-0x%x sap_credo_chip_id_get fail", slice_info->phy_addr);
            return SAP_STATUS_FAILURE;
        } else {
            if (chip_id == 0xFFFF) {
                SAP_CR_LOG_ERR("PHY-0x%x sap_credo_chip_id_get chip_id is invalid", slice_info->phy_addr);
                return SAP_STATUS_FAILURE;
            }
            if (chip_id != CREDO_55321_CHIP_ID) {
                SAP_COMMON_MEM_FREE(global_mdio_info);
                SAP_COMMON_MEM_FREE(global_dev_info);
                SAP_COMMON_MEM_FREE(global_slice_info);
                SAP_CR_LOG_ERR("PHY-0x%x sap_credo_chip_id_get chip_id isn't credo 55321", slice_info->phy_addr);
...
static int sap_credo_slot_init(int unit, int slot_id)
{
    ...
    for (dev_id = 0; dev_id < global_dev_num; dev_id++) {
        ...
        if (dev_info->cr_device == NULL) {
            rv = cr_device_create(sdk, CREDO_BLACKHAWK_800, &dev_info->cr_device);
            if (rv) {
...
static void *sap_credo_slice_init_thread(void *arg)
{
    int *rv, mdio_id;
    mdio_id = *(int *)arg;
    SAP_CR_LOG_INFO("credo test in");
    rv = (int *)malloc(sizeof(int));
    if (rv == NULL) {
        pthread_exit(NULL);
    }
    *rv = sap_credo_slice_init(mdio_id);
    SAP_CR_LOG_INFO("credo test end");
...
static int sap_credo_port_init(sap_slice_info_t *slice_info)
{
    ...
    list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
        ...
        rv = cr_port_configure(slice_info->cr_slice, &port_config, CR_FORCE);
...
static int sap_credo_get_profile_info(sap_slice_info_t *slice_info, int port_id, CredoPortConfig_t *conf)
{
    ...
    conf->connection_mode = GET_NUMBER_VALUE(port_profile, "connection_mode");
    conf->port_mode = GET_NUMBER_VALUE(port_profile, "port_mode");
    conf->flags = GET_NUMBER_VALUE(port_profile, "flags");
    conf->line_no_of_lanes = GET_NUMBER_VALUE(port_profile, "line_no_of_lanes");
    conf->host_no_of_lanes = GET_NUMBER_VALUE(port_profile, "host_no_of_lanes");
    conf->line_start_lane = GET_NUMBER_VALUE(port_profile, "line_start_lane");
    conf->host_start_lane = GET_NUMBER_VALUE(port_profile, "host_start_lane");
    conf->host_fec_type = GET_NUMBER_VALUE(port_profile, "host_fec_type");
    conf->line_fec_type = GET_NUMBER_VALUE(port_profile, "line_fec_type");
    SAP_CR_LOG_DBG("port %d host_fec:%d line_fec:%d", port_id, conf->host_fec_type, conf->line_fec_type);
    conf->speed = speed;
    return SAP_STATUS_SUCCESS;
}
...
static int sap_credo_set_port_admin(int unit, int port, bool enable)
{
    ...
    rv = cr_port_query(port_info->slice_info->cr_slice, port_info->port_id, &port_config);
    SAP_CR_RV_CHECK(port_info->slice_info, "cr_port_query", rv);
    for (lane_id = 0; lane_id < port_config.host_no_of_lanes; lane_id++) {
        lane = port_config.host_start_lane + lane_id;
        if (enable) {
    ...
    for (lane_id = 0; lane_id < port_config.line_no_of_lanes; lane_id++) {
        lane = port_config.host_start_lane + lane_id;
        if (enable) {
            rv = cr_lane_rx_no_disable(port_info->slice_info->cr_slice, lane);
            SAP_CR_RV_CHECK(port_info->slice_info, "cr_lane_rx_no_disable", rv);
...
static int sap_credo_slice_init(int mdio_id)
{
    ...
    init_type = CR_INIT_NO_FIRMWARE; /* TODO: 需要从配置文件里面获取 */
    ...
    SAP_CR_LOG_DBG("broadcast_slices_num:%d", broadcast_slices_num);
    /* 加载固件 */
...
static int sap_credo_loopback_set(int unit, int port, int if_side, int lb_dir, int enable)
{
    ...
    rv = cr_port_query(slice_info->cr_slice, port_info->port_id, &port_config);
    
    SAP_CR_RV_CHECK(slice_info, "cr_port_query", rv);
    if (if_side == SAP_PORT_SIDE_LINE) {
        for (i = 0; i < port_config.line_no_of_lanes; i ++) {
            lane_id = port_config.line_start_lane + i;
        ...
        for (i = 0; i < port_config.host_no_of_lanes; i ++) {
            lane_id = port_config.host_start_lane + i;
            rv = cr_lane_set_loopback_mode(slice_info->cr_slice, lane_id, mode);
            SAP_CR_RV_CHECK(slice_info, "cr_lane_set_loopback_mode", rv);
        }
    }
...
static int sap_credo_polarity_set(int unit, int port, int if_side, int tx_rx, uint32_t polarity, bool override)
{
    ...
    slice_info = port_info->slice_info;
    rv = sap_credo_query_port(port_info, if_side, tx_rx, &port_config);
    SAP_RV_CHECK(slice_info, "sap_credo_query_port", rv);
...
static int sap_credo_query_port(sap_credo_port_info_t *port_info, int if_side, int tx_rx, CredoPortConfig_t *port_config)
{
    ...
    rv = cr_port_query(port_info->slice_info->cr_slice, port_id, port_config);
    SAP_CR_RV_CHECK(port_info->slice_info, "cr_port_query", rv);
...
static int sap_credo_prbs_change(int poly, CredoPrbsPattern_t *cr_poly)
{
    ...
    SAP_CR_LOG_DBG("sap_credo_prbs_change sap poly:%d cr_poly:%d", poly, *cr_poly);
    return SAP_STATUS_SUCCESS;
}

static int sap_credo_prbs_operation_set(int tx_rx, sap_slice_info_t *slice_info, int if_side, 
                       const CredoPortConfig_t *port_config, int lane, CredoPrbsPattern_t cr_poly, int admin) {
    int rv, lane_id, i, count, start_lane, enable_t;
    const int *lanes;

    if (if_side == SAP_PORT_SIDE_LINE) {
        count = port_config->line_no_of_lanes;
        start_lane = port_config->line_start_lane;
    } else if (if_side == SAP_PORT_SIDE_SYS) {
        count = port_config->host_no_of_lanes;
        start_lane = port_config->host_start_lane;
    } else {
        return SAP_STATUS_INVALID_PARAMETER;
    }
    if (lane == SAP_LANE_ID_ALL) {
        for (i = 0; i < count; i++) {
            lane_id = start_lane + i;
            if (tx_rx == SAP_CHANNEL_TX) {
                if (!admin) {
                    /* need use valid cr_poly even disable situation */
                    rv = cr_prbs_get_tx_generator(slice_info->cr_slice,  lane_id, &enable_t, &cr_poly);
                    SAP_RV_CHECK(slice_info, "cr_prbs_get_tx_generator", rv);
                }

                rv = cr_prbs_set_tx_generator(slice_info->cr_slice, lane_id, admin, cr_poly);
                SAP_CR_RV_CHECK(slice_info, "cr_prbs_set_tx_generator", rv);
            } else {
                if (!admin) {
                    /* need use valid cr_poly even disable situation */
                    rv = cr_prbs_get_rx_checker(slice_info->cr_slice,  lane_id, &enable_t, &cr_poly);
                    SAP_RV_CHECK(slice_info, "cr_prbs_get_rx_checker", rv);
                }
                rv = cr_prbs_set_rx_checker(slice_info->cr_slice, lane_id, admin, cr_poly);
                SAP_CR_RV_CHECK(slice_info, "cr_prbs_set_rx_checker", rv);
                rv = cr_prbs_reset_rx_count(slice_info->cr_slice, lane_id);
                SAP_CR_RV_CHECK(slice_info, "cr_prbs_reset_rx_count", rv);
            }
        }
    } else {
        ...
        lane_id = start_lane + lane  - SAP_LANE_ID_START;
        if (tx_rx == SAP_CHANNEL_TX) {
            if (!admin) {
                /* need use valid cr_poly even disable situation */
                rv = cr_prbs_get_tx_generator(slice_info->cr_slice,  lane_id, &enable_t, &cr_poly);
...
static int sap_credo_port_prbs_set_ex(int unit, int port, int poly, int inv, int if_side, int lane, int admin)
{
    ...
    rv = sap_credo_query_port(port_info, if_side, SAP_CHANNEL_TX, &port_config);
    SAP_RV_CHECK(slice_info, "sap_credo_query_port", rv);

    rv = sap_credo_prbs_operation_set(SAP_CHANNEL_TX, slice_info, if_side, &port_config, lane, cr_poly, admin);
    SAP_RV_CHECK(slice_info, "sap_credo_prbs_operation_set", rv);
    /* Configure RX Checker to the correct PRBS pattern */
    rv = sap_credo_query_port(port_info, if_side, SAP_CHANNEL_RX, &port_config);
    SAP_RV_CHECK(slice_info, "sap_credo_query_port", rv);

    rv = sap_credo_prbs_operation_set(SAP_CHANNEL_RX, slice_info, if_side, &port_config, lane, cr_poly, admin);
    SAP_RV_CHECK(slice_info, "sap_credo_prbs_operation_set", rv);
    return SAP_STATUS_SUCCESS;
}

static int sap_credo_port_prbs_set_phy_mode(int unit, int port, int poly, int inv, int if_side, int lane, int admin)
{
    int i,rv;
    int lane_id, enable;
    sap_credo_port_info_t *port_info;
    ...
    slice_info = port_info->slice_info;
    if (admin) {
        SAP_RV_CHECK(slice_info, "sap_credo_prbs_change", sap_credo_prbs_change(poly, &cr_poly));
        rv = sap_credo_query_port(port_info, if_side, SAP_CHANNEL_TX, &port_config);
        SAP_RV_CHECK(slice_info, "sap_credo_query_port", rv);
    } else {
        rv = sap_credo_get_profile_info(slice_info, port_info->port_id, &port_config);
    ...
    if (if_side) {
        lane_start = port_config.host_start_lane;
        lane_end = port_config.host_start_lane + port_config.host_no_of_lanes;
    } else {
        lane_start = port_config.line_start_lane;
        lane_end = port_config.line_start_lane + port_config.line_no_of_lanes;
    }
    ...
    if (admin) {
        rv = cr_port_destroy(slice_info->cr_slice, port_config.port_id);
        SAP_RV_CHECK(slice_info, "cr_port_destroy", rv);
        
        for (lane = lane_start; lane < lane_end; lane++) {
            /* old api */
            // rv = cr_lane_destroy_mode(slice_info->cr_slice, lane);
            // SAP_RV_CHECK(slice_info, "cr_lane_destroy_mode", rv);
            // rv = cr_lane_configure_mode(slice_info->cr_slice, lane, lane_mode, lane_speed);
            // SAP_RV_CHECK(slice_info, "cr_lane_configure_mode", rv);

            /* new api */
            rv = cr_phy_destroy(slice_info->cr_slice, lane);
            SAP_RV_CHECK(slice_info, "cr_phy_destroy", rv);
            rv = cr_phy_configure(slice_info->cr_slice, lane, lane_mode, lane_speed, 0);
            SAP_RV_CHECK(slice_info, "cr_lane_configure_mode", rv);
        }

        usleep(200000);
    } else {
        rv = cr_prbs_get_tx_generator(slice_info->cr_slice,  lane_start, &enable, &cr_poly);
        SAP_RV_CHECK(slice_info, "cr_prbs_get_tx_generator", rv);
    }

    for (lane = lane_start; lane < lane_end; lane++) {
        rv = cr_prbs_set_tx_generator(slice_info->cr_slice, lane, admin, cr_poly);
        SAP_RV_CHECK(slice_info, "cr_prbs_set_tx_generator", rv);
        err = cr_prbs_set_rx_checker(slice_info->cr_slice, lane, admin, cr_poly);
        SAP_RV_CHECK(slice_info, "cr_prbs_set_rx_checker", rv);
    }

    if (!admin) {
        /* It need to destroy phy lane first when port_config mode is no force */
        /* If port_config mode is force, it doesn't matter whether the phy lane is destoryed or not. */
        SAP_CR_RV_CHECK(slice_info, "cr_phy_destroy", cr_phy_destroy(slice_info->cr_slice, lane));
        SAP_CR_RV_CHECK(slice_info, "cr_port_configure", cr_port_configure(slice_info->cr_slice, &port_config, CR_FORCE));
    }
...
static int sap_credo_port_prbs_set(int unit, int port, int poly, int inv, int if_side, int lane)
{
    ...
    // return sap_credo_port_prbs_set_phy_mode(unit, port, poly, inv, if_side, lane, CR_ENABLE);
}

/* 读完清除 */
static int sap_credo_prbs_operation_get(sap_slice_info_t *slice_info, int if_side, const CredoPortConfig_t *port_config, int lane, sap_prbs_status_t *prbs_info) {
    int lane_id, rv, i, count, start_lane;
    int link_up = 1;
    unsigned rdy;
    int all_rdy = 0;
    ...
    if (count != 0) {
        is_pam4 = ((port_config->speed / (uint32_t)count) > 25000);
    } else {
        is_pam4 = false;
        SAP_CR_LOG_ERR("count[%d] err", count);
    }
    if ((lane - SAP_LANE_ID_START) >= count) {
        return SAP_STATUS_INVALID_PARAMETER;
    }
    if (lane == SAP_LANE_ID_ALL) {
        for (i = 0; i < count; i++) {
            ...
            if (is_pam4) {
                ...
                /* credo 55321 unsupport get rx lock when work in NRZ mode */
                prbs_info[i].prbs_lock = 1;
            }
...
static int sap_credo_port_prbs_get(int unit, int port, int if_side, sap_prbs_status_t *prbs_info)
{
    ...
    rv = sap_credo_prbs_operation_get(slice_info, if_side, &port_config, lane, prbs_info);
...
static int sap_credo_port_prbs_clear(int unit, int port, int if_side)
{
    ...
    // return sap_credo_port_prbs_set_phy_mode(unit, port, 0, 0, if_side, SAP_LANE_ID_ALL, CR_DISABLE);
}
...
static int sap_credo_linktraining_set(int unit, int port, int if_side, int enable)
{
    ...
    slice_info = port_info->slice_info;
    rv = sap_credo_query_port(port_info, if_side, SAP_CHANNEL_BOTH, &port_config);
    SAP_RV_CHECK(slice_info, "sap_credo_query_port", rv);
...
static int sap_credo_port_status_get(int unit, int port, sap_port_status_t *port_status)
{
    ...
    rv = cr_port_query(slice_info->cr_slice, port_info->port_id, &port_config);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_CR_LOG_ERR("cr_port_query failed, unit:%d, port:%d", unit, port);
        return SAP_STATUS_FAILURE;
    }
    ...
    port_status->speed = port_config.speed;
    port_status->line_lane_num = port_config.line_no_of_lanes;
    ...
    port_status->speed = port_config.speed;
    port_status->host_lane_num = port_config.host_no_of_lanes;
    port_status->fec_line = port_config.line_fec_type;
    port_status->fec_sys = port_config.host_fec_type;

    return SAP_STATUS_SUCCESS;  
}

static int sap_credo_fec_convert_phy2sap(int phy_fec, sap_fec_e *sap_fec)
{
    int i;
...
static int sap_credo_fec_convert_sap2phy(sap_fec_e sap_fec, int *phy_fec)
{
    int i;
    for (i = 0; i < (sizeof(fec_map) / sizeof(fec_map[0])); i++) {
...
static int sap_credo_port_fec_get(int unit, int port, int if_side, sap_fec_e *fec_type)
{
    ...
    SAP_POINT_CHECK_RV(fec_type, "fec_type", SAP_STATUS_INVALID_PARAMETER);
    ...
    slice_info = port_info->slice_info;
    rv = cr_port_query(slice_info->cr_slice, port, &port_config);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_CR_LOG_ERR("cr_port_query failed, unit:%d, port:%d", unit, port);
        return SAP_STATUS_FAILURE;
    }

    if (if_side == CR_SIDE_HOST) {
        fec = port_config.host_fec_type;
    } else {
        fec = port_config.line_fec_type;
    }
    rv = sap_credo_fec_convert_phy2sap(fec, fec_type);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_CR_LOG_ERR("unit[%d] port[%d], sap_bcm_fec_convert_phy2sap failed with rv:%d", unit, port, rv);
        return rv;
    }
    SAP_CR_LOG_DBG("phy fec:%d sap_fec:%d", fec, *fec_type);
...
static int sap_credo_shell_cmd_run(int unit, int port, char *cmdline)
{
    ...
    cr_slash_command_log(&slice_info->cr_slice, 1, cmdline);
    return SAP_STATUS_SUCCESS;
}
