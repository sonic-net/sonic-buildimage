...
#define SAP_BCM_CFG_FILE    "/usr/share/sonic/hwsku/sap_bcm_85344.cfg"
...
sap_bcm_speed_mode_table aperta2_speed_mode_table[] = {
...
sap_bcm_latency_table aperta2_ingress_latency_table[] = {
    {SPEED_800G, 8, false, false, 109}
};

sap_bcm_latency_table aperta2_egress_latency_table[] = {
    {SPEED_800G, 8, false, false, 109}
};
...
static bool sap_validate_vco_for_speed(bcm_plp_pll_vco_t vco, int speed, int lanes);
static bool sap_validate_vco_for_fec_type(bcm_plp_pll_vco_t vco, int speed, int lanes, int fec_type);
static int sap_slot_init(int unit, int slot_id);
static int sap_aperta2_config_init(int unit);
static int sap_aperta2_fw_load(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_phy_init(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_pre_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta2_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta2_post_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta2_lane_init(int unit, int port, int if_side);
static int sap_aperta2_fw_repair(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_clean_up(sap_bcm_phy_info_t *phy_info);
static void sap_aperta2_fw_params_init(sap_bcm_phy_info_t *phy_info, bcm_plp_aperta2_fw_init_params_t *fw_init_params);
static int sap_aperta2_fw_load_unicast(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_fw_load_broadcast(sap_bcm_phy_info_t **phy_info_list, int phy_num);
static int sap_aperta2_fw_load_eeprom(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_fw_load_none(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_fw_set_eeprom(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_fw_soft_reset(sap_bcm_phy_info_t *phy_info);
static int sap_aperta2_fw_hard_reset(sap_bcm_phy_info_t *phy_info);
...
static void *sap_aperta2_phy_init_thread(void *arg);
...
static int sap_aperta2_fill_egr_latency_config(int speed, int lanes, bcm_plp_aperta2_egr_fixed_latency_config *latency_cfg)
{
...
static int sap_aperta2_fill_ing_latency_config(int speed, int lanes, bcm_plp_aperta2_ing_fixed_latency_config *latency_cfg)
{
    latency_cfg->enable = 1;
    latency_cfg->start_point = 0;
    latency_cfg->dp_ck_cycles = 109;
...
static int sap_bcm_platform_init(int unit, bool warm_boot)
{
    ...
    if (fileExists(SAP_BCM_CFG_FILE) != 1) {
        SAP_BCM_LOG_DBG("config file %s not exist", SAP_BCM_CFG_FILE);
        return SAP_STATUS_NOT_SUPPORTED;
    }
    if (SAP_STATUS_SUCCESS != sap_aperta2_config_init(unit)) {
...
static bool sap_validate_vco_for_speed(bcm_plp_pll_vco_t vco, int speed, int lanes)
{
    int i;
    int table_len;
    sap_bcm_speed_mode_table *item;
    table_len = sizeof(aperta2_speed_mode_table) / sizeof(sap_bcm_speed_mode_table);
...
static int sap_bcm_speed_set(int unit, int port, int speed)
{
    /* 设置速率需要校验vco，校验不通过的不予设置，用户调用前需要先调用sap_validate_vco_for_speed进行判断，
...
sap_apis_t sap_extphy_aperta2_apis = {
    .type                           = "BCM_85344",
...
static int sap_slot_init(int unit, int slot_id)
{
    ...
    if (g_fw_load_method == BCM_FW_BROADCAST_LOAD) {
        ...
        sap_aperta2_fw_load_broadcast(phy_infos, count);
        return SAP_STATUS_SUCCESS;
    }
    ...
    for (i=0; i<g_phy_num; i++) {
        ...
        rv = pthread_create(&tid, NULL, sap_aperta2_phy_init_thread, (void *)phy_info);
        if (rv) {
...
static int sap_slot_init(int unit, int slot_id)
{
    ...
    for (i=0; i<g_phy_num; i++) {
        ...
        rv = sap_aperta2_fw_load(phy_info);
        if (rv != SAP_STATUS_SUCCESS) {
            rv = sap_aperta2_clean_up(phy_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_LOG_WARN("PHY-0x%x sap_aperta2_clean_up failed with rv: %d", phy_info->phy_addr, rv);
            }
            rv = sap_aperta2_fw_load(phy_info);
            if (rv != SAP_STATUS_SUCCESS) {
        ...
        /* phy init */
        rv = sap_aperta2_phy_init(phy_info);
        if (rv != SAP_STATUS_SUCCESS) {
        ...
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta2_pre_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta2_pre_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta2_pre_port_init succeed", port_info->unit, port_info->port);
            }
        }
        /* port init */
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta2_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta2_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta2_port_init succeed", port_info->unit, port_info->port);
                port_info->inited = 1;
            }
        }
        /* post port init */
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta2_post_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta2_post_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta2_post_port_init succeed", port_info->unit, port_info->port);
            }
        }
    }
    return SAP_STATUS_SUCCESS;
...
static int sap_aperta2_fw_load(sap_bcm_phy_info_t *phy_info)
{
    ...
    if (g_fw_load_method == BCM_FW_UNICAST_LOAD) {
        rv = sap_aperta2_fw_load_unicast(phy_info);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta2_fw_load_unicast failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    } else if (g_fw_load_method == BCM_FW_EEPROM_LOAD) {
        rv = sap_aperta2_fw_load_eeprom(phy_info);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta2_fw_load_eeprom failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    } else if (g_fw_load_method == BCM_FW_NONE_LOAD) {
        rv = sap_aperta2_fw_load_none(phy_info);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta2_fw_load_none failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    } else if (g_fw_load_method == BCM_FW_UPGRADE) {
        rv = sap_aperta2_fw_set_eeprom(phy_info);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta2_fw_set_eeprom failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    }
...
//     rv = sap_aperta2_fw_repair(phy_info);
//     if (rv != SAP_STATUS_SUCCESS) {
...
static int sap_aperta2_phy_init(sap_bcm_phy_info_t *phy_info)
{
    ...
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_polarity_get failed for lane-map: 0x%x at system side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully get the polarity (tx_pol : 0x%x, rx_pol : 0x%x) for lane-map 0x%x of PHY-0x%x at System side!",
    ...
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_polarity_set failed for lane-map: 0x%x at system side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully set the polarity (tx_pol : 0x%x, rx_pol : 0x%x) for lane-map 0x%x of PHY-0x%x at System side!",
                read_tx_pol, read_rx_pol, plp_access.lane_map, plp_access.phy_addr);
    ...
    rv = bcm_plp_polarity_get(phy_info->chip_name, plp_access, &read_tx_pol, &read_rx_pol);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_polarity_get failed for lane-map: 0x%x at line side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully get the polarity (tx_pol : 0x%x, rx_pol : 0x%x) for lane-map 0x%x of PHY-0x%x at Line side!",
                read_tx_pol, read_rx_pol, plp_access.lane_map, plp_access.phy_addr);
    ...
    rv = bcm_plp_polarity_set(phy_info->chip_name, plp_access, read_tx_pol, read_rx_pol);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_polarity_set failed for lane-map: 0x%x at line side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully set the polarity (tx_pol : 0x%x, rx_pol : 0x%x) for lane-map 0x%x of PHY-0x%x at Line side!",
                read_tx_pol, read_rx_pol, plp_access.lane_map, plp_access.phy_addr);
    if (g_i2c_slave_en == 1) {
...
static int sap_aperta2_clean_up(sap_bcm_phy_info_t *phy_info)
{
    ...
    if (rv != BCM_PM_IF_SUCCESS) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_mac_cleanup failed with rv: %d", plp_access.phy_addr, rv);
        return SAP_STATUS_FAILURE;
    } else {
    ...
    if (rv != BCM_PM_IF_SUCCESS) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_cleanup failed with rv: %d", plp_access.phy_addr, rv);
        return SAP_STATUS_FAILURE;
    } else {
...
static int sap_aperta2_fw_soft_reset(sap_bcm_phy_info_t *phy_info)
{
    ...
    plp_access.platform_ctxt = phy_info;
    plp_access.phy_addr = phy_info->phy_addr;
    // sap_aperta2_fw_params_init(phy_info, &fw_init_params);
    firmware_load_type.firmware_load_method = bcmpmFirmwareLoadMethodProgEEPROM;
    firmware_load_type.force_load_method = bcmpmFirmwareLoadForce;
    // firmware_load_type.fw_init_params = &fw_init_params;
    rv = bcm_plp_init_fw_bcast(phy_info->chip_name, plp_access, sap_bcm_mdio_read, sap_bcm_mdio_write,
...
static int sap_aperta2_polarity_init(sap_bcm_phy_info_t *phy_info)
{
    // sap_bcm_port_info_t *port_info;
...
static int sap_aperta2_post_port_init(sap_bcm_port_info_t *port_info)
{
    ...
    if (port_info->linktrain_sys == 1) {
        ...
        rv = sap_aperta2_lane_init(port_info->unit, port_info->port, PLP_SYS_IF_SIDE);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_LOG_NOTICE("unit[%d] port[%d], sys side sap_aperta2_lane_init failed with rv: %d", port_info->unit, port_info->port, rv);
            return SAP_STATUS_FAILURE;
        }
    }
    if (port_info->linktrain_line == 1) {
        ...
        rv = sap_aperta2_lane_init(port_info->unit, port_info->port, PLP_LINE_IF_SIDE);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_LOG_NOTICE("unit[%d] port[%d], line side sap_aperta2_lane_init failed with rv: %d", port_info->unit, port_info->port, rv);
            return SAP_STATUS_FAILURE;
        }
    }
    return SAP_STATUS_SUCCESS;
...
static int sap_aperta2_port_init(sap_bcm_port_info_t *port_info)
{
    ...
    rv = bcm_plp_mac_fault_option_set((port_info->phy_info)->chip_name, mac_access, BCM_FAULT_PASS_THROUGH);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_mac_fault_option_set failed for line side with rv: %d", plp_access.phy_addr, rv);
        return rv;
    }
...
static int sap_bcm_linktraining_set(int unit, int port, int if_side, int enable)
{
    ...
    if (!enable) {
        rv = sap_aperta2_lane_init(unit, port, if_side);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_LOG_NOTICE("unit[%d] port[%d], sap_aperta2_lane_init at %s side failed with rv: %d", unit, port, BCM_IF_SIDE_STR(if_side), rv);
            return SAP_STATUS_FAILURE;
        }
...
static int sap_bcm_reg_get(int unit, int port, int if_side, int devaddr, int regaddr, int flag, uint64_t *data)
{
    ...
    if (rv != 0) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], bcm_plp_reg_value_get failed with rv:%d", unit, port, rv);
    } else {
...
static int sap_bcm_phy_reset(int unit, int port, int flags)
{
    ...
    rv = sap_aperta2_clean_up(phy_info);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta2_clean_up failed with rv:%d", unit, port, rv);
    }
    phy_info->loaded = 0;
    return rv;
}
