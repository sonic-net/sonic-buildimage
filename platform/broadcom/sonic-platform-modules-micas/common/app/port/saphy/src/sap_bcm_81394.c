...
#define BCM_PLP_DEVICE_AUX_MODE bcm_plp_aperta_device_aux_modes_t
...
#define MUTIL_THREAD
#define aperta_VCO_53G bcmplpVco53G /* bcmplpVco53G = 1 */
#define aperta_VCO_51G bcmplpVco51G /* bcmplpVco51G = 0 */
...
#define SAP_BCM_CFG_FILE    "/usr/share/sonic/hwsku/sap_bcm_81394.cfg"
...
sap_bcm_speed_mode_table aperta_speed_mode_table[] = {
...
static sap_phy_fec_map fec_map[] = {
    {SAP_FEC_NONE,        bcmplpapertaNoFEC     },
    {SAP_FEC_BASER,       bcmplpapertaBaseR     },
    {SAP_FEC_RSFEC,       bcmplpapertaRSFEC     },
    {SAP_FEC_RS544,       bcmplpapertaRS544     },
    {SAP_FEC_RS272,       bcmplpapertaRS272     },
    {SAP_FEC_RS544_2XN,   bcmplpapertaRS544_2XN },
};
...
sap_bcm_latency_table aperta_ingress_latency_table[] = {
    {SPEED_800G, 8, false, false, 109}
};

sap_bcm_latency_table aperta_egress_latency_table[] = {
    {SPEED_800G, 8, false, false, 109}
};
...
static int sap_aperta_config_init(int unit);
static int sap_aperta_fw_load(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_phy_init(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_pre_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta_post_port_init(sap_bcm_port_info_t *port_info);
static int sap_aperta_lane_init(int unit, int port, int if_side);
static int sap_aperta_fw_repair(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_clean_up(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_fw_load_type(sap_bcm_phy_info_t *phy_info, int type);
static int sap_aperta_fw_load_broadcast(sap_bcm_phy_info_t **phy_info_list, int phy_num);
static int sap_aperta_fw_load_full_process(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_fw_soft_reset(sap_bcm_phy_info_t *phy_info);
static int sap_aperta_fw_hard_reset(sap_bcm_phy_info_t *phy_info);
...
static int sap_bcm_fec_set(int unit, int port, int if_side, sap_fec_e fec_type);
static int sap_bcm_fec_get(int unit, int port, int if_side, sap_fec_e *fec_type);
...
static void *sap_aperta_phy_init_thread(void *arg);
...
static int sap_bcm_platform_init(int unit, bool warm_boot)
{
    ...
    rv = sap_aperta_config_init(unit);
    if (SAP_STATUS_SUCCESS != rv) {
...
static int sap_aperta_chip_id_get(sap_bcm_phy_info_t *phy_info, uint32_t phy_addr, uint32_t *chip_id)
{
    int rv;
    uint32_t data;
    ...
    *chip_id = 0xFFFF;
    ...
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("mdio_read fail rv: %d", rv);
        return rv;
    } else {
        SAP_BCM_LOG_DBG("mdio_read chip id addr:0x%x date:0x%x", BCM_81394_CHIP_ID_ADDR, data);
        *chip_id = data;
    }
...
static int sap_aperta_config_init(int unit)
{
    ...
    for (i=0; i<g_phy_num; i++) {
        ...
        /* check chip id */
        chip_id = 0xFFFF;
        rv = sap_aperta_chip_id_get(phy_info, phy_info->phy_addr, &chip_id);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_chip_id_get fail", phy_info->phy_addr);
            return SAP_STATUS_FAILURE;
        } else {
            if (chip_id == 0xFFFF) {
                SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_chip_id_get chip_id is invalid", phy_info->phy_addr);
                return SAP_STATUS_FAILURE;
            }
            if (chip_id != BCM_81394_CHIP_ID) {
                SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_chip_id_get chip_id isn't 81394", phy_info->phy_addr);
...
static int sap_slot_init(int unit, int slot_id)
{
    ...
    SAP_BCM_LOG_DBG("slot %d init", slot_id);
    if (g_fw_load_method == BCM_FW_BROADCAST_LOAD) {
        ...
        sap_aperta_fw_load_broadcast(phy_infos, count);
        return SAP_STATUS_SUCCESS;
    }
    ...
    for (i=0; i<g_phy_num; i++) {
        ...
        rv = pthread_create(&tid, NULL, sap_aperta_phy_init_thread, (void *)phy_info);
        if (rv) {
...
static int sap_slot_init(int unit, int slot_id)
{
    ...
    for (i=0; i<g_phy_num; i++) {
        ...
        rv = sap_aperta_fw_load(phy_info);
        if (rv != SAP_STATUS_SUCCESS) {
            rv = sap_aperta_clean_up(phy_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_LOG_WARN("PHY-0x%x sap_aperta_clean_up failed with rv: %d", phy_info->phy_addr, rv);
            }
            rv = sap_aperta_fw_load(phy_info);
            if (rv != SAP_STATUS_SUCCESS) {
        ...
        /* phy init */
        rv = sap_aperta_phy_init(phy_info);
        if (rv != SAP_STATUS_SUCCESS) {
        ...
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta_pre_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta_pre_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta_pre_port_init succeed", port_info->unit, port_info->port);
            }
        }
        /* port init */
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta_port_init succeed", port_info->unit, port_info->port);
                port_info->inited = 1;
            }
        }
        /* post port init */
        list_for_each_entry(sap_port_info, &global_port_list_head, node, sap_port_info_t) {
            ...
            rv = sap_aperta_post_port_init(port_info);
            if (rv != SAP_STATUS_SUCCESS) {
                SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta_post_port_init failed with rv: %d", port_info->unit, port_info->port, rv);
                continue;
            } else {
                SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_aperta_post_port_init succeed", port_info->unit, port_info->port);
            }
        }
    }
    return SAP_STATUS_SUCCESS;
...
static int sap_aperta_fw_load(sap_bcm_phy_info_t *phy_info)
{
    ...
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-%s-0x%x bcm_plp_mutex_info_set failed with rv: %d", phy_info->chip_name, phy_info->phy_addr, rv);
        return rv;
    }
    if (g_fw_load_method == BCM_FW_LOAD_FP) {
        rv = sap_aperta_fw_load_full_process(phy_info);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_fw_load_full_process failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    } else {
        rv = sap_aperta_fw_load_type(phy_info, g_fw_load_method);
        if (rv != 0) {
            SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_fw_load_type failed with rv: %d", phy_info->phy_addr, rv);
            return rv;
        }
    }
...
//     rv = sap_aperta_fw_repair(phy_info);
//     if (rv != SAP_STATUS_SUCCESS) {
...
static int sap_aperta_phy_init(sap_bcm_phy_info_t *phy_info)
{
    ...
    uint32_t fw_ver, fw_crc;
    bcm_plp_access_t plp_access;
    bcm_plp_laneswap_map_t sys_lane_map, line_lane_map;
    sap_bcm_port_info_t *port_info;
    if (phy_info == NULL) {
        return SAP_STATUS_INVALID_PARAMETER;
    }
    ...
    /* 线序初始化 */
    memset(&sys_lane_map, 0, sizeof(bcm_plp_laneswap_map_t));
    memset(&line_lane_map, 0, sizeof(bcm_plp_laneswap_map_t));
    ...
    rv = bcm_plp_rxtx_laneswap_set(phy_info->chip_name, plp_access, &sys_lane_map);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_rxtx_laneswap_set failed at system side with rv: %d", phy_info->phy_addr, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully set rx logical lane (sys : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x) for PHY-0x%x at System side!",
                sys_lane_map.lane_map_rx[0], sys_lane_map.lane_map_rx[1], sys_lane_map.lane_map_rx[2], sys_lane_map.lane_map_rx[3],
                sys_lane_map.lane_map_rx[4], sys_lane_map.lane_map_rx[5], sys_lane_map.lane_map_rx[6], sys_lane_map.lane_map_rx[7],
                plp_access.phy_addr);
    SAP_BCM_LOG_DBG("PASS: Successfully set tx logical lane (sys : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x) for PHY-0x%x at System side!",
                sys_lane_map.lane_map_tx[0], sys_lane_map.lane_map_tx[1], sys_lane_map.lane_map_tx[2], sys_lane_map.lane_map_tx[3],
                sys_lane_map.lane_map_tx[4], sys_lane_map.lane_map_tx[5], sys_lane_map.lane_map_tx[6], sys_lane_map.lane_map_tx[7],
                plp_access.phy_addr);

    plp_access.if_side  = BCM_LINE_SIDE;
    rv = bcm_plp_rxtx_laneswap_set(phy_info->chip_name, plp_access, &line_lane_map);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_rxtx_laneswap_set failed for lane-map: 0x%x at system side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("PASS: Successfully set rx logical lane (line : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x) for PHY-0x%x at System side!",
                line_lane_map.lane_map_rx[0], line_lane_map.lane_map_rx[1], line_lane_map.lane_map_rx[2], line_lane_map.lane_map_rx[3],
                line_lane_map.lane_map_rx[4], line_lane_map.lane_map_rx[5], line_lane_map.lane_map_rx[6], line_lane_map.lane_map_rx[7],
                plp_access.phy_addr);
    SAP_BCM_LOG_DBG("PASS: Successfully set tx logical lane (line : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x) for PHY-0x%x at System side!",
                line_lane_map.lane_map_tx[0], line_lane_map.lane_map_tx[1], line_lane_map.lane_map_tx[2], line_lane_map.lane_map_tx[3],
                line_lane_map.lane_map_tx[4], line_lane_map.lane_map_tx[5], line_lane_map.lane_map_tx[6], line_lane_map.lane_map_tx[7],
                plp_access.phy_addr);

    memset(&sys_lane_map, 0, sizeof(bcm_plp_logical_lane_map_t));
    plp_access.if_side = BCM_SYSTEM_SIDE;
    rv = bcm_plp_rxtx_laneswap_get(phy_info->chip_name, plp_access, &sys_lane_map);
    if (rv != 0) {
    ...
    plp_access.if_side = BCM_SYSTEM_SIDE;
    rv = bcm_plp_rxtx_laneswap_get(phy_info->chip_name, plp_access, &line_lane_map);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_logical_lane_get failed for lane-map: 0x%x at system side with rv: %d", phy_info->phy_addr, plp_access.lane_map, rv);
        return rv;
    }
...
static int sap_aperta_fw_load_type(sap_bcm_phy_info_t *phy_info, int type)
{
    ...
    bcm_plp_firmware_load_type_t firmware_load_type;
    bcm_plp_aperta_fw_init_params_t aperta_init_params;
    bcm_pm_firmware_broadcast_method_t load_method;
    ...
    if (type == BCM_FW_NONE_LOAD) {
        ...
        load_method = bcmpmFirmwareBroadcastNone;
    } else if (type == BCM_FW_UNICAST_LOAD) {
        firmware_load_type.firmware_load_method = bcmpmFirmwareLoadMethodInternal;
        firmware_load_type.force_load_method = bcmpmFirmwareLoadForce;
        load_method = bcmpmFirmwareBroadcastNone;
    } else if (type == BCM_FW_EEPROM_LOAD) {
        firmware_load_type.firmware_load_method = bcmpmFirmwareLoadMethodProgEEPROM;
        firmware_load_type.force_load_method = bcmpmFirmwareLoadForce;
        load_method = bcmpmFirmwareBroadcastNone;
    } else if (type == BCM_FW_UPGRADE) {
        firmware_load_type.firmware_load_method = bcmpmFirmwareLoadMethodProgEEPROM;
        firmware_load_type.force_load_method = bcmpmFirmwareLoadForce;
        load_method = bcmpmFirmwareBroadcastFirmwareExecute;
    } else if (type == BCM_FW_LOAD_SKIP) {
        firmware_load_type.firmware_load_method = bcmpmFirmwareLoadMethodProgEEPROM;
        firmware_load_type.force_load_method = bcmpmFirmwareLoadSkip;
        load_method = bcmpmFirmwareBroadcastNone;
    } else {
        SAP_BCM_LOG_ERR("PHY-0x%x sap_aperta_fw_load_type type %d is unknown", phy_info->phy_addr, type);
        return SAP_STATUS_INVALID_PARAMETER;
    }
    firmware_load_type.firmware_loader = NULL;
    firmware_load_type.fw_init_params = &aperta_init_params; 
    rv = bcm_plp_init_fw_bcast(phy_info->chip_name, plp_access, sap_bcm_mdio_read, sap_bcm_mdio_write,
            &firmware_load_type, load_method);
    if (rv != BCM_PM_IF_SUCCESS) {
        SAP_BCM_LOG_ERR("PHY-0x%x load_method:%d API err %d", phy_info->phy_addr, load_method, rv);
        return SAP_STATUS_FAILURE;
    } else {
        SAP_BCM_LOG_INFO("PHY-0x%x load_method:%d API success", phy_info->phy_addr, load_method);
    }
    sleep(1);
    return SAP_STATUS_SUCCESS;
}
...
static int sap_aperta_polarity_init(sap_bcm_phy_info_t *phy_info)
{
    // sap_bcm_port_info_t *port_info;
...
static int sap_aperta_pre_port_init(sap_bcm_port_info_t *port_info)
{
    ...
    /* enable flow control */
    rv = sap_bcm_flow_control_set(port_info->unit, port_info->port, PLP_LINE_IF_SIDE, bcmplpFlowcontrolTerminateGenerate);
    // rv = sap_bcm_flow_control_set(port_info->unit, port_info->port, PLP_LINE_IF_SIDE, bcmplpFlowcontrolPassthrough);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_bcm_flow_control_set failed with rv: %d", port_info->unit, port_info->port, rv);
        return SAP_STATUS_FAILURE;
    } else {
        SAP_BCM_LOG_INFO("unit[%d] port[%d], sap_bcm_flow_control_set succeed", port_info->unit, port_info->port);
    }
#endif
    return SAP_STATUS_SUCCESS;
}
...
static int sap_aperta_post_port_init(sap_bcm_port_info_t *port_info)
{
    ...
    if (port_info->linktrain_sys == 1) {
        ...
        //     return SAP_STATUS_FAILURE;
        // }
        // rv = sap_aperta_lane_init(port_info->unit, port_info->port, PLP_SYS_IF_SIDE);
        // if (rv != SAP_STATUS_SUCCESS) {
        //     SAP_LOG_NOTICE("unit[%d] port[%d], sys side sap_aperta_lane_init failed with rv: %d", port_info->unit, port_info->port, rv);
        //     return SAP_STATUS_FAILURE;
        // }
    }
    if (port_info->linktrain_line == 1) {
        ...
        //     return SAP_STATUS_FAILURE;
        // }
        // rv = sap_aperta_lane_init(port_info->unit, port_info->port, PLP_LINE_IF_SIDE);
        // if (rv != SAP_STATUS_SUCCESS) {
        //     SAP_LOG_NOTICE("unit[%d] port[%d], line side sap_aperta_lane_init failed with rv: %d", port_info->unit, port_info->port, rv);
        //     return SAP_STATUS_FAILURE;
        // }
    }
    return SAP_STATUS_SUCCESS;
...
static int sap_aperta_port_init(sap_bcm_port_info_t *port_info)
{
    ...
    SAP_BCM_LOG_INFO("PHY-0x%x port_type: %d", plp_access.phy_addr, port_type);
    ...
    if (rv != 0) {
        SAP_BCM_LOG_ERR("PHY-0x%x bcm_plp_mac_fault_option_set failed for system side with rv: %d", plp_access.phy_addr, rv);
        return rv;
    }
#endif
...
static int sap_bcm_linktraining_set(int unit, int port, int if_side, int enable)
{
    ...
    if (!enable) {
        rv = sap_aperta_lane_init(unit, port, if_side);
        if (rv != SAP_STATUS_SUCCESS) {
            SAP_LOG_NOTICE("unit[%d] port[%d], sap_aperta_lane_init at %s side failed with rv: %d", unit, port, BCM_IF_SIDE_STR(if_side), rv);
            return SAP_STATUS_FAILURE;
        }
...
static int sap_bcm_fec_convert_phy2sap(int phy_fec, sap_fec_e *sap_fec)
{
    int i;
    for (i = 0; i < (sizeof(fec_map) / sizeof(fec_map[0])); i++) {
        if (phy_fec == fec_map[i].phy_fec) {
            *sap_fec = fec_map[i].sap_fec;
            return SAP_STATUS_SUCCESS;
        }
    }
    return SAP_STATUS_INVALID_PARAMETER;
...
static int sap_bcm_fec_convert_sap2phy(sap_fec_e sap_fec, int *phy_fec)
{
    int i;
    for (i = 0; i < (sizeof(fec_map) / sizeof(fec_map[0])); i++) {
        if (sap_fec == fec_map[i].sap_fec) {
            *phy_fec = fec_map[i].phy_fec;
            return SAP_STATUS_SUCCESS;
        }
    }
    return SAP_STATUS_INVALID_PARAMETER;
...
{
    int rv = 0;
    int speed = 0, if_type = 0, ref_clk = 0, if_mode = 0, fec_mode;
    sap_bcm_phy_info_t *phy_info;
    bcm_plp_access_t  phy_access;
    bcm_plp_aperta_device_aux_modes_t aux_mode;

    rv = sap_bcm_fec_convert_sap2phy(fec_type, &fec_mode);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_bcm_fec_convert_sap2phy failed with rv:%d", unit, port, rv);
        return rv;
    }
    memset(&phy_access, 0, sizeof(bcm_plp_access_t));
    rv = sap_phy_access_get(unit, port, if_side, &phy_access);
    ...
    memset(&aux_mode, 0, sizeof(bcm_plp_aperta_device_aux_modes_t));
    rv = bcm_plp_mode_config_get(phy_info->chip_name, phy_access, &speed, &if_type, &ref_clk, &if_mode, (void*)&aux_mode);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], bcm_plp_mode_config_get failed with rv:%d", unit, port, rv);
        return SAP_STATUS_FAILURE;
    }
    aux_mode.fec_mode_sel = fec_mode;
    rv = bcm_plp_mode_config_set(phy_info->chip_name, phy_access, speed, if_type, ref_clk, if_mode, (void*)&aux_mode);
    if (rv != 0) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], bcm_plp_mode_config_set failed with rv:%d", unit, port, rv);
        return SAP_STATUS_FAILURE;
    }
...
static int sap_bcm_fec_get(int unit, int port, int if_side, sap_fec_e *fec_type)
{
    ...
    rv = sap_bcm_fec_convert_phy2sap(aux_mode.fec_mode_sel, fec_type);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_bcm_fec_convert_phy2sap failed with rv:%d", unit, port, rv);
        return rv;
    }
    SAP_BCM_LOG_DBG("phy fec:%d sap_fec:%d", aux_mode.fec_mode_sel, *fec_type);
...
static int sap_bcm_phy_reset(int unit, int port, int flags)
{
    ...
    rv = sap_aperta_clean_up(phy_info);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta_clean_up failed with rv:%d", unit, port, rv);
    }
    phy_info->loaded = 0;
    return rv;
}
...
static int sap_bcm_eeprom_update(int unit, int port, bool use_file)
{
    ...
    rv = sap_aperta_fw_repair(phy_info);
    if (rv != SAP_STATUS_SUCCESS) {
        SAP_BCM_LOG_ERR("unit[%d] port[%d], sap_aperta_fw_repair failed with rv:%d", unit, port, rv);
        return SAP_STATUS_ITEM_NOT_FOUND;
    }
