...
#elif defined(PLP_APERTA2_SUPPORT)
  #include "aperta2_common.h"
  #define  LANE_MAX                 16
  #define  LANEMAP_MIN              0x01
  #define  LANEMAP_MAX              0x10000
  #define  LANE_WIDTH               1
...
#define  PLP_ACCESS_INFO            ((bcm_plp_access_t) {&p_ctxt, PHY_ID, LINE_SIDE, LANE_MAP})
#else
...
int timesync_verify_tc_dpll(char* chipname, char *opt, int port, unsigned int arg[]);
...
void getipv6bytes(const char* str, char sep, unsigned int* bytes, int maxb, int b) {
    int i = 0;
    for (i = 0; i < maxb; i++) {
        bytes[i] = strtoul(str, NULL, b);
        str = strchr(str, sep);
...
scallop(char cmdline[])
{
        ...
        if ( PHY_INVALID_ARG != phy_id ) {
            ...
            if ( PHY_INVALID_ARG != val ) {
                rv = bcm_plp_reg_value_set(chipname,
                                    ...
                                    1, regad,  val);
                printf("rv=%d PhyID=[%02x.0x%04x]%s Reg.0x%08x <== 0x%04x\n", rv,
...
#if defined(PLP_APERTA_25G_50G_FOV) || defined(PLP_APERTA2_FAILOVER)
    ...
    IS_STRMATCH2(cmd, "failover", "fov") {
        ...
        if ( ! rv ) {
            printf("PhyID 0x%04X  LaneMap 0x%04X ==>> 0x%04X\n", phy_info.phy_addr,
    ...
    IS_STRMATCH2(cmd, "tcdpll", "tc") {
        ...
        rv = timesync_verify_tc_dpll(chipname, opt, port, arg);
        if ( rv )   printf("!! rv = %d\n", rv);
    } else
    ...
    IS_STRMATCH2(cmd, "tsibf", "ibf") {
        ...
        IS_STRMATCH2(first, "valid", "v") {
            ...
            filter.flags = (macip << 3 | dst << 1 | src) & 0xFF;
            printf("filter.flags:0x%02x, macip:%d, dst:%d, src:%d\n", filter.flags, macip, dst, src);
            ...
            if (macip == 1) {
                ...
                unsigned int ipv6[8];
                getipv6bytes(addrval, ':', ipv6, 8, 16);
                for (ii = 15, jj = 0; jj < 8; jj++) {
                    filter.match_addr.ip6_addr[ii--] = (ipv6[jj] >> 8) & 0xFF;
                    filter.match_addr.ip6_addr[ii--] = ipv6[jj] & 0xFF;
                }
                printf("Set IBF entry: Index[%d] Valid[%d] Action[%d] Flags[0x%x] "
                       "Direction[%d] IPV6[%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x]\n",
