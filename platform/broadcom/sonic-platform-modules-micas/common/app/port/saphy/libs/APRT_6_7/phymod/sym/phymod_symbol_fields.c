 ...
 */
uint32_t *
plp_aperta_phymod_field_get(const uint32_t *entbuf, int sbit, int ebit, uint32_t *fbuf)
{
    int i, wp, bp, len;
 ...
 */
int
plp_aperta_phymod_field_set(uint32_t *entbuf, int sbit, int ebit, uint32_t *fbuf)
{
    uint32_t mask;
    int i, wp, bp, len;
...
uint32_t *
plp_aperta_phymod_field_info_decode(uint32_t* fp, plp_aperta_phymod_field_info_t* finfo, const char** fnames)
{
    if(!fp) {
        return NULL; 
    }
...
{    
    int count = 0; 
    while(fp) {
        fp = plp_aperta_phymod_field_info_decode(fp, NULL, NULL); 
        count++; 
    }   
    return count; 
}
