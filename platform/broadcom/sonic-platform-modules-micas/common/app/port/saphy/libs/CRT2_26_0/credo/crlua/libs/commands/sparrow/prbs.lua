...
slash.register_chip_command({"prbs", "monitor"}, credo.FAMILY_SPARROW, [[
...
Arguments:
    ...
    -,--duration (optional time)           Switch from global time to a window time
...
Examples:
    ...
    if not argt.init and argt.duration ~= nil then
        command = command .. " %d" % {argt.duration * 1000}
    end
