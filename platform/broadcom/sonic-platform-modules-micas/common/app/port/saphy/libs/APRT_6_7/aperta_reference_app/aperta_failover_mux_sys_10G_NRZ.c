 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Aperta chip in system side failover mode  *
 * as follows:                                                                      *
 *   1. Connect to the board.                                                       *
 *   2. Initialize the PHY and download the firmware                                *
 *   3. Initialize MACsec in static bypass mode                                     *
 *   4. Configure Tx/Rx polarity                                                    *
 *   5. Configure PHY-0 system side in failover mode                                *
...
#define SYS_FAILOVER_LANEMAP_PHY0   0x02    /* Failover lane map of PHY-0 */
#define SYS_FAILOVER_LANEMAP_PHY1   0x00
#define SYS_FAILOVER_MUXLOC_PHY0    0x00    /* 0x00: Failover after macsec. 0x01: Failover before macsec */
#define SYS_FAILOVER_MUXLOC_PHY1    0x00

/* Set System side primary lane map */
...
#define LINE_FAILOVER_LANEMAP_PHY0  0x00
#define LINE_FAILOVER_LANEMAP_PHY1  0x00
#define LINE_FAILOVER_MUXLOC_PHY0   0x00
#define LINE_FAILOVER_MUXLOC_PHY1   0x00

/* Set Line side primary lane map */
...
int main(int argc, char *argv[])
{
    ...
    }
    {
        ...
        phy_info.if_side  = SYS_SIDE ;
        phy_info.lane_map = SYS_FAILOVER_LANEMAP_PHY0 ;
        retval = bcm_plp_link_status_get(CHIP_NAME, phy_info,  &link_status_get);
        if (retval != TEST_SUCCESS) {
            printf("FAIL: Failed to get link status for lane-map 0x%x of PHY-%d at %s side (ret = %d)!\n",
            phy_info.lane_map, phy_info.phy_addr, (side == SYS_SIDE) ? "System" : "Line", retval);
    ...
    printf("\n++++++++++++++++++++++++++++++++++++++++++++++++");
    printf("\n Switch over to failover path - System side PHY-0:\n");
    printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
