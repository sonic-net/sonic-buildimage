 ...
 *   5. Configure operating mode parameters for 400G PAM4 (8x53G) mode               *
 *   6. Setup link training or configure TX FIR settings                            *
 *   7. Configure MACsec cleartag functionality using Secy/Cfye.                    *
 *      This application can be used to validate VLAN QinQ, EoMPLS, MPLS and PBB    *
 *      packet types.                                                               *
 *      - To verify different packet types, configure pkt_type variable to          *
 *        following values:                                                         *
 *         PKT_TYPE_PBB                                                             *
 *         PKT_TYPE_EoMPLS_2LABELS                                                  *
 *         PKT_TYPE_VLAN_QINQ                                                       *
 *         PKT_TYPE_MPLS_2LABELS                                                    *
 *      - Configure correct rules for Destination MAC, VLAN, or MPLS labels         *
 *        in function populate_tcam_rules() based on the different packet types.    *
 *        The Destination MAC, VLAN or MPLS labels used in the reference            *
 *        application are based on an example packet. These values need to be       *
 *        changed with the actual packet values.                                    *
 *   7. Cleanup and close the connection to the board if uninitialization path is   *
 *       enabled                                                                    *
 * -------------------------------------------------------------------------------- *
...
#define PKT_TYPE_PBB              0x1
#define PKT_TYPE_EoMPLS_2LABELS   0x2
#define PKT_TYPE_VLAN_QINQ        0x3
#define PKT_TYPE_MPLS_2LABELS     0x4
#define MAX_NUM_OF_PHY            1024

const char *pkt_type_name[] = {"INVALID", "PBB", "EoMPLS", "VLAN QinQ", "MPLS"};
...
int populate_tcam_rules(int macsec_side, bcm_plp_cfye_rule_t *ruleparams, int pkt_type)
{
    ...
    switch(pkt_type) {
        case PKT_TYPE_PBB:
            if (macsec_side == EGRESS || macsec_side == INGRESS) {
                ruleparams->mask.packet_type = 0x2; /* exact match on all these fields. */
                ruleparams->mask.num_tags = 0x7F;
                ruleparams->key.packet_type = BCM_PLP_CFYE_RULE_PKT_TYPE_PBB;
            ...
            break;
        case PKT_TYPE_EoMPLS_2LABELS:
            if (macsec_side == EGRESS || macsec_side == INGRESS) {
                ruleparams->mask.packet_type = 0x3; /* exact match on all these fields. */
                ruleparams->mask.num_tags = 0x7F;
                ruleparams->key.packet_type = BCM_PLP_CFYE_RULE_PKT_TYPE_MPLS;
                ruleparams->key.num_tags = (0x1 << 2);
                ruleparams->data_mask[0] = 0xffffffff; /* match on destination address. */
                ruleparams->data_mask[1] = 0x0000ffff;
                ruleparams->data_mask[2] = 0x00ffffff;
                ruleparams->data_mask[3] = 0x00ffffff;
                /* Set the Dest MAC: 00:00:00:09:01:01 */
                ruleparams->data[0] = 0x9000000;
                ruleparams->data[1] = 0x101;
                /* Set MPLS Label 1 (Byte 0 to 3) and 2 (Byte 0 to 3)*/
                ruleparams->data[2] = 0xD2 | (0x2D << 8) | (0x3C << 16);
                ruleparams->data[3] = 0xA5 | (0x5A << 8) | (0x77 << 16);
            }
            break;
...
/*
 * This function configures the pre_sectag_auth_start and pre_sectag_auth_length
 * parameters for egress or ingress packets which represents number of bytes in front
 * of MAC SA/DA and length of MAC SA/DA fields.
 *
 * macsec_side: 0 for Egress and 1 for Ingress.
 * sa: bcm_plp_secy_sa_t structure to configure SA parameters.
 * pkt_type: Selects the types of packet to configure the rule.
 */
int populate_sa_params(int macsec_side, bcm_plp_secy_sa_t *sa, int pkt_type)
{
    int rv = 0;
    ...
    switch(pkt_type) {
        case PKT_TYPE_PBB:
            if (macsec_side == INGRESS) {
                sa->params.ingress.fmac_inmac = 1;
                sa->params.ingress.pre_sectag_auth_start = 22;
                sa->params.ingress.pre_sectag_auth_length = 12;
            } else {
                sa->params.egress.pre_sectag_auth_start = 22;
                sa->params.egress.pre_sectag_auth_length = 12;
            }
            break;
        case PKT_TYPE_EoMPLS_2LABELS:
            if (macsec_side == INGRESS) {
                sa->params.ingress.feompls = 1;
                sa->params.ingress.pre_sectag_auth_start = 22;
                sa->params.ingress.pre_sectag_auth_length = 12;
            } else {
                sa->params.egress.pre_sectag_auth_start = 22;
        ...
        case PKT_TYPE_VLAN_QINQ:
            if (macsec_side == INGRESS) {
                sa->params.ingress.pre_sectag_auth_start = 0;
                sa->params.ingress.pre_sectag_auth_length = 12;
            } else {
                sa->params.egress.pre_sectag_auth_start = 0;
                sa->params.egress.pre_sectag_auth_length = 12;
            }
            break;
        case PKT_TYPE_MPLS_2LABELS:
            if (macsec_side == INGRESS) {
                sa->params.ingress.pre_sectag_auth_start = 22;
                sa->params.ingress.pre_sectag_auth_length = 0;
            } else {
                sa->params.egress.pre_sectag_auth_start = 22;
                sa->params.egress.pre_sectag_auth_length = 0;
            }
            break;
        default:
            printf("%s(): Invalid pkt_type, macsec_side[%d], pkt_type[0x%x] \n",
...
/* Install parsers settings for SecTAG and MPLS labels */
int macsec_install_parser_settings(bcm_plp_sec_phy_access_t sec_info, int pkt_type)
{
    ...
    bcm_plp_cfye_pbb_parser_t pbb_parser;
    bcm_plp_cfye_mpls_parser_t MPLSParams;
    ...
    memset(&pbb_parser, 0, sizeof(bcm_plp_cfye_pbb_parser_t));
    memset(&MPLSParams, 0, sizeof(bcm_plp_cfye_mpls_parser_t));
    ...
    switch(pkt_type) {
        case PKT_TYPE_PBB:
            pbb_parser.fcompare = 1;
            pbb_parser.pbb_etype.btag_etype = 0x88a8;
            pbb_parser.pbb_etype.itag_etype = 0x88e7;
            HeaderParams.pbb_parser_p = &pbb_parser;
            break;
        case PKT_TYPE_EoMPLS_2LABELS:
            MPLSParams.mpls3_select1 = BCM_PLP_CFYE_MPLS3_SELECT_LABEL1;
            MPLSParams.mpls3_select2 = BCM_PLP_CFYE_MPLS3_SELECT_LABEL2;
            MPLSParams.mpls4_select1 = BCM_PLP_CFYE_MPLS4_SELECT_LABEL1;
            MPLSParams.mpls4_select2 = BCM_PLP_CFYE_MPLS4_SELECT_LABEL4;
            MPLSParams.mpls_etype[0].fcompare = 1;
            MPLSParams.mpls_etype[0].mpls_etype = 0x8847;
            MPLSParams.mpls_etype[1].fcompare = 1;
            MPLSParams.mpls_etype[1].mpls_etype = 0x8848;
            MPLSParams.mpls_etype[2].fcompare = 1;
            MPLSParams.mpls_etype[2].mpls_etype = 0x9947;
            HeaderParams.mpls_parser_p = &MPLSParams;
            break;
        case PKT_TYPE_VLAN_QINQ:
...
int macsec_install_sa_with_ct_params(bcm_plp_sec_phy_access_t sec_info, macsec_policy_bypass_option_t policy, int pkt_type)
{
    ...
    if (populate_sa_params(macsec_side, &sa_params, pkt_type) < 0) {
        printf("FAIL: populate_sa_params failed for phy_index[%d], macsec_side[%d], return code =[%d] \n",
        sec_info.phy_info.phy_addr, sec_info.macsec_side, retval);
        return retval;
    }
...
int main(int argc, char *argv[])
{
    ...
    /*NOTE: pkt_type needs to be modified to test VLAN, MPLS, or EoMPLS packets*/
    /* int pkt_type = PKT_TYPE_PBB; */
    int pkt_type = PKT_TYPE_VLAN_QINQ;
    /* int pkt_type = PKT_TYPE_EoMPLS_2LABELS; */
    /* int pkt_type = PKT_TYPE_MPLS_2LABELS; */
   ...
   total_lane_maps = NUM_ARR_ELEMENTS(sys_lane_map_list);
    ...
    for (phy_index = 0; phy_index < NUM_OF_PHY; phy_index++) {
        ...
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            ...
            for (macsec_side = 0; macsec_side < 2; macsec_side++) {
                ...
                switch(pkt_type) {
                    case PKT_TYPE_PBB:
                        /* sectag_offset values should be as follows:
                          42: Leave PBB Header + Both Outer and Inner VLAN tag in the clear.
                          38: Leave PBB Header + Outer VLAN tag in the clear.
                          34: Leave PBB Header + Customer MAC in the clear.
                          22: Leave PBB Header in the clear and encrypt the packet.
                          12: Standard MACsec encrypted packet including PBB Header.
                        */
                        vportparams.sectag_offset = 34;
                        break;
                    case PKT_TYPE_EoMPLS_2LABELS:
                        vportparams.sectag_offset = 34;
                        vportparams.feompls_support = 1;
                        break;
                    case PKT_TYPE_VLAN_QINQ:
                        vportparams.sectag_offset = 16;
                        break;
                    case PKT_TYPE_MPLS_2LABELS:
                        vportparams.sectag_offset = macsec_side ? 0 : 22;
                        break;
                    default:
                        printf("Invalid pkt_type for PHY-ID[%d],  macsec_side[%d], LANE_MAP[0x%x], pkt_type[0x%x] \n",
                            phy_info.phy_addr, sec_info.macsec_side, phy_info.lane_map, pkt_type);
                        goto _aperta_config_error;
                }
    ...
    printf("Please send %s type packet(s), Waiting for 10 seconds for packets to pass\n",
        pkt_type_name[pkt_type]);
    printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
    sleep(10);
    printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
