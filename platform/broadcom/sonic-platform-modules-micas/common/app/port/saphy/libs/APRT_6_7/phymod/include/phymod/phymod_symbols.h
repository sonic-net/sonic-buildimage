...
#ifndef __plp_aperta_PHYMOD_SYMBOLS_H__
#define __plp_aperta_PHYMOD_SYMBOLS_H__
 ...
 */
typedef struct plp_aperta_phymod_field_info_s {
    const char *name; 
    int fid; 
    uint16_t minbit;
    uint16_t maxbit; 
} plp_aperta_phymod_field_info_t; 
...
extern uint32_t* plp_aperta_phymod_field_info_decode(uint32_t* fp, 
                                       plp_aperta_phymod_field_info_t* finfo, 
                                       const char** fnames);
extern uint32_t
plp_aperta_phymod_field_info_count(uint32_t* fp);  
...
{                                                              \
    ...
    for(;;) {                                                  \
      if(!__fp) {                                              \
        ...
        __fp = plp_aperta_phymod_field_info_decode(__fp, &(finfo), (fnames));    
 ...
 *
 */
typedef struct plp_aperta_phymod_symbol_s {
...
} plp_aperta_phymod_symbol_t; 


/*
 * Symbols Information Structure (plural)
 */
typedef struct plp_aperta_phymod_symbols_s {    
    
    const plp_aperta_phymod_symbol_t *symbols; 
    const uint32_t size; 
    const char** field_names; 

} plp_aperta_phymod_symbols_t; 


/*
 * Search a symbol array. Utility function. 
 */
extern const plp_aperta_phymod_symbol_t *phymod_symbol_find(const char *name, 
                                           const plp_aperta_phymod_symbol_t *table, 
                                           int size); 
...
extern int plp_aperta_phymod_symbols_find(const char *name, const plp_aperta_phymod_symbols_t *symbols, 
                            plp_aperta_phymod_symbol_t *rsym); 

/*
 * Get index of a specific symbol
 */
extern int plp_aperta_phymod_symbols_index(const plp_aperta_phymod_symbols_t *symbols,
                             const plp_aperta_phymod_symbol_t *symbol);


/*
 * Get a specific symbol (by index)
 */
extern int plp_aperta_phymod_symbols_get(const plp_aperta_phymod_symbols_t *symbols, uint32_t sindex, 
                           plp_aperta_phymod_symbol_t *rsym); 
...
typedef struct plp_aperta_phymod_symbols_iter_s {
    ...
    const plp_aperta_phymod_symbols_t* symbols; 

    int (*function)(const plp_aperta_phymod_symbol_t *sym, void *vptr);
    void* vptr; 

} plp_aperta_phymod_symbols_iter_t; 

typedef int (*phymod_symbol_filter_cb_t)(const plp_aperta_phymod_symbol_t *symbol,
                                      const char **field_names,
                                      const char *encoding,
                                      void *cookie);


extern int plp_aperta_phymod_symbols_iter(plp_aperta_phymod_symbols_iter_t *iter);

extern int plp_aperta_phymod_symbol_field_filter(const plp_aperta_phymod_symbol_t *symbol,
                                   const char **fnames,
                                   const char *encoding,
                                   void *cookie);

extern int plp_aperta_phymod_symbol_show_fields(const plp_aperta_phymod_symbol_t *symbol,
                                     const char **fnames, uint32_t *data, int nz,
                                     int (*print_str)(const char *str),
                                     phymod_symbol_filter_cb_t fcb, void *cookie);

extern int plp_aperta_phymod_symbol_dump(const char *name, const plp_aperta_phymod_symbols_t *symbols,
                              uint32_t *data,
                              int (*print_str)(const char *str));

extern uint32_t *plp_aperta_phymod_field_get(const uint32_t *entbuf,
                                  int sbit, int ebit,
                                  uint32_t *fbuf);

extern int plp_aperta_phymod_field_set(uint32_t *entbuf, int sbit, int ebit,
                            uint32_t *fbuf);

#endif /* __plp_aperta_PHYMOD_SYMBOLS_H__ */
