...
int main(int argc, char *argv[])
{
    ...
    phy_info.flags = 1; /* Set Warmboot flag; Skip HW configuration */
    ...
    for (phy_index = 0; phy_index < NUM_OF_PHY; phy_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
                ...
                vport_handle[phy_index][port][macsec_side] = cvh = nvh;
                printf("\n Restored vport handle for phy:%d port:%d side:%d is %p\n",phy_index, port, macsec_side, cvh);
                if(vport_handle[phy_index][port][macsec_side]){
                ...
                secy_sahandle[phy_index][port][macsec_side].p = ch.p = nh.p;
                printf("\n Restored SA handle for phy:%d port:%d side:%d is %p\n",phy_index, port, macsec_side, ch.p);
                ...
                phy_index, macsec_side, phy_info.lane_map, retval);
                rule_handle[phy_index][port][macsec_side] = crh = nrh;
                printf("\n Restored Rule handle for phy:%d port:%d side:%d is %p\n",phy_index, port, macsec_side, crh);
            }
        }
    }
     ...
     * -------------------------------------------------------------------------------- */
#ifdef TRAFFIC_DEBUG
     retval = macsec_traffic_verify("00:00:00:08:01:01", 0xF, "allow");
     if (retval) {
         printf("Print_statistics() observed failure. Exiting... \n");
         goto _aperta_config_error;
         return retval;
     }
     retval = macsec_traffic_verify("00:00:00:08:01:01", 0xF0, "allow");
     if (retval) {
         printf("Print_statistics() observed failure. Exiting... \n");
         goto _aperta_config_error;
         return retval;
