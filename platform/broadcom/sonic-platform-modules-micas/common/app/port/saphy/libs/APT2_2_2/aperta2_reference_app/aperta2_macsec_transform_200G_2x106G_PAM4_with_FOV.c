 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Aperta2 chip for 200G(2x106G) PAM4 with FOV*
 * MACSec enabled mode as follows:                                                  *
 *   1.  Connect to the board.                                                      *
 *   2.  Download the firmware and Initialize the PHY in MACSec enabled mode        *
 *   3.  Initialize MACsec                                                          *
 *   4.  Configure Tx/Rx polarity                                                   *
 *   5.  Configure operating mode parameters for 200G(2x106G) PAM4 mode with FOV    *
 *   6.  Setup link training on System and Line side                                *
 *   7.  Add vport                                                                  *
 *   8.  Add SA with transfer record                                                *
 *   9.  Add rule                                                                   *
 *   10. Check the Link status of ports                                             *
 *   11. Remove rule                                                                *
 *   12. Remove SA                                                                  *
 *   13. Remove vport                                                               *
 *   14. Uninitialize MACSec                                                        *
...
unsigned int sys_lane_map_list[] =  {
                                      0x0003, 0x000C, 0x0300, 0x0C00
                                    };

/* System side FOV lanemaps */
unsigned int sys_fov_lane_map_list[] =  {
                                      0x0030, 0x00C0, 0x3000, 0xC000
                                    };
...
int main(int argc, char *argv[])
{
    ...
    /* -------------------------------------------------------------------------------- *
     * Section 1: PHY, MACsec Initialization                                            *
     * -------------------------------------------------------------------------------- */
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        sys_fov_lane_map = sys_fov_lane_map_list[lane_map_index];
        line_lane_map = line_lane_map_list[lane_map_index];
        ...
        aux_mode_sys_set.failover_config.lane_map = sys_fov_lane_map;
        ...
        aux_mode_line_set.failover_config.lane_map = 0;
     ...
     * -------------------------------------------------------------------------------- */
    /* -------------------------------------------------------------------------------- *
     * Start Force Tx training on primary port at system side                           *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_lane_map_list[lane_map_index];
    ...
    sleep(5);
    /* -------------------------------------------------------------------------------- *
     * Get Force Tx training status on primary port at system side                      *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_lane_map_list[lane_map_index];
    ...
    /* -------------------------------------------------------------------------------- *
     * Start Force Tx training on secondary port at system side                         *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_fov_lane_map_list[lane_map_index];
        phy_info.if_side = SYS_SIDE;
        phy_info.lane_map = sys_lane_map ;
        retval = bcm_plp_force_tx_training_set(CHIP_NAME, phy_info, ENABLE);
        if (retval != TEST_SUCCESS) {
    ...
    sleep(5);
    /* -------------------------------------------------------------------------------- *
     * Get Force Tx training status on secondary port at system side                    *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_fov_lane_map_list[lane_map_index];
    ...
    /* -------------------------------------------------------------------------------- *
     * Ports are setup, Primary ports are active  and ready to test with traffic        *
     * -------------------------------------------------------------------------------- */
     printf("\n\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
     printf("Ports are MACSec enabled and UP. Please send the traffic\n");
     printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
    /* -------------------------------------------------------------------------------- *
     * Section 8: Failover mode set                                                     *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_lane_map_list[lane_map_index];
    ...
    /* -------------------------------------------------------------------------------- *
     * Ports are setup, Secondary ports are active  and ready to test with traffic      *
     * -------------------------------------------------------------------------------- */
     printf("\n\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
     printf("Ports are MACSec enabled and UP. Please send the traffic\n");
     printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
