...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk_barchetta_debug_functions.c                                  *
...
/** @file blackhawk_barchetta_debug_functions.c
...
#include "blackhawk_barchetta_reg_dump.h"


static err_code_t _plp_barchetta_blackhawk_barchetta_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section);
static err_code_t _plp_barchetta_blackhawk_barchetta_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static err_code_t _plp_barchetta_blackhawk_barchetta_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static uint64_t _plp_barchetta_blackhawk_barchetta_int_pow(uint64_t base, uint8_t exp); 
...
err_code_t plp_barchetta_blackhawk_barchetta_stop_uc_lane(srds_access_t *sa__, uint8_t enable) {
    ...
    return(plp_barchetta_blackhawk_barchetta_stop_rx_adaptation(sa__,enable));
}
...
err_code_t plp_barchetta_blackhawk_barchetta_set_usr_ctrl_disable_startup(srds_access_t *sa__, struct blackhawk_barchetta_usr_ctrl_disable_functions_st set_val) {
  EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_update_usr_ctrl_disable_functions_byte(&set_val));
...
err_code_t plp_barchetta_blackhawk_barchetta_set_usr_ctrl_disable_startup_dfe(srds_access_t *sa__, struct blackhawk_barchetta_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_update_usr_ctrl_disable_dfe_functions_byte(&set_val));
...
err_code_t plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_startup(srds_access_t *sa__, struct blackhawk_barchetta_usr_ctrl_disable_functions_st *get_val) {
  ...
  EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_update_usr_ctrl_disable_functions_st(get_val));
  return (ERR_CODE_NONE);
}
...
err_code_t plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_startup_dfe(srds_access_t *sa__, struct blackhawk_barchetta_usr_ctrl_disable_dfe_functions_st *get_val) {
  ...
  EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_update_usr_ctrl_disable_dfe_functions_st(get_val));
  return (ERR_CODE_NONE);
}
...
static err_code_t _plp_barchetta_blackhawk_barchetta_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section)
{
    ...
    for (addr = plp_barchetta_blackhawk_barchetta_reg_dump_arr[reg_section][0]; addr < plp_barchetta_blackhawk_barchetta_reg_dump_arr[reg_section][1]; addr++) 
    {
        if (!(addr & 0x000F))  
        {
            EFUN_PRINTF(("\n%04x ",addr));
        }
        EFUN(plp_barchetta_blackhawk_barchetta_acc_rdt_reg(sa__, addr,&rddata));
...
static err_code_t _plp_barchetta_blackhawk_barchetta_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    if(reg_buffer == NULL)
    {
         EFUN(_plp_barchetta_blackhawk_barchetta_reg_print_no_buf(sa__, reg_section));
    }
    else
    {
        EFUN(_plp_barchetta_blackhawk_barchetta_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
...
{    
    ...
    while(reg_section < BLACKHAWK_BARCHETTA_REG_DUMP_SECTIONS) 
    {
        if(plp_barchetta_blackhawk_barchetta_reg_dump_arr[reg_section][0] == BLACKHAWK_BARCHETTA_REG_PLL_START)
        {
            ...
            for (pll_idx = 0; pll_idx < DUAL_PLL_NUM_PLLS; pll_idx++)
            {
                ...
                EFUN(plp_barchetta_blackhawk_barchetta_set_pll_idx(sa__,pll_idx));
                while( reg_section < BLACKHAWK_BARCHETTA_REG_DUMP_SECTIONS && plp_barchetta_blackhawk_barchetta_reg_dump_arr[reg_section][0] != BLACKHAWK_BARCHETTA_REG_UC_START)
                {
                    EFUN(_plp_barchetta_blackhawk_barchetta_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
            }
            EFUN(plp_barchetta_blackhawk_barchetta_set_pll_idx(sa__,pll_orig));
        }
        else if (plp_barchetta_blackhawk_barchetta_reg_dump_arr[reg_section][0] == BLACKHAWK_BARCHETTA_REG_UC_START)
        {
...
err_code_t plp_barchetta_blackhawk_barchetta_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][250]) {
  uint8_t  addr, rddata, core_var_ram_size;
    ...
    core_var_ram_size = (uint8_t)blackhawk_barchetta_info_ptr->core_var_ram_size;
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM_PRINTF(("%02x ", plp_barchetta_blackhawk_barchetta_rdbc_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t index=0;
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM(rddata = plp_barchetta_blackhawk_barchetta_rdbc_uc_var(sa__, __ERR, addr));
...
err_code_t plp_barchetta_blackhawk_barchetta_uc_micro_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < 0x10; addr++) {
         ...
         ESTM_PRINTF(("%02x ", plp_barchetta_blackhawk_barchetta_rdbuc_uc_var(sa__, __ERR, addr)));
      }
  }
  else
  {
      ...
      EFUN_PRINTF(("\n**** SERDES UC CORE %d MICRO %d RAM VARIABLE DECODED ****\n", core,(blackhawk_barchetta_get_physical_lane(sa__)/2)));
      ...
      for (addr = 0x0; addr < 0x10; addr++) {
         ...
         ESTM(rddata = plp_barchetta_blackhawk_barchetta_rdbuc_uc_var(sa__, __ERR, addr));
         USR_SPRINTF(tempBuf,"%02x ", rddata);
         USR_STRNCAT(ram_buffer[index], tempBuf, 6);
     }
     ram_buffer[index+1][0] = 0xA;
...
err_code_t plp_barchetta_blackhawk_barchetta_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
  ...
  srds_info_t * blackhawk_barchetta_info_ptr = plp_barchetta_blackhawk_barchetta_INTERNAL_get_blackhawk_barchetta_info_ptr_with_check(sa__);
  CHECK_AND_RETURN_IF_NULL(blackhawk_barchetta_info_ptr, 0);
  lane_var_ram_size = (uint16_t)blackhawk_barchetta_info_ptr->lane_var_ram_size;

  ESTM(rx_lock = rd_pmd_rx_lock());

  {
      err_code_t err_code=ERR_CODE_NONE;
      uc_stopped = plp_barchetta_blackhawk_barchetta_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM_PRINTF(("%02x ", plp_barchetta_blackhawk_barchetta_rdbl_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t rddata, index = 0;
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM(rddata = plp_barchetta_blackhawk_barchetta_rdbl_uc_var(sa__, __ERR, addr));
         USR_SPRINTF(tempBuf,"%02x ", rddata);
         USR_STRNCAT(ram_buffer[index], tempBuf, 6);
      }
      ram_buffer[index+1][0] = 0xA;
...
err_code_t plp_barchetta_blackhawk_barchetta_read_event_log(srds_access_t *sa__) {
    ...
    for (; micro_num<blackhawk_barchetta_info_ptr->micro_count; ++micro_num)
    {
        ...
        EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_read_event_log_with_callback(sa__, micro_num, 0, &state, plp_barchetta_blackhawk_barchetta_INTERNAL_event_log_dump_callback));
...
err_code_t plp_barchetta_blackhawk_barchetta_dig_lpbk_rptr(srds_access_t *sa__, uint8_t enable, enum plp_barchetta_srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
            EFUN(plp_barchetta_blackhawk_barchetta_loop_timing(sa__, 0));
            EFUN(plp_barchetta_blackhawk_barchetta_loop_timing(sa__, 1)); /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
        } else {
            EFUN(plp_barchetta_blackhawk_barchetta_loop_timing(sa__, 0));
        }
        break;
    ...
    case DIG_LPBK_SIDE:
        ...
        EFUN(plp_barchetta_blackhawk_barchetta_tx_rptr_mode_timing(sa__, 1)); /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
...
err_code_t plp_barchetta_blackhawk_barchetta_nl_from_dig_lpbk(srds_access_t *sa__, enum plp_barchetta_srds_rptr_mode_enum mode) {
    EFUN(plp_barchetta_blackhawk_barchetta_dig_lpbk_rptr(sa__, 0,mode));
    return (ERR_CODE_NONE);
...
err_code_t plp_barchetta_blackhawk_barchetta_tx_pi_jitt_gen(srds_access_t *sa__, uint8_t enable, int16_t freq_override_val, enum plp_barchetta_srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_tx_pi_freq_override(sa__, enable, freq_override_val));
...
err_code_t plp_barchetta_blackhawk_barchetta_isolate_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    for(pll = 0; pll < DUAL_PLL_NUM_PLLS; pll++) {
        ...
        EFUN(plp_barchetta_blackhawk_barchetta_isolate_core_ctrl_pins(sa__, enable));
    }
    EFUN(plp_barchetta_blackhawk_barchetta_set_pll_idx(sa__, pll_orig));

    ESTM(lane_orig = plp_barchetta_blackhawk_barchetta_get_lane(sa__));
    ...
    for(lane = 0; lane < num_lanes; lane++) {
        ...
        EFUN(plp_barchetta_blackhawk_barchetta_isolate_lane_ctrl_pins(sa__, enable));
    }
...
/* BLACKHAWK TODO:  Differentify from Falcon16. */
...
err_code_t plp_barchetta_blackhawk_barchetta_log_full_pmd_state_noPRBS (srds_access_t *sa__, struct blackhawk_barchetta_detailed_lane_status_st *lane_st) {
    ...
    int8_t   tmp;
    uint8_t  tmp1;
    const uint8_t big_endian = plp_barchetta_blackhawk_barchetta_INTERNAL_is_big_endian();
    ...
    {
        err_code_t err_code=ERR_CODE_NONE;
        lane_st->stop_state = plp_barchetta_blackhawk_barchetta_INTERNAL_stop_micro(sa__,lane_st->pmd_lock,&err_code);
        if(err_code) USR_PRINTF(("Unable to stop microcontroller,  following data is suspect\n"));
    }
  ...
  }
  {
    ...
    ESTM(lane_st->txfir_use_pam4_range = rd_txfir_nrz_tap_range_sel() ? 0 : 1);
    {
        ...
        for (tap_num=0; tap_num<12; ++tap_num) {
            EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_tx_tap(sa__, tap_num, &(lane_st->txfir.tap[tap_num])));
        }
    ...
    ESTM(plp_barchetta_blackhawk_barchetta_INTERNAL_get_rx_vga(sa__, &tmp1)); lane_st->vga = tmp1;
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_rx_pf_main(sa__, &tmp1)); lane_st->pf = tmp1;
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_rx_pf2(sa__, &tmp1)); lane_st->pf2 = tmp1;
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_rx_pf3(sa__, &tmp1)); lane_st->pf3 = tmp1;
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_lms_thresh_bin(sa__, &tmp)); lane_st->lms_thresh = tmp;
    ...
    lane_st->lms_dac4ck[0] = (int8_t)((lane_st->lms_dac4ck[0]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->lms_dac4ck[0]&0x07));
    lane_st->lms_dac4ck[1] = (int8_t)((lane_st->lms_dac4ck[1]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->lms_dac4ck[1]&0x07));
    ...
    /* Convert from sign+gray to sign+bin */
    lane_st->phase_dac4ck[0] = (int8_t)((lane_st->phase_dac4ck[0]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->phase_dac4ck[0]&0x07));
    lane_st->phase_dac4ck[1] = (int8_t)((lane_st->phase_dac4ck[1]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->phase_dac4ck[1]&0x07));
    ...
    /* Convert from sign+gray to sign+bin */
    lane_st->data_dac4ck[0] = (int8_t)((lane_st->data_dac4ck[0]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->data_dac4ck[0]&0x07));
    lane_st->data_dac4ck[1] = (int8_t)((lane_st->data_dac4ck[1]&0x8)|plp_barchetta_blackhawk_barchetta_INTERNAL_gray_to_uint8(lane_st->data_dac4ck[1]&0x07));
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_eye_margin_est(sa__, &lane_st->heye_left, &lane_st->heye_right, &lane_st->veye_upper, &lane_st->veye_lower));
...
err_code_t plp_barchetta_blackhawk_barchetta_log_full_pmd_state (srds_access_t *sa__, struct blackhawk_barchetta_detailed_lane_status_st *lane_st) {
    enum plp_barchetta_srds_prbs_polynomial_enum prbs_poly_mode = PRBS_7;
    enum plp_barchetta_srds_prbs_checker_mode_enum prbs_checker_mode = PRBS_SELF_SYNC_HYSTERESIS;

    if(!lane_st)
      return(blackhawk_barchetta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

    ENULL_MEMSET(lane_st, 0, sizeof(struct blackhawk_barchetta_detailed_lane_status_st));
    EFUN(plp_barchetta_blackhawk_barchetta_log_full_pmd_state_noPRBS(sa__, lane_st));

    ESTM(lane_st->prbs_chk_en = rd_prbs_chk_en());
    EFUN(plp_barchetta_blackhawk_barchetta_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &lane_st->prbs_chk_inv));
    lane_st->prbs_chk_order = (uint8_t)prbs_poly_mode;

    EFUN(plp_barchetta_blackhawk_barchetta_prbs_chk_lock_state(sa__, &lane_st->prbs_chk_lock));
    EFUN(plp_barchetta_blackhawk_barchetta_prbs_err_count_ll(sa__, &lane_st->prbs_chk_errcnt));
    if (lane_st->prbs_chk_lock && (lane_st->rx_pam_mode != NRZ)) {
       EFUN(plp_barchetta_blackhawk_barchetta_collect_dbgfb_stats(sa__, 8000, &lane_st->dbgfb_stats));
    }
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_get_BER_string(sa__,100,&lane_st->ber_string[0]));
...
err_code_t plp_barchetta_blackhawk_barchetta_disp_full_pmd_state (srds_access_t *sa__, struct blackhawk_barchetta_detailed_lane_status_st const * const lane_st, uint8_t num_lanes) {
    ...
    if ((num_lanes != 4) && (num_lanes != 8) && (num_lanes != 1)) {
      return(blackhawk_barchetta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));  /* Number of lanes has to be 1, 4 or 8 */
    }

    size_of_lane_st = sizeof(struct blackhawk_barchetta_detailed_lane_status_st);

    EFUN_PRINTF(("\n**** SERDES EXTENDED LANE DIAG DATA CORE %d LANE %d DUMP ****", plp_barchetta_blackhawk_barchetta_get_core(sa__), plp_barchetta_blackhawk_barchetta_get_lane(sa__)));
    EFUN_PRINTF(("\n0000 "));
    ESTM_PRINTF(("%02x ", num_lanes));
    ESTM_PRINTF(("%02x ", big_endian));
    ESTM_PRINTF(("%02x ", (uint8_t)sizeof(struct blackhawk_barchetta_check_platform_info_st)));
...
/*  Note: No EFUNs or ESTMs should be used in this function as this print routine is called by _error() handler. */
    blackhawk_barchetta_triage_info info;
    ...
    memset(&info, 0, sizeof(blackhawk_barchetta_triage_info));
    info.error = err_code;
    info.line = line;
    ...
    if (plp_barchetta_blackhawk_barchetta_version(sa__, &info.api_ver)) {
    ...
    if(plp_barchetta_blackhawk_barchetta_INTERNAL_sigdet_status(sa__, &info.sig_det, &info.sig_det_chg)) {
        error_seen = 1;
    }
    if (plp_barchetta_blackhawk_barchetta_INTERNAL_pmd_lock_status(sa__, &info.pmd_lock, &info.pmd_lock_chg)) {
        error_seen = 1;
    }
    ...
    info.core = plp_barchetta_blackhawk_barchetta_get_core(sa__);
    info.lane = plp_barchetta_blackhawk_barchetta_get_lane(sa__);
    if (print_data) {
        if ((err_code == ERR_CODE_UC_CMD_POLLING_TIMEOUT) || (err_code == ERR_CODE_UC_NOT_STOPPED)) {
            USR_PRINTF(("%4d, %4d,  %X_%X,    %X_%X, %17d, %7d,%d       , %11d,   0x%04x, %8d, %6d,    0x%x,0x%x   , %s\n",
                        ...
                        info.dsc_one_hot[0],
                        info.dsc_one_hot[1],
                        plp_barchetta_blackhawk_barchetta_INTERNAL_e2s_err_code(info.error) ));
        } else {
...
err_code_t plp_barchetta_blackhawk_barchetta_set_pam_eye_margin_usr_ctrl(srds_access_t *sa__, uint8_t enable) {
    struct blackhawk_barchetta_usr_ctrl_disable_functions_st val;

    /* get current usr ctrl disable value */
    ESTM(plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_steady_state(sa__, &val));
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_set_usr_ctrl_disable_steady_state(sa__, val));
    return ERR_CODE_NONE;
}
...
err_code_t plp_barchetta_blackhawk_barchetta_rand_err_gen_enable(srds_access_t *sa__, uint8_t enable) {
    EFUN(wr_error_injection_en(enable));
    return (ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_rand_err_gen_config_ber(srds_access_t *sa__, uint16_t base, uint8_t exp) {
    uint8_t thresh_adjust;
    uint16_t thresh_val, ber_adjust;
    uint64_t ber_inverse, scaling_factor;

    if(base == 0 || exp < 3) {
        return (ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }

    scaling_factor = _plp_barchetta_blackhawk_barchetta_int_pow(10,exp);
    ber_inverse = scaling_factor / base;

    if(ber_inverse >= 1e3 && ber_inverse < 1e4) {
        thresh_adjust = 0x0;
        ber_adjust = 8;
    }
    else if (ber_inverse >= 1e4 && ber_inverse < 1e5) {
        thresh_adjust = 0x1;
        ber_adjust = 128;
    }
    else if (ber_inverse >= 1e5 && ber_inverse < 1e7) {
        thresh_adjust = 0x2;
        ber_adjust = 2048;
    }
    else if (ber_inverse >= 1e7 && ber_inverse <= 1e9) {
        thresh_adjust = 0x3;
        ber_adjust = 32768;
    }
    else {
    ...
    thresh_val = (uint16_t)((65536 * (ber_adjust * (scaling_factor  - (uint64_t)(40 * base)) - (uint64_t)(ber_adjust - 1) * scaling_factor)) / scaling_factor);

    EFUN(wr_error_injection_thresh_adjust(thresh_adjust));
    EFUN(wr_error_injection_threshold(thresh_val));
    return (ERR_CODE_NONE);
}
