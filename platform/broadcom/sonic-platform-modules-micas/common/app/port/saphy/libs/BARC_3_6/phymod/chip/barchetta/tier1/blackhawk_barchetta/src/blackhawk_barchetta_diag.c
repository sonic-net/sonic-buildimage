...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk_barchetta_diag.c                                             *
...
/** @file blackhawk_barchetta_diag.c
...
#include "blackhawk_barchetta_diag.h"
...
err_code_t plp_barchetta_blackhawk_barchetta_test_uc_timestamp(srds_access_t *sa__) {
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_TIMESTAMP,100));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_eye_scan(srds_access_t *sa__) {
    ...
    ESTM(core=plp_barchetta_blackhawk_barchetta_get_core(sa__));
    ESTM(lane=plp_barchetta_blackhawk_barchetta_get_lane(sa__));
    EFUN_PRINTF(("\n\n****  SERDES EYE SCAN CORE %d LANE %d   ****\n",core,lane));
    EFUN(plp_barchetta_blackhawk_barchetta_display_eye_scan_header(1));

    /* start horizontal acquisition */
    {   err_code_t err_code = plp_barchetta_blackhawk_barchetta_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code)
        {
            ...
            {
                EFUN((plp_barchetta_blackhawk_barchetta_meas_eye_scan_done(sa__), err_code));
            }
        }
    }
    ...
    {
        {   err_code_t err_code = plp_barchetta_blackhawk_barchetta_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN((plp_barchetta_blackhawk_barchetta_meas_eye_scan_done(sa__), err_code));
            }
        }
        EFUN(plp_barchetta_blackhawk_barchetta_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(plp_barchetta_blackhawk_barchetta_meas_eye_scan_done(sa__));
    EFUN(plp_barchetta_blackhawk_barchetta_display_eye_scan_footer(1));

    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_INTERNAL_poll_diag_data(srds_access_t *sa__, srds_info_t *blackhawk_barchetta_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    uint32_t lane_diag_size;
    uint16_t loop;

    CHECK_AND_RETURN_IF_NULL(blackhawk_barchetta_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
    lane_diag_size = blackhawk_barchetta_info_ptr->diag_mem_ram_size;
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : plp_barchetta_blackhawk_barchetta_INTERNAL_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
typedef struct {
    uint32_t *buffer_ptr;
    } blackhawk_barchetta_read_eye_scan_stripe_callback_arg_t;

static err_code_t plp_barchetta_blackhawk_barchetta_read_eye_scan_stripe_callback(void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk_barchetta_read_eye_scan_stripe_callback_arg_t * const cast_arg = (blackhawk_barchetta_read_eye_scan_stripe_callback_arg_t *)arg;
    *(cast_arg->buffer_ptr++) = plp_barchetta_blackhawk_barchetta_INTERNAL_float8_to_int32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = plp_barchetta_blackhawk_barchetta_INTERNAL_float8_to_int32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    const uint8_t lane = plp_barchetta_blackhawk_barchetta_get_lane(sa__);
    const uint8_t stripe_size = 64;

    blackhawk_barchetta_read_eye_scan_stripe_callback_arg_t arg;
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_verify_blackhawk_barchetta_info(blackhawk_barchetta_info_ptr, sa__));
        ...
        lane_diag_base = blackhawk_barchetta_info_ptr->diag_mem_ram_base + ((lane%blackhawk_barchetta_info_ptr->lane_count) * blackhawk_barchetta_info_ptr->diag_mem_ram_size) +
            (uint32_t)(blackhawk_barchetta_info_ptr->grp_ram_size*plp_barchetta_blackhawk_barchetta_INTERNAL_grp_idx_from_lane(blackhawk_barchetta_get_physical_lane(sa__)));

    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_poll_diag_data(sa__, blackhawk_barchetta_info_ptr, status, &diag_rd_ptr, stripe_size, 400));

    arg.buffer_ptr = buffer;
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)blackhawk_barchetta_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              plp_barchetta_blackhawk_barchetta_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % blackhawk_barchetta_info_ptr->diag_mem_ram_size);
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_display_eye_scan_stripe(srds_access_t *sa__, int8_t y,uint32_t *buffer) {
    ...
    level = plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, y,0);
...
err_code_t plp_barchetta_blackhawk_barchetta_display_eye_scan_header(int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_eye_scan_footer(int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
err_code_t plp_barchetta_blackhawk_barchetta_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
...
err_code_t plp_barchetta_blackhawk_barchetta_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}


err_code_t plp_barchetta_blackhawk_barchetta_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    if((sts & 0x8000) == 0) {
        return(blackhawk_barchetta_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
    *cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < *cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = plp_barchetta_blackhawk_barchetta_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (plp_barchetta_blackhawk_barchetta_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t plp_barchetta_blackhawk_barchetta_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ESTM(offset_start = (int16_t)rd_uc_dsc_data());
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("offset_start = %d:%dmV\n",offset_start,plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start,0)));
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(plp_barchetta_blackhawk_barchetta_read_ber_scan_data(sa__, &errs[0], &time[0], &cnt, 2000));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
 ...
 /* }                                                                  */
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_display_ber_scan_data(sa__, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)SRDS_ABS(offset_start)));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                /* BLACKHAWK TODO:  This was copied from Falcon16. */
                margins[i] = direction*plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0);
            } else {
                margins[i] = direction*plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
...
err_code_t plp_barchetta_blackhawk_barchetta_display_lane_state_hdr(void) {
...
err_code_t plp_barchetta_blackhawk_barchetta_display_lane_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = plp_barchetta_blackhawk_barchetta_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t plp_barchetta_blackhawk_barchetta_display_core_state_hdr(void) {
    char core_type[23] = "blackhawk_barchetta_b0";
...
err_code_t plp_barchetta_blackhawk_barchetta_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = plp_barchetta_blackhawk_barchetta_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t plp_barchetta_blackhawk_barchetta_display_core_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
/**********************************************/
err_code_t plp_barchetta_blackhawk_barchetta_display_core_config(srds_access_t *sa__) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    EFUN_PRINTF((    "**** SERDES CORE CONFIGURATION ****\n"  ));
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        struct   blackhawk_barchetta_uc_core_config_st core_cfg;
        struct   blackhawk_barchetta_uc_core_config_st pll1_cfg;
        ...
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
        {
            ...
            EFUN(plp_barchetta_blackhawk_barchetta_get_uc_core_config(sa__, &pll1_cfg));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_lane_config(srds_access_t *sa__) {
    struct blackhawk_barchetta_uc_lane_config_st lane_cfg;
    int chip_id = 0;
    PHYMOD_IF_ERR_RETURN(_plp_barchetta_get_chip_id(sa__, &chip_id));
    ENULL_MEMSET(&lane_cfg, 0, sizeof(lane_cfg));

    EFUN_PRINTF(("\n\n*************************************\n"                                          ));
    ESTM_PRINTF((   "**** SERDES LANE %d CONFIGURATION ****\n"  , plp_barchetta_blackhawk_barchetta_get_lane(sa__)                 ));
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    ...
    ESTM_PRINTF((    "TXFIR Range                 = %s\n"       , (chip_id == BARCHETTA_CHIP_81381) ? "NRZ" :
...
err_code_t plp_barchetta_blackhawk_barchetta_display_core_state(srds_access_t *sa__) {
    EFUN(plp_barchetta_blackhawk_barchetta_display_core_state_hdr());
    EFUN(plp_barchetta_blackhawk_barchetta_display_core_state_line(sa__));
    EFUN(plp_barchetta_blackhawk_barchetta_display_core_state_legend());
    return ERR_CODE_NONE;
}


err_code_t plp_barchetta_blackhawk_barchetta_display_lane_debug_status(srds_access_t *sa__) {
    /* startup */
    struct blackhawk_barchetta_usr_ctrl_disable_functions_st     ds;
    struct blackhawk_barchetta_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct blackhawk_barchetta_usr_ctrl_disable_functions_st     dss;
    struct blackhawk_barchetta_usr_ctrl_disable_dfe_functions_st dssd;
    uint8_t core,lane;
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_startup(sa__, &ds));
    /* BLACKHAWK TODO:  This is copied from Falcon16. */
    ...
    EFUN(                                                            plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_startup_dfe(sa__, &dsd));
    /* BLACKHAWK TODO:  This is copied from Falcon16. */
    ...
    EFUN(                                                           plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_steady_state(sa__, &dss));
    /* BLACKHAWK TODO:  This is copied from Falcon16. */
    ...
    EFUN(                                                      plp_barchetta_blackhawk_barchetta_get_usr_ctrl_disable_steady_state_dfe(sa__, &dssd));
    /* BLACKHAWK TODO:  This is copied from Falcon16. */
...
static err_code_t _plp_barchetta_display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t i,prbs_byte,prbs_multi,time_byte,time_multi;
    uint16_t sts,dataword;
    int16_t offset_start;
    uint8_t cnt;
    ...
    struct blackhawk_barchetta_uc_core_config_st core_config;
    ...
    if((osr_mode.rx > BLACKHAWK_BARCHETTA_OSX4) && (osr_mode.rx != BLACKHAWK_BARCHETTA_OSX2P5)) {
#else
    if(osr_mode.rx > BLACKHAWK_BARCHETTA_OSX4) {
#endif
        EFUN_PRINTF(("ERROR DIAG display_ber_data: osr mode too high\n"));
        return(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }
    if (osr_mode.rx > BLACKHAWK_BARCHETTA_OSX4) { /* BER supported for only BLACKHAWK_BARCHETTA_OSX2P5 mode when osr_mode > osx4 */
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN_PRINTF(("STARTING OFFSET = %d : %d mV\n",offset_start,plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start, range250)));
    }
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("start done\n"));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = plp_barchetta_blackhawk_barchetta_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (plp_barchetta_blackhawk_barchetta_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        if(DIAG_VERBOSE < 5) {
            if (!(i % 4))  {
                EFUN_PRINTF(("\n"));
            }
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN_PRINTF(("%d %d %d ",direction*plp_barchetta_blackhawk_barchetta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)(SRDS_ABS(offset_start)-i), range250),errors,timer_values));
            }
    ...
    EFUN_PRINTF(("\n"));
    EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t plp_barchetta_blackhawk_barchetta_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_version(sa__, &api_version));
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = plp_barchetta_blackhawk_barchetta_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
    ...
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_state_hdr());
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_state(sa__));

    if(diag_level & SRDS_DIAG_LANE) {
        struct blackhawk_barchetta_detailed_lane_status_st lane_st;

        ENULL_MEMSET(&lane_st, 0, sizeof(lane_st));
        EFUN(plp_barchetta_blackhawk_barchetta_log_full_pmd_state(sa__, &lane_st));

        EFUN(plp_barchetta_blackhawk_barchetta_disp_full_pmd_state(sa__, &lane_st, 1));
    }

    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(plp_barchetta_blackhawk_barchetta_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        blackhawk_barchetta_INTERNAL_event_log_dump_state_t state;
        uint8_t micro_num = 0;
        srds_info_t * blackhawk_barchetta_info_ptr;
        ESTM(blackhawk_barchetta_info_ptr = plp_barchetta_blackhawk_barchetta_INTERNAL_get_blackhawk_barchetta_info_ptr_with_check(sa__));
        CHECK_AND_RETURN_IF_NULL(blackhawk_barchetta_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
        for (; micro_num<blackhawk_barchetta_info_ptr->micro_count; ++micro_num)
        {
            state.index = 0;
            state.line_start_index = 0;
            EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, plp_barchetta_blackhawk_barchetta_INTERNAL_event_log_dump_callback));
            EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_event_log_dump_callback(&state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(plp_barchetta_blackhawk_barchetta_read_event_log(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    core = plp_barchetta_blackhawk_barchetta_get_core(sa__);
    lane = plp_barchetta_blackhawk_barchetta_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(plp_barchetta_blackhawk_barchetta_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(plp_barchetta_blackhawk_barchetta_uc_core_var_dump(sa__, core, NULL));

        {
	    uint8_t orig_lane, num_micros, lane;

	    ESTM(num_micros = rdc_micro_num_uc_cores());
	    orig_lane = blackhawk_barchetta_get_physical_lane(sa__);
	    for (lane = 0; lane < num_micros*2; lane =(uint8_t)(lane + 2)) {
		    EFUN(blackhawk_barchetta_set_physical_lane(sa__, lane));

		    EFUN(plp_barchetta_blackhawk_barchetta_uc_micro_var_dump(sa__, core, lane, NULL));
	    }
	    EFUN(blackhawk_barchetta_set_physical_lane(sa__, orig_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(plp_barchetta_blackhawk_barchetta_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(plp_barchetta_blackhawk_barchetta_display_eye_scan(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(plp_barchetta_blackhawk_barchetta_display_lane_debug_status(sa__));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_plp_barchetta_display_ber_scan_data(sa__, ber_mode, timer_control,  err_threshold));
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_display_info_table(sa__));
    }

    /* re enable micro */
    if (!micro_stop) {
        EFUN(plp_barchetta_blackhawk_barchetta_stop_rx_adaptation(sa__, 0));
    }
...
} blackhawk_barchetta_diag_access_callback_state_t;

static err_code_t plp_barchetta_blackhawk_barchetta_diag_access_read_byte_callback(void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk_barchetta_diag_access_callback_state_t * const state_ptr = (blackhawk_barchetta_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t plp_barchetta_blackhawk_barchetta_diag_access_read_word_callback(void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk_barchetta_diag_access_callback_state_t * const state_ptr = (blackhawk_barchetta_diag_access_callback_state_t *)arg;
    UNUSED(byte_count);
...
/* Required Diagnostic Functions */
err_code_t plp_barchetta_blackhawk_barchetta_diag_access(srds_access_t *sa__, enum plp_barchetta_srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    srds_info_t * blackhawk_barchetta_info_ptr = plp_barchetta_blackhawk_barchetta_INTERNAL_get_blackhawk_barchetta_info_ptr_with_check(sa__);
    blackhawk_barchetta_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(plp_barchetta_blackhawk_barchetta_acc_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(plp_barchetta_blackhawk_barchetta_acc_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(plp_barchetta_blackhawk_barchetta_acc_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           addr + blackhawk_barchetta_info_ptr->core_var_ram_base,
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                                                           plp_barchetta_blackhawk_barchetta_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,plp_barchetta_blackhawk_barchetta_rdbc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK LANE RAM READ BYTE   ****"));
                                 EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + blackhawk_barchetta_info_ptr->lane_var_ram_base + (blackhawk_barchetta_get_physical_lane(sa__) * blackhawk_barchetta_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,plp_barchetta_blackhawk_barchetta_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           (uint16_t)(((data+1)>>1)<<1), /* Round up to nearest word count */
                                                                           &state,
                                                                           plp_barchetta_blackhawk_barchetta_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,plp_barchetta_blackhawk_barchetta_rdwc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,plp_barchetta_blackhawk_barchetta_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_wrbl_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,plp_barchetta_blackhawk_barchetta_INTERNAL_rdb_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,plp_barchetta_blackhawk_barchetta_INTERNAL_rdw_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = plp_barchetta_blackhawk_barchetta_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(plp_barchetta_blackhawk_barchetta_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(plp_barchetta_blackhawk_barchetta_pmd_uc_cmd_with_data(sa__, (enum plp_barchetta_srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
    ...
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_plp_barchetta_display_ber_scan_data(sa__, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    ...
    default: EFUN_PRINTF(("Invalid request type plp_barchetta_blackhawk_barchetta_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_display_state (srds_access_t *sa__) {
    EFUN(plp_barchetta_blackhawk_barchetta_display_core_state(sa__));
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_state_hdr());
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_state(sa__));
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_state_legend());
    return(ERR_CODE_NONE);
}

err_code_t plp_barchetta_blackhawk_barchetta_display_config (srds_access_t *sa__) {
    EFUN(plp_barchetta_blackhawk_barchetta_display_core_config(sa__));
    EFUN(plp_barchetta_blackhawk_barchetta_display_lane_config(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t plp_barchetta_blackhawk_barchetta_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    /* disable force */
    if(die_temp == -255) {
...
/* BLACKHAWK TODO:  Update with appropriate changes for 50G link training */

err_code_t plp_barchetta_blackhawk_barchetta_display_linktrn_status(srds_access_t *sa__) {
  EFUN_PRINTF(("\n\n***************************\n"                                                                                                                                    ));
