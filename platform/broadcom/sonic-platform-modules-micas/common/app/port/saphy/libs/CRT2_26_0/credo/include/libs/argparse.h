...
enum argparse_exit {
    ARGPARSE_EXIT_OK = 0,      //!< parsed everything, do whatever else you want
    ARGPARSE_EXIT_ERROR = -1,  //!< some sort of error, stop everythin
    ARGPARSE_EXIT_SKIP = -2,   //!< everything is fine, but dont do anything more
};
...
typedef void (*argparse_writer)(void *ud, const char *fmt, ...);
...
struct argparse {
    ...
    // writer
    argparse_writer writer;
    void *ud;
};

#define ARGP_PRINTF(A, ...) (A)->writer((A)->ud, __VA_ARGS__);
...
#define OPT_UINTEGER(...) \
    { ARGPARSE_OPT_UINTEGER, __VA_ARGS__ }
#define OPT_FLOAT(...) \
...
#define OPT_HELP() \
    OPT_BOOLEAN('h', "help", NULL, "show this help message and exit", argparse_help_cb_no_exit, 0, OPT_NONEG)
