...
local config = {
    ...
    fnW = 35, -- Width of the function name column
...
local function functionReport(information)
    ...
    elseif string.sub(src, #src - 3, #src) == ".lua" then
        ...
        src = stringx.shorten(src, config.fW, true)
    end
...
function module.attachPrintFunction(fn)
    printFun = fn
end
...
function module.report(filename)
    ...
    fileWriter:write(divider)
    if printFun ~= nil then
        printFun(divider:sub(1, -2))
        printFun(formatHeader:sub(1, -2))
        printFun(divider:sub(1, -2))
    end
    ...
    fileWriter:close()
    if printFun ~= nil and filename ~= nil then
