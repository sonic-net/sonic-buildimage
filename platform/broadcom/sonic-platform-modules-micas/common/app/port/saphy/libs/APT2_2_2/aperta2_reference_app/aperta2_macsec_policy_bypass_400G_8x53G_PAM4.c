 ...
 *   5.  Configure operating mode parameters for 400G(8x53G) PAM4 mode              *
 *   6.  Setup link training on System and Line side                                *
 *   7.  Add vport                                                                  *
 *   8.  Add SA with transfer record with policy based bypass                       *
 *   9.  Add rule with policy based bypass                                          *
...
int main(int argc, char *argv[])
{
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            retval = macsec_add_sa_with_transform_record(sec_info, POLICY_BYPASS_CONTROL_PACKET);
#else 
            retval = macsec_add_sa_with_transform_record(sec_info, POLICY_BYPASS_DATA_PACKET);
#endif
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
