 ...
 * --------------------------------------------------------------------------------------- *
 * This sample application demonstrate the usage of System side Force Tx Training/AN API's *
 ...
 * #else                                                                                   *
 *   7  - Set AN ability and Enable AN (with training) at System side for all the ports    *
 * #endif                                                                                  *
 * #ifdef BCM_PLP_FORCE_TX_TRAINING_ENABLE                                                 *
 *   8  - Check the Force Tx Training at System side for all the ports                     *
 * #else                                                                                   *
 *   8  - Check the AN status at System side for all the ports                             *
 * #endif                                                                                  *
 *   9  - Check for link status, PRBS lock and Rx PMD lock on both sides after Training/AN *
...
#ifdef BCM_PLP_FORCE_TX_TRAINING_ENABLE
#define OP_MODE_STR             "100G NRZ (4x25G) Retimer - Force Tx Training Demonstration (System Side)"
#else
#define OP_MODE_STR             "100G NRZ (4x25G) Retimer - AN Demonstration (System Side)"
#endif
...
int main(void)
{
    ...
    an_config_set.tech_ability = bcmplpAnCap100G_KR4 ;
#endif
    ...
    for (phy_index = 0; phy_index <  NUM_OF_PHY; phy_index++) {
        ...
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            ...
            /* Set PRBS generator (Tx) configuration at Line side */
            phy_info.lane_map = line_lane_map;
            phy_info.if_side  = LINE_SIDE;
            ...
            printf("Enabled PRBS generators for lane-map 0x%x of Line side\n", phy_info.lane_map);
        }
    }
    sleep(3);
    /* -------------------------------------------------------------------------------- *
    ...
    sleep(3);
    /* -------------------------------------------------------------------------------- *
     * Start Force Tx training / AN at System side                                      *
     * -------------------------------------------------------------------------------- */
    for (phy_index = 0; phy_index <  NUM_OF_PHY; phy_index++) {
        phy_info.phy_addr = (phy_index == 0) ? PHY_ID0 : PHY_ID1;
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            ...
            if (retval != TEST_SUCCESS) {
                printf("Failed to set cl73 ability for lane-map 0x%x of PHY-%d at System side (ret = %d)!\n",
                        phy_info.lane_map, phy_info.phy_addr, retval);
               goto _barchetta_reference_app_error;
            }
            ...
            if (retval != TEST_SUCCESS) {
                printf("Failed to enable AN for lane-map 0x%x of PHY-%d at System side (ret = %d)!\n",
                        phy_info.lane_map, phy_info.phy_addr, retval);
               goto _barchetta_reference_app_error;
            }
    ...
    sleep(5);
    /* -------------------------------------------------------------------------------- *
     * Get Force Tx training/AN status at System side                                   *
     * -------------------------------------------------------------------------------- */
    for (phy_index = 0; phy_index <  NUM_OF_PHY; phy_index++) {
        phy_info.phy_addr = (phy_index == 0) ? PHY_ID0 : PHY_ID1;
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            ...
            if (retval != TEST_SUCCESS) {
                printf("Failed to get cl73 ability for lane-map 0x%x of PHY-%d at System side (ret = %d)!\n",
                phy_info.lane_map, phy_info.phy_addr, retval);
                goto _barchetta_reference_app_error;
            } else {
                printf("CL73 ability get parameters for lane-map 0x%x of PHY-%d at System side as below :\n",
                phy_info.lane_map, phy_info.phy_addr);
            ...
            if (retval != TEST_SUCCESS) {
              printf("Failed to get AN status for lane-map 0x%x of PHY-%d at System side (ret = %d)!\n",
              phy_info.lane_map, phy_info.phy_addr, retval);
              goto _barchetta_reference_app_error;
            } else {
                printf("AN status for lane-map 0x%x of PHY-%d at System side as below :\n", phy_info.lane_map, phy_info.phy_addr);
            ...
            if (an_done != 1) {
                retval = TEST_FAILURE;
                printf("AN not done for lane-map 0x%x of PHY-%d at System side!\n", phy_info.lane_map, phy_info.phy_addr);
                goto _barchetta_reference_app_error;
            }
#endif
        }
