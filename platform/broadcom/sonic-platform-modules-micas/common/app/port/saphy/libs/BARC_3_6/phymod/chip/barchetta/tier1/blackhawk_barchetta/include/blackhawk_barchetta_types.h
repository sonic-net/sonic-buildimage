...
/** @file blackhawk_barchetta_types.h
 * Common types used by Serdes API functions
 */

#ifndef BLACKHAWK_BARCHETTA_API_TYPES_H
#define BLACKHAWK_BARCHETTA_API_TYPES_H

#include "blackhawk_barchetta_usr_includes.h"

typedef struct blackhawk_barchetta_triage_info_st {
   uint32_t     api_ver;
   uint32_t     ucode_ver;
   uint8_t      core;
   uint8_t      lane;
...
} blackhawk_barchetta_triage_info;


/** TX FIR tap values */
typedef struct blackhawk_barchetta_txfir {
  int16_t tap[12];
} blackhawk_barchetta_txfir_st;

#define _Y_ (6)
#define _X_ (5)
typedef struct blackhawk_barchetta_dbgfb_stats_s {
        int32_t  data1[_X_][_Y_];
        uint32_t data2[_X_][_Y_];
        uint32_t data3[_X_][_Y_];
        int32_t  data4[_X_][_Y_];
} blackhawk_barchetta_dbgfb_data_st;

typedef struct blackhawk_barchetta_dbgfb_data_s {
    uint32_t time_in_us;
    blackhawk_barchetta_dbgfb_data_st data;
    uint8_t pam4es_en;
    uint8_t temp[3];
} blackhawk_barchetta_dbgfb_stats_st;

#define SIZE_OF_DBGFB_STATS_STRUCT 488  /* Change the size if blackhawk_barchetta_dbgfb_stats_st size changes */
typedef struct {
    uint8_t dbgfb_tmp[sizeof(blackhawk_barchetta_dbgfb_stats_st)==SIZE_OF_DBGFB_STATS_STRUCT ? 1: -1];
} blackhawk_barchetta_dbgfb_stats_st_size_check;

typedef struct blackhawk_barchetta_dbgfb_cfg_s {
    uint8_t y;
    uint8_t x;
    uint32_t time_in_us;
    int32_t data1;
    uint32_t data2;
    uint32_t data3;
} blackhawk_barchetta_dbgfb_cfg_st;
...
struct blackhawk_barchetta_uc_lane_config_field_st {
...
struct blackhawk_barchetta_uc_core_config_field_st {
  uint8_t  vco_rate          ;
...
/** Lane Config Struct */
struct  blackhawk_barchetta_uc_lane_config_st {
  struct blackhawk_barchetta_uc_lane_config_field_st field;
  uint16_t word;
};

/** Core Config Struct */
struct  blackhawk_barchetta_uc_core_config_st {
  struct blackhawk_barchetta_uc_core_config_field_st field;
  uint16_t word;
...
struct blackhawk_barchetta_usr_ctrl_func_st {
...
struct blackhawk_barchetta_usr_ctrl_dfe_func_st {
  /* BLACKHAWK TODO:  This was copied from Falcon16. */
...
struct blackhawk_barchetta_usr_ctrl_disable_functions_st {
  struct blackhawk_barchetta_usr_ctrl_func_st field;
  uint16_t word;
};

/** Lane User Control Disable DFE Function Struct */
struct blackhawk_barchetta_usr_ctrl_disable_dfe_functions_st {
  struct blackhawk_barchetta_usr_ctrl_dfe_func_st field;
  uint8_t  byte;
};
struct blackhawk_barchetta_ber_data_st {
    uint64_t num_errs;
    uint64_t num_bits;
    uint8_t lcklost;
    uint8_t prbs_chk_en;
...
struct blackhawk_barchetta_check_platform_info_st {
    char c;
    uint32_t d;
};
...
struct blackhawk_barchetta_detailed_lane_status_st {
    ...
    blackhawk_barchetta_dbgfb_stats_st dbgfb_stats;
...
/* Change the size if blackhawk_barchetta_detailed_lane_status_st size changes */
/* Add new extended lane struct fields > 1byte to the reverse byte list in the decoder */
#define SIZE_OF_EXTENDED_LANE_STATE 916

typedef struct {
    uint8_t extended_lane_st_tmp[sizeof(struct blackhawk_barchetta_detailed_lane_status_st)==SIZE_OF_EXTENDED_LANE_STATE ? 1: -1];
} blackhawk_barchetta_detailed_lane_status_st_size_check;
