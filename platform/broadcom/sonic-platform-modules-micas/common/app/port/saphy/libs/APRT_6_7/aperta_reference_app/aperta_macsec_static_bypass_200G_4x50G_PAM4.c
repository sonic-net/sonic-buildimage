 ...
 *   8. Check link status   (Not mandetory)                                         *
 *   9. Run the Scallop Shell (CLI command line interface)                          *
...
/* 200G PAM4 (4x25G) MACsec static bypass mode */
#define OP_MODE_STR    "200G PAM4 mixed mode demonstration"
#define MAX_FLEXING_ITERATION   1   /* 2 */
...
int main(int argc, char *argv[])
{
    ...
    if ( argv[2] ) {
        speed_mode = ('2' == argv[2][0]) ? 200 : 100;
    }
    if ( argv[3] ) {
        if ( 100 == speed_mode ) {
            fec_mode = ('f' == argv[3][0]) ? bcmplpapertaRSFEC: bcmplpapertaNoFEC;
        } else {
            fec_mode = bcmplpapertaRS544;
        }
    }
    ...
    for ( flex_cnt=0; flex_cnt<MAX_FLEXING_ITERATION; flex_cnt++ ) {
        if ( 100 ==  speed_mode ) {
            ...
            sys_fec_mode    = fec_mode;    /*  SYS_FEC_MODE_4X_1; */
            line_fec_mode   = fec_mode;    /* LINE_FEC_MODE_4X_1; */
     ...
     * -------------------------------------------------------------------------------- */
     printf("\n========  %s CLI Reference App  ======== (%dG FEC=%d)\n", CHIP_NAME, speed_mode, fec_mode);
