...
#define OP_MODE_STR    "25G NRZ (1x25G) PCS Dump Demonstration"
...
int main(int argc, char *argv[])
{
   ...
   total_lane_maps = NUM_ARR_ELEMENTS(sys_lane_map_list);
    ...
    printf("\n PCS Diag Dump :\n");
    printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
    phy_info.if_side  = LINE_SIDE;/* Unused */
    phy_info.lane_map = 0xFF; /* Unused */
    for (phy_index = 0; phy_index < NUM_OF_PHY; phy_index++) {
        phy_info.phy_addr = (phy_index == 0) ? PHY_ID0 : PHY_ID1;
        memcpy(&mac_info.phy_info, &phy_info, sizeof(bcm_plp_access_t));
        retval = bcm_plp_pcs_diagnostic_dump(CHIP_NAME, mac_info);
        if (retval != TEST_SUCCESS) {
            printf("FAIL: Failed to perform PCS diagonistic dump for PHY-%d (ret = %d)!\n", phy_info.phy_addr, retval);
            goto _aperta_config_error;
        }
    }
