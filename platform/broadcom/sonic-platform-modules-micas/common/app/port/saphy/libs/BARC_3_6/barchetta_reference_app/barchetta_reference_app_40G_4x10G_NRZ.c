 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Barchetta chip for 40G NRZ (4x10G)        *
 * mode as follow:                                                                  *
 *   1. Connect to the board.                                                       *
 *   2. Resest all the cores on MDIO Bus                                            *
 *   3. Enable broadcast firmware download method for all cores on MDIO bus         *
 *   4. Load the firmware download only on one PHY Id                               *
 *   5. Verify the broadcast firmware download method for all cores on MDIO bus     *
 *   6. Disable the broadcast firmware download method for all cores on MDIO bus    *
 *   7. Read Firmware Version, Firmware CRC and Chip Revision Id for all cores  
 *   8. Setup logical lane-map for Tx/Rx at System and Line side.                   *
 *   9. Configure Tx/Rx polarity.                                                   *
 *  10. Configure operating mode parameters.                                        *
 *  11. Configure PRBS generator (Tx) and checker (Rx).                             *
 *  12. Check for link status, PRBS lock and Rx PMD lock.                           *
 *  13. Reads chip register, dump PHY status and display eye scan.                  *
 *  14. Disable Tx/Rx PRBS at both System and Line side.                            *
 *  15. Cleanup and close the connection to the board.                              *
 * -------------------------------------------------------------------------------- */

/* 40G NRZ (4x10G) Retimer */
#define OP_MODE_STR             "40G NRZ (4x10G) Retimer"
...
int main(void)
{
    ...
    for (phy_index = 0; phy_index <  NUM_OF_PHY ; phy_index++) {
        ...
        if (retval != TEST_SUCCESS) {
            printf("Failed to enable broadcast method for PHY-%d (ret = %d)!\n", phy_info.phy_addr, retval);
            goto _barchetta_reference_app_error;
        }
     }    
    ...
    if (retval != TEST_SUCCESS) {
            printf("Failed to load broadcast firmware method for PHY-%d (ret = %d)!\n", phy_info.phy_addr, retval);
            goto _barchetta_reference_app_error;
    }
   
    printf("Broadcast firmware download method verificaiton for all PHY Ids on MDIO bus\n"); 
    for (phy_index = 0; phy_index <  NUM_OF_PHY ; phy_index++) {
        phy_info.phy_addr = (phy_index == 0) ? PHY_ID0 : PHY_ID1;
        ...
        if (retval != TEST_SUCCESS) {
            printf("Failed to verify firmware broadcast method for PHY-%d (ret = %d)!\n", phy_info.phy_addr, retval);
            goto _barchetta_reference_app_error;
        }
     }    
    ...
    for (phy_index = 0; phy_index <  NUM_OF_PHY ; phy_index++) {
        ...
        if (retval != TEST_SUCCESS) {
            printf("Failed to disable firmware broadcast method for PHY-%d (ret = %d)!\n", phy_info.phy_addr, retval);
            goto _barchetta_reference_app_error;
        }
     }    
