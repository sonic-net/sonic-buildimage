...
bcm_if_phymod_ctrl_t plp_aperta2_phy_ctrl;
bcm_if_phymod_phy_id_t _plp_aperta2_phyid_list[BCM_PM_IF_MAX_PHY];
...
void _bcm_plp_aperta2_pm_if_core_init(_bcm_if_phymod_core_t *core,
                   plp_aperta2_phymod_bus_t *core_bus, uint32_t phy_addr, void *user_acc)
{
    plp_aperta2_phymod_core_access_t *pm_core;
    plp_aperta2_phymod_access_t *pm_acc;
    ...
    plp_aperta2_phymod_core_access_t_init(pm_core);
    pm_acc = &pm_core->access;
    plp_aperta2_phymod_access_t_init(pm_acc);
    ...
    PHYMOD_ACC_MISC(pm_acc) = &(_plp_aperta2_phyid_list[phy_addr]);
...
{
    /* invalid the _plp_aperta2_phyid_list entry before initialization */
    _plp_aperta2_phyid_list[BCM_PHY_ADDR_LOGICAL(phy_id_combo)].valid &= ~BCM_PHY_ID_VALID;
}

void _bcm_plp_aperta2_pm_if_get_phy_id_idx(uint32_t phy_id_combo, uint32_t *phy_id_idx, uint32_t *exist_phy)
{
    unsigned int  found  = FALSE;
    uint32_t      phy_id = BCM_PHY_ADDR_LOGICAL(phy_id_combo);
    *phy_id_idx = phy_id;

    if ( (BCM_PHY_ID_VALID & _plp_aperta2_phyid_list[phy_id].valid) && (_plp_aperta2_phyid_list[phy_id].phy_id == phy_id) ) {
       found      = TRUE;
       *exist_phy = TRUE;
       return;
    }
    ...
    if ( ! found ) {
        if ( ! (BCM_PHY_ID_VALID & _plp_aperta2_phyid_list[phy_id].valid) ) {
            if ( BCM_PHY_IS_PHYSICAL_ADDR_GIVEN_BY_APP(phy_id_combo) ) {
                _plp_aperta2_phyid_list[phy_id].valid        |=  BCM_PHY_PHYSICAL_ADDR_GIVEN_BY_APP;
                _plp_aperta2_phyid_list[phy_id].physical_addr =  BCM_PHY_ADDR_PHYSICAL(phy_id_combo);
            } else {
                _plp_aperta2_phyid_list[phy_id].valid        &= ~BCM_PHY_PHYSICAL_ADDR_GIVEN_BY_APP;
                _plp_aperta2_phyid_list[phy_id].physical_addr =  phy_id;
            }

            if ( BCM_PHY_IS_MASTER_PORT(phy_id_combo) ) {
                _plp_aperta2_phyid_list[phy_id].valid        |=  BCM_PHY_MASTER_PORT;
            } else {
                _plp_aperta2_phyid_list[phy_id].valid        &= ~BCM_PHY_MASTER_PORT;
            }

            _plp_aperta2_phyid_list[phy_id].phy_id            = phy_id;            /* logical PHY address */
            _plp_aperta2_phyid_list[phy_id].valid            |= BCM_PHY_ID_VALID;  /* validate this entry */
            *exist_phy = FALSE;
        }
    }
}
...
{
    return (BCM_PHY_MASTER_PORT & _plp_aperta2_phyid_list[BCM_PHY_ADDR_LOGICAL(phy_id)].valid) ? TRUE : FALSE;
}

#else  /* ! XGBASET_SUPPORT  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  */

void _bcm_plp_aperta2_pm_if_get_phy_id_idx(uint32_t phy_id, uint32_t *phy_id_idx, uint32_t *exist_phy) 
{
    unsigned int found = 0;
    *phy_id_idx = phy_id;

    if (_plp_aperta2_phyid_list[phy_id].valid && (_plp_aperta2_phyid_list[phy_id].phy_id == phy_id)) {
           found = 1;
           *exist_phy = 1;
    }
    if (!found) { /* New PHYID*/
        if (!_plp_aperta2_phyid_list[phy_id].valid) {
            _plp_aperta2_phyid_list[phy_id].phy_id = phy_id;
            _plp_aperta2_phyid_list[phy_id].valid = 1;
                 *exist_phy = 0;
        }
    }
}
...
int _bcm_plp_aperta2_pm_phy_init(_bcm_if_phymod_phy_t *phy, bcm_pm_firmware_load_method_t firmware_load_method, void *init_param)
{
    ...
    if (core->init == FALSE  && PHYMOD_CORE_INIT_F_RESET_CORE_FOR_FW_LOAD_GET(&core->init_config)) {
        rv = plp_aperta2_phymod_core_init(&core->pm_core, &core->init_config, &core_sts);
        ...
        /* bcm_plp_aperta2_init needs to be multithreaded, so for PHY's other than
    ...
    /* read serdes id info */
    rv = plp_aperta2_phymod_core_info_get(&core->pm_core, &core_info); 
    if (rv != 0) {
        return rv;
    }
...
uint8_t _bcm_plp_aperta2_pm_if_log2n(uint32_t n)
{
    return (n > 1 ? 1 + _bcm_plp_aperta2_pm_if_log2n(n >> 1) : 0);
}

uint8_t _bcm_plp_aperta2_pm_if_get_right_most_set_bit_pos(uint32_t n)
{
    return (_bcm_plp_aperta2_pm_if_log2n(n & -n));
}
...
int _bcm_plp_aperta2_pm_if_get_port_information(bcm_plp_access_t phy_info, bcm_if_port_information_t *bcm_port_information )
{
    ...
    rv = plp_aperta2_phymod_get_port_information(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &port_information);
