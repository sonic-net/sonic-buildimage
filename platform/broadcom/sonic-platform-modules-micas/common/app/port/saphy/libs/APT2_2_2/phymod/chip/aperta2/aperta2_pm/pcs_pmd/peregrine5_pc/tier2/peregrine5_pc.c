/*
 *
 * $Id: phymod.xml,v 1.1.2.5 2013/09/12 10:43:06 nirf Exp $
 *
 * $Copyright:
 * All Rights Reserved.$
 *
 *
 */

#include <phymod/phymod.h>
#include <phymod/phymod_system.h>
#include <phymod/phymod_util.h>
#include <phymod/phymod_dispatch.h>
#include <peregrine5_pc.h>
#include "peregrine5_pc/tier1/peregrine5_pc_cfg_seq.h"
#include "peregrine5_pc/tier1/peregrine5_pc_enum.h"
#include "peregrine5_pc/tier1/peregrine5_pc_common.h"
#include "peregrine5_pc/tier1/peregrine5_pc_interface.h"
#include "peregrine5_pc/tier1/peregrine5_pc_dependencies.h"
#include "peregrine5_pc/tier1/peregrine5_pc_internal.h"
#include "peregrine5_pc/tier2/peregrine5_pc_ucode.h"
#include <aperta2_pm_seq.h>

#define PEREGRINE5_PC_MODEL               0x2a
#define PEREGRINE5_PC_NOF_LANES_IN_CORE   0x8
#define PEREGRINE5_PC_PHY_ALL_LANES       0xff
#define PEREGRINE5_PC_TX_TAP_NUM          12
#define PEREGRINE5_PC_PMD_CRC_UCODE       1

#define PEREGRINE5_PC_REF_CLOCK_312P5_HZ  312500000

#define DEACODE 0

#define PEREGRINE5_PC_CORE_TO_PHY_ACCESS(_phy_access, _core_access) \
    do{\
        PHYMOD_MEMCPY(&(_phy_access)->access, &(_core_access)->access, sizeof((_phy_access)->access));\
        (_phy_access)->type           = (_core_access)->type; \
        (_phy_access)->port_loc       = (_core_access)->port_loc; \
        (_phy_access)->device_op_mode = (_core_access)->device_op_mode; \
        (_phy_access)->access.lane_mask = PEREGRINE5_PC_PHY_ALL_LANES; \
    }while(0)

int plp_aperta2_peregrine5_pc_core_identify(const plp_aperta2_phymod_core_access_t* core,
                                uint32_t core_id, uint32_t* is_identified)
{
    plp_aperta2_phymod_core_access_t  core_copy;

    peregrine5_pc_rev_id0_t rev_id0;
    peregrine5_pc_rev_id1_t rev_id1;
    *is_identified = 0;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));

    /* PHY IDs match - now check model */
    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_identify((plp_aperta2_phymod_phy_access_t*)&core_copy, &rev_id0, &rev_id1));

    if (rev_id0.revid_model == PEREGRINE5_PC_MODEL)  {
        *is_identified = 1;
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_core_info_get(const plp_aperta2_phymod_core_access_t* core, plp_aperta2_phymod_core_info_t* info)
{
    info->core_version = 0;
    info->serdes_id = 0;
    info->phy_id0 = 0;
    info->phy_id1 = 0;

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_core_lane_map_get(const plp_aperta2_phymod_core_access_t* core, plp_aperta2_phymod_lane_map_t* lane_map)
{
    plp_aperta2_phymod_core_access_t  core_copy;
    uint32_t tx_lane_map, rx_lane_map;
    int i = 0;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_pmd_lane_map_get((plp_aperta2_phymod_phy_access_t*)&core_copy, &tx_lane_map, &rx_lane_map));

    /*next get the lane map into serdes spi format */
    for (i = 0; i < PEREGRINE5_PC_NOF_LANES_IN_CORE; i++) {
        lane_map->lane_map_tx[tx_lane_map >> (4 * i) & 0xf] = i;
        lane_map->lane_map_rx[rx_lane_map >> (4 * i) & 0xf] = i;
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_core_reset_set(const plp_aperta2_phymod_core_access_t* core,
                                  plp_aperta2_phymod_reset_mode_t reset_mode,
                                  plp_aperta2_phymod_reset_direction_t direction)
{
    plp_aperta2_phymod_core_access_t  core_copy;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;

    if (reset_mode == phymodResetModeSoft) {
        ucode_info_t ucode;
        PHYMOD_MEMSET(&ucode,0,sizeof(ucode));
        ucode.stack_size = PEREGRINE5_PC_UCODE_STACK_SIZE;
        ucode.ucode_size = PEREGRINE5_PC_UCODE_IMAGE_SIZE;
        ucode.crc_value  = PEREGRINE5_PC_UCODE_IMAGE_CRC;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_core_reset((plp_aperta2_phymod_phy_access_t*)&core_copy, ucode));
    }
    return PHYMOD_E_NONE;

}

int plp_aperta2_peregrine5_pc_core_reset_get(const plp_aperta2_phymod_core_access_t* core,
                                  plp_aperta2_phymod_reset_mode_t reset_mode,
                                  plp_aperta2_phymod_reset_direction_t* direction)
{
    return PHYMOD_E_NONE;

}

int plp_aperta2_peregrine5_pc_phy_tx_lane_control_set(const plp_aperta2_phymod_phy_access_t* phy,
                                          plp_aperta2_phymod_phy_tx_lane_control_t tx_control)
{
    int start_lane, num_lane, i;
    uint32_t lane_reset;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    /*first check if lane is in reset */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset_get(&pm_phy_copy, &lane_reset));

    /* if lane is not in reset, then reset the lane first */
    if (!lane_reset) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 1));
    }

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        switch (tx_control)
        {
            case phymodTxElectricalIdleEnable:
                PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_electrical_idle_set(&pm_phy_copy, 1));
                break;
            case phymodTxElectricalIdleDisable:
                PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_electrical_idle_set(&pm_phy_copy, 0));
                break;
            case phymodTxSquelchOn:
                PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_tx_disable(&pm_phy_copy, 1));
                break;
            case phymodTxSquelchOff:
                PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_tx_disable(&pm_phy_copy, 0));
                break;
            default:
                PHYMOD_DEBUG_ERROR(("This control is NOT SUPPORTED!! (plp_aperta2_peregrine5_pc_phy_tx_lane_control_set) \n"));
                break;
        }
    }

    /* if lane is not in reset, then reset the lane first */
    if (!lane_reset) {
        PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 0));
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_lane_control_get(const plp_aperta2_phymod_phy_access_t* phy,
                                           plp_aperta2_phymod_phy_tx_lane_control_t *tx_control)
{

    uint8_t disable, idle_enable;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));


    *tx_control = phymodTxSquelchOff;

    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_tx_disable_get(&pm_phy_copy, &disable));
    if(disable) {
      *tx_control = phymodTxSquelchOn;
    } else {
      PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_electrical_idle_get(&pm_phy_copy, &idle_enable));
      if (!idle_enable) {
        *tx_control = phymodTxElectricalIdleDisable;
      }
    }

    return PHYMOD_E_NONE;
}

/*Rx control*/
int plp_aperta2_peregrine5_pc_phy_rx_lane_control_set(const plp_aperta2_phymod_phy_access_t* phy,
                                          plp_aperta2_phymod_phy_rx_lane_control_t rx_control)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    int start_lane, num_lane, i;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    /* next program the tx fir taps and driver current based on the input */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    /*put the lane into dp reset */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 1));


    switch (rx_control) {
    case phymodRxSquelchOn:
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
            pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pmd_force_signal_detect(&pm_phy_copy, 1, 0));
        }
        break;
    case phymodRxSquelchOff:
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
            pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pmd_force_signal_detect(&pm_phy_copy, 0, 0));
        }
        break;
    default:
        break;
    }

    /*release the lane dp reset */
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_lane_control_get(const plp_aperta2_phymod_phy_access_t* phy,
                                          plp_aperta2_phymod_phy_rx_lane_control_t* rx_control)
{
    uint8_t force_en, force_val;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    /* first get the force enabled bit and forced value */
    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pmd_force_signal_detect_get(&pm_phy_copy, &force_en, &force_val));

    if ((force_en) && (force_val == 0)) {
        *rx_control = phymodRxSquelchOn;
    } else {
        *rx_control = phymodRxSquelchOff;
    }

    return PHYMOD_E_NONE;
}

/* load tscp fw. the fw_loader parameter is valid just for external fw load*/
STATIC
int _plp_aperta2_peregrine5_pc_core_firmware_load(const plp_aperta2_phymod_core_access_t* core, const plp_aperta2_phymod_core_init_config_t* init_config)
{
    plp_aperta2_phymod_core_access_t  core_copy;
    plp_aperta2_phymod_phy_access_t phy_access;
    phymod_firmware_load_info_t info;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    PEREGRINE5_PC_CORE_TO_PHY_ACCESS(&phy_access, core);

    /* Get ucode load info */
    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_phy_firmware_load_info_get(&phy_access, &info));

    switch(init_config->firmware_load_method){
    case phymodFirmwareLoadMethodInternal:
        break;
    case phymodFirmwareLoadMethodExternal:
       break;
    case phymodFirmwareLoadMethodNone:
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_CONFIG, (_PHYMOD_MSG("illegal fw load method %u"), init_config->firmware_load_method));
    }

    return PHYMOD_E_NONE;

}

int plp_aperta2_peregrine5_pc_phy_firmware_lane_config_get(const plp_aperta2_phymod_phy_access_t* phy,
                                                phymod_firmware_lane_config_t* fw_config)
{
    struct peregrine5_pc_uc_lane_config_st lane_config;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    enum peregrine5_pc_force_cdr_mode_enum cdr_mode = 0;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_MEMSET(&lane_config, 0x0, sizeof(lane_config));
    PHYMOD_MEMSET(fw_config, 0, sizeof(*fw_config));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_get_uc_lane_cfg(&pm_phy_copy, &lane_config));

    fw_config->LaneConfigFromPCS     = lane_config.field.lane_cfg_from_pcs;
    fw_config->AnEnabled             = lane_config.field.an_enabled;
    fw_config->DfeOn                 = lane_config.field.dfe_on;
    fw_config->LpDfeOn               = lane_config.field.rx_low_power;
    fw_config->ForceBrDfe            = lane_config.field.force_cdr_mode;
    fw_config->MediaType             = lane_config.field.media_type;
    fw_config->UnreliableLos         = lane_config.field.unreliable_los;
    fw_config->Cl72AutoPolEn         = lane_config.field.linktrn_auto_polarity_en;
    fw_config->Cl72RestTO            = lane_config.field.linktrn_restart_timeout_en;
    fw_config->ForceES     = lane_config.field.force_er;
    fw_config->ForceNS      = lane_config.field.force_nr;
    fw_config->LinkPartnerPrecoderEn     = lane_config.field.lp_has_prec_en;
    fw_config->ForcePAM4Mode         = lane_config.field.force_pam4_mode;
    fw_config->ForceNRZMode          = lane_config.field.force_nrz_mode;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_get_force_lane_cdr_mode(&pm_phy_copy, &cdr_mode));
    if (cdr_mode == PEREGRINE5_PC_OSCDR_FORCE_ENABLE) {
        fw_config->ForceOsCdr = 1;
        fw_config->ForceBrDfe = 0;
    } else if (cdr_mode == PEREGRINE5_PC_BRCDR_FORCE_ENABLE) {
        fw_config->ForceOsCdr = 0;
        fw_config->ForceBrDfe = 1;
    } else {
        fw_config->ForceOsCdr = 0;
        fw_config->ForceBrDfe = 0;
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pam4_tx_set(const plp_aperta2_phymod_phy_access_t* phy, const phymod_pam4_tx_t* tx)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    enum peregrine5_pc_txfir_tap_enable_enum enable_taps = PEREGRINE5_PC_NRZ_6TAP;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 1));

    for (i = 0; i < num_lane; i++) {
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        /*next check 3 tap mode or 6 tap mode */
        if (tx->serdes_tx_tap_mode == phymodTxTapModeNRZ_LP_3TAP ||
                tx->serdes_tx_tap_mode == phymodTxTapModePAM4_LP_3TAP) {
            if (tx->serdes_tx_tap_mode == phymodTxTapModeNRZ_LP_3TAP) {
                enable_taps = PEREGRINE5_PC_NRZ_LP_3TAP;
            } else {
                enable_taps = PEREGRINE5_PC_PAM4_LP_3TAP;
            }
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_apply_txfir_cfg(&phy_copy,
                                                 enable_taps,
                                                 tx->pre3,
                                                 0,
                                                 tx->pre,
                                                 tx->main,
                                                 tx->post,
                                                 0));
        } else {
            if (tx->serdes_tx_tap_mode == phymodTxTapModeNRZ_6TAP) {
                enable_taps= PEREGRINE5_PC_NRZ_6TAP;
            } else {
                enable_taps= PEREGRINE5_PC_PAM4_6TAP;
            }
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_apply_txfir_cfg(&phy_copy,
                                                 enable_taps,
                                                 tx->pre3,
                                                 tx->pre2,
                                                 tx->pre,
                                                 tx->main,
                                                 tx->post,
                                                 tx->post2));
        }
    }

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_core_lane_map_set(const plp_aperta2_phymod_core_access_t* core, const plp_aperta2_phymod_lane_map_t* lane_map)
{
    plp_aperta2_phymod_core_access_t  core_copy;
    uint8_t pmd_tx_addr[8], pmd_rx_addr[8];
    int i = 0;
    uint8_t tmp_phy_lane;

    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;

    /*next get the lane map into serdes spi format */
    for (i = 0; i < PEREGRINE5_PC_NOF_LANES_IN_CORE; i++) {
        tmp_phy_lane = (uint8_t) lane_map->lane_map_tx[i];
        pmd_tx_addr[tmp_phy_lane] = i;
        tmp_phy_lane = (uint8_t) lane_map->lane_map_rx[i];
        pmd_rx_addr[tmp_phy_lane] = i;
    }

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_map_lanes((plp_aperta2_phymod_phy_access_t*)&core_copy,
                                   PEREGRINE5_PC_NOF_LANES_IN_CORE,
                                   pmd_tx_addr,
                                   pmd_rx_addr));

    return PHYMOD_E_NONE;
}

static
int _plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(const plp_aperta2_phymod_phy_access_t* phy,
                                                phymod_firmware_lane_config_t fw_config)
{
    struct peregrine5_pc_uc_lane_config_st serdes_firmware_config;
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_MEMSET(&serdes_firmware_config, 0x0, sizeof(serdes_firmware_config));
    serdes_firmware_config.field.lane_cfg_from_pcs      = fw_config.LaneConfigFromPCS;
    serdes_firmware_config.field.an_enabled             = fw_config.AnEnabled;
    serdes_firmware_config.field.dfe_on                 = fw_config.DfeOn;
    serdes_firmware_config.field.force_cdr_mode         = fw_config.ForceBrDfe;
    serdes_firmware_config.field.unreliable_los         = fw_config.UnreliableLos;
    serdes_firmware_config.field.media_type             = fw_config.MediaType;
    serdes_firmware_config.field.rx_low_power            = fw_config.LpDfeOn;
    serdes_firmware_config.field.linktrn_auto_polarity_en  = fw_config.Cl72AutoPolEn;
    serdes_firmware_config.field.linktrn_restart_timeout_en = fw_config.Cl72RestTO;
    serdes_firmware_config.field.force_er               = fw_config.ForceES;
    serdes_firmware_config.field.force_nr               = fw_config.ForceNS;
    serdes_firmware_config.field.force_nrz_mode         = fw_config.ForceNRZMode;
    serdes_firmware_config.field.force_pam4_mode        = fw_config.ForcePAM4Mode;
    serdes_firmware_config.field.lp_has_prec_en         = fw_config.LinkPartnerPrecoderEn;


    
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_set_uc_lane_cfg(&phy_copy, serdes_firmware_config));
        if (fw_config.ForceBrDfe) {
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_force_lane_cdr_mode(&phy_copy, PEREGRINE5_PC_BRCDR_FORCE_ENABLE));
        } else if (fw_config.ForceOsCdr) {
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_force_lane_cdr_mode(&phy_copy, PEREGRINE5_PC_OSCDR_FORCE_ENABLE));
        } else if ((fw_config.ForceOsCdr == 0) && (fw_config.ForceBrDfe == 0)) {
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_force_lane_cdr_mode(&phy_copy, PEREGRINE5_PC_CDR_FORCE_DISABLE));
        }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(const plp_aperta2_phymod_phy_access_t* phy,
                                                phymod_firmware_lane_config_t fw_config)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    uint32_t lane_reset;
    int start_lane, num_lane, i;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    /* need to check if both FORCE os/BR cdr bit are set */
    if ((fw_config.ForceBrDfe) && (fw_config.ForceOsCdr)) {
        PHYMOD_DEBUG_ERROR(("force both OS/BR CDR mode is NOT supported\n"));
        return PHYMOD_E_UNAVAIL;
    }

    /*first check if lane is in reset */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset_get(&phy_copy, &lane_reset));

    /* if lane is not in reset, then reset the lane first */
    if (!lane_reset) {
        PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 1));
    }

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (_plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(&phy_copy, fw_config));
    }

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    if (!lane_reset) {
        PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 0));
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_restart(const plp_aperta2_phymod_phy_access_t* phy)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 1));

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_rx_restart(&phy_copy, 1));
    }

    PHYMOD_USLEEP(1000);

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_rx_restart(&phy_copy, 0));
    }

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_polarity_set(const plp_aperta2_phymod_phy_access_t* phy, const plp_aperta2_phymod_polarity_t* polarity)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_rx_polarity_set(&phy_copy, polarity->tx_polarity, polarity->rx_polarity));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_polarity_get(const plp_aperta2_phymod_phy_access_t* phy, plp_aperta2_phymod_polarity_t* polarity)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_rx_polarity_get(&phy_copy, &polarity->tx_polarity, &polarity->rx_polarity));
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pam4_tx_get(const plp_aperta2_phymod_phy_access_t* phy, phymod_pam4_tx_t* tx)
{
    uint8_t pmd_tx_tap_mode;
    int16_t val;
    uint16_t tx_tap_nrz_mode = 0;
    plp_aperta2_phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));


    /* read current tx tap mode */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_tap_mode_get(&phy_copy, &pmd_tx_tap_mode));

    /*read current tx NRZ mode control info */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_nrz_mode_get(&phy_copy, &tx_tap_nrz_mode));

    if (pmd_tx_tap_mode == 0) {
        /* 3 tap mode */
        if (tx_tap_nrz_mode) {/*NRZ*/
            tx->serdes_tx_tap_mode = phymodTxTapModeNRZ_LP_3TAP;
        } else {
            tx->serdes_tx_tap_mode = phymodTxTapModePAM4_LP_3TAP;
        }
    } else {
        if (tx_tap_nrz_mode) {/*NRZ*/
            tx->serdes_tx_tap_mode = phymodTxTapModeNRZ_6TAP;
        } else {
            tx->serdes_tx_tap_mode = phymodTxTapModePAM4_6TAP;
        }
    }

    /*next check 3 tap mode or 6 tap mode */
    if (tx->serdes_tx_tap_mode == phymodTxTapModeNRZ_LP_3TAP || 
            tx->serdes_tx_tap_mode == phymodTxTapModePAM4_LP_3TAP) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_PRE1, &val));
        tx->pre = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_MAIN, &val));
        tx->main = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_POST1, &val));
        tx->post = val;
        tx->pre2 = 0;
        tx->post2 = 0;
        tx->post3 = 0;
        tx->pre3 = 0;
    } else {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_PRE3, &val));
        tx->pre3 = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_PRE2, &val));
        tx->pre2 = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_PRE1, &val));
        tx->pre = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_MAIN, &val));
        tx->main = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_POST1, &val));
        tx->post = val;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_read_tx_afe(&phy_copy, PEREGRINE5_PC_TX_AFE_POST2, &val));
        tx->post2 = val;
        tx->post3 = 0;
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_override_set(const plp_aperta2_phymod_phy_access_t* phy, const plp_aperta2_phymod_tx_override_t* tx_override)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 1));

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
             (plp_aperta2_peregrine5_pc_tx_pi_freq_override(&phy_copy,
                                                 tx_override->phase_interpolator.enable,
                                                 tx_override->phase_interpolator.value));
    }

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_override_get(const plp_aperta2_phymod_phy_access_t* phy, plp_aperta2_phymod_tx_override_t* tx_override)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int16_t value;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_pi_control_get(&phy_copy, &value));

    tx_override->phase_interpolator.value = (int32_t) value;
    tx_override->phase_interpolator.enable = (value != 0) ? 1 : 0;

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_set(const plp_aperta2_phymod_phy_access_t* phy, const plp_aperta2_phymod_rx_t* rx)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    uint8_t uc_lane_stopped;
    phymod_phy_signalling_method_t signalling_mode;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    /* next read the PAM4 mode or  not */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_signalling_mode_status_get(&phy_copy, &signalling_mode));

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        /* first check if uc lane is stopped already */
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_stop_uc_lane_status(&phy_copy, &uc_lane_stopped));
        if (!uc_lane_stopped) {
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_stop_rx_adaptation(&phy_copy, 1));
        }

        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_write_rx_afe(&phy_copy, RX_AFE_VGA, (int8_t) rx->vga.value));

        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_write_rx_afe(&phy_copy, RX_AFE_PF, (int8_t) rx->peaking_filter.value));

        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_write_rx_afe(&phy_copy, RX_AFE_PF2, (int8_t) rx->low_freq_peaking_filter.value));

        /* next check the peaking value */
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_write_rx_afe(&phy_copy, RX_AFE_PF3, (int8_t) rx->high_freq_peaking_filter.value));
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_power_set(const plp_aperta2_phymod_phy_access_t* phy, const plp_aperta2_phymod_phy_power_t* power)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerOff)) {
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_lane_pwrdn(&pm_phy_copy, PWRDN));
    } else if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOn)) {
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_lane_pwrdn(&pm_phy_copy, PWR_ON));
    } else if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerOn)) {
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_lane_pwrdn(&pm_phy_copy, PWRDN_TX));
    } else if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOff)) {
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_lane_pwrdn(&pm_phy_copy, PWRDN_RX));
    } else {
       return PHYMOD_E_CONFIG;
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_power_get(const plp_aperta2_phymod_phy_access_t* phy, plp_aperta2_phymod_phy_power_t* power)
{
    return PHYMOD_E_UNAVAIL;
}

int plp_aperta2_peregrine5_pc_phy_rx_get(const plp_aperta2_phymod_phy_access_t* phy, plp_aperta2_phymod_rx_t* rx)
{
    int8_t val;
    plp_aperta2_phymod_phy_access_t phy_copy;
    phymod_phy_signalling_method_t signalling_mode;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_MEMSET(rx, 0, sizeof(*rx));
    /* next read the PAM4 mode or  not */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_signalling_mode_status_get(&phy_copy, &signalling_mode));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_read_rx_afe(&phy_copy, RX_AFE_VGA, &val));
    rx->vga.value = val;
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_read_rx_afe(&phy_copy, RX_AFE_PF, &val));
    rx->peaking_filter.value = val;
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_read_rx_afe(&phy_copy, RX_AFE_PF2, &val));
    rx->low_freq_peaking_filter.value = val;
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_read_rx_afe(&phy_copy, RX_AFE_PF3, &val));
    rx->high_freq_peaking_filter.value = val;

    rx->num_of_dfe_taps = 0;

    rx->vga.enable = 1;
    rx->peaking_filter.enable = 1;
    rx->low_freq_peaking_filter.enable = 1;
    rx->high_freq_peaking_filter.enable = 1;

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_speed_config_set(const plp_aperta2_phymod_phy_access_t* phy,
                                   const phymod_phy_speed_config_t* speed_config,
                                   const phymod_phy_pll_state_t* old_pll_state,
                                   phymod_phy_pll_state_t* new_pll_state)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    uint32_t  tvco_pll_div = 0, request_pll_div;
    uint32_t is_pam4, osr_mode;
    int i, start_lane, num_lane;
    phymod_firmware_lane_config_t firmware_lane_config;

    firmware_lane_config = speed_config->pmd_lane_config;

    /*first make sure that tvco pll index is valid */
    if (phy->access.tvco_pll_index > 0) {
        PHYMOD_DEBUG_ERROR(("Unsupported tvco index\n"));
        return PHYMOD_E_UNAVAIL;
    }

    /* first copy the PLL state */
    *new_pll_state = *old_pll_state;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    /* Get TVCO because it's not allowed to change during speed set */
    pm_phy_copy.access.pll_idx = 0;
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_INTERNAL_read_pll_div(&pm_phy_copy, &tvco_pll_div));

    /* next check the request speed VCO */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_speed_config_get(speed_config->data_rate, &request_pll_div, &is_pam4, &osr_mode));

    if (tvco_pll_div != request_pll_div) {
        /*this speed request can not be configured */
        PHYMOD_DEBUG_ERROR(("ERROR :: this speed can not be configured \n"));
        return PHYMOD_E_CONFIG;
    }

    /* first assert the ln dp reset */
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 1));

    /* config oversample for each lane */
    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_osr_mode_set(&pm_phy_copy, osr_mode));
    }

    /*next need to set certain firmware lane config to be zero*/
    firmware_lane_config.LaneConfigFromPCS = 0;
    firmware_lane_config.AnEnabled = 0;

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
             (_plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
    }

    /* next need to enable/disable link training based on the input */
    if (speed_config->linkTraining) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_phy_cl72_set(phy, speed_config->linkTraining));
    } else {
        /* disable cl72 and avoid overwriting the value from above _firmware_lane_config_set */
        for (i = 0; i < num_lane; i++) {
            pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
            PHYMOD_IF_ERR_RETURN
                 (plp_aperta2_peregrine5_pc_clause72_control(&pm_phy_copy, speed_config->linkTraining));
        }
    }

    /* next release the ln dp reset */
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 0));

    /* first clear the current lane _mask from both */
    new_pll_state->pll0_lanes_bitmap &= ~(phy->access.lane_mask);

    /* need to update the pll_state */
    new_pll_state->pll0_lanes_bitmap |= phy->access.lane_mask;

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_speed_config_get(const plp_aperta2_phymod_phy_access_t* phy, phymod_phy_speed_config_t* speed_config)
{
    int osr_mode = 0;
    uint32_t pll_div, vco_freq_khz, cl72_enable;
    plp_aperta2_phymod_phy_access_t phy_copy;
    phymod_firmware_lane_config_t firmware_lane_config;
    int start_lane, num_lane;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    phy_copy.access.lane_mask = 0x1 << start_lane;

    phy_copy.access.pll_idx = 0;
     /* get the PLL div from HW */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_INTERNAL_read_pll_div(&phy_copy, &pll_div));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_get_vco_from_refclk_div(&phy_copy, PEREGRINE5_PC_REF_CLOCK_312P5_HZ, pll_div, &vco_freq_khz, 0));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_osr_mode_get(&phy_copy, &osr_mode));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_phy_firmware_lane_config_get(&phy_copy, &firmware_lane_config));

    speed_config->pmd_lane_config.AnEnabled          = firmware_lane_config.AnEnabled;
    speed_config->pmd_lane_config.Cl72AutoPolEn      = firmware_lane_config.Cl72AutoPolEn;
    speed_config->pmd_lane_config.Cl72RestTO         = firmware_lane_config.Cl72RestTO;
    speed_config->pmd_lane_config.DfeOn              = firmware_lane_config.DfeOn;
    speed_config->pmd_lane_config.ForceBrDfe         = firmware_lane_config.ForceBrDfe;
    speed_config->pmd_lane_config.ForceES  = firmware_lane_config.ForceES;
    speed_config->pmd_lane_config.ForceNS   = firmware_lane_config.ForceNS;
    speed_config->pmd_lane_config.ForceNRZMode       = firmware_lane_config.ForceNRZMode;
    speed_config->pmd_lane_config.ForcePAM4Mode      = firmware_lane_config.ForcePAM4Mode;
    speed_config->pmd_lane_config.LaneConfigFromPCS  = firmware_lane_config.LaneConfigFromPCS;
    speed_config->pmd_lane_config.LpDfeOn            = firmware_lane_config.LpDfeOn;
    speed_config->pmd_lane_config.LinkPartnerPrecoderEn  = firmware_lane_config.LinkPartnerPrecoderEn;
    speed_config->pmd_lane_config.MediaType          = firmware_lane_config.MediaType;
    speed_config->pmd_lane_config.ScramblingDisable  = firmware_lane_config.ScramblingDisable;
    speed_config->pmd_lane_config.UnreliableLos      = firmware_lane_config.UnreliableLos;

    /* next get the cl72 enable status */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_clause72_control_get(&phy_copy, &cl72_enable));
    speed_config->linkTraining = cl72_enable;

    /* next check if PAM4 mode enabled */
    if (firmware_lane_config.ForcePAM4Mode) {
        /* to get the over sample value */
        if (osr_mode == 0) {
            speed_config->data_rate = (vco_freq_khz  << 1) / 1000;
        } else if(osr_mode == 1) {
            speed_config->data_rate = (vco_freq_khz) / 1000;
        }
    } else {
        /* to get the over sample value */
        if (osr_mode == 0) {
            speed_config->data_rate = (vco_freq_khz) / 1000;
        } else if(osr_mode == 1) {
            speed_config->data_rate = (vco_freq_khz  >> 1) / 1000;
        } else if (osr_mode == 2) {
            speed_config->data_rate = (vco_freq_khz  >> 2) / 1000;
        }
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_cl72_set(const plp_aperta2_phymod_phy_access_t* phy, uint32_t cl72_en)
{
    int start_lane, num_lane, i, precoder_en;
    uint32_t lane_reset;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    phymod_firmware_lane_config_t firmware_lane_config;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    /*first check if lane is in reset */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset_get(&pm_phy_copy, &lane_reset));

    /* if lane is not in reset, then reset the lane first */
    if (!lane_reset) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 1));
    }

    /* next need to clear both force ER and NR config on the firmware lane config side
    if link training enable is passed*/
    if (cl72_en) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_phy_firmware_lane_config_get(phy, &firmware_lane_config));

        firmware_lane_config.ForceNS = 0;
        firmware_lane_config.ForceES = 0;

         PHYMOD_IF_ERR_RETURN
            (_plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(phy, firmware_lane_config));
    } else {
        /* disable Tx pre-coding and set Rx in NR mode */
        for (i = 0; i < num_lane; i++) {
            pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
            precoder_en = 0;
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_phy_tx_pam4_precoder_enable_get(&pm_phy_copy, &precoder_en));
            if (precoder_en) {
                PHYMOD_IF_ERR_RETURN
                    (plp_aperta2_peregrine5_pc_phy_tx_pam4_precoder_enable_set(&pm_phy_copy, 0));
            }
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_phy_firmware_lane_config_get(&pm_phy_copy, &firmware_lane_config));

            if (firmware_lane_config.ForcePAM4Mode) {
                firmware_lane_config.ForceNS = 1;
                firmware_lane_config.ForceES = 0;
                PHYMOD_IF_ERR_RETURN
                    (_plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
            }
        }
    }

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_clause72_control(&pm_phy_copy, cl72_en));
    }

    /* release the ln dp reset */
    if (!lane_reset) {
        PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 0));
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_cl72_get(const plp_aperta2_phymod_phy_access_t* phy, uint32_t* cl72_en)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_clause72_control_get(&pm_phy_copy, cl72_en));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_cl72_status_get(const plp_aperta2_phymod_phy_access_t* phy, plp_aperta2_phymod_cl72_status_t* status)
{
    int i;
    uint32_t tmp_status;
    int start_lane, num_lane;
    plp_aperta2_phymod_phy_access_t phy_copy;


    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    status->locked = 1;

    /* next figure out the lane num and start_lane based on the input */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        tmp_status = 1;
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pmd_cl72_receiver_status(&phy_copy, &tmp_status));
        if (tmp_status == 0) {
            status->locked = 0;
            return PHYMOD_E_NONE;
        }
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_loopback_set(const plp_aperta2_phymod_phy_access_t* phy,
                                    plp_aperta2_phymod_loopback_mode_t loopback,
                                    uint32_t enable)
{
    int i;
    int start_lane, num_lane;
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    /* next figure out the lane num and start_lane based on the input */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    switch (loopback) {
    case phymodLoopbackGlobal :
    case phymodLoopbackGlobalPMD :
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_dig_lpbk(&phy_copy, (uint8_t) enable));
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pmd_force_signal_detect(&phy_copy,  (int) enable, (int) enable));
        }
        break;
    case phymodLoopbackRemotePMD :
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
                continue;
            }
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_rmt_lpbk(&phy_copy, (uint8_t)enable));
        }
        break;
    case phymodLoopbackRemotePCS :
    default :
         PHYMOD_RETURN_WITH_ERR(PHYMOD_E_UNAVAIL,
                               (_PHYMOD_MSG("This mode is not supported\n")));
        break;
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_loopback_get(const plp_aperta2_phymod_phy_access_t* phy,
                                    plp_aperta2_phymod_loopback_mode_t loopback,
                                    uint32_t* enable)
{
    int start_lane, num_lane;
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    /*next figure out the lane num and start_lane based on the input*/
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    switch (loopback) {
    case phymodLoopbackGlobal :
    case phymodLoopbackGlobalPMD :
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_dig_lpbk_get(&phy_copy, enable));
        break;
    case phymodLoopbackRemotePMD :
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_rmt_lpbk_get(&phy_copy, enable));
        break;
    case phymodLoopbackRemotePCS :
    default :
         return PHYMOD_E_UNAVAIL;
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_core_init(const plp_aperta2_phymod_core_access_t* core,
                             const plp_aperta2_phymod_core_init_config_t* init_config,
                             const plp_aperta2_phymod_core_status_t* core_status)
{
    int rv;
    int lane = 0;
    plp_aperta2_phymod_phy_access_t phy_access, phy_access_copy;
    plp_aperta2_phymod_core_access_t  core_copy;
    uint32_t uc_enable = 0;
    plp_aperta2_phymod_polarity_t tmp_pol;
    ucode_info_t ucode;
    unsigned int peregrine5_pc_ucode_len;
    unsigned char *plp_aperta2_peregrine5_pc_ucode;

    PEREGRINE5_PC_CORE_TO_PHY_ACCESS(&phy_access, core);
    PHYMOD_MEMSET(&tmp_pol, 0x0, sizeof(tmp_pol));
    phy_access_copy = phy_access;
    PHYMOD_MEMCPY(&core_copy, core, sizeof(core_copy));
    core_copy.access.lane_mask = 0x1;
    phy_access_copy = phy_access;
    phy_access_copy.access = core->access;
    phy_access_copy.access.lane_mask = 0x1;
    phy_access_copy.type = core->type;

    ucode.stack_size = PEREGRINE5_PC_UCODE_STACK_SIZE;
    ucode.ucode_size = PEREGRINE5_PC_UCODE_IMAGE_SIZE;
    ucode.ucode_dr_code_size_kb = 0;
    peregrine5_pc_ucode_len = PEREGRINE5_PC_UCODE_IMAGE_SIZE;
    plp_aperta2_peregrine5_pc_ucode = plp_aperta2_peregrine5_pc_ucode_get();
    ucode.crc_value = PEREGRINE5_PC_UCODE_IMAGE_CRC;


    /* 1. De-assert PMD core power and core data path reset */
    if(!PHYMOD_CORE_INIT_F_RESUME_AFTER_FW_LOAD_GET(init_config)) {
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_pmd_reset_seq((plp_aperta2_phymod_phy_access_t*)&core_copy, core_status->pmd_active));

        core_copy.access.pll_idx = 0;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_core_dp_reset((plp_aperta2_phymod_phy_access_t*)&core_copy, 1));

        /*wait until com clk and ref clk is stable */
        PHYMOD_USLEEP(1000);

        /* De-assert PMD lane reset */
        /* FIXME: check if still need reset the lane registers */
        for (lane = 0; lane < PEREGRINE5_PC_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_lane_hard_soft_reset_release(&phy_access_copy, 0));
        }

        for (lane = 0; lane < PEREGRINE5_PC_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_lane_hard_soft_reset_release(&phy_access_copy, 1));
        }

        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_uc_active_get(&phy_access, &uc_enable));
        if (uc_enable) return PHYMOD_E_NONE;

        /* 2. Set the heart beat COM Clock, default is 312.5M */
        if (init_config->interface.ref_clock == phymodRefClk312Mhz) {
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_comclk_set((plp_aperta2_phymod_phy_access_t*)&core_copy, init_config->interface.ref_clock));
        } else {
            PHYMOD_DEBUG_ERROR(("Unsupported reference clock.\n"));
            return PHYMOD_E_UNAVAIL;
        }
     
        /*now config the lane mapping*/
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_core_lane_map_set(&core_copy, &init_config->lane_map));

        PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_uc_reset_with_info((plp_aperta2_phymod_phy_access_t*)&core_copy , 1, ucode));
    }

    rv = _plp_aperta2_peregrine5_pc_core_firmware_load(&core_copy, init_config);

    if (rv != PHYMOD_E_NONE) {
        PHYMOD_DEBUG_ERROR(("devad 0x%"PRIx32" lane 0x%"PRIx32": UC firmware-load failed\n", core->access.addr, core->access.lane_mask));
        PHYMOD_IF_ERR_RETURN(rv);
    }

    if(PHYMOD_CORE_INIT_F_UNTIL_FW_LOAD_GET(init_config)) {
        return PHYMOD_E_NONE;
    }

    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        if(PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
            rv = plp_aperta2_peregrine5_pc_ucode_load_verify((plp_aperta2_phymod_phy_access_t*)&core_copy, (uint8_t *) plp_aperta2_peregrine5_pc_ucode, peregrine5_pc_ucode_len);
            if (rv != PHYMOD_E_NONE) {
                PHYMOD_DEBUG_ERROR(("devad 0x%x lane 0x%x: UC load-verify failed\n", core->access.addr, core->access.lane_mask));
                PHYMOD_IF_ERR_RETURN(rv);
            }
        }
    }

    /*next we need to check if the load is correct or not */
    if(init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        /*next we need to set the uc active and release uc */
        PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_uc_active_set((plp_aperta2_phymod_phy_access_t*)&core_copy ,1));

        /*release the uc reset */
        PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_uc_reset_with_info((plp_aperta2_phymod_phy_access_t*)&core_copy , 0, ucode));

        PHYMOD_IF_ERR_RETURN (plp_aperta2_peregrine5_pc_wait_uc_active((plp_aperta2_phymod_phy_access_t*)&core_copy));
        /* FIXME: check if this is needed here */
        for (lane = 0; lane < PEREGRINE5_PC_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_pmd_ln_h_rstb_pkill_override( &phy_access_copy, 0x1));
        }

        /* we need to wait at least 10ms for the uc to settle */
        /* FIXME: to be reviewed after silicom is back */
        PHYMOD_USLEEP(10000);

        /* 7. Initialize software information table for the micro */
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_init_peregrine5_pc_info((plp_aperta2_phymod_phy_access_t*)&core_copy));

        if(!PHYMOD_CORE_INIT_F_BYPASS_CRC_CHECK_GET(init_config)) {
            PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_ucode_crc_verify((plp_aperta2_phymod_phy_access_t*)&core_copy, ucode.ucode_size, ucode.crc_value));
        }

        for (lane = 0; lane < PEREGRINE5_PC_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
           PHYMOD_IF_ERR_RETURN
                (plp_aperta2_peregrine5_pc_pmd_ln_h_rstb_pkill_override( &phy_access_copy, 0x0));

        }
    } 

    /* program the rx/tx polarity */
    for (lane = 0; lane < PEREGRINE5_PC_NOF_LANES_IN_CORE; lane++) {
        phy_access_copy.access.lane_mask = 1 << lane;
        tmp_pol.tx_polarity = (init_config->polarity_map.tx_polarity) >> lane & 0x1;
        tmp_pol.rx_polarity = (init_config->polarity_map.rx_polarity) >> lane & 0x1;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_phy_polarity_set(&phy_access_copy, &tmp_pol));
        /* clear the tmp vairiable */
        PHYMOD_MEMSET(&tmp_pol, 0x0, sizeof(tmp_pol));

    }

    /* default PLL config for PLL0 */
    if (init_config->pll0_div_init_value != 0) {
        core_copy.access.pll_idx = 0;
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_configure_pll_refclk_div((plp_aperta2_phymod_phy_access_t*)&core_copy,
                                                    PEREGRINE5_PC_PLL_REFCLK_312P5MHZ,
                                                    init_config->pll0_div_init_value));
    }

    /* release core soft reset for PLL0 */
    core_copy.access.lane_mask = 0x1;
    core_copy.access.pll_idx = 0;
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_core_dp_reset((plp_aperta2_phymod_phy_access_t*)&core_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_init(const plp_aperta2_phymod_phy_access_t* phy, const plp_aperta2_phymod_phy_init_config_t* init_config)
{
    const plp_aperta2_phymod_access_t *pm_acc = &phy->access;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    int start_lane, num_lane, i;
    int lane_bkup;
    phymod_firmware_lane_config_t firmware_lane_config;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(pm_acc, &start_lane, &num_lane));
    /*per lane based  dp reset release */

    lane_bkup = pm_phy_copy.access.lane_mask;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 0));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&pm_phy_copy, 1));

    pm_phy_copy.access.lane_mask = lane_bkup;

    /* clearing all the lane config */
    PHYMOD_MEMSET(&firmware_lane_config, 0x0, sizeof(firmware_lane_config));

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
             (plp_aperta2_peregrine5_pc_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_pmd_locked_get(const plp_aperta2_phymod_phy_access_t* phy, uint32_t* pmd_lock)
{
    int start_lane, num_lane, i;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    uint8_t tmp_lock;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    *pmd_lock = 1;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_pmd_lock_status(&pm_phy_copy, &tmp_lock));
        *pmd_lock &= (uint32_t) tmp_lock;
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_pmd_lock_status_get(const plp_aperta2_phymod_phy_access_t* phy,
                                         uint32_t* pmd_lock,
                                         uint32_t* pmd_lock_change)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_rx_pmd_lock_status_get(&pm_phy_copy,
                                          pmd_lock,
                                          pmd_lock_change));
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_signal_detect_get(const plp_aperta2_phymod_phy_access_t* phy, uint32_t* signal_detect)
{
    int start_lane, num_lane, i;
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    uint32_t tmp_detect;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    *signal_detect = 1;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_signal_detect(&pm_phy_copy, &tmp_detect));
        *signal_detect &= tmp_detect;
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_reg_read(const plp_aperta2_phymod_phy_access_t* phy, uint32_t reg_addr, uint32_t* val)
{

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_reg_write(const plp_aperta2_phymod_phy_access_t* phy, uint32_t reg_addr, uint32_t val)
{
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_taps_default_get(const plp_aperta2_phymod_phy_access_t* phy, phymod_phy_signalling_method_t mode, phymod_pam4_tx_t* tx)
{
    /*always default to 6-taps mode */
    tx->serdes_tx_tap_mode = phymodTxTapModePAM4_6TAP;
    if (mode == phymodSignallingMethodNRZ) {
        tx->pre3 = 0;
        tx->pre2 = 0;
        tx->pre = -12;
        tx->main = 88;
        tx->post = -26;
        tx->post2 = 0;
        tx->post3 = 0;
    } else {
        tx->pre3 = -4;
        tx->pre2 = 14;
        tx->pre = -36;
        tx->main = 112;
        tx->post = 0;
        tx->post2 = 0;
        tx->post3 = 0;
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pll_multiplier_get(const plp_aperta2_phymod_phy_access_t* phy, uint32_t* core_vco_pll_multiplier)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_INTERNAL_read_pll_div(&pm_phy_copy, core_vco_pll_multiplier));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_firmware_load_info_get(const plp_aperta2_phymod_phy_access_t* phy,
                                              phymod_firmware_load_info_t* info)
{
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 0x1;

    info->ucode_ptr = plp_aperta2_peregrine5_pc_ucode_get();
    info->ucode_len = PEREGRINE5_PC_UCODE_IMAGE_SIZE;

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_adaptation_resume(const plp_aperta2_phymod_phy_access_t* phy)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    uint8_t uc_lane_stopped;
    int start_lane, num_lane, i;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_stop_uc_lane_status(&phy_copy, &uc_lane_stopped));
        if (uc_lane_stopped) {
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_stop_rx_adaptation(&phy_copy, 0));
        }
    }
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_pam4_precoder_enable_set(const plp_aperta2_phymod_phy_access_t* phy, int enable)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 1));

    for (i = 0; i < num_lane; i++) {
        if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            continue;
        }
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_tx_pam4_precoder_enable_set(&phy_copy, enable));
    }

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_lane_soft_reset(&phy_copy, 0));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pll_pwrdn(const plp_aperta2_phymod_phy_access_t* phy, uint32_t pll_index, uint32_t pwrdn)
{
    plp_aperta2_phymod_phy_access_t phy_copy;
    uint32_t is_pwrdn = 0;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 1 << 0;
    phy_copy.access.pll_idx = pll_index;

    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pll_pwrdn_get(&phy_copy, &is_pwrdn));
    if (is_pwrdn != pwrdn) {
        if (pwrdn) {
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_core_pwrdn(&phy_copy, PWRDN));
        } else {
            PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_core_pwrdn(&phy_copy, PWR_ON));
        }
    }

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_tx_pam4_precoder_enable_get(const plp_aperta2_phymod_phy_access_t* phy, int *enable)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_tx_pam4_precoder_enable_get(&pm_phy_copy, enable));
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_eye_margin_est_get(const plp_aperta2_phymod_phy_access_t* phy,
                                          phymod_eye_margin_mode_t eye_margin_mode,
                                          uint32_t* value)
{

#if DEADCODE
    int start_lane, num_lane;
    uint16_t hz_l, hz_r, vt_u, vt_d;
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 0x1 << start_lane;

    PHYMOD_IF_ERR_RETURN
        (peregrine5_pc_INTERNAL_get_eye_margin_est(&phy_copy, &hz_l, &hz_r, &vt_u, &vt_d));

    switch (eye_margin_mode) {
    case phymod_eye_marign_HZ_L:
        *value = hz_l;
        break;
    case phymod_eye_marign_HZ_R:
        *value = hz_r;
        break;
    case phymod_eye_marign_VT_U:
        *value = vt_u;
        break;
    case phymod_eye_marign_VT_D:
        *value = vt_d;
        break;
    default:
        *value = 0;
        break;
    }
#endif
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_channel_loss_hint_set(const plp_aperta2_phymod_phy_access_t* phy,  uint32_t channel_loss)
{
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_channel_loss_hint_get(const plp_aperta2_phymod_phy_access_t* phy,  uint32_t *channel_loss)
{
    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pmd_info_init(const plp_aperta2_phymod_phy_access_t* phy)
{
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 0x1;

    /* call PMD api to init the global PMD data structure */
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_init_peregrine5_pc_info(&phy_copy));

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_rx_ppm_get(const plp_aperta2_phymod_phy_access_t* phy, int16_t* rx_ppm)
{

    return PHYMOD_E_NONE;
}

int plp_aperta2_peregrine5_pc_phy_pll_powerdown_get(const plp_aperta2_phymod_phy_access_t* phy,
                                         uint32_t pll_index, uint32_t* powerdown)
{
    int octal_shift =0;
    plp_aperta2_phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    if (APERTA2_GET_OCTAL(phy->access.lane_mask) == APERTA2_OCTAL1) {
        octal_shift = 1;
    }

    phy_copy.access.pll_idx = 0;
    phy_copy.access.lane_mask = 1 << (octal_shift *8);
    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_pll_pwrdn_get(&phy_copy, powerdown));

    return PHYMOD_E_NONE;
}


int plp_aperta2_peregrine5_pc_phy_pll_reconfig(const plp_aperta2_phymod_phy_access_t* phy,
                               uint8_t pll_index,
                               uint32_t pll_div, 
                               plp_aperta2_phymod_ref_clk_t ref_clk)
{
    plp_aperta2_phymod_phy_access_t pm_phy_copy;
    uint32_t  pll_is_pwrdn;
    enum peregrine5_pc_pll_refclk_enum refclk;
    uint32_t cnt = 0, pll_lock = 0;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    if (pll_index > 1) {
        PHYMOD_DEBUG_ERROR(("Unsupported PLL index\n"));
        return PHYMOD_E_UNAVAIL;
    }

    pm_phy_copy.access.lane_mask = 1 << 0;

    if (ref_clk == phymodRefClk312Mhz) {
        refclk = PEREGRINE5_PC_PLL_REFCLK_312P5MHZ;
    } else {
        PHYMOD_DEBUG_ERROR(("Unsupported reference clock.\n"));
        return PHYMOD_E_UNAVAIL;
    }

    /*next check if the PLL is power down or not */
    pm_phy_copy.access.pll_idx = pll_index;
    pm_phy_copy.access.lane_mask = 1 << 0;
    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pll_pwrdn_get(&pm_phy_copy, &pll_is_pwrdn));

    /* if PLL is power down, need to power up first */
    if (pll_is_pwrdn) {
        /* Power up vco if it's power down */
        PHYMOD_IF_ERR_RETURN
            (plp_aperta2_peregrine5_pc_core_pwrdn(&pm_phy_copy, PWR_ON));
    }

    PHYMOD_IF_ERR_RETURN
        (plp_aperta2_peregrine5_pc_core_dp_reset(&pm_phy_copy, 1));

    /*next re-config pll divider */
    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_configure_pll_refclk_div(&pm_phy_copy,
                                                                refclk,
                                                                pll_div));

    PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_core_dp_reset(&pm_phy_copy, 0));

    /* need to wait for the PLL lock */
    cnt = 0;
    while (cnt < 500) {
        PHYMOD_IF_ERR_RETURN(plp_aperta2_peregrine5_pc_pll_lock_get(&pm_phy_copy, &pll_lock));
        cnt = cnt + 1;
        if(pll_lock) {
            break;
        } else {
            if(cnt == 500) {
                PHYMOD_DEBUG_ERROR(("WARNING :: core 0x%x PLL Index %d is not locked within 5 milli second \n", pm_phy_copy.access.addr, pll_index));
                break;
            }
        }
        PHYMOD_USLEEP(10);
    }

    return PHYMOD_E_NONE;
}

