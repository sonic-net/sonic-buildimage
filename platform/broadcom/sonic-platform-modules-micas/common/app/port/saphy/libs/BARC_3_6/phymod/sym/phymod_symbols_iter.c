...
static int
_sym_match(plp_barchetta_phymod_symbols_iter_t *iter, const char *sym_name)
{
    char search_name[64]; 
    char symbol_name[64]; 
...
int
plp_barchetta_phymod_symbols_iter(plp_barchetta_phymod_symbols_iter_t *iter)
{       
    int count = 0; 
    int rc; 
    int match;
    uint32_t idx; 
    plp_barchetta_phymod_symbol_t s; 

    for (idx = 0; plp_barchetta_phymod_symbols_get(iter->symbols, idx, &s) >= 0; idx++) {
