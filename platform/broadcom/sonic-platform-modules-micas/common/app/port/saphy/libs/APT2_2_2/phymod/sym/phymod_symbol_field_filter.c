...
/*
 * Function:
 *	plp_aperta2_phymod_symbol_field_filter
 * Purpose:
 ...
 */
int 
plp_aperta2_phymod_symbol_field_filter(const plp_aperta2_phymod_symbol_t *symbol, const char **fnames,
    ...
    *ptr++ = 0;
    ...
    PHYMOD_SYMBOL_FIELDS_ITER_BEGIN(symbol->fields, finfo, fnames) {
        ...
        if (finfo.name && PHYMOD_STRCMP(finfo.name, keyfield) == 0) {
            ...
            plp_aperta2_phymod_field_get(data, finfo.minbit, finfo.maxbit, val);
            kval = val[0];
            break;
        }
