 ...
 *   7. Configure Recovered Clock (SyncE)                                           *
 *   8. Configure MACsec parameters for transform operation                         *
...
int main(int argc, char *argv[])
{
   ...
   total_lane_maps = NUM_ARR_ELEMENTS(sys_lane_map_list);
    ...
    /* -------------------------------------------------------------------------------- *
     * Configure Recovered Clock (SyncE) settings on 1st PHY                            *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID0; /* SyncE can only be configured on one PHY for a particular clk output pin */
    line_lane_map = 0xFF;
    phy_info.lane_map = line_lane_map;
    phy_info.if_side = LINE_SIDE;
    divider = bcmplpDivider2048;
    rclk_out_pin_sel = 0;
    recoveredClkLane = bcmplpRecoveredClkLane0;
     ...
     * -------------------------------------------------------------------------------- */
#ifdef TRAFFIC_DEBUG
     retval = macsec_traffic_verify("00:00:00:08:01:01", 0xFF, "drop");
     if (retval) {
     ...
     * -------------------------------------------------------------------------------- */
#ifdef TRAFFIC_DEBUG
     retval = macsec_traffic_verify("00:00:00:08:01:01", 0xFF, "flow");
     if (retval) {
         printf("Print_statistics() observed failure. Exiting... \n");
         goto _aperta_config_error;
         return retval;
...
_aperta_macsec_init_error:
#ifdef MACSEC_UNINIT_EN
    /*
     *  Disable SyncE configuration
     */
    phy_info.phy_addr = PHY_ID0; /* SyncE can only be configured on one PHY for a particular clk output pin */
    line_lane_map = 0xFF;
    phy_info.lane_map = line_lane_map;
    phy_info.if_side = LINE_SIDE;
    squelchMonitorLanemap = 0;
    recoveredClkLane = 0;
    divider = 0;
    rclk_side = 0;
    ...
    retval = synce_test(phy_info, clkGenSquelchCfg, squelchMonitorLanemap, recoveredClkLane, divider, rclk_side, rclk_out_pin_sel);
    if (retval != 0) {
        printf("Disabling SyncE failed with status:0x%x\n", retval);
    }
