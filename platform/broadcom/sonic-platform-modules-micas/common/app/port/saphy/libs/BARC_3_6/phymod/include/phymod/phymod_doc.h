...
\code
phymod_core_func(plp_barchetta_phymod_core_access_t, ...);
phymod_phy_func(plp_barchetta_phymod_phy_access_t, ...);
\endcode

The phymod_core APIs receive plp_barchetta_phymod_core_access_t structure. This set of APIs
performs operations that affect the entire core. The phymod_phy APIs receive
plp_barchetta_phymod_phy_access_t structure. This set of APIs performs operations that affect
single lane, or a given group of lanes within a core.

Both phy and core structure contains the following two members:
\li plp_barchetta_phymod_dispatch_type_t - the driver type, this parameter is used for the
 dispatcher (can be retrieved using the probe function that will be presented
 later in this document).
\li plp_barchetta_phymod_access_t Access information structure.
...
\li user_acc - data that provided for each plp_barchetta_phymod_access_t, and is used in every
...
In order to initialize the structure call the function plp_barchetta_phymod_bus_t_init(bus_structure);
...
\li user_acc - data that provided for each plp_barchetta_phymod_access_t, and can be used in
...
\code
plp_barchetta_phymod_core_access_t core; 
plp_barchetta_phymod_core_access_t_init(&core.access);
\endcode
For Phy (i.e. for each port)
\code
plp_barchetta_phymod_phy_access_t phy;
plp_barchetta_phymod_phy_access_t_init(&phy.access);
\endcode
...
For more information of how to get the core type read about plp_barchetta_phymod_core_probe.

\subsubsection plp_barchetta_phymod_core_probe plp_barchetta_phymod_core_probe
This function is used to get the core type.
\code
int plp_barchetta_phymod_core_probe(const plp_barchetta_phymod_access_t* access, plp_barchetta_phymod_dispatch_type_t* type);
\endcode
Parameters:
