...
/*
 * Legacy MAX, MIN, and ABS Macros for SERDES code.
...
/*
 * These macros are prefered over the legacy macros above, particularly for microcode.
 * Note that these ALWAYS return an 'int32_t' (aka S32) type since they promote the parameters to int32_t values.
 * Because of this, assigning their result to variables other then 'int32_t's requires a cast.
 *
 * The primary advantage of the new macros is smaller code size for those use cases where the
 * parameters require evaluation (e.g pointer dereferences, equations, function calls).
 * This also eliminates the the risk of evaluating a macro parameter more then once with unexpected effects.
 */
#define SRDS_MAX_S32(_A_, _B_)  ( {                                                         \
    int32_t SRDS_MAX_S32_reg_a = (_A_);                                                     \
    int32_t SRDS_MAX_S32_reg_b = (_B_);                                                     \
    ((SRDS_MAX_S32_reg_a > SRDS_MAX_S32_reg_b) ? SRDS_MAX_S32_reg_a : SRDS_MAX_S32_reg_b);  \
} )

#define SRDS_MIN_S32(_A_, _B_)  ( {                                                         \
    int32_t SRDS_MIN_S32_reg_a = (_A_);                                                     \
    int32_t SRDS_MIN_S32_reg_b = (_B_);                                                     \
    ((SRDS_MIN_S32_reg_a < SRDS_MIN_S32_reg_b) ? SRDS_MIN_S32_reg_a : SRDS_MIN_S32_reg_b);  \
} )

#define SRDS_ABS_S32(_X_)       ( {                                                         \
    int32_t SRDS_ABS_S32_reg_x = (_X_);                                                     \
    ((SRDS_ABS_S32_reg_x >= 0) ? SRDS_ABS_S32_reg_x : (-SRDS_ABS_S32_reg_x));               \
} )

#define CMD_RELEASE_DBSTOP_TIMEOUT 100
...
enum plp_aperta2_srds_pmd_uc_cmd_enum {
    SRDS_CMD_NULL                  =  0,
    SRDS_CMD_UC_CTRL               =  1,
    ...
    SRDS_CMD_MEASURE_SNR           = 38,
    SRDS_CMD_PRE_LSN_SNAPSHOT      = 39,
    SRDS_CMD_MEASURE_MSE_PER_LVL   = 40,
    SRDS_CMD_CONFIG_LINKCAT        = 50,
    SRDS_CMD_TX_ADAPTATION_STOP    = 51,
    SRDS_CMD_TX_ADAPTATION_RESUME  = 52,
...
enum plp_aperta2_srds_pmd_uc_ctrl_cmd_enum {
...
enum plp_aperta2_srds_pmd_uc_diag_cmd_enum {
...
#define EVENT_SOURCE_VERID   9

/** SERDES_EVENT_LOG_READ Enum */
enum plp_aperta2_srds_pmd_event_rd_cmd_enum {
...
enum plp_aperta2_srds_pmd_uc_dbg_cmd_enum {
...
/** Media type Enum */
enum plp_aperta2_srds_media_type_enum {
...
enum plp_aperta2_srds_diag_ber_mode_enum {
    SRDS_DIAG_BER_POS  = 0,
    SRDS_DIAG_BER_NEG  = 1,
...
#define INFO_TABLE_OFFS_LANE_STATIC_MEM_PTR     (0x74)   /* Not used in BHK7 and MER7 uCode. */
#define INFO_TABLE_OFFS_LANE_STATIC_MEM_SIZE    (0x78)   /* Not used in BHK7 and MER7 uCode. */

#define INFO_TABLE_OFFS_COMMON_BLOCK_MEM_PTR    (0x7C)   /* Only used in DSP IPs uCode. */
#define INFO_TABLE_OFFS_COMMON_BLOCK_MEM_SIZE   (0x80)   /* Only used in DSP IPs uCode. */

#define INFO_TABLE_OFFS_DEBUG_BLOCK_MEM_PTR     (0x84)   /* Only used in DSP IPs uCode. */
#define INFO_TABLE_OFFS_DEBUG_BLOCK_MEM_SIZE    (0x88)   /* Only used in DSP IPs uCode. */
