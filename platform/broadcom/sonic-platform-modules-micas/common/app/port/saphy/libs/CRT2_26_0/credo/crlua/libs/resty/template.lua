    ...
    load_view = function(template)
        return function(view, plain)
            ...
            if (not root or root == EMPTY) and vars then
                ...
                if not root or root == EMPTY then
                    root = var.document_root or prefix
                end
            end
            if root and root ~= EMPTY then
                if byte(root, -1) == SOL then
...
local function load_file(func)
    return function(view)
        return func(view, false)
    end
end

local function load_string(func)
    return function(view)
        return func(view, true)
    end
end
...
local function new(template, safe)
    ...
    template.cache = {}
    -- credo change to load using module system
    template.load = function(view, plain)
        if plain == true then
            return view
        end
        view = "templates." .. gsub(view, "%.html", "")
        return require(view)
    end
    -- /end credo change
    ...
    function template.new(view, layout)
        ...
        return setmetatable({render = render, process = process}, {__tostring = process})
    end
...
local function echo(...) for i=1,select("#", ...) do ___[#___+1] = tostring(select(i, ...)) end end
        ...
        c[j] =
            "return layout and include(layout,setmetatable({view=table.concat(___),blocks=blocks},{__index=context})) or table.concat(___)" -- luacheck: ignore
        return concat(c)
    end
