 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Barchetta chip for 25G NRZ (1x25G)        *
 * mode as follow:                                                                  *
 ...
 *   5. Configure Recovered Clock Generation                                        *
...
/* 25G NRZ (1x25G) Retimer */
#define OP_MODE_STR             "25G NRZ (1x25G) Retimer"
...
int main(void)
{
    ...
    unsigned int recoveredClkLane = 0, divider = 0, rclk_side = 0;
    ...
    /* -------------------------------------------------------------------------------- *
     * Enable Recovered Clock Generation                                                *
     * -------------------------------------------------------------------------------- */
    ...
    /* Barchetta supports following SyncE options:
     *  - clkGenSquelchCfg
     *     bcmplpClkGenSquelchDisable
     *     bcmplpClkGenEnSquelchNone
     *  - divider
     *     bcmplpDivider1
     *     bcmplpDivider2
     *     bcmplpDivider4
     *     bcmplpDivider8
     *  - recoveredClkLane
     *    bcmplpRecoveredClkLane0 to bcmplpRecoveredClkLane7
     *  - rclk_if_side is supported only on BCM81381 and not supported on BCM81338
     *
     * Barchetta does not support following SyncE options:
     * - squelchMonitorLanemap
     * - rclk_out_pin_sel
     */
    clkGenSquelchCfg = bcmplpClkGenEnSquelchNone;
    squelchMonitorLanemap = phy_info.lane_map;
    recoveredClkLane = bcmplpRecoveredClkLane7;
    divider = bcmplpDivider4;
    rclk_side = phy_info.if_side;
    retval = synce_test(CHIP_NAME, phy_info, clkGenSquelchCfg, squelchMonitorLanemap, recoveredClkLane, divider, rclk_side);
    if (retval != 0)
    {
        printf("Configure SyncE failed with status:0x%x\n", retval);
        goto _barchetta_reference_app_error;
    }
    sleep(5);
    ...
    /*Other parameters are not needed if clkGenSquelchCfg is set to disable*/
    retval = synce_test(CHIP_NAME, phy_info, clkGenSquelchCfg, squelchMonitorLanemap, recoveredClkLane, divider, rclk_side);
    if (retval != 0)
    {
       printf("Disable SyncE failed with status:0x%x\n", retval);
       goto _barchetta_reference_app_error;
    }
