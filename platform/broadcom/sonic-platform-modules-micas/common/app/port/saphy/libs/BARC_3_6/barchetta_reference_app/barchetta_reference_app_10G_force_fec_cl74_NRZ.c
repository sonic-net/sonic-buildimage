 ...
 *   7. Enable Froce FEC - CL74 on line side of barchetta                           *
 *   8. Check for link status and Rx PMD lock.                                      *
 *   9. Disable Tx/Rx PRBS at both System and Line side.                            *
 *  10. Cleanup and close the connection to the board.                              *
 * -------------------------------------------------------------------------------- */

/* 10G NRZ (1x10G) Retimer */
#define OP_MODE_STR             "10G NRZ (1x10G) Retimer - Froced FEC-CL74"
...
int main(void)
{
    ...
    unsigned int fec_enable, fec_get;
	...
	/* We have to enable PCS traffic before enabling force FEC */
	printf("\nPlease send PCS traffic....\n");
	
    /* -------------------------------------------------------------------------------- *
     * Enable foce FEC CL74                                                             *
     * -------------------------------------------------------------------------------- */
    fec_enable = 0x10001; /* enable cl74 froce fec */
    for (phy_index = 0; phy_index <  NUM_OF_PHY; phy_index++) {
        phy_info.phy_addr = (phy_index == 0) ? PHY_ID0 : PHY_ID1;
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            line_lane_map = line_lane_map_list[lane_map_index];
            ...
            retval = bcm_plp_fec_enable_set(CHIP_NAME,phy_info, fec_enable);
            if (retval != TEST_SUCCESS) {
                printf("Failed to enable force fec cl74 for lane-map 0x%x of PHY-%d at Line side (ret = %d)!\n", phy_info.lane_map, phy_info.phy_addr, retval);
                goto _barchetta_reference_app_error;
            }
            ...
            if (retval != TEST_SUCCESS) {
                printf("Failed to get force fec cl74 status for lane-map 0x%x of PHY-%d at Line side (ret = %d)!\n", phy_info.lane_map, phy_info.phy_addr, retval);
                goto _barchetta_reference_app_error;
            }
            printf("Force FEC CL74 is %s for lane-map 0x%x of PHY-%d at Line side \n", (fec_get == 1 )? "enabled" : "disabled", phy_info.lane_map, phy_info.phy_addr);
        }
    }
