...
#define _PHYMOD_PHY_MSG(phy, string) ("Phy 0x%x: %s: lanes 0x%02x: " string), (phy)->access.addr,((phy)->port_loc == phymodPortLocLine) ? "LINE":"SYS", (phy)->access.lane_mask
#define _PHYMOD_CORE_MSG(core, string) ("Core 0x%x: " string), (core)->access.addr

#define PHYMOD_DIAG_PRINT_FUNC_VALIDATE if(plp_aperta2_phymod_diag_print_func == NULL) return PHYMOD_E_IO

print_func_f plp_aperta2_phymod_diag_print_func = NULL;


int plp_aperta2_phymod_diag_symbols_table_get(plp_aperta2_phymod_phy_access_t *phy, plp_aperta2_phymod_symbols_t **symbols)
{
    switch(phy->type){
#ifdef PHYMOD_QSGMIIE_SUPPORT
...
int plp_aperta2_phymod_diag_firmware_load(plp_aperta2_phymod_core_access_t *cores, int array_size, char *firwmware_file){
    int i = 0;
...
/******************************************************************************
 Register read/write
******************************************************************************/
int plp_aperta2_phymod_diag_reg_read(plp_aperta2_phymod_phy_access_t *phys, int array_size, uint32_t reg_addr){
    uint32_t val = 0;
    int rv;
    int i;
    ...
    for(i = 0 ; i < array_size ; i++){
       rv = plp_aperta2_phymod_phy_reg_read(&phys[i], reg_addr, &val);
       if(rv == PHYMOD_E_NONE){
...
int plp_aperta2_phymod_diag_reg_write(plp_aperta2_phymod_phy_access_t *phys, int array_size, uint32_t reg_addr, uint32_t val){
    int i;
    int rv;
    ...
    for(i = 0 ; i < array_size ; i++){
       rv = plp_aperta2_phymod_phy_reg_write(&phys[i], reg_addr, val);
       if(rv != PHYMOD_E_NONE){
...
******************************************************************************/
STATIC
void plp_aperta2_phymod_diag_prbs_get_results_print(plp_aperta2_phymod_phy_access_t *phy, plp_aperta2_phymod_prbs_status_t *status) {
    if (status->prbs_lock == 0) {
        if (status->error_count) {
            PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG
                                       (phy, "prbs unlocked with %d errors during PRBS test\n"),
                                        status->error_count));   
        } else {
            PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG(phy, "prbs unlocked\n")));
        } 
    } else if (status->prbs_lock_loss == 1) {
        PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG(phy, "prbs unlocked during the PRBS test\n")));
    } else {
        if (status->error_count) {
            PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG
            ...
            PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG(phy, "prbs locked\n")));
            PHYMOD_DIAG_OUT((_PHYMOD_PHY_MSG(phy, "PRBS OK!\n")));
        }
    }
}
...
STATIC
int plp_aperta2_phymod_diag_prbs_set(plp_aperta2_phymod_phy_access_t *phys, int array_size, plp_aperta2_phymod_diag_prbs_set_args_t *params){
    int i = 0;
    int rv;
    
    plp_aperta2_phymod_prbs_t_validate(&params->prbs_options);
    
       
    if(params->loopback == 1){
        for(i = 0 ; i < array_size ; i++){
            rv = plp_aperta2_phymod_phy_loopback_set(&phys[i], phymodLoopbackGlobal, 1);
            if(rv != PHYMOD_E_NONE){
    ...
    for(i = 0 ; i < array_size ; i++){
        if(plp_aperta2_phymod_phy_prbs_config_set(&phys[i], params->flags, &params->prbs_options) != PHYMOD_E_NONE){
            PHYMOD_DEBUG_ERROR((_PHYMOD_PHY_MSG(&phys[i],"Failed set PRBS\n")));
            return PHYMOD_E_FAIL;
        }
        if(plp_aperta2_phymod_phy_prbs_enable_set(&phys[i], params->flags,  params->enable) != PHYMOD_E_NONE){
...
STATIC
int plp_aperta2_phymod_diag_prbs_get(plp_aperta2_phymod_phy_access_t *phys, int array_size, plp_aperta2_phymod_diag_prbs_get_args_t *params){
    int i = 0;
    int rv;
    ...
    for(i = 0 ; i < array_size ; i++){
        ...
        rv = plp_aperta2_phymod_phy_prbs_enable_get(&phys[i], flags, &enable);
        if(rv != PHYMOD_E_NONE){
        ...
        /*clear the status*/
        rv = plp_aperta2_phymod_phy_prbs_status_get(&phys[i], PHYMOD_PRBS_STATUS_F_CLEAR_ON_READ, &status);
        if(rv != PHYMOD_E_NONE){
            PHYMOD_DEBUG_WARN_ERROR((_PHYMOD_PHY_MSG(&phys[i],"PRBS clear status failed\n")));
            return rv;
        }
    }
    /*wait*/
    ...
    for(i = 0 ; i < array_size ; i++){
        ...
        plp_aperta2_phymod_diag_prbs_get_results_print(&phys[i], &status);
    }
    return PHYMOD_E_NONE;
}
...
STATIC
int plp_aperta2_phymod_diag_prbs_clear(plp_aperta2_phymod_phy_access_t *phys, int array_size, plp_aperta2_phymod_diag_prbs_clear_args_t *params){
    int i = 0;
    int rv;
    
    for(i = 0 ; i < array_size ; i++){
        rv = plp_aperta2_phymod_phy_prbs_enable_set(&phys[i], params->flags, 0);
        if(rv  != PHYMOD_E_NONE){
            PHYMOD_DEBUG_WARN_ERROR((_PHYMOD_PHY_MSG(&phys[i],"Failed clear prbs\n")));
            return rv;
        }
    }
    return PHYMOD_E_NONE;
...
int plp_aperta2_phymod_diag_prbs(plp_aperta2_phymod_phy_access_t *phys, int array_size, plp_aperta2_phymod_diag_prbs_args_t *prbs_diag_args){
    ...
    case PhymodDiagPrbsClear:
        rv = plp_aperta2_phymod_diag_prbs_clear(phys, array_size, &prbs_diag_args->args.clear_params);
        break;
    case PhymodDiagPrbsGet:
        rv = plp_aperta2_phymod_diag_prbs_get(phys, array_size, &prbs_diag_args->args.get_params);
        break;
    case PhymodDiagPrbsSet:
        rv = plp_aperta2_phymod_diag_prbs_set(phys, array_size, &prbs_diag_args->args.set_params);
        break;
    default:
...
DSC
******************************************************************************/
int plp_aperta2_phymod_diag_dsc(plp_aperta2_phymod_phy_access_t *phys, int array_size){
    ...
    for(i = 0 ; i < array_size ; i++){
        ...
        for( lane_index = 0 ; lane_index < PHYMOD_MAX_LANES_PER_CORE; lane_index++){
            ...
            /*collect info*/
            rv = plp_aperta2_phymod_phy_pmd_info_dump(&(phys[i]), NULL); 
...
DSC
******************************************************************************/
int plp_aperta2_phymod_diag_dsc_std(plp_aperta2_phymod_phy_access_t *phys, int array_size){
    ...
    for(i = 0 ; i < array_size ; i++){
        ...
        if (phys[i].access.lane_mask) {
            rv = plp_aperta2_phymod_phy_pmd_info_dump(&(phys[i]), "STD"); 
...
DSC
******************************************************************************/
int plp_aperta2_phymod_diag_dsc_config(plp_aperta2_phymod_phy_access_t *phys, int array_size){
    ...
    for(i = 0 ; i < array_size ; i++){
        ...
        for( lane_index = 0 ; lane_index < PHYMOD_MAX_LANES_PER_CORE; lane_index++){
            ...
            phys[i].access.lane_mask = lane & (1<<lane_index);
            /*collect info*/
            rv = plp_aperta2_phymod_phy_pmd_info_dump(&(phys[i]), "config"); 
...
Eyescan
******************************************************************************/
int plp_aperta2_phymod_diag_eyescan_run( 
    plp_aperta2_phymod_phy_access_t *phys,
    int unit,
    int* port_ids,
    int* line_rates, 
    int num_phys,
...
{
    const plp_aperta2_phymod_phy_access_t    *pa;
    ...
    for (idx = 0; idx < num_phys; idx++) {
        ...
        if (pa->access.lane_mask){
            rc = plp_aperta2_phymod_phy_eyescan_run(pa, flags, mode, NULL);
        }
        if (rc != PHYMOD_E_NONE) {
            PHYMOD_DIAG_OUT(("Failed in plp_aperta2_phymod_phy_eyescan_run (enable) \n"));
            goto exit;
        }
    }
    ...
    for (idx = 0; idx < num_phys; idx++) {
        ...
        rc = plp_aperta2_phymod_phy_eyescan_run(pa, flags, mode, eyescan_options);
        if (rc != PHYMOD_E_NONE) {
            PHYMOD_DIAG_OUT(("Failed in plp_aperta2_phymod_phy_eyescan_run (process) \n"));
        }
    }
...
exit:    
    ...
    for (idx = 0; idx < num_phys; idx++) {
        ...
        rc_done = plp_aperta2_phymod_phy_eyescan_run(pa, flags, mode, eyescan_options);
        if (rc_done != PHYMOD_E_NONE) {
            PHYMOD_DIAG_OUT(("Failed in plp_aperta2_phymod_phy_eyescan_run (done) \n"));
