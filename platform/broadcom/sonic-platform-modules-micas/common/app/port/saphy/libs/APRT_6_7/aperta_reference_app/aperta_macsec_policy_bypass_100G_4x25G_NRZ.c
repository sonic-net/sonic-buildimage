 ...
 *   6. Setup link training, if needed                          *
 ...
 *          BCM_PLP_CONTROL_PACKET_POLICY_BYPASS in Makefile and enable             *
 *          BCM_PLP_DATA_PACKET_POLICY_BYPASS                                       *
 * -------------------------------------------------------------------------------- */

/* 100G NRZ (4x25G) MACsec policy bypass mode */
#define OP_MODE_STR    "100G NRZ (4x25G) MACsec policy bypass"

/* Enable below for Tx forced training */
...
int main(int argc, char *argv[])
{
    ...
    for (phy_index = 0; phy_index < NUM_OF_PHY; phy_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            retval = macsec_initialize(sec_info, 0); /* disable bypass */
            if (retval != TEST_SUCCESS) {
   ...
   total_lane_maps = NUM_ARR_ELEMENTS(sys_lane_map_list);
     ...
     * -------------------------------------------------------------------------------- */
...
#ifdef TRAFFIC_DEBUG
     retval = macsec_traffic_verify("00:00:00:09:01:01", 0xF, "flow");
     if (retval)
     {
     ...
     retval = macsec_traffic_verify("00:00:00:09:01:01", 0xF0, "flow");
     if (retval)
     {
         printf("Print_statistics() observed failure. Exiting... \n");
         goto _aperta_config_error;
