...
#ifndef BLACKHAWK_BARCHETTA_API_UC_COMMON_H
#define BLACKHAWK_BARCHETTA_API_UC_COMMON_H

/* Add Blackhawk specific items below this */

/* Please note that when adding entries here you should update the #defines in the blackhawk_barchetta_common.h */

/** OSR_MODES Enum */
enum blackhawk_barchetta_osr_mode_enum {
  /* If the enumerations change, then consider updating OSR_MODE_SUPPORTS_EYE_TESTS(). */
  BLACKHAWK_BARCHETTA_OSX1      = 0,
  BLACKHAWK_BARCHETTA_OSX2      = 1,
  BLACKHAWK_BARCHETTA_OSX4      = 2,
  BLACKHAWK_BARCHETTA_OSX21P25  = 4,
  BLACKHAWK_BARCHETTA_OSX8      = 5,
  BLACKHAWK_BARCHETTA_OSX16P5   = 8,
  BLACKHAWK_BARCHETTA_OSX16     = 9,
  BLACKHAWK_BARCHETTA_OSX20P625 = 12,
  BLACKHAWK_BARCHETTA_OSX32     = 13
};

/* UNIQUIFY_PUBLIC_END  - Marker used by API Uniquify script */

/** Return true if an OSR mode supports eye tests. **/
#define OSR_MODE_SUPPORTS_EYE_TESTS(mode) ((mode) < BLACKHAWK_BARCHETTA_OSX16P5)

/** CDR mode Enum **/
enum blackhawk_barchetta_cdr_mode_enum {
  BLACKHAWK_BARCHETTA_CDR_MODE_OS_ALL_EDGES             = 0,
  BLACKHAWK_BARCHETTA_CDR_MODE_OS_PATTERN               = 1,
  BLACKHAWK_BARCHETTA_CDR_MODE_OS_PATTERN_ENHANCED      = 2,
  BLACKHAWK_BARCHETTA_CDR_MODE_BR_PATTERN               = 3,
  BLACKHAWK_BARCHETTA_CDR_MODE_PAM4_NR_OS_PATTERN       = 4,
  BLACKHAWK_BARCHETTA_CDR_MODE_PAM4_NR_BR_PATTERN       = 5,
  BLACKHAWK_BARCHETTA_CDR_MODE_PAM4_ER_DFE_DATA_PATTERN = 6,
  BLACKHAWK_BARCHETTA_CDR_MODE_PAM4_ER_PATTERN          = 7
};

/** Lane User Control Clause93/72 Force Value **/
enum blackhawk_barchetta_cl93n72_frc_val_enum {
  BLACKHAWK_BARCHETTA_CL93N72_FORCE_OS  = 0,
  BLACKHAWK_BARCHETTA_CL93N72_FORCE_BR  = 1
};

/** FLR Timeout Time Enum
 *  Please change flr_timeout_time_ary in process_pmd_lane.c to match any changes here.
 */
enum blackhawk_barchetta_flr_timeout_enum {
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_50MS  = 0,
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_75MS  = 1,
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_100MS = 2,
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_150MS = 3,
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_250MS = 4,
  BLACKHAWK_BARCHETTA_FLR_TIMEOUT_500MS = 5
};
