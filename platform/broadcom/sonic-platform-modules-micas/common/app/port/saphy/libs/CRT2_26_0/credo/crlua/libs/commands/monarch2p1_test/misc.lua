...
local function get_clk_phase(slice, lane)
    local mnreg = regmap.slice(slice)
    return {
        gray_bin(mnreg.pam100.vdacclkphase1_tx:read(slice, lane)),
        gray_bin(mnreg.pam100.vdacclkphase2_tx:read(slice, lane)),
        gray_bin(mnreg.pam100.vdacclkphase0:read(slice, lane)),
        gray_bin(mnreg.pam100.vdacclkphase1_rx:read(slice, lane)),
        gray_bin(mnreg.pam100.vdacclkphase2_rx:read(slice, lane)),
        gray_bin(mnreg.pam100.vdacclkphase3:read(slice, lane)),
    }
end
...
local function set_rx_phase(slice, lane, idx, value)
    ...
    if idx == 0 then
        mnreg.pam100.vdacclkphase0:write(slice, value, lane)
    elseif idx == 1 then
        mnreg.pam100.vdacclkphase1_rx:write(slice, value, lane)
    elseif idx == 2 then
        mnreg.pam100.vdacclkphase2_rx:write(slice, value, lane)
    elseif idx == 3 then
        mnreg.pam100.vdacclkphase3:write(slice, value, lane)
    end
    time.sleep(0.00002)
end
...
slash.register_chip_command({"scan_rx_clk_phase"}, credo.FAMILY_MONARCH2P1_TEST, [[
Get top pll status.
Arugments:
...
local function get_half_rate(slice, lane)
    local mnreg = regmap.slice(slice)
    if mnreg.pam100.reg_pam4_en:read(slice, lane) == 1 then
        return mnreg.pam100.reg_half_rate:read(slice, lane) == 1
    else
        return mnreg.nrz50.reg_halfrate_rx:read(slice, lane) == 1
    end
end
...
local function set_supercal(slice, lane, en)
    local mnreg = regmap.slice(slice)
    if en then
        mnreg.pam100.ow_mu_offset_q:write(slice, 3, lane)
        mnreg.pam100.owen_mu_offset_q:write(slice, 1, lane)
        mnreg.pam100.tosm_mu_offset_s6:write(slice, 1, lane)
        mnreg.pam100.tosm_mu_polarity:write(slice, 1, lane)
        ...
        mnreg.pam100.cntr_sel0:write(slice, (val0 >> 13) & 0x7, lane)
        mnreg.pam100.cntr_sel1:write(slice, (val0 >> 10) & 0x7, lane)
        mnreg.pam100.cntr_sel2:write(slice, (val0 >> 7) & 0x7, lane)
        mnreg.pam100.cntr_sel3:write(slice, (val0 >> 4) & 0x7, lane)
        mnreg.pam100.cntr_sel4:write(slice, (val0 >> 1) & 0x7, lane)
        mnreg.pam100.cntr_sel5:write(slice, (val1 >> 13) & 0x7, lane)
        mnreg.pam100.cntr_sel6:write(slice, (val1 >> 10) & 0x7, lane)
        mnreg.pam100.cntr_sel7:write(slice, (val1 >> 7) & 0x7, lane)
    else
        mnreg.pam100.ow_mu_offset_q:write(slice, 0, lane)
        mnreg.pam100.owen_mu_offset_q:write(slice, 1, lane)
        mnreg.pam100.tosm_mu_offset_S6:write(slice, 0, lane)
    end
end
...
local function bp1(slice, lane, en, state)
    ...
    if mnreg.pam100.reg_pam4_en:read(slice, lane) == 1 then
        if state then
            mnreg.pam100.breakpoint_1_state:write(slice, state, lane)
        end
        if en then
            mnreg.pam100.breakpoint_1_en:write(slice, en, lane)
        end
    else
        if state then
            mnreg.nrz50.breakpoint_1_state:write(slice, state, lane)
        end
        if en then
            mnreg.nrz50.breakpoint_1_en:write(slice, en, lane)
        end
    end
end
...
local function sm_cont(slice, lane)
    local mnreg = regmap.slice(slice)
    mnreg.pam100.statemachine_continue:write(slice, 0, lane)
    time.sleep(0.001)
    mnreg.pam100.statemachine_continue:write(slice, 1, lane)
end
...
local function acal(slice, lane, dac_settle_time, timeout)
    ...
    local mnreg = regmap.slice(slice)
    mnreg.pam100.rg_adc_wait_sel_s:write(slice, dac_settle_time, lane)
    bp1(slice, lane, 1, 2)
    mnreg.pam100.sm_reset:write(slice, 1, lane)
    sm_cont(slice, lane)
    mnreg.pam100.sm_reset:write(slice, 0, lane)
    local start = time.time()
    while (time.time() - start) < timeout do
        if (gDebug and mnreg.pam100.adc_cal_done:read(slice, lane) == 1) or
           (not gDebug and mnreg.pam100.breakpoint_1_reached:read(slice, lane) == 1) then
            break
        end
    end
    local adc_cal_done = mnreg.pam100.adc_cal_done:read(slice, lane)
    if gDebug then
        timer_print("ADC_CAL_DONE %d time %.4fs" % {adc_cal_done, time.time() - start})
    else
        timer_print("bp1 state2 reach %d, time %.4fs"
            % {mnreg.pam100.breakpoint_1_reached:read(slice, lane), time.time() - start})
    end
    return adc_cal_done
end
...
local function read_sum(slice, lane, row)
    ...
    for dir=0, 2^dir_bits-1 do
        for phase=0, 2^phase_bits-1 do
            mnreg.pam100.adr_comp:write(slice, (dir << 5) + (row << 2) + phase, lane)
            local calp_caln = mnreg.pam100.calp_caln:read(slice, lane)
...
local function tune_acal(slice, lane)
    ...
    mnreg.pam100.pu_adccal_vref2com:write(slice, 0, lane)
    debug_print("\n"..lane_str)
    for row=1, #params do
        local param = params[row]
        debug_print(param, crutil.twos_to_int(mnreg.pam100["offset_"..param]:read(slice, lane), 7))
    end
    for row=1, #params do
        local param = params[row]
        debug_print("\n    row",row,"offset_%s"%{param})
        debug_print("        Stg1 acal")
        mnreg.pam100["offset_"..param]:write(slice, crutil.int_to_twos(0, 7), lane)
        mnreg.pam100.pu_stg2cal:write(slice, 0, lane)
        acal(slice, lane)
        local target = read_sum(slice, lane, row)
        debug_print("        Target=", target)
        debug_print("\n        Stg2 acal")
        mnreg.pam100.pu_stg2cal:write(slice, 1, lane)
        acal(slice, lane)
        ...
        for i=1, iter_count do
            ...
            k = k + step
            mnreg.pam100["offset_"..param]:write(slice, crutil.int_to_twos(k, 7), lane)
            time.sleep(0.00001)
            acal(slice, lane)
...
slash.register_chip_command({"tune_acal"}, credo.FAMILY_MONARCH2P1_TEST, [[
