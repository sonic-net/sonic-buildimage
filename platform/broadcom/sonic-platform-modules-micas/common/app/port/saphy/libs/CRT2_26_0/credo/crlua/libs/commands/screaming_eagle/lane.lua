...
slash.register_chip_command({"lane", "statelogs"}, credo.FAMILY_SCREAMING_EAGLE, [[
Display lane(s) state machine history logs.
    ...
    for lane in iter(lanes) do
        ...
        ftable:print_ln('Lane %s(%d) State|||||||Time|Cumulative|Timedelta|Timestamp' %
                            {crutil.get_lane_id(slice, lane), lane})
        ...
        for i, id in ipairs(state_id) do
            ...
            local state_timestamp_wall = start_time + state_time_stamp[i] / 1000
            ftable:print_ln("%d|%s|%X|%s|%X|%s|%2X|%d|%s|%s|%s" % {
                ...
                timedelta.pretty(state_time_stamp[i] / 1000, true), timedelta.timestamp_iso8601(state_timestamp_wall)
            })
        end
        ...
        local cur_time = credo.time_system(slice)
        ftable:add_separator()
        ftable:print_ln("now|%s|%X|%s|%X|%s|%2X|-|-|%s|%s" % {
            state_mode_str, state_mode, state_major_str, state_major, state_minor_str, state_minor,
            timedelta.pretty(cur_time, true), timedelta.timestamp_iso8601(cur_time + start_time)
        })
...
slash.register_chip_command({"lane", "datalogs"}, credo.FAMILY_SCREAMING_EAGLE, [[
Display lane(s) state machine history logs.
...
slash.register_chip_command({"lane", "exitlogs"}, credo.FAMILY_SCREAMING_EAGLE, [[
...
Arguments:
    ...
    local ftable = fort.create()
    local start_time = credo.time_start(slice)
    ftable:print_ln("Lane|Exit|Mode|Major|Minor|Raw|Error|Raw|Timedelta|Timestamp")
    ...
    for lane in iter(lanes) do
        ...
        local exit_code_timestamps_wall = list.map(exit_code_timestamps, function(v)
            return start_time + v
        end)
        ...
        for idx, exit_code in ipairs(exit_codes) do
            ...
            local timestamp_wall = exit_code_timestamps_wall[idx] or 0
            ...
            ftable:print("%s|%s|%s|0x%04X|%s|%d" %
                             {state_mode_str, state_major_str, state_minor_str, exit_code, error_str, error_code})
            if timestamp ~= 0 then
                ftable:print("%s|%s" % {timedelta.pretty(timestamp, true), timedelta.timestamp_iso8601(timestamp_wall)})
            else
                ftable:print("|")
            end
            ftable:ln()
