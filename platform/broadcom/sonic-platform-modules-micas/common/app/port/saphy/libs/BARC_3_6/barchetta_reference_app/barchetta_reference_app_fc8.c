 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Barchetta chip for 8.5G NRZ (1x25G)       *
 * mode as follow:                                                                  *
 ...
 *   5. Configure PRBS generator (Tx) and checker (Rx).                             *
 *   6. Check for link status, PRBS lock and Rx PMD lock.                           *
 *   7. Reads chip register, dump PHY status and display eye scan.                  *
 *   8. Disable Tx/Rx PRBS at both System and Line side.                            *
 *   9. Cleanup and close the connection to the board.                              *
 * -------------------------------------------------------------------------------- */

/* 25G NRZ (1x25G) Retimer */
#define OP_MODE_STR             "FC8 : 8.5G NRZ (1x8.5G) Retimer"

#define FC8_REF_CLOCK           bcm_pm_RefClk106P25Mhz
/* System side parameters */
#define SYS_PORT_SPEED          8500
#define SYS_LANE_RATE           bcmpLplaneDataRate_8P5G
#define SYS_MODULATION          bcmplpModulationNRZ
#define SYS_FEC_MODE            bcmplpNoFEC
#define SYS_IF_TYPE             bcm_pm_InterfaceSFI
...
#define LINE_PORT_SPEED         8500
#define LINE_LANE_RATE          bcmpLplaneDataRate_8P5G
#define LINE_MODULATION         bcmplpModulationNRZ
#define LINE_FEC_MODE           bcmplpNoFEC
#define LINE_IF_TYPE            bcm_pm_InterfaceSFI
...
int main(void)
{
    ...
    for (phy_index = 0; phy_index <  NUM_OF_PHY; phy_index++) {
        ...
        for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
            ...
            retval = bcm_plp_mode_config_set(CHIP_NAME, phy_info, SYS_PORT_SPEED, SYS_IF_TYPE, FC8_REF_CLOCK, SYS_IF_MODE, (void *)&aux_mode_sys);
            if (retval != TEST_SUCCESS) {
            ...
            retval = bcm_plp_mode_config_set(CHIP_NAME, phy_info, LINE_PORT_SPEED, LINE_IF_TYPE, FC8_REF_CLOCK, LINE_IF_MODE, (void *)&aux_mode_line);
            if (retval != TEST_SUCCESS) {
