...
int plp_barchetta_phymod_core_probe(const plp_barchetta_phymod_access_t* access, plp_barchetta_phymod_dispatch_type_t* type, int* is_probed)
{
    ...
    plp_barchetta_phymod_dispatch_type_t idx;
    ...
    *is_probed = 0;
    ...
    if(PHYMOD_E_OK != plp_barchetta_phymod_access_t_validate(access)) {
    ...
    for(idx = 0 ; idx < phymodDispatchTypeCount ; idx++) {
        if(NULL != __plp_barchetta_phymod__dispatch__[idx]->f_phymod_core_identify) {
            rv = __plp_barchetta_phymod__dispatch__[idx]->f_phymod_core_identify(&core, core_id, &is_identified);
...
{
    if(PHYMOD_E_OK != plp_barchetta_phymod_osr_mode_t_validate(osr_mode)) {
