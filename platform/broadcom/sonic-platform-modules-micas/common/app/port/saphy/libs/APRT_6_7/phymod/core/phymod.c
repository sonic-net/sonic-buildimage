...
int plp_aperta_phymod_core_probe(const plp_aperta_phymod_access_t* access, plp_aperta_phymod_dispatch_type_t* type, int* is_probed)
{
    uint32_t is_identified;
    plp_aperta_phymod_dispatch_type_t idx;
    ...
    *is_probed = 0;
    ...
    if(PHYMOD_E_OK != plp_aperta_phymod_access_t_validate(access)) {
    ...
    for(idx = 0 ; idx < phymodDispatchTypeCount ; idx++) {
        if(NULL != __plp_aperta_phymod__dispatch__[idx]->f_phymod_core_identify) {
            rv = __plp_aperta_phymod__dispatch__[idx]->f_phymod_core_identify(&core, core_id, &is_identified);
...
int plp_aperta_phymod_osr_mode_to_actual_os(plp_aperta_phymod_osr_mode_t osr_mode, uint32_t* os_int, uint32_t* os_remainder)
{
    if(PHYMOD_E_OK != plp_aperta_phymod_osr_mode_t_validate(osr_mode)) {
