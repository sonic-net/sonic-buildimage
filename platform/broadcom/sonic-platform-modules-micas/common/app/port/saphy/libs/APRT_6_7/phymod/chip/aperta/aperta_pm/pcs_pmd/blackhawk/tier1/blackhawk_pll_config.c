...
static const uint32_t plp_aperta_pll_fracn_ndiv_int_bits = 10;
static const uint32_t plp_aperta_pll_fracn_frac_bits     = 18;
...
err_code_t plp_aperta_blackhawk_tsc_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    EFUN(plp_aperta_blackhawk_tsc_INTERNAL_resolve_pll_parameters(sa__, refclk, &refclk_freq_hz, &srds_div, &vco_freq_khz, pll_option));
    ...
    if (!int_configured) {
         ...
         */
        const uint32_t div_fraction_0p5 = 1 << (div_fraction_width - plp_aperta_pll_fracn_frac_bits - 1);
        ...
        const uint32_t pll_fracn_div = ((div_fraction_plus_0p5 & ((1U << div_fraction_width)-1))
                                        >> (div_fraction_width - plp_aperta_pll_fracn_frac_bits));

        if (pll_fracn_ndiv_int != (pll_fracn_ndiv_int & ((1 << plp_aperta_pll_fracn_ndiv_int_bits)-1))) {
        ...
        if (pll_fracn_div != 0) {
            ...
            if ((refclk_freq_hz > 300000000) && (plp_aperta_blackhawk_tsc_get_pll_idx(sa__) == 0)) {
                EFUN(wrc_ams_pll_fp3_rh(0x0));
                EFUN(wrc_ams_pll_fp3_ctrl(0x0));
                EFUN(wrc_ams_pll_iqp(0x4));
            } else if ((refclk_freq_hz > 300000000) && (plp_aperta_blackhawk_tsc_get_pll_idx(sa__) == 1)) {
                EFUN(wrc_ams_pll_fp3_rh(0x0));
...

    {
        ...
        ESTM(pll_idx = plp_aperta_blackhawk_tsc_get_pll_idx(sa__));
        EFUN(plp_aperta_blackhawk_tsc_set_pll_idx(sa__, ((pll_idx == 0) ? 1 : 0)));
...
err_code_t plp_aperta_blackhawk_tsc_INTERNAL_read_pll_div(srds_access_t *sa__, uint32_t *srds_div) {
    ...
    if (fracn_sel) {
        ...
        
        {
             ...
             * the integer part.
             */
            const uint32_t pll_fracn_div_masked = pll_fracn_div & ((1U << plp_aperta_pll_fracn_frac_bits)-1);
        ...
        *srds_div = (uint32_t)(SRDS_INTERNAL_COMPOSE_PLL_DIV(pll_fracn_ndiv_int, pll_fracn_div, plp_aperta_pll_fracn_frac_bits));
    } else {
        uint8_t pll_mode;
        ...
        switch (pll_mode) {
            ...
            default:
                EFUN_PRINTF(("ERROR: plp_aperta_blackhawk_tsc_INTERNAL_read_pll_div() found invalid pll_mode value:  %d\n", pll_mode));
                return (blackhawk_tsc_error(sa__, ERR_CODE_PLL_DIV_INVALID));
        }
    }
    return (ERR_CODE_NONE);
