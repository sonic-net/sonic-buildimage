 ...
 */
err_code_t plp_aperta_blackhawk_tsc_pmd_rdt_reg(srds_access_t *sa__, uint16_t address, uint16_t *val);
 ...
 */
err_code_t plp_aperta_blackhawk_tsc_pmd_wr_reg(srds_access_t *sa__, uint16_t address, uint16_t val);
 ...
 *
 *    plp_aperta_blackhawk_tsc_pmd_wr_reg(addr, ((plp_aperta_blackhawk_tsc_pmd_rd_reg(addr) & ~mask) | (mask & (val << lsb))));
 *
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t plp_aperta_blackhawk_tsc_delay_ns(srds_access_t *sa__, uint16_t delay_ns);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t plp_aperta_blackhawk_tsc_delay_us(srds_access_t *sa__, uint32_t delay_us);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t plp_aperta_blackhawk_tsc_delay_ms(srds_access_t *sa__, uint32_t delay_ms);
#else
...
#define blackhawk_tsc_get_physical_lane plp_aperta_blackhawk_tsc_get_lane
...
#define blackhawk_tsc_set_physical_lane plp_aperta_blackhawk_tsc_set_lane
