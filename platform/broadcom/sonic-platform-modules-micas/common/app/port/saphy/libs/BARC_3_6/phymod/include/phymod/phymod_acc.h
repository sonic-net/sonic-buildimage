...
#ifndef __plp_barchetta_PHYMOD_ACC_H__
#define __plp_barchetta_PHYMOD_ACC_H__

#include <phymod/phymod_system.h>
#include <phymod/phymod.h>

/*
 * Occasionally the plp_barchetta_phymod_bus_t changes to support new features. This define
...
/* Ensures that plp_barchetta_phymod_access_t structure is properly initialized */
#define PHYMOD_ACC_CHECK(pa_) \
    do { \
        if (plp_barchetta_phymod_acc_check(pa_) < 0) return PHYMOD_E_INTERNAL; \
    } while (0)

/* Access to PHY bus with instance adjustment (addr_offset) */
#define PHYMOD_BUS_READ(pa_,r_,v_) plp_barchetta_phymod_bus_read(pa_,r_,v_)
#define PHYMOD_BUS_WRITE(pa_,r_,v_) plp_barchetta_phymod_bus_write(pa_,r_,v_)
#define PHYMOD_IS_WRITE_DISABLED(pa_,v_) plp_barchetta_phymod_is_write_disabled(pa_,v_)
...
#endif /* __plp_barchetta_PHYMOD_ACC_H__ */
