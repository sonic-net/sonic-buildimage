...
bcm_if_phymod_ctrl_t plp_aperta_phy_ctrl;
bcm_if_phymod_phy_id_t _plp_aperta_phyid_list[BCM_PM_IF_MAX_PHY];

int _bcm_plp_aperta_if_phymod_phy_create(_bcm_if_phymod_phy_t **phy)
{
...
int _bcm_plp_aperta_if_phymod_core_create(_bcm_if_phymod_core_t **core)
{
...
void _bcm_plp_aperta_pm_if_core_init(_bcm_if_phymod_core_t *core,
                   plp_aperta_phymod_bus_t *core_bus, uint32_t phy_addr, void *user_acc)
{
    plp_aperta_phymod_core_access_t *pm_core;
    plp_aperta_phymod_access_t *pm_acc;
    ...
    plp_aperta_phymod_core_access_t_init(pm_core);
    pm_acc = &pm_core->access;
    plp_aperta_phymod_access_t_init(pm_acc);
    ...
    PHYMOD_ACC_MISC(pm_acc) = &(_plp_aperta_phyid_list[phy_addr]);
...
{
    /* invalid the _plp_aperta_phyid_list entry before initialization */
    _plp_aperta_phyid_list[BCM_PHY_ADDR_LOGICAL(phy_id_combo)].valid &= ~BCM_PHY_ID_VALID;
}

void _bcm_plp_aperta_pm_if_get_phy_id_idx(uint32_t phy_id_combo, uint32_t *phy_id_idx, uint32_t *exist_phy)
{
    unsigned int  found  = FALSE;
    uint32_t      phy_id = BCM_PHY_ADDR_LOGICAL(phy_id_combo);
    *phy_id_idx = phy_id;

    if ( (BCM_PHY_ID_VALID & _plp_aperta_phyid_list[phy_id].valid) && (_plp_aperta_phyid_list[phy_id].phy_id == phy_id) ) {
       found      = TRUE;
       *exist_phy = TRUE;
       return;
    }
    ...
    if ( ! found ) {
        if ( ! (BCM_PHY_ID_VALID & _plp_aperta_phyid_list[phy_id].valid) ) {
            if ( BCM_PHY_IS_PHYSICAL_ADDR_GIVEN_BY_APP(phy_id_combo) ) {
                _plp_aperta_phyid_list[phy_id].valid        |=  BCM_PHY_PHYSICAL_ADDR_GIVEN_BY_APP;
                _plp_aperta_phyid_list[phy_id].physical_addr =  BCM_PHY_ADDR_PHYSICAL(phy_id_combo);
            } else {
                _plp_aperta_phyid_list[phy_id].valid        &= ~BCM_PHY_PHYSICAL_ADDR_GIVEN_BY_APP;
                _plp_aperta_phyid_list[phy_id].physical_addr =  phy_id;
            }

            if ( BCM_PHY_IS_MASTER_PORT(phy_id_combo) ) {
                _plp_aperta_phyid_list[phy_id].valid        |=  BCM_PHY_MASTER_PORT;
            } else {
                _plp_aperta_phyid_list[phy_id].valid        &= ~BCM_PHY_MASTER_PORT;
            }

            _plp_aperta_phyid_list[phy_id].phy_id            = phy_id;            /* logical PHY address */
            _plp_aperta_phyid_list[phy_id].valid            |= BCM_PHY_ID_VALID;  /* validate this entry */
            *exist_phy = FALSE;
        }
    }
}
...
{
    return (BCM_PHY_MASTER_PORT & _plp_aperta_phyid_list[BCM_PHY_ADDR_LOGICAL(phy_id)].valid) ? TRUE : FALSE;
}

#else  /* ! XGBASET_SUPPORT  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  */

void _bcm_plp_aperta_pm_if_get_phy_id_idx(uint32_t phy_id, uint32_t *phy_id_idx, uint32_t *exist_phy) 
{
    unsigned int found = 0;
    *phy_id_idx = phy_id;

    if (_plp_aperta_phyid_list[phy_id].valid && (_plp_aperta_phyid_list[phy_id].phy_id == phy_id)) {
           found = 1;
           *exist_phy = 1;
    }
    if (!found) { /* New PHYID*/
        if (!_plp_aperta_phyid_list[phy_id].valid) {
            _plp_aperta_phyid_list[phy_id].phy_id = phy_id;
            _plp_aperta_phyid_list[phy_id].valid = 1;
                 *exist_phy = 0;
        }
    }
}
...
int _bcm_plp_aperta_pm_phy_init(_bcm_if_phymod_phy_t *phy, bcm_pm_firmware_load_method_t firmware_load_method, void *init_param)
{
...
#if defined(PHYMOD_APERTA_SUPPORT) || defined (PHYMOD_AGERA_SUPPORT) || defined(PHYMOD_AGERALITE_SUPPORT) || defined( PHYMOD_APERTA2_SUPPORT) || defined (PHYMOD_AGERA2_SUPPORT)
...
#if !defined(PHYMOD_MILLENIO_SUPPORT) && !defined(PHYMOD_AGERA_SUPPORT) && !defined(PHYMOD_AGERALITE_SUPPORT)&& !defined(PHYMOD_APERTA2_SUPPORT) && !defined (PHYMOD_AGERA2_SUPPORT)
    ...
    if (core->init == FALSE  && PHYMOD_CORE_INIT_F_RESET_CORE_FOR_FW_LOAD_GET(&core->init_config)) {
        rv = plp_aperta_phymod_core_init(&core->pm_core, &core->init_config, &core_sts);
        ...
        /* bcm_plp_aperta_init needs to be multithreaded, so for PHY's other than
    ...
    /* read serdes id info */
    rv = plp_aperta_phymod_core_info_get(&core->pm_core, &core_info); 
    if (rv != 0) {
        return rv;
    }
...
int _bcm_plp_aperta_phy_init_bcast(_bcm_if_phymod_phy_t *phy, bcm_plp_firmware_load_type_t *firmware_load_type)
{
    _bcm_if_phymod_core_t *core;
    int rv = 0;
    ...
    phy->init_config.interface.data_rate = 10000;
    phy->init_config.interface.ref_clock = phymodRefClk156Mhz;
#if defined( PHYMOD_APERTA_SUPPORT)|| defined(PHYMOD_EVORA_SUPPORT) || defined (PHYMOD_AGERA_SUPPORT) || defined(PHYMOD_AGERALITE_SUPPORT) || defined( PHYMOD_APERTA2_SUPPORT) || defined (PHYMOD_AGERA2_SUPPORT)
    ...
    phy->init_config.op_mode = phymodOperationModeRepeater;
	core->init_config.op_datapath = phymodDatapathNormal;
#if !defined(PHYMOD_MILLENIO_SUPPORT) && !defined(PHYMOD_BARCHETTA2_SUPPORT) && !defined(PHYMOD_AGERA_SUPPORT) && !defined(PHYMOD_AGERALITE_SUPPORT) && !defined(PHYMOD_APERTA2_SUPPORT) && !defined (PHYMOD_AGERA2_SUPPORT)
    if (phy->static_config) {
        core->init_config.op_datapath = ((plp_static_config_t*)phy->static_config)->ull_dp;
    }
#endif
...
int _bcm_plp_aperta_phy_user_phymod_ability(unsigned int tech_ability, unsigned int fec_ability, unsigned short pause_ability, bcm_plp_an_config_t an_config, plp_aperta_phymod_autoneg_ability_t* ability)
{
    ability->an_cap = 0;
    ...
    if (an_config.tech_ability_ext & bcmplpAnCapMSA_800G_CR8_KR8) {
        PHYMOD_AN_CAP1_MSA_800G_CR8_KR8_SET(ability->an_cap_ext);
    }
    if (an_config.tech_ability_ext & bcmplpAnCapBAM_800G_CR8_KR8) {
        PHYMOD_AN_CAP1_BAM_800G_CR8_KR8_SET(ability->an_cap_ext);
    }
    ...
    if (fec_ability & bcmplpFecRS544_2XN) {
        PHYMOD_AN_FEC_RS544_2XN_SET(ability->an_fec);
    }
    if (fec_ability & bcmplpFecRS272_2XN) {
        PHYMOD_AN_FEC_RS272_2XN_SET(ability->an_fec);
    }
    if (fec_ability & bcmplpFecRS272Requested) {
        PHYMOD_AN_FEC_RS272_REQUESTED_SET(ability->an_fec);
    }
...
int _bcm_plp_aperta_phy_phymod_user_ability(plp_aperta_phymod_autoneg_ability_t ability, unsigned short *fec_ability, unsigned short *pause_ability, bcm_plp_an_config_t* an_config)
{
    ...
    *fec_ability = 0;
    ...
    if (PHYMOD_AN_CAP1_MSA_800G_CR8_KR8_GET(ability.an_cap_ext)) {
        an_config->tech_ability_ext |= bcmplpAnCapMSA_800G_CR8_KR8;
    }
    if (PHYMOD_AN_CAP1_BAM_800G_CR8_KR8_GET(ability.an_cap_ext)) {
        an_config->tech_ability_ext |= bcmplpAnCapBAM_800G_CR8_KR8;
    }
    ...
    if (PHYMOD_AN_FEC_RS544_2XN_GET(ability.an_fec)) {
        *fec_ability |= bcmplpFecRS544_2XN;
    }
    if (PHYMOD_AN_FEC_RS272_2XN_GET(ability.an_fec)) {
        *fec_ability |= bcmplpFecRS272_2XN;
    }
    if (PHYMOD_AN_FEC_RS272_REQUESTED_GET(ability.an_fec)) {
        *fec_ability |= bcmplpFecRS272Requested;
    }
...
int bcm_plp_aperta_phy_get_phy_type (bcm_plp_access_t phy_info, plp_aperta_phymod_dispatch_type_t *phy_type,
                         int (*read)(void* user_acc, unsigned int core_addr, unsigned int reg_addr, unsigned int* val),int (*write)(void* user_acc, unsigned int core_addr, unsigned int reg_addr, unsigned int val))
{
    ...
    *phy_type = 0xFFFF;
    ...
    BCM_PM_IF_ERR_RETURN(
            read(phy_info.platform_ctxt, phy_info.phy_addr, 0x18b00, &id0));
    PHYMOD_DEBUG_INFO(("Aperta2 CHIPID:%x for PHY:%x\n", id0, phy_info.phy_addr));
    BCM_PM_IF_ERR_RETURN(
            read(phy_info.platform_ctxt, phy_info.phy_addr, 0x18b01, &id1));
    PHYMOD_DEBUG_INFO(("Aperta2 chip ID and REV:0x%x\n", id1));
    if (id0 == 0x5343 || id0 == 0x9611 || id0 == 0x5344) {
        *phy_type = phymodDispatchTypeAperta2;
    }
#endif
    ...
    /* Reset slice register to avoid chip id register read in case of a crash */
    BCM_PM_IF_ERR_RETURN(write(phy_info.platform_ctxt, phy_info.phy_addr, 0x18000, 0x00));
    BCM_PM_IF_ERR_RETURN(read(phy_info.platform_ctxt, phy_info.phy_addr, 0x18b00, &id0));
    BCM_PM_IF_ERR_RETURN(read(phy_info.platform_ctxt, phy_info.phy_addr, 0x18b01, &id1));
    if (id0 == 0x5361 || id0 == 0x5362 || id0 == 0x5748 || id0 == 0x5851 || id0 == 0x5854) {
         PHYMOD_DEBUG_INFO(("AGERA2_SUPPORT\n"));
        *phy_type = phymodDispatchTypeAgera2;
    }
#endif
...
/* ID Mapping for KAUAI
 *   Part       ID1     PKG_ID0
 * BCM54908   0x53C0    0x0000
 * BCM54908E  0x53C0    0x0001
 * BCM54904   0x53C0    0x0002
 *
 */
#ifdef PHYMOD_KAUAI_SUPPORT  /* 5490x */
    if ( (id0 == 0x3590) && ((id1 == 0x53C0) || (id1 == 0x53C1)) ) {
        PHYMOD_DEBUG_INFO(("KAUAI CHIPID id0:0x%x id1:0x%x for PHY:%x\n",
                                         id0,     id1,    phy_info.phy_addr));
        *phy_type = phymodDispatchTypeXgbaset;
    }
#endif

/* ID Mapping for LANAI
 *   Part     ID0       ID1
 * BCM54901   0xF7A6    0x1C10
 * BCM50901   0xF7A6    0x1C14
 * BCM54110   0xF7A6    0x1C18
 *
 */
#ifdef PHYMOD_LANAI_SUPPORT  /* 54901 */
    if ( (id0 == 0xF7A6) && ((id1 == 0x1C10) || (id1 == 0x1C14) || (id1 == 0x1C18)) ) {
        PHYMOD_DEBUG_INFO(("LANAI CHIPID id0:0x%x id1:0x%x for PHY:%x\n",
                                         id0,     id1,    phy_info.phy_addr));
        *phy_type = phymodDispatchTypeXgbaset;
    }
...
uint8_t _bcm_plp_aperta_pm_if_log2n(uint32_t n)
{
    return (n > 1 ? 1 + _bcm_plp_aperta_pm_if_log2n(n >> 1) : 0);
}

uint8_t _bcm_plp_aperta_pm_if_get_right_most_set_bit_pos(uint32_t n)
{
    return (_bcm_plp_aperta_pm_if_log2n(n & -n));
}

/*! \brief This is an internal function to get the port information.
 *         Calling function should acquire Mutex.
 *
 *  This API is used to get port information 
 *
 *  @param phy_info  <pre> Represents PHY access\n
 *  @param port_information [OUT] Port information </pre>
 *
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */

int _bcm_plp_aperta_pm_if_get_port_information(bcm_plp_access_t phy_info, bcm_if_port_information_t *bcm_port_information )
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    phymod_phy_port_information_t port_information;

    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
   
    if (!bcm_port_information) {
        rv = BCM_PM_IF_MEMORY;
        goto ERR;
    }
#if defined (OCTAL_CROSSING_SUPPORT)
    rv = plp_aperta_phymod_get_port_information(&plp_aperta_phy_ctrl.phy[phy_id_idx]->pm_phy, &port_information);
    PHYMOD_MEMCPY(bcm_port_information, &port_information, sizeof(phymod_phy_port_information_t));
#else
    PHYMOD_MEMSET(bcm_port_information, 0, sizeof(bcm_if_port_information_t));
    (void)port_information;
#endif
