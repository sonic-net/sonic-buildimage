...
#ifndef PEREGRINE5_PC_API_DIAG_H
#define PEREGRINE5_PC_API_DIAG_H
...
#define DB_STOPPED         (0x08)
#define DIAG_POLL_TIMEOUT  (400)

/** Eyescan Options Struct */
struct peregrine5_pc_eyescan_options_st {
...
/** Column definition header for plp_aperta2_peregrine5_pc_display_core_state() API output.
 * To be called before plp_aperta2_peregrine5_pc_display_core_state_line() API.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * Call plp_aperta2_peregrine5_pc_display_core_state_hdr() API before and plp_aperta2_peregrine5_pc_display_core_state_legend() after
...
/** Detailed description of each column in plp_aperta2_peregrine5_pc_display_core_state_line() API output.
 * To be called after plp_aperta2_peregrine5_pc_display_core_state_line() API to display the legends.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @code
        srds_access_t *sa__ ;
        struct peregrine5_pc_event_st event;
        event.event_id = 2; event.data1 = 5; event.data2 = 6;
        plp_aperta2_peregrine5_pc_log_api_event(sa__, event);
...
#define EYE_SCAN_NRZ_INTRUSIVE_BUF_SIZE     (EYE_SCAN_INTRUSIVE_WIDTH * (((EYE_SCAN_NRZ_VERTICAL_IDX_MAX+1) * 2) / EYE_SCAN_NRZ_VERTICAL_STEP))
#define EYE_SCAN_PAM_INTRUSIVE_BUF_SIZE     (EYE_SCAN_INTRUSIVE_WIDTH * (((EYE_SCAN_PAM_VERTICAL_IDX_MAX+1) * 2) / EYE_SCAN_PAM_VERTICAL_STEP))
 ...
 * This function only initiates the processor actions. User must use plp_aperta2_peregrine5_pc_read_eye_scan_stripe() function
 ...
 *
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] ber_scan_mode configures the type of test (use #plp_aperta2_srds_diag_ber_mode_enum).         \n
 * \verbatim
  bit  7  - reserved.
 ...
 * plp_aperta2_peregrine5_pc_start_ber_scan_test(), plp_aperta2_peregrine5_pc_read_ber_scan_data(), plp_aperta2_peregrine5_pc_display_ber_scan_data().
 *
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] rate specifies the data rate in Hz.
 * @param[in] ber_scan_mode the type of test used to take the data(use #plp_aperta2_srds_diag_ber_mode_enum).
 ...
 * \n Note: Call functions plp_aperta2_peregrine5_pc_display_lane_state_hdr(sa__) before and plp_aperta2_peregrine5_pc_display_lane_state_legend() after
...
/** Column definition header for peregrine5_pc display state.
 * To be called before plp_aperta2_peregrine5_pc_display_lane_state() function.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t plp_aperta2_peregrine5_pc_display_lane_state_hdr(srds_access_t *sa__);

/** Detailed explanation of each column in peregrine5_pc display state.
 * To be called after plp_aperta2_peregrine5_pc_display_lane_state() function to display the legends.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 *     enum plp_aperta2_srds_diag_level_enum can be used to help set the diag_level. \n
 * \verbatim
 ...
 * This function is required to be implemented by upper level software to enable peregrine5_pc support.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] diag_level is a MASK style parameter to enable variable level of display.
 *     enum plp_aperta2_srds_diag_level_enum can be used to help set the diag_level. \n
 * \verbatim
...
/** Parameterized diagnostic function which provides access to all peregrine5_pc control and status bits
 * This function is required to be implemented by upper level software to enable peregrine5_pc support.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] type controls the type of access requested specified through enum plp_aperta2_srds_diag_access_enum. \n
 * \verbatim
