...
--- does s start with a prefix, if so remove it from the string
---@param s string
---@param prefix string
---@return string
function stringx.removeprefix(s, prefix)
    assert_string(1, s)
    assert_string(2, prefix)
    if stringx.startswith(s, prefix) then
...
---@param suffix string
---@return string
function stringx.removesuffix(s, suffix)
    assert_string(1, s)
    assert_string(2, suffix)
    if stringx.endswith(s, suffix) then
        return string.sub(s, 0, -(#suffix + 1))
    end
    return s
end
--- Strings and Lists
