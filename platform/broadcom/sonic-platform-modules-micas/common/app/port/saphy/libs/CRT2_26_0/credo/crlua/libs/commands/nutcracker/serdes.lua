...
slash.register_chip_command({"serdes", "param"}, credo.FAMILY_NUTCRACKER, [[
...
slash.register_chip_command({"serdes", "control"}, credo.FAMILY_NUTCRACKER, [[
...
slash.register_chip_command({"serdes", "isi"}, credo.FAMILY_NUTCRACKER, [[
...
serdes.register_tx_taps(credo.FAMILY_NUTCRACKER, {"pre3", "pre2", "pre1", "main", "post1", "post2", "post3"})

slash.register_chip_command({"serdes", "tx_taps_nl"}, credo.FAMILY_NUTCRACKER, [[
...
Examples:
    ...
    > /serdes tx_taps_nl b*                    # display tx taps for lanes on line side
    > /serdes tx_taps_nl -r                    # reset all tx taps
    ...
    if argt.full ~= nil or argt.partial ~= nil then
        ...
        for lane in iter(lanes) do
            ...
            if lane < 8 then
                ...
                print("lane %d set tx_taps_nl {%d,%d,%d,%d,%d,%d,%d} {%d,%d,%d,%d}" % {lane, table.unpack(val)})
            end
    ...
    local ncreg = require"regmap".slice(slice)
    local ftablea = fort.create_table()
    local ftableb = fort.create_table()
    fort.write_ln(ftablea, "", "auto", "Taps", "", "", "Taps_bar", "", "", "Taps_3", "", "", "Taps_bar_3", "", "")
    fort.write_ln(ftablea, "Lane", "select", "-1 M +1 +2", "Scale", "Sum", "-1 M +1 +2", "Scale", "Sum", "-1 M +1 +2",
                  "Scale", "Sum", "-1 M +1 +2", "Scale", "Sum")
    fort.add_separator(ftablea)
    local is_host = false
    local is_line = false
    for lane in iter(argt.lanes) do
        if lane >= 8 then
        ...
        fort.printf(ftablea, "%s(%d)", crutil.get_lane_id(slice, lane), lane)
        local auto_sel = ""
        auto_sel = auto_sel .. ncreg.pam100_v.reg_tx_coef_4_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. ncreg.pam100_v.reg_tx_coef_3_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. ncreg.pam100_v.reg_tx_coef_2_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. ncreg.pam100_v.reg_tx_coef_1_auto_sel:read(slice, lane)
        fort.printf(ftablea, "%s" % {auto_sel})
        local pr1, m, po1, po2, scale, sum = table.unpack(get_table_range(taps, 1, 6))
        fort.printf(ftablea, "%d  %d  %d  %d" % {pr1, m, po1, po2})
        fort.printf(ftablea, "%d|%d" % {scale, sum})
    ...
    fort.write_ln(ftableb, "", "auto", "Taps", "", "Taps_bar", "", "Taps_3", "", "Taps_bar_3", "")
    fort.write_ln(ftableb, "Lane", "select", "-3 -2 -1 M +1 +2 +3", "Sum", "-3 -2 -1 M +1 +2 +3", "Sum",
                  "-3 -2 -1 M +1 +2 +3", "Sum", "-3 -2 -1 M +1 +2 +3", "Sum")
    fort.add_separator(ftableb)
    for lane in iter(argt.lanes) do
        if lane < 8 then
            goto continue
        end
        is_line = true
        local taps = credo.serdes_get_param(slice, 'tx_taps_nl', lane)
        fort.printf(ftableb, "%s(%d)", crutil.get_lane_id(slice, lane), lane)
        local auto_sel = ""
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_m3:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_m2:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_m1:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_0:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_1:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_2:read(slice, lane - 8)
        auto_sel = auto_sel .. ncreg.pam100.reg_tx_coef_auto_sel_3:read(slice, lane - 8)
        fort.printf(ftableb, "%s" % {auto_sel})
        local pr3, pr2, pr1, m, po1, po2, po3, sum = table.unpack(taps, 1, 8)
        fort.printf(ftableb, "%d  %d  %d  %d  %d  %d  %d|%d" % {pr3, pr2, pr1, m, po1, po2, po3, sum})
        pr3, pr2, pr1, m, po1, po2, po3, sum = table.unpack(taps, 9, 16)
        fort.printf(ftableb, "%d  %d  %d  %d  %d  %d  %d|%d" % {pr3, pr2, pr1, m, po1, po2, po3, sum})
        pr3, pr2, pr1, m, po1, po2, po3, sum = table.unpack(taps, 17, 24)
        fort.printf(ftableb, "%d  %d  %d  %d  %d  %d  %d|%d" % {pr3, pr2, pr1, m, po1, po2, po3, sum})
        pr3, pr2, pr1, m, po1, po2, po3, sum = table.unpack(taps, 25, 32)
        fort.printf(ftableb, "%d  %d  %d  %d  %d  %d  %d|%d" % {pr3, pr2, pr1, m, po1, po2, po3, sum})
        fort.ln(ftableb)
        ::continue::
    end
...
slash.register_chip_command({"serdes", "tx_6taps"}, credo.FAMILY_NUTCRACKER, [[
Get/Set tx 6-taps
...
Examples:
    ...
    local regmap = require"regmap".slice(slice)
    local ftable = fort.create_table()
    ...
    for lane in iter(argt.lanes) do
        ...
        local auto_sel = ""
        auto_sel = auto_sel .. regmap.nrz50_v.reg_tx_coef_4_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. regmap.nrz50_v.reg_tx_coef_3_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. regmap.nrz50_v.reg_tx_coef_2_auto_sel:read(slice, lane)
        auto_sel = auto_sel .. regmap.nrz50_v.reg_tx_coef_1_auto_sel:read(slice, lane)
