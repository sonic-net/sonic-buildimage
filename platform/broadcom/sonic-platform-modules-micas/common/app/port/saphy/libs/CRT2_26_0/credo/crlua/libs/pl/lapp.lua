...
local filetypes = {stdin = {io.stdin, "file-in"}, stdout = {io.stdout, "file-out"}, stderr = {io.stderr, "file-out"}}
...
function lapp.process_options_string(str, args)
    ...
    for line in lines(str) do
        ...
        elseif check "$<{name} $" then -- is it <parameter_name>?
            ...
            optparm = optparm:gsub("%W", "_") -- transfer non-alphanumeric to underline
            varargs = rest == "..."
    ...
    while i <= #arg do
        ...
        -- custom fix to enable negative numbers
        if not end_of_flags and
            (match("--$S{long}", theArg, res) or (match("-$S{short}", theArg, res) and not string.match(theArg, "^-%d"))) then
