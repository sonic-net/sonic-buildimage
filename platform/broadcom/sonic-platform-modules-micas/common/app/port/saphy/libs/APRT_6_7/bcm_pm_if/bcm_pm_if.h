...
#define BCM_PM_IF_MAJOR_VER_NO           6
#define BCM_PM_IF_MINOR_VER_NO           7
...
#define BCM_PM_MUTEX_LOCK(_PHY_ID, _P_CTXT) \
    do {\
        ...
        if ((plp_aperta_phy_ctrl.phy[_PHY_ID] != NULL) && (plp_aperta_phy_ctrl.phy[_PHY_ID]->mutex_info.mutex_take != NULL)) {\
           PHYMOD_IF_ERR_RETURN(plp_aperta_phy_ctrl.phy[_PHY_ID]->mutex_info.mutex_take(_PHY_ID, _P_CTXT));\
        }\
    } while (0)
...
#define BCM_PM_MUTEX_UNLOCK(_PHY_ID, _P_CTXT) \
    do {\
        BCM_PM_ERR_CHECK_PHYID_RET_INVALID(_PHY_ID);    \
        if ((plp_aperta_phy_ctrl.phy[_PHY_ID] != NULL) && (plp_aperta_phy_ctrl.phy[_PHY_ID]->mutex_info.mutex_give != NULL)) {\
           PHYMOD_IF_ERR_RETURN(plp_aperta_phy_ctrl.phy[_PHY_ID]->mutex_info.mutex_give(_PHY_ID, _P_CTXT));\
        }\
    } while (0)
...
#define BCM_PM_CHK_INIT_DONE(_PHY_ID)\
    do {\
        ...
        if ((_plp_aperta_phyid_list[_PHY_ID].valid) && (_plp_aperta_phyid_list[_PHY_ID].phy_id == _PHY_ID)) {\
...
typedef struct _bcm_if_phymod_core_s {
    ...
    plp_aperta_phymod_bus_t pm_bus;
    int ref_cnt;
    int init;
    int unit;
    int port;
...
typedef struct bcm_if_phymod_ctrl_s {
    ...
    plp_aperta_phymod_symbols_t *symbols;
...
extern bcm_if_phymod_phy_id_t _plp_aperta_phyid_list[BCM_PM_IF_MAX_PHY];


#if defined(PHYMOD_XGBASET_SUPPORT)
void _bcm_pm_if_init_phy_id_idx(uint32_t phy_id);
void _bcm_plp_aperta_pm_if_get_phy_id_idx(uint32_t phy_id_combo, uint32_t *idx, uint32_t *exist_phy);
int  _bcm_pm_if_phy_id_is_master(uint32_t phy_id);
#else  /* ! XGBASET_SUPPORT  */
void _bcm_plp_aperta_pm_if_get_phy_id_idx(uint32_t phy_id, uint32_t *idx, uint32_t *exist_phy);
#endif  /*  XGBASET_SUPPORT  */

void _bcm_plp_aperta_pm_if_core_init(_bcm_if_phymod_core_t *core, plp_aperta_phymod_bus_t *core_bus, uint32_t phy_addr, void *user_acc);
...
int _bcm_plp_aperta_pm_if_get_port_information(bcm_plp_access_t phy_info, bcm_if_port_information_t* port_information);
...
#define BCM_PLP_PHY_GET_P_CTXT(_PI, IDX, E_P)                              \
    do {                                                                   \
        ...
        if (E_P != 1) {                                                    \
            _plp_aperta_phyid_list[laddr].valid = 0;                                  \
            PHYMOD_DEBUG_ERROR(("PHY does not exist\n"));                  \
            rv = BCM_PM_IF_PHY_NA;                                         \
            goto ERR;                                                      \
        }                                                                  \
...
#define BCM_PLP_FILL_PHY_ACCESS(_PI, IDX)                                               \
    do {                                                                                \
        ...
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.flags &= ~(1U << BCM_PM_INTERFACE_SIDE_SHIFT); \
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.flags |=                                       \
                                ( (is_line_side ? BCM_LINE_SIDE : BCM_SYSTEM_SIDE)      \
                                  << BCM_PM_INTERFACE_SIDE_SHIFT );                     \
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.port_loc = (is_line_side) ? phymodPortLocLine         \
                                                            : phymodPortLocSys;         \
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.misc = &(_plp_aperta_phyid_list[IDX]);                    \
    } while (0)
...
#define BCM_PLP_CORE_GET_P_CTXT(_PI, IDX, E_P)                                    \
    do {                                                                          \
        _bcm_plp_aperta_pm_if_get_phy_id_idx(_PI.phy_addr, &IDX, &E_P);                      \
        if (E_P != 1) {                                                           \
            _plp_aperta_phyid_list[BCM_PHY_ADDR_LOGICAL(_PI.phy_addr)].valid = 0;            \
            PHYMOD_DEBUG_ERROR(("PHY does not exist\n"));                         \
            rv = BCM_PM_IF_PHY_NA;                                                \
            goto ERR;                                                             \
        }                                                                         \
        ...
        if (_PI.platform_ctxt == NULL) {                                          \
            _PI.platform_ctxt = plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.access.user_acc; \
        }                                                                         \
    } while (0)
...
#define BCM_PLP_FILL_CORE_ACCESS(_PI, IDX)                                        \
    do {                                                                          \
        ...
        plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.access.lane_mask = _PI.lane_map;         \
        BCM_SET_CORE_INTF_SIDE(plp_aperta_phy_ctrl,_PI.if_side);                             \
        plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.port_loc =                               \
                     (_PI.if_side == 0) ? phymodPortLocLine : phymodPortLocSys;   \
        plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.access.misc = &(_plp_aperta_phyid_list[IDX]);       \
    } while (0)
...
#define BCM_PLP_FILL_PHY_ACCESS(_PI, IDX)                             \
    do{                                                               \
        ...
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.lane_mask = _PI.lane_map;     \
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.flags &= ~(1 << BCM_PM_INTERFACE_SIDE_SHIFT);\
        plp_aperta_phy_ctrl.phy[IDX]->pm_phy.access.flags |= (((_PI.if_side == 0 || _PI.if_side == 0xFF ) ? BCM_LINE_SIDE : BCM_SYSTEM_SIDE) << BCM_PM_INTERFACE_SIDE_SHIFT); \
        (_PI.if_side == 0 ||_PI.if_side == 0xFF ) ? (plp_aperta_phy_ctrl.phy[IDX]->pm_phy.port_loc = phymodPortLocLine) : (plp_aperta_phy_ctrl.phy[IDX]->pm_phy.port_loc = phymodPortLocSys);\
    }while(0)
...
#define BCM_PLP_FILL_CORE_ACCESS(_PI, IDX)                            \
    do{                                                               \
        ...
        plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.access.lane_mask = _PI.lane_map;     \
        BCM_SET_CORE_INTF_SIDE(plp_aperta_phy_ctrl,_PI.if_side);                            \
        (_PI.if_side == 0) ? (plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.port_loc = phymodPortLocLine) : (plp_aperta_phy_ctrl.phy[IDX]->core->pm_core.port_loc = phymodPortLocSys);\
    }while(0)
