 ...
 * APIs. This program might not work for all environments.
 *
 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Aperta2 chip for 400G(4x106G) PAM4 with   *
 * IPSec enabled mode as follows:                                                   *
 *   1.  Connect to the board.                                                      *
 *   2.  Download the firmware and Initialize the PHY in IPSec enabled mode         *
 *   3.  Initialize IPSec                                                           *
 *   4.  Configure Tx/Rx polarity                                                   *
 *   5.  Configure operating mode parameters for 400G(4x106G) PAM4 mode             *
 *   6.  Setup link training on System and Line side                                *
 *   7.  Add channel configuration                                                  *
 *   8.  Add vport                                                                  *
 *   9.  Add SA with transfer record                                                *
 *   10. Add rule                                                                   *
 *   11. Add CfyE parser                                                            *
 *   12. Check the Link status of ports                                             *
 *   13. Remove rule                                                                *
 *   14. Remove SA                                                                  *
 *   15. Remove vport                                                               *
 *   16. Uninitialize IPSec                                                         *
 *   17. Cleanup and close the connection to the board.                             *
 * -------------------------------------------------------------------------------- *
...
int main(int argc, char *argv[])
{
    ...
    /* -------------------------------------------------------------------------------- *
     * Section 1: PHY, IPSec Initialization                                             *
     * -------------------------------------------------------------------------------- */

    /* -------------------------------------------------------------------------------- *
     * Initialize the PHYs and download firmware based on the test setup and Initialize *
     *                                     IPSec                                        *
     * -------------------------------------------------------------------------------- */
    fw_init_params.macsec_option = 0x0;  /* IPSec Mode */
    ...
    /* -------------------------------------------------------------------------------- *
     * Initialize IPSec CfyE and SecY for both Octals                                   *
     * -------------------------------------------------------------------------------- */
    ...
    for (macsec_side = 0; macsec_side < 2; macsec_side++) {
        ...
        /* device_bypass_enable is set to 0, to trigger IPSec transformation & validation */
        retval = macsec_ipsec_initialize(sec_info, device_bypass_enable);
        if (retval != TEST_SUCCESS) {
            printf("FAIL: IPSec Initialize for PHY-ID[%d], macsec_side[%d], return code [%d]\n",
                    phy_info.phy_addr, sec_info.macsec_side, retval);
            goto _aperta2_macsec_init_error;
        } else {
            printf("PASS: IPSec Initialize for PHY-ID[%d], macsec_side [%d] \n",
    ...
    /* -------------------------------------------------------------------------------- *
     * Section 6: IPSec Configuration                                                   *
     * -------------------------------------------------------------------------------- */

    /* -------------------------------------------------------------------------------- *
     * Add chnanel configuration                                                        * 
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        line_lane_map = line_lane_map_list[lane_map_index];
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.if_side = macsec_side ? BCM_LINE_SIDE : BCM_SYSTEM_SIDE;
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = ipsec_channel_mode_config_set(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = ipsec_add_vport(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = ipsec_add_sa_with_transform_record(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            if (sec_info.macsec_side == INGRESS) { 
                retval = ipsec_add_rule_policy(sec_info, POLICY_BYPASS_NONE);
                if (retval != TEST_SUCCESS) {
                    goto _aperta2_config_error;
                }
            }
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = ipsec_install_parser_settings(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
     ...
     * Note: Packet format used for Vport ID0 "ipsec_sample_pkt_vid0.tx" as per configuration of IPSec *
     * ------------------------------------------------------------------------------------------------ */
     printf("\n\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
     printf("Ports are IPSec enabled and UP. Please send the traffic\n");
     printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
     
    /* -------------------------------------------------------------------------------- *
     * Section 8: Deconfigure IPSec                                                    *
     * -------------------------------------------------------------------------------- */
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = macsec_remove_rule_policy(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
    ...
    /* -------------------------------------------------------------------------------- *
     * Remove SA with transform record
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        line_lane_map = line_lane_map_list[lane_map_index];
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = macsec_remove_sa_with_transform_record(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for (macsec_side = 0; macsec_side < 2; macsec_side++) {
            ...
            phy_info.lane_map = macsec_side ? line_lane_map : sys_lane_map;
            memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t)); 
            retval = macsec_remove_vport(sec_info);
            if (retval != TEST_SUCCESS) {
                goto _aperta2_config_error;
            }
        }
...
#ifdef MACSEC_UNINIT_EN
    /* -------------------------------------------------------------------------------- *
     * Uninitialize IPSec CfyE and SecY                                                 *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
        memcpy(&sec_info.phy_info, &phy_info, sizeof(bcm_plp_access_t));
        for(macsec_side = 0; macsec_side < 2; macsec_side++) {
            sec_info.macsec_side = macsec_side; /* macsec_side = 0 -> Egress ; macsec_side = 1 -> Ingress */
            retval = macsec_ipsec_uninitialize(sec_info);
            if (retval) {
                printf("FAIL: Failed to uninitialize IPSec device for PHY-%d in %s path (ret = %d)!\n",
                sec_info.phy_info.phy_addr, sec_info.macsec_side ? "INGRESS" : "EGRESS", retval);
            } else {
                printf("PASS: Successfully uninitialized IPSec device for PHY-%d in %s path!\n",
