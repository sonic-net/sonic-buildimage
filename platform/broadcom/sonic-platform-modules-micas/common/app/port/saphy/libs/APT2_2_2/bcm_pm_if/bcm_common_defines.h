...
typedef enum bcm_pm_usxgmii_mode_e {
    ...
    bcmpmUsxgmiiAggrGroupOp    = 0x8000, /**< USXGMII aggregate group operation */
...
typedef enum bcm_pm_firmware_load_method_e {
    ...
    bcmpmFirmwareLoadMethodExternal,      /**< Load FW to internal memory via MDIO using a user registered function */
    bcmpmFirmwareLoadMethodProgEEPROM,    /**< Load FW to internal memory via MDIO and flash it into EEPROM */
    bcmpmFirmwareLoadMethodAuto,          /**< Auto download FW to internal memory via MDIO in case of firmware change */
    bcmpmFirmwareLoadMethodExternalNVM,   /**< Load FW via MDIO to Non Volatile Memory such as EEPROM using a user registered function */
 ...
 */
 typedef enum bcm_plp_failover_mode_e {
    ...
    /* For Millenio Chip Family only */
    bcmplpFailovermodePrimary = 0,
    bcmplpFailovermodeSecondary = 1,
    /* For Barchetta Chip Family only  */
...
typedef struct bcm_plp_pm_firmware_lane_config_s {
    ...
    unsigned int OppositeCdrFirst;              /**< 1:Wait for opposite cdr_lock is ready before link training 
                                                      start 0: Start link training firstly, then check oppositei
                                                      cdr_lock status */
    unsigned int AnTimer;                       /**< 0:IEEE standard 3s link inhibit timer, 1:6s  
                                                     link inhibit timer: Applicable only PAM4 modes */
    unsigned int DfeOn;                         /**< Enable DFE */
    unsigned int ForceBrDfe;                    /**< Force Baud rate DFE */
    unsigned int LpDfeOn;                       /**< Enable low power DFE */
    unsigned int TxSquelchOrPrbs;               /**< Squelch TX or send PRBS58 pattern if a fault occurs.
                                                     0: Squelch TX, 1: Send PRBS58 */
...
typedef struct bcm_plp_dsrds_firmware_lane_config_s {
    ...
    unsigned int OppositeCdrFirst;              /**< 1:Wait for opposite cdr_lock is ready before link training start, 
    ...
    unsigned int LinkTrainingReStartTimeOut;    /**< Link Training restart timeout */
    unsigned int LinkTrainingCL72_CL93PresetEn; /**< LT RX send cl72/cl93 preset request: 0:Disable (non-IEEE compatible mode);
                                                     1: Enable(IEEE compatible mode, default value) */
    unsigned int LinkTraining802_3CDPresetEn;   /**< LT RX send 802.3cd preset1 request: 0:Disable (non-IEEE compatible mode);
                                                     1: Enable (IEEE compatible mode, default value) */
    unsigned int LinkTrainingTxAmpTuning;       /**< Copper PAM4 Link training, 0: RX adjust LP's TX TXFIR with fixed 7 steps;
                                                     1: Copper PAM4 Link training, RX adjust LP's TX TXFIR dynamically */
    unsigned int LpDfeOn;                       /**< Enable Low poer DFE */
    unsigned int AnTimer;                       /**< 0:IEEE standard 3s link inhibit timer, 1:6s  link inhibit timer:
                                                     Applicable only PAM4 modes */
    unsigned int TxSquelchOrPrbs;               /**< Squelch TX or send PRBS58 pattern if a fault occurs.  
                                                     0: Squelch TX, 1: Send PRBS58 */
...
typedef struct bcm_plp_kp4_fec_config_s {
    unsigned int kp4_fec_config_enable;                        /**< KP4 FEC config enable. This field must be set 1 to
                                                                    configure KP4 FEC. Input only, Output is ignored*/
    ...
    bcm_plp_kp4_prbs_t kp4_prbs;                               /**< KP4 FEC Prbs: Note: This field must be set to 1 to
                                                                     configure KP4 PRBS */
...
typedef struct bcm_plp_ieee_fec_rx_status_s {
    ...
    unsigned short ieee_fec_lane_mapping;          /**< ln0_map[1:0]; ln1_map[3:2]; ln2_map[5:4]; ln3_map[7:6] 1.206
                                                        for 100G/50G only */
    ...
    unsigned short ieee_symbols_corr_cnt_fln[8];   /**< 1.210+1.211..1.216+1.217 - Symbol error per fec lane for 
                                                        50G(2ln)/100G(4ln); 3.600+3.601..3.614+3.615 for 200G(8ln) */
...
typedef struct bcm_plp_fec_dump_status_s {
    ...
    bcm_plp_fec_rx_err_cnt_t fec_b_err_cnt;    /**< fec err counter struct - only for 100G-Interleaved FEC, 200G , 400G 
                                                    or 800G */
...
typedef struct bcm_plp_fec_status_s {
    ...
    unsigned char fec_status_clear;                  /**< used to clear the fec status */
    bcm_plp_fec_status_init_t fec_status_init;       /**< used to enable/disable the fec status counters*/
 ...
 * @brief Aux PHY Controls from setting BASE-T features thru bcm_plp_aperta2_mode_config_set()/_get()
 */
typedef enum  bcm_plp_base_t_ctrl_e {
    ...
    bcmplpBaseTCtrlPreambleAlignment,               /**< Control preamble alignment to interoperate with non-IEEE-standard PHYs */
...
/** PTP-v2.1 user-defined PTP lookup actions for the nine classified PTP packet types defined
     in bcm_plp_timesync_user_action_t */
typedef enum  bcm_plp_timesync_lookup_action_e {
...
typedef struct bcm_plp_timesync_config_s {
    ...
    unsigned short otpid1;                                  /**< 1588 Alternative outer Vlan tag protocol identifier
                                                               (Egress) */
    ...
    unsigned short otpid1_igr;                              /**< 1588 Alternative outer Vlan tag protocol identifier
                                                               (Ingress) */
    unsigned short otpid2_igr;                              /**< 1588 Alternative outer Vlan tag protocol identifier2
                                                               (Ingress) */
    ...
    unsigned int   sopmem_wr_on_err;                        /**< Enable writing to SOPMEM when pkt_err or crc_err is
                                                               received */
...
typedef struct bcm_plp_avs_config_s {
    ...
    unsigned char i2c_slave_address[3];                 /**< i2c slave address in case of external avs regulator 
                                                             as master */
    bcm_plp_avs_external_ctrl_step_t external_ctrl_step;/**< AVS external control step control */
    unsigned char regulator_i2c_address;                /**< Set Regulator i2c slave address or DVDD rail address 
                                                             for multiple regulators, if not set, keep it 0 to 
                                                             use default regulator slave address*/
    unsigned char regulator_legs;                       /**< Regulator legs set to 0 by default. Enables user to 
                                                             use any of the two legs for supplying DVDD */
    unsigned char internal_master_slave_package;        /** < 0:slave package; 1:master package */
    bcm_plp_avs_regulator_config_t regulator_config[BCM_PLP_AVS_REGULATORS_MAX];/**< Regulator Info structure only
                                                              applicable for multiple regulators support */
...
typedef struct bcm_plp_pcs_rx_status_phylane_s {
    unsigned char  pcs_block_lock_stat;           /*!< per pcs lane for each phy lane - read followed by clear;
                                                       Applicable to all modes 10GE, 40GE, 25GE, 50GE and 100GE */
    ...
    unsigned char  pcs_dskw_error_sticky;         /*!< per pcs lane for each phy lane - read followed by clear;
                                                       Not applicable to single lane speeds of 10GE and 25GE */
    unsigned char  pcs_lane_mapping[BCM_PLP_PCS_LOGICAL_LANES];          /*!< per pcs lane for each phy lane - valid only if
                                                                           corresponding am_lolock=0; Not applicable to single
                                                                           lane speeds of 10GE and 25GE */
    unsigned short pcs_bip_err_cnt[BCM_PLP_PCS_LOGICAL_LANES];           /*!< per pcs lane for each phy lane - valid only if
                                                                           corresponding am_lolock=0; Not applicable to single
                                                                           lane speeds of 10GE and 25GE */
    unsigned char pcs_skew_stat[BCM_PLP_PCS_LOGICAL_LANES];              /*!< per pcs lane for each phy lane - valid only if
                                                                           corresponding am_lolock=0; Not applicable to single
                                                                           lane speeds of 10GE and 25GE */
    unsigned int fcfec_corrected_block_cnt[BCM_PLP_PCS_LOGICAL_LANES];   /*!< per pcs lane for each phy lane - valid only
                                                                           if corresponding am_lolock=0; Applicable for 25GE
                                                                           and 50GE */
    unsigned int fcfec_uncorrected_block_cnt[BCM_PLP_PCS_LOGICAL_LANES]; /*!< per pcs lane for each phy lane - valid only
                                                                           if corresponding am_lolock=0; Applicable for
                                                                           25GE and 50GE */
...
/*! Line and system side VCO selection */
typedef struct bcm_plp_pai_pll_vco_config_s {
    bcm_plp_pll_vco_t sys_vco;       /**< System side VCO */
    bcm_plp_pll_vco_t line_vco;      /**< Line Side VCO */
} bcm_plp_pai_pll_vco_config_t;
...
typedef struct bcm_plp_pai_fw_init_config_s {
    ...
    unsigned int phy_mode;        /**< Select chip specific phy mode operation. 
                                       See bcm_plp_phy_mode_sel_t for examples  */
    unsigned int ptp_option;      /**< Configure timesync operation*/
    bcm_plp_laneswap_map_t sys_laneswap_map;  /**< System side logical lane map*/
    bcm_plp_laneswap_map_t line_laneswap_map; /**< Line side logical lane map*/ 
    bcm_plp_pai_pll_vco_config_t octal0;      /**< Octal 0 VCO config*/
    bcm_plp_pai_pll_vco_config_t octal1;      /**< Octal 1 VCO config*/
...
typedef enum bcm_plp_failover_switch_mode_e {
    bcmplpFailoverSwitchModeGlobal = 0, /**< active contexts are switched and the scope is global use bcm_plp_aperta2_failover_mode_set to switch context */
    bcmplpFailoverSwitchModePerPort,    /**< active contexts are switched and the scope is per port determined by API bcm_plp_aperta2_failover_mode_set */
    bcmplpFailoverSwitchModePinBased    /**< active contexts are switched and the scope is HMUX_GLOBAL by pin call to API bcm_plp_aperta2_failover_mode_set returns error */
...
typedef enum bcm_plp_fw_upgrade_side_e {
    bcmplpFwUpgradeSideDC,   /**< Side don't care   */
