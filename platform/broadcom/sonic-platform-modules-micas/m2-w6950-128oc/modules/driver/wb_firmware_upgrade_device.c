...
static firmware_upgrade_device_t firmware_upgrade_device_data0 = {
    ...
    .en_level[1]         = 1,
    .en_gpio[2]          = 56 + GPIO_D1700_OFFSET,
    .en_level[2]         = 0,
...
static int __init firmware_upgrade_device_init(void)
{
    ...
    gdev = gpio_device_find_by_label("wb_gpio_d1500");
    if (gdev) {
        gpio_base = gpio_device_get_base(gdev);
        gpio_device_put(gdev);
    } else {
        printk(KERN_WARNING "firmware_upgrade_device: gpio chip wb_gpio_d1500 not found, GPIO offset=0\n");
    }
    ...
    for (i = 0; i < ARRAY_SIZE(firmware_upgrade_device); i++) {
        ...
        if (gpio_base >= 0) {
            ...
            if (!strcmp(firmware_upgrade_device_data->type, "JTAG")) {
                firmware_upgrade_device_data->upg_type.jtag.tdi += gpio_base;
                firmware_upgrade_device_data->upg_type.jtag.tck += gpio_base;
                firmware_upgrade_device_data->upg_type.jtag.tms += gpio_base;
                firmware_upgrade_device_data->upg_type.jtag.tdo += gpio_base;
            }

            for (j = 0; j < firmware_upgrade_device_data->en_gpio_num; j++) {
                firmware_upgrade_device_data->en_gpio[j] += gpio_base;
            }
        }
