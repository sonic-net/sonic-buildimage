...
/**
 * Shows switch temperature
 * @param  buf   register value in hexstring
 * @return       number of bytes after software calculation with the read raw data, or an error code
 */
static ssize_t get_temp_TH4_core(struct device *dev,
                                 ...
                                 char *buf)
{
    ...
    mutex_lock(&fpga->fpga_lock);
    raw_data = ioread32(fpga->base + FPGA_MAX_TEMP_REG) & 0xffff;
...
static DEVICE_ATTR( temp_TH4_core, 0400, get_temp_TH4_core, NULL);
