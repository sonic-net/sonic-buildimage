...
/* i2c / mux */
#include "ds5000_fpga_i2c_tree_cfg.h"
...
static struct ds5000_fpga_data *fpga_data;
...
static void ds5000_dev_release(struct device *dev)
{
	return;
}
				...
				char *buf)
{
	...
	ready = ioread32(fpga_dev.data_base_addr + TH5_SDK_READY_REG);
...
static void ds5000_drv_remove(struct platform_device *pdev)
{
	int portid_count;
...
static int ds5000_drv_probe(struct platform_device *pdev)
{
	int ret = 0;
	int portid_count;
	...
	fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct ds5000_fpga_data),
							 GFP_KERNEL);
...
static struct platform_driver ds5000_drv = {
	.probe  = ds5000_drv_probe,
	.remove = __exit_p(ds5000_drv_remove),
	.driver = {
		.name   = DRV_NAME,
	},
};

static struct platform_device ds5000_dev = {
	.name           = DRV_NAME,
	.id             = -1,
	.num_resources  = 0,
	.dev = {
		.release = ds5000_dev_release,
	}
};
...
static int cls_fpga_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	platform_device_register(&ds5000_dev);
	platform_driver_register(&ds5000_drv);
...
static void cls_fpga_remove(struct pci_dev *dev)
{
	...
	platform_driver_unregister(&ds5000_drv);
	platform_device_unregister(&ds5000_dev);
...
MODULE_DESCRIPTION("Celestica DS5000 sw_fpga driver");
