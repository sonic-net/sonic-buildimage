...
static int icp201xx_probe(struct i2c_client *client)
{
	...
	st->trig = devm_iio_trigger_alloc(&client->dev,"%s-dev%d",indio_dev->name,
	...
	if (ret) {
        ...
        ret = icp201xx_reg_update(st,ICP201XX_REG_MODE_SELECT, BIT_MEAS_MODE_MASK, BIT_FORCED_MEAS_MODE_POS, (uint8_t)ICP201XX_MEAS_MODE_CONTINUOUS);
        if (ret)
        {
              pr_info("Failed to set Continuous mode");
        }
...
static void icp201xx_remove(struct i2c_client *client)
{
