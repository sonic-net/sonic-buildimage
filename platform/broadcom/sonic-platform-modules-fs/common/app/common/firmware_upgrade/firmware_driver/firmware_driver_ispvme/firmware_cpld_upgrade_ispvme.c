...
static int firmware_file_read(const char *path, uint32_t addr, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        FIRMWARE_DRIVER_DEBUG_ERROR("vfs_iter_read failed, path=%s, addr=0x%x, size=%ld, ret=%d\r\n", path, pos, size, ret);
        goto exit;
    }
...
static int firmware_file_write(const char *path, uint32_t addr, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        FIRMWARE_DRIVER_DEBUG_ERROR("vfs_iter_write failed, path=%s, addr=0x%x, size=%ld, ret=%d\r\n", path, pos, size, ret);
        goto exit;
    }
