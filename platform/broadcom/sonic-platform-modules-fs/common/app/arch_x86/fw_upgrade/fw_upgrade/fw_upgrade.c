...
{
    /* Enable read and write permission for the port */
    if ( iopl(3) < 0) {
...
/* Writes the address of the register to be accessed to the address register of the iLPC2AHB bridge */
...
/* Writes the data to be sent to the iLPC2AHB bridge's data register */
...
/* Read the value of the iLPC2AHB bridge's data register */
...
/* Close the bridge from LPC to AHB and close the SIO*/
...
static int check_chip(uint32_t file_size, uint8_t *p, flash_info_t* info)
{
    ...
    write_bmc_flash_addr(info->flash_base_addr); /* Start IP address of the flash read */
    while (1) {
...
static int read_single_bmc_flash(flash_info_t* info, uint32_t start_addr, int read_size, int is_print)
{
    ...
    pull_ce_down(info);
    send_cmd(info->flash_base_addr, COMMON_FLASH_READ); /* flash read command */
    write_bmc_flash_addr(flash_start_addr); /* Start IP address of the flash read */
    while (1) {
