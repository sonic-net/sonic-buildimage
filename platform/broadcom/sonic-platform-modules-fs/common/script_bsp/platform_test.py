...
try:
    import click
    from platform_intf import platform_reg_read, platform_reg_write, platform_get_optoe_type, platform_get_mgmt_version, \
            platform_bmc_read, platform_bmc_write
...
@click.argument('offset', required=True)
@click.argument('size', required=True)
def bmc_rd(offset, size):
    '''read bmc reg'''
    offset = str_to_int(offset)

    ret, info = platform_bmc_read(offset, int(size))
    print(ret)
    if ret is True:
        print_reg(info, offset)
    else:
        print(info)
...
@main.command()
@click.argument('offset', required=True)
@click.argument('value', required=True, nargs=-1)
def bmc_wr(offset, value):
    '''write bmc reg'''
    offset = str_to_int(offset)
    value_list = [str_to_int(v) for v in value]

    ret, info = platform_bmc_write(offset, value_list)
    print(ret)
    print(info)
...
def get_mgmt_version():
    '''get_mgmt_version'''
    ret, info = platform_get_mgmt_version()
    print(ret)
    print(info)
