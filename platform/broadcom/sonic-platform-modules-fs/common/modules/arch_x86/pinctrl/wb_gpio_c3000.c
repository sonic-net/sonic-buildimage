...
static int wb_gpio_driver_probe(struct platform_device *plat_dev)
{
    ...
    DEBUG_VERBOSE("domain:0x%04x, bus:0x%02x, slot:0x%02x, fn:%u, bar:%u, irq: %d\n",
    ...
    DEBUG_VERBOSE("pci_dev_init success, pci_mem_bae: 0x%pK, res0: 0x%pK, res1: 0x%pK\n",
...
static int wb_gpio_driver_remove(struct platform_device *plat_dev)
{
    ...
    DEBUG_VERBOSE("c3000_gpio_pcie_remove.\n");
