...
static void intel_spi_enable_bios_write(struct pci_dev *pci_dev, struct intel_spi_boardinfo *info)
{
    ...
    if (writeable) {
        ...
        if (value == 0x3) {
            DEBUG_VERBOSE("invalid prefetching/caching settings, ");
        } else {
            DEBUG_VERBOSE("prefetching %sabled, caching %sabled, ",
                (value & 0x2) ? "en" : "dis",
                (value & 0x1) ? "dis" : "en");
        }
        ...
        /* BIOS lock disabled */
        want &= ~BIOS_CNTL_BLE;
        DEBUG_VERBOSE("bios cntl is:0x%x, want is:0x%x\n", bios_cntl, want);
        pci_write_config_byte(pci_dev, BIOS_CNTL, want);
        pci_read_config_byte(pci_dev, BIOS_CNTL, &new);
        DEBUG_VERBOSE("\nBIOS_CNTL = 0x%02x: ", new);
        DEBUG_VERBOSE("BIOS Lock Enable: %sabled, ", (new & BIOS_CNTL_BLE) ? "en" : "dis");
        DEBUG_VERBOSE("BIOS Write Enable: %sabled\n", (new & BIOS_CNTL_WN) ? "en" : "dis");

        if (new & BIOS_CNTL_SMM_BMP) {
            DEBUG_VERBOSE("BIOS region SMM protection is enabled!\n");
        }

        if (new != want) {
            DEBUG_VERBOSE("Warning: Setting Bios Control at 0x%x from 0x%02x to 0x%02x failed.\n"
                "New value is 0x%02x.\n", BIOS_CNTL, value, want, new);
        } else {
            info->writeable = !!(new & BIOS_CNTL_WN);
        }
        DEBUG_VERBOSE("Bios Control is 0x%x\n", new);
    } else {
        DEBUG_VERBOSE("Bios don't write\n");
    }
...
static int intel_spi_platform_probe(struct platform_device *pdev)
{
    ...
    if (!pci_dev) {
        DEBUG_ERROR("pci_get_device(0x8086, 0x8c54) failed!\n");
        return -1;
    }
    ...
    case INTEL_SPI_LPT:
        ...
        break;
    default:
        DEBUG_ERROR("info type[%d] not need set writeable.\n",info->type);
        break;
    }
    DEBUG_VERBOSE("intel spi boardinfo writeable is %sabled\n",
