...
{
    u32 irq_info_mode, gpio, irq_type, pirq_line;
    int ret;
    struct uio_info *dfd_irq_info;
    dfd_irq_t *dfd_irq;
    uio_irq_device_t *uio_irq_device;
    ...
    if (pdev->dev.of_node) {
        ...
        if (ret) {
            ...
            irq_info_mode = MODE_PIRQ_LINE;
        }

        if (irq_info_mode == MODE_PIRQ_LINE) {
            /* get pirq line for x86 */
            ret = of_property_read_u32(pdev->dev.of_node, "pirq_line", &pirq_line);
            if (ret != 0) {
                dev_err(&pdev->dev, "get dts config pirq_line error. ret:%d.\n", ret);
                ret = -ENXIO;
                goto free_mem;
            }
            DEBUG_VERBOSE("use pirq-line method, pirq-line:%u", pirq_line);
            dfd_irq_info->irq = pirq_line;
        } else if (irq_info_mode == MODE_GPIO) {
            ...
            dfd_irq->gpio = gpio;
            devm_gpio_request(&pdev->dev, dfd_irq->gpio, "GPIOA");
            ...
            DEBUG_ERROR("DTS Unsupport irq_info_mode type:%d.\n", irq_info_mode);
            ret = -EINVAL;
            goto free_mem;
        }
    } else {
        ...
        uio_irq_device = pdev->dev.platform_data;
        irq_info_mode = uio_irq_device->irq_info_mode;
        if (irq_info_mode == MODE_PIRQ_LINE) {
            pirq_line = uio_irq_device->pirq_line;
            DEBUG_VERBOSE("use pirq-line method, pirq-line:%u", pirq_line);
            dfd_irq_info->irq = pirq_line;
        } else if (irq_info_mode == MODE_GPIO) {
            gpio = uio_irq_device->gpio;
            irq_type = uio_irq_device->irq_type;
            dfd_irq->gpio = gpio;
            devm_gpio_request(&pdev->dev, dfd_irq->gpio, "GPIOA");
            dfd_irq_info->irq = gpio_to_irq(dfd_irq->gpio);
            DEBUG_VERBOSE("use gpio:%u, irq num:%ld", gpio, dfd_irq_info->irq);
        ...
        set_irq_type(dfd_irq_info->irq, dfd_irq->irq_type);
            ...
            DEBUG_ERROR("Unsupport irq_info_mode type:%d.\n", irq_info_mode);
            ret = -EINVAL;
            goto free_mem;
        }
    }
