...
static int oc_spi_reg_write(struct spioc *spioc, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    pfunc = (device_func_write)spioc->write_intf_addr;
    return pfunc(spioc->dev_name, pos, val, size);
}
...
static int oc_spi_reg_read(struct spioc *spioc, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    pfunc = (device_func_read)spioc->read_intf_addr;
    return pfunc(spioc->dev_name, pos, val, size);
}
...
static void oc_spi_setreg_8(struct spioc *spioc, int reg, u32 value)
{
    ...
    DEBUG_VERBOSE("path:%s, access mode:%d, pos:0x%x, value0x%x.\n",
...
static int spioc_get_clkdiv(struct spioc *spioc, u32 speed)
{
    ...
    rate = spioc->freq;
    DEBUG_VERBOSE("clk get rate:%u, speed:%u\n", rate, speed);
    /* fs = fw/((DIV+2)*2) */

    if (speed > (rate / 4)) {
        div = 0;
        DEBUG_VERBOSE("spi device speed[%u] more than a quarter of clk rate[%u].\n",
            speed, rate);
        return div;
    }
    div = (rate/(2 * speed)) - 2;
    if (div > SPIOC_MAX_DIV) {
        DEBUG_ERROR("Unsupport spi device speed, div:%u.\n", div);
        return -1;
    }
    DEBUG_VERBOSE("DIV is:0x%x\n", div);
    return div;
}
...
static void spioc_copy_tx(struct spioc *spioc)
{
    ...
    if (!spioc->txp) {
        DEBUG_ERROR("spioc->txp is NULL.\n");
        return;
    }

    src = (u8 *)spioc->txp + spioc->cur_pos;
    DEBUG_VERBOSE("current tx len:0x%x, tx pos:[0x%x]\n", spioc->cur_len, spioc->cur_pos);

    for (i = 0; i < spioc->cur_len; i++) {
        DEBUG_VERBOSE("write %d, val:[0x%x]\n", i, src[i]);
...
static void spioc_copy_rx(struct spioc *spioc)
{
    ...
    if (!spioc->rxp) {
        DEBUG_ERROR("spioc->rxp is NULL.\n");
        return;
    }

    dest = (u8 *)spioc->rxp + spioc->cur_pos;
    DEBUG_VERBOSE("current rx len:0x%x, rx pos:[0x%x]\n", spioc->cur_len, spioc->cur_pos);

    for (i = 0; i < spioc->cur_len; i++) {
        dest[i] = oc_spi_getreg(spioc, SPIOC_RX(i));
        DEBUG_VERBOSE("read %d, val:[0x%x]\n", i, dest[i]);
    }
}
...
static int spioc_setup_transfer(struct spi_device *spi, struct spi_transfer *transfer)
{
    ...
    if (hz == 0) {
        DEBUG_ERROR("Unsupport zero speed.\n");
        return -EINVAL;
    }

    div = spioc_get_clkdiv(spioc, hz);
    if (div < 0) {
        DEBUG_ERROR("get div error, div:%d.\n", div);
        return -EINVAL;
    }
    ctrl |= SPIOC_DIV(div);

    DEBUG_VERBOSE("ctrl:[0x%x].\n", ctrl);
...
static int spioc_spi_setup(struct spi_device *spi)
{
    ...
    if (!(spi->mode & SPI_CPHA)) {
        DEBUG_ERROR("Unsupport spi device mde:0x%x, SPI_CPHA must be 1.\n", spi->mode);
        return -EINVAL;
    }

    spioc = spi_master_get_devdata(spi->master);
    if (spi->chip_select >= spioc->num_chipselect) {
        DEBUG_ERROR("Spi device chipselect:%u, more than max chipselect:%u.\n",
            spi->chip_select, spioc->num_chipselect);
        return -EINVAL;
    }
    DEBUG_VERBOSE("Support spi device mode:0x%x, chip_select:%u.\n",
...
static int spioc_rx_start_one(struct spioc *spioc)
{
    ...
    if (!spioc->reamin_len) {
        DEBUG_VERBOSE("spioc reamin_len:[0x0]\n");
        return 0;
    }
...
static int spioc_tx_rx_start_one(struct spioc *spioc)
{
    ...
    txlen = spioc->cur_len;
    spioc->reamin_len -= txlen;
    DEBUG_VERBOSE("tx len:[0x%x], tx len remain:[0x%x]\n", txlen, spioc->reamin_len);
    ...
    if (spioc->reamin_len) {
        spioc->cur_pos += txlen;
        DEBUG_VERBOSE("cur_txrx pos:[0x%x]\n", spioc->cur_pos);
    }
    return 0;
}
...
static int spioc_spi_txrx_bufs(struct spi_device *spi, struct spi_transfer *t)
{
    ...
    if(!t->len || (!t->tx_buf && !t->rx_buf)) {
        DEBUG_ERROR("params error, tx_buf and rx_buf may both NULL, transfer len:0x%x.\n",
            t->len);
        return 0;
    }
    ...
    if (spioc->irq >= 0) {
        ...
        if (t->tx_buf && t->rx_buf) {
            DEBUG_VERBOSE("start tx rx, len:0x%x\n", t->len);
            ...
            DEBUG_VERBOSE("start tx, txlen:0x%x\n", t->len);
            while (spioc->reamin_len) {
            ...
            DEBUG_VERBOSE("start rx, rxlen:0x%x\n", t->len);
            while (spioc->reamin_len) {
...
static int ocores_spi_config_init(struct spioc *spioc)
{
    ...
    if (dev->of_node) {
        ...
        if (spioc->dev->platform_data == NULL) {
            DEBUG_ERROR("platform data config error.\n");
            ret = -ENXIO;
            return ret;
        }
    ...
    ret = find_intf_addr(&spioc->write_intf_addr, &spioc->read_intf_addr, spioc->reg_access_mode);
    if (ret) {
        dev_err(dev, "find_intf_addr func mode %d fail, ret: %d.\n", spioc->reg_access_mode, ret);
        return ret;
    }

    if (!spioc->write_intf_addr || !spioc->read_intf_addr) {
        DEBUG_ERROR("Fail: func mode %u rw symbol undefined.\n", spioc->reg_access_mode);
        return -ENOSYS;
    }

    DEBUG_VERBOSE("name:%s, base:0x%x, reg_shift:0x%x, io_width:0x%x, clock-frequency:0x%x.\n",
        spioc->dev_name, spioc->base_addr, spioc->reg_shift, spioc->reg_io_width, spioc->freq);
    DEBUG_VERBOSE("reg access mode:%u, num_chipselect:%u, irq_flag: %u\n",
...
static int spioc_probe(struct platform_device *pdev)
{
    ...
    if (spioc->irq_flag == 1) {
        ...
        if (spioc->irq >= 0) {
            DEBUG_VERBOSE("spi oc use irq, irq number:%d.\n", spioc->irq);
