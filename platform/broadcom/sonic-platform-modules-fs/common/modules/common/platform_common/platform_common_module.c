    ...
    for (j = 0 ;j < size ;j++){
        for (i = 0; i < PLATFORM_I2C_RETRY_TIMES; i++) {
          if ((val_t = i2c_smbus_read_byte_data(&client, (offset_addr + j)))< 0) {
              DEBUG_VERBOSE("read try(%d)time  offset_addr:%x \n", i, offset_addr);
              continue;
          }  else {
              * (buf + j) = val_t;
              break;
...
static int dfd_tlvinfo_get_cardtype(void)
{
    ...
    DEBUG_VERBOSE("Read device eeprom info:(dev:%s, addr:%02x).\n", i2c_path, i2c_dev.addr);

    ret = dfd_i2c_read(i2c_path, i2c_dev.addr, 0, eeprom, DFD_E2PROM_MAX_LEN);
    if (ret != 0) {
        DEBUG_ERROR("Read eeprom info error(dev: %s, addr: %02x).\n", i2c_path, i2c_dev.addr);
        return ret;
    }
    ...
    if (ret) {
        DEBUG_ERROR("dfd_tlvinfo_get_e2prom_info failed ret %d.\n", ret);
        return -1;
    }
    for (ret = 0; ret < 4; ret++) {
        DEBUG_VERBOSE("buf 0x%02x.\n", buf[ret]);
    }

    cardtype = ntohl(*((uint32_t *)buf));
    DEBUG_VERBOSE("cardtype 0x%x.\n", cardtype);
    return cardtype;
}
...
int dfd_get_my_card_type(void)
{
    ...
    while (cnt--) {
        ...
        if (type < 0) {
            DEBUG_ERROR("__dfd_get_my_card_type fail cnt %d, ret %d.\n", cnt, type);
            msleep(PLATFORM_CARDTYPE_RETRY_TIMES);
            continue;
        }
        DEBUG_VERBOSE("success to get type 0x%x.\n", type);
        break;
    }
...
static int __init platform_common_init(void)
{
    ...
    if (ret <= 0) {
        DEBUG_ERROR("dfd_get_my_card_type failed, ret %d.\n", ret);
        printk(KERN_ERR "Warning: Device type get failed, please check the TLV-EEPROM!\n");
        return -1;
    }

    DEBUG_VERBOSE("Leave success type 0x%x.\n", ret);
    return 0;
}
