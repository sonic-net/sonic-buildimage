...
int parse_tlv_eeprom(u_int8_t *eeprom, u_int32_t size)
{
    ...
    if (!is_valid_tlvinfo_header(eeprom_hdr)) {
        DEBUG_ERROR("Failed to check tlv header.\n");
        return -1;
    }

    if (!is_checksum_valid(eeprom)) {
        DEBUG_ERROR("Failed to check tlv crc.\n");
        return -1;
    }
...
static int dfd_parse_tlv_eeprom(u_int8_t *eeprom, u_int32_t size, u_int8_t main_type, tlv_decode_value_t *decode_value)
{
    ...
    if (!ret) {
        DEBUG_ERROR("No found type: %d\n", main_type);
        return -1;
    }

    DEBUG_VERBOSE("Found type: %d, value: %s\n", main_type,decode_value->value);
    ...
    u_int8_t *buf, u_int8_t *buf_len)
{
    ...
    while (eeprom_index < eeprom_end) {
        ...
        if (!is_valid_tlv(eeprom_tlv)) {
            DEBUG_ERROR("tlv is not valid, eeprom_tlv->type 0x%x.\n", eeprom_tlv->type);
            return -1;
        }

        DEBUG_VERBOSE("eeprom_tlv->length %d.\n", eeprom_tlv->length);
    ...
    DEBUG_ERROR("ext_type %d: tlv is not found.\n", ext_type);
    return -1;
}
...
int dfd_tlvinfo_get_e2prom_info(u_int8_t *eeprom, u_int32_t size, dfd_tlv_type_t *tlv_type, u_int8_t* buf, u_int8_t *buf_len)
{
    ...
    if (eeprom == NULL || tlv_type == NULL || buf == NULL) {
        DEBUG_ERROR("Input para invalid.\n");
        return -1;
    }
