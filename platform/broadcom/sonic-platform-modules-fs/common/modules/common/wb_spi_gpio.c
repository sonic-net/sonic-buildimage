...
static int nsecs_debug_time = 0;
module_param(nsecs_debug_time, int, S_IRUGO | S_IWUSR);
...
int spi_gpio_get_delay_time(unsigned nsecs)
{
	int delay_time_us;

	/* Convert ns to us and round up */
	delay_time_us = (nsecs + 1000 - 1) / 1000;
...
void spidelay(unsigned nsecs)
{
	u32 delay_time_us;
	/* Convert ns to us and round up */
	delay_time_us = spi_gpio_get_delay_time(nsecs);
	if (delay_time_us >= 1) {
...
static void print_spi_debug_info(const char *mode, unsigned nsecs)
{
    if (nsecs_debug_time > 0) {
        nsecs_debug_time--;
        printk("%s, nsecs = %d, delay = %d\n", mode, nsecs, spi_gpio_get_delay_time(nsecs));
    }
}
		...
		unsigned nsecs, u32 word, u8 bits, unsigned flags)
{
    ...
    print_spi_debug_info(__func__, nsecs);
