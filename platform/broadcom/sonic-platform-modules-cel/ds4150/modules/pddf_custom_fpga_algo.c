...
#define HAWKEYE_I2C_PCI_MAX_BUS 20
...
static struct fpgalogic_i2c fpgalogic_i2c[HAWKEYE_I2C_PCI_MAX_BUS];
...
static inline u32 xiic_getreg32(struct fpgalogic_i2c *i2c, int reg)
{
	u32 ret;
...
static int fpgai2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	if (!i2c) {
		printk(KERN_INFO "%s: i2c adapter data is NULL\n", __FUNCTION__);
		return -ENODEV;
	}
...
static int adap_data_init(struct i2c_adapter *adap, int i2c_ch_index)
{
...
#endif
    if (i2c_ch_index >= pci_privdata->max_fpga_i2c_ch || pci_privdata->max_fpga_i2c_ch > HAWKEYE_I2C_PCI_MAX_BUS) {
        printk("[%s]: ERROR i2c_ch_index=%d max_ch_index=%d out of range: %d\n",
             __FUNCTION__, i2c_ch_index, pci_privdata->max_fpga_i2c_ch, HAWKEYE_I2C_PCI_MAX_BUS);
        return -EINVAL;
    }
    ...
    int ret = fpgai2c_init(&fpgalogic_i2c[i2c_ch_index]);
	if (ret) return ret;
