        ...
        try:
            for fantray in chassis.get_all_fan_drawers():
                ...
                for fan in fantray_fans:
                    ...
                    if fan_rpm < self.FAN_LOW_WARNING_SPEED and fan not in self._low_warning_fans and fantray.get_presence():
                        time.sleep(5)
                        fan_rpm = fan.get_speed_rpm()
                        if fan_rpm < self.FAN_LOW_WARNING_SPEED:
                    ...
                    # FAN failure detection, <3000 RPM seen as failure
                    if fan_rpm < self.FAN_LOW_WARNING_SPEED and fan not in self._failure_fans and fantray.get_presence():
                        self._failure_fans.add(fan)
                        fantray_failure_fan_count += 1
                    elif fan_rpm > self.FAN_LOW_WARNING_SPEED and fan in self._failure_fans:
...
class ThermalInfo(ThermalPolicyInfoBase):
    ...
    B2F_INLET_SENSORS = ["Inlet 1 Temp", "Inlet 2 Temp"]      #["Inlet Temp U68", "Inlet Temp U3"]
    F2B_INLET_SENSORS = ["Inlet 3 Temp", "Inlet 4 Temp"]      #["Inlet Temp U26", "Inlet Temp U25"]

    THERMAL_INLET_SENSORS = F2B_INLET_SENSORS
    THERMAL_INLET_CRITICAL_TEMP = 47
    THERMAL_SHUTDOWN_TEMP_MAP = {##"CPU Internal Temp": 108, 
                                 "Switch Chip Internal Temp": 115}
    THERMAL_STEP_COMP_TEMP_MAP = {"Switch Chip Internal Temp": 103,
                                  "CPU Internal Temp": 105,
                                  "DIMM0 Temp": 85,
                                  "Switch Board Rear Center Temp": 80,
                                  "Switch Board Center Left Temp": 80,
                                  "Switch Board Center Right Temp": 80,
                                  "Switch Board Front Right Temp": 80,
                                  "Base Board Temp": 80}
        ...
        try:
            ...
            for thermal in chassis.get_all_thermals():
                ...
                if name in self.THERMAL_INLET_SENSORS:
                    ...
                    thermal_data = self._inlet_thermals_data.get(name, None)
                    if thermal_data == None:
                        thermal_data = ThermalData(name)
                        self._inlet_thermals_data[name] = thermal_data

                    if sys_airflow == "INTAKE":
                        thermal_data.update_temp(temp-3).update_temp_trend()
                    else:
    ...
    def is_any_inlet_over_high_critical_threshold(self):
        ...
        :return: True if the temperature is warm up and over high threshold else False
        """
        for thermal_info in list(self.get_inlet_thermals_data().values()):
