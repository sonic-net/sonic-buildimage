...
# @thermal_json_object('fantray.less.then.one.absence')
...
@thermal_json_object('fan.rotor.more.than.one.failure')
class FanRotorMoreThanOneFailureCondition(FanCondition):
    def is_match(self, thermal_info_dict):
        fan_info_obj = self.get_fan_info(thermal_info_dict)
        return len(fan_info_obj.get_failure_fans()) > 1 if fan_info_obj else False
