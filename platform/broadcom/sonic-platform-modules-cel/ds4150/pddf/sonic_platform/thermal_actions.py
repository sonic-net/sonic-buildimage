...
LPC_CPLD_SETREG_PATH = "/sys/bus/platform/devices/sys_cpld/setreg"
sys_airflow = None
...
class StepFanController():
    ...
    F2B_STEP_TEMP = [28, 36, 42]
    B2F_STEP_TEMP = [27, 35, 42]

    def __init__(self):
        global sys_airflow
        self.step_temp = StepFanController.F2B_STEP_TEMP
        
        if sys_airflow == "INTAKE":
            self.step_temp = StepFanController.B2F_STEP_TEMP    

    def _get_temp_pwm(self, is_cooling, temp):
        if is_cooling:
            if temp > self.step_temp[2] - 3:
                target_pwm = 100
            elif temp > self.step_temp[1] - 3:
                target_pwm = 85
            elif temp > self.step_temp[0] - 3:
                target_pwm = 70
            else:
                target_pwm = 50
        else:
            if temp <= self.step_temp[0]:
                target_pwm = 50
            elif temp <= self.step_temp[1]:
                target_pwm = 70
            elif temp <= self.step_temp[2]:
                target_pwm = 85
            else:
                target_pwm = 100
        return target_pwm
    ...
    def execute(self, thermal_info_dict):
        ...
        if ThermalInfo.INFO_NAME in thermal_info_dict and \
           isinstance(thermal_info_dict[ThermalInfo.INFO_NAME], ThermalInfo):
            ...
            thermals_data = list(thermal_info_obj.get_inlet_thermals_data().values())
...
class SwitchPolicyAction(ThermalPolicyActionBase):
    ...
    def execute(self, thermal_info_dict):
        ...
        import time
        time.sleep(30)
        # Power off SB & COMe through CPLD
        api_helper = APIHelper()
        api_helper.lpc_setreg(LPC_CPLD_SETREG_PATH, 0xA20, 0x3f)
        api_helper.lpc_setreg(LPC_CPLD_SETREG_PATH, 0xA26, 0x7f)
...
class SystemPolicyAction(ThermalPolicyActionBase):
    ...
    def execute(self, thermal_info_dict):
        ...
        # Step1: set all fan speed to 100%
        self.fan.set_all_fan_speed(thermal_info_dict, 100)   
        # Step2: Wait for 120s then shutdown
        import time
        time.sleep(120)
        # Power off SB & COMe through CPLD
        api_helper = APIHelper()
