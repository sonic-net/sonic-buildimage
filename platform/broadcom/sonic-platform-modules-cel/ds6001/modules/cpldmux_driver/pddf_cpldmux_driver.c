 ...
 *
 * Based on: PDDF's pddf_cpldmux_driver.c
 * Definition of CPLDMUX to access XCVR status
 */
...
int pddf_cpldmux_select(struct i2c_mux_core *muxc, uint32_t chan)
{
    ...
    if ( (pdata->chan_cache != 1) || (private->last_chan != chan) )
    {
        ...
        // pddf_dbg(CPLDMUX, KERN_INFO "%s: Writing 0x%x at 0x%x offset of cpld 0x%x to enable chan %d\n", __func__, sdata->cpld_sel, sdata->cpld_offset, pdata->cpld->addr, chan);
        dev_dbg(muxc->dev, "%s: Writing 0x%x at 0x%x offset of cpld 0x%x to enable chan %d\n", __func__, sdata->cpld_sel, sdata->cpld_offset, pdata->cpld->addr, chan);
