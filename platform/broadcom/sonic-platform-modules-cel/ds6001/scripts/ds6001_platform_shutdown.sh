...
# get platform x86_64-cls_ds6000-r0, not used in this file
...
echo "grn_amb_4hz" > /sys/bus/platform/devices/sys_cpld/sys_led
...
if [[ "$bmc_present" -eq 0 ]]; then
    ...
    echo 0xA64 0x00 > /sys/devices/platform/sys_cpld/setreg &> /dev/null 
fi
