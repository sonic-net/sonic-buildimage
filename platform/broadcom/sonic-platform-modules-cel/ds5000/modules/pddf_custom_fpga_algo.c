 ...
 * FPGAPCI APIs
 */
int fpgapci_read(uint32_t offset)
{
    int data;
...
int fpgapci_write(uint32_t offset, uint32_t value)
{
...
static int __init pddf_custom_fpga_algo_init(void)
{
    ...
    ptr_fpgapci_read = fpgapci_read;
    ptr_fpgapci_write = fpgapci_write;
    return 0;
}
