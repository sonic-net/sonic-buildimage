...
class LinearFanController():
    ...
    def calc_fan_speed(self, thermal_data):
        ...
        descend = not thermal_data.temp_cooling
        ...
        if temp <= low_temp:
            sonic_logger.log_debug(
                "[LinearController] temp: {} equal or lower than low temp: {}, "
                "set to lowest pwm: {}".format(temp, low_temp, self._low_pwm))
            self._last_pwm = self._low_pwm
            return self._low_pwm
        elif temp >= high_temp:
            sonic_logger.log_debug(
                "[LinearController] temp: {} equal or higher than high temp: {}, "
                "set to highest pwm: {}".format(temp, high_temp, self._high_pwm))
            ...
            self._last_pwm = pwm
            sonic_logger.log_debug(
                "[LinearController] temp: {}, slope: {}, low_temp: {}, low_pwm: {}, "
                "set to pwm: {}".format(temp, self._linear_slope, low_temp, self._low_pwm, pwm))
            return round(pwm)


class PIDFanController():
    """
    Common FAN PID controller for CPU, switch, OSFP Temp
    """
    MAX_SPEED = 100
    MIN_SPEED = 45
...
class ThermalAlgorithmAction(SetFanSpeedAction):
    ...
    SWITCH_PID_PARAMS = "switch_pid_params"
    ...
    def load_from_json(self, json_obj):
        ...
        Construct ThermalAlgorithmAction via JSON. JSON example:
            {
                ...
                "switch_pid_params": [88, 4, 0.3, 0.4],
        ...
        if self.SWITCH_PID_PARAMS in json_obj:
            switch_pid_params = json_obj[self.SWITCH_PID_PARAMS]
            if not isinstance(switch_pid_params, list) or len(switch_pid_params) != 4:
                raise ValueError('ThermalAlgorithmAction invalid SetPoint PID {} in JSON policy file, valid value should be [point, p, i, d]'.
                                 format(switch_pid_params))
            self.switch_pid_params = switch_pid_params
        else:
            raise ValueError('ThermalAlgorithmAction missing mandatory field [setpoint, p, i, d] in JSON policy file')
        ...
        sonic_logger.log_info("[ThermalAlgorithmAction] cpu_pid: {}, bcm_pid: {}, f2b_linear: {}, b2f_linear: {}".format(
            self.cpu_pid_params, self.switch_pid_params, self.f2b_linear_params, self.b2f_linear_params))
    ...
    def execute(self, thermal_info_dict):
        ...
        if self.switch_fan_controller == None:
            self.switch_fan_controller = PIDFanController(self.switch_pid_params[0], self.switch_pid_params[1],
                                                          self.switch_pid_params[2], self.switch_pid_params[3])
        ...
        if ThermalInfo.INFO_NAME in thermal_info_dict and \
           isinstance(thermal_info_dict[ThermalInfo.INFO_NAME], ThermalInfo):
            ...
            switch_thermal_data = thermals_data["TH5_CORE_TEMP"]
            switch_fan_pwm = self.switch_fan_controller.calc_fan_speed(switch_thermal_data)
            if self.sys_airflow == 'INTAKE':
                thermal_data = thermals_data["BB_TEMP_U3"]
                linear_fan_pwm1 = self.linear_fan_controller.calc_fan_speed(thermal_data)
                thermal_data = thermals_data["BB_TEMP_U68"]
                linear_fan_pwm2 = self.linear_fan_controller.calc_fan_speed(thermal_data)
            else:
                thermal_data = thermals_data["SB_TEMP2"]
                linear_fan_pwm1 = self.linear_fan_controller.calc_fan_speed(thermal_data)
                thermal_data = thermals_data["SB_TEMP1"]
                linear_fan_pwm2 = self.linear_fan_controller.calc_fan_speed(thermal_data)
            target_fan_pwm = max(cpu_fan_pwm, switch_fan_pwm, linear_fan_pwm1, linear_fan_pwm2)
            sonic_logger.log_info("[ThermalAlgorithmAction] cpu_pid_pwm: {}, switch_pid_pwm: {}, linear_fan_pwm1: {}, linear_fan_pwm2: {}, target_pwm: {}".format(
                cpu_fan_pwm, switch_fan_pwm, linear_fan_pwm1, linear_fan_pwm2, target_fan_pwm))
...
class SwitchPolicyAction(ThermalPolicyActionBase):
    ...
    def execute(self, thermal_info_dict):
        ...
        api_helper.cpld_lpc_write(0xA20, 0x00)
