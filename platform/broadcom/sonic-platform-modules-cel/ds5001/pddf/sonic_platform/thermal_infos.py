...
class FanInfo(ThermalPolicyInfoBase):
    ...
    INFO_NAME = 'fan_info'
    FAN_LOW_WARNING_SPEED = 10308
    FAN_HIGH_WARNING_SPEED = 47500
    FAN_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld/fan_led"
            ...
            for fantray in chassis.get_all_fan_drawers():
                ...
                for fan in fantray.get_all_fans():
                    ...
                    if fan_rpm > self.FAN_HIGH_WARNING_SPEED and fan not in self._high_warning_fans:
                        ...
                        sonic_logger.log_warning("FAN {} rpm speed {}, high speed warning".format(fan_name, fan_rpm))
...
class PsuInfo(ThermalPolicyInfoBase):
    ...
    """
    INFO_NAME = 'psu_info'
    PSU_HIGH_FAN_SPEED_THRESHOLD = 36800
    PSU_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld/pwr_led"
    ...
    def collect(self, chassis):
        ...
        try:
            for psu in chassis.get_all_psus():
                ...
                # PSU Fan high speed failure
                for fan in psu.get_all_fans():
                    psu_fan_name = fan.get_name()
                    psu_fan_rpm = fan.get_speed_rpm()

                    if psu_fan_rpm is None:
                        continue

                    if psu_fan_rpm > self.PSU_HIGH_FAN_SPEED_THRESHOLD and psu not in self._high_warning_psus:
                        self._high_warning_psus.add(psu)
                        sonic_logger.log_warning("PSU {} Fan speed {}, high speed warning".format(psu_fan_name, psu_fan_rpm))
                    elif psu_fan_rpm < self.PSU_HIGH_FAN_SPEED_THRESHOLD and psu in self._high_warning_psus:
                        self._high_warning_psus.remove(psu)
                        sonic_logger.log_notice("PSU {} restore from high speed warning".format(psu_fan_name))
...
class ThermalInfo(ThermalPolicyInfoBase):
    ...
    THERMAL_SHUTDOWN_TEMP_MAP = {"TH5_CORE_TEMP": 115}
    ALARM_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld/alarm_led"
