...
#define DRIVER_NAME			"ds5001_fpgapci"
#define MAX_PCI_NUM_BARS	6

struct pci_driver ds5001_fpgapci_driver;
struct pci_device_id *ds5001_fpgapci_ids=NULL;
...
static int ds5001_fpgapci_probe(struct pci_dev *dev, const struct pci_device_id *id);
static void ds5001_fpgapci_remove(struct pci_dev *dev);
...
static int pddf_pci_add_adapter(struct pci_dev *dev)
{
	...
	for (i = 0 ; i < total_i2c_pci_bus; i ++) {
		...
		/* Add by sandy for skip i2c-114 & i2c - 115 since these two are skiped by FPGA */
		/*
		if(i == 14 || i == 15)
		{
			pddf_dbg(FPGA, "....pddf_pci_add_adapter skip i2c-%d due to FPGA design limitation\n", i);
			continue;
		}
        */
...
static void pddf_pci_del_adapter(void)
{
	...
	for( i = 0; i < total_i2c_pci_bus; i++ ){
		if(i == 14 || i == 15)
		{
			pddf_dbg(FPGA, "....pddf_pci_del_adapter  skip i2c-%d due to FPGA design limitation\n", i);
			continue;
		}
...
static int map_bars(struct fpgapci_devdata *pci_privdata, struct pci_dev *dev)
{
	...
	if (FPGAPCI_BAR_INDEX != -1) {
        ...
        //added by sandy for test
		if (!pci_privdata->fpga_data_base_addr) {
            pddf_dbg(FPGA, KERN_INFO "[%s] ioremap for data base failed!\n", __FUNCTION__);
            return -ENOMEM;
        }
		//added end
...
int ds5001_fpgapci_register(FPGA_OPS_DATA* ptr_ops_data)
{
    ...
    if ((ds5001_fpgapci_ids=kmalloc(size, GFP_KERNEL)) == NULL) {
		pddf_dbg(FPGA, KERN_INFO "%s kmalloc failed\n", __FUNCTION__);
		return 0;
	}

	memcpy(ds5001_fpgapci_ids, fpgapci_ids, size);

    ds5001_fpgapci_driver.name=DRIVER_NAME;
    ds5001_fpgapci_driver.id_table=ds5001_fpgapci_ids;
    ds5001_fpgapci_driver.probe=ds5001_fpgapci_probe;
    ds5001_fpgapci_driver.remove=ds5001_fpgapci_remove;

    if (pci_register_driver(&ds5001_fpgapci_driver)) {
		pddf_dbg(FPGA, KERN_INFO "%s: pci_unregister_driver\n", __FUNCTION__);
		pci_unregister_driver(&ds5001_fpgapci_driver);
		return -ENODEV;
    }
	return 0;
}

EXPORT_SYMBOL(ds5001_fpgapci_register);

static int __init ds5001_fpgapci_driver_init(void)
{
    pddf_dbg(FPGA, KERN_INFO "[%s]\n", __FUNCTION__);
...
static void __exit ds5001_fpgapci_driver_exit(void)
{
    pddf_dbg(FPGA, KERN_INFO "[%s]\n", __FUNCTION__);

    if (ds5001_fpgapci_ids) {
        /* unregister this driver from the PCI bus driver */
        pci_unregister_driver(&ds5001_fpgapci_driver);
        kfree(ds5001_fpgapci_ids);
    }

}


module_init (ds5001_fpgapci_driver_init);
module_exit (ds5001_fpgapci_driver_exit);
MODULE_LICENSE("GPL");
