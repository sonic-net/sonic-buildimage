/*
 * ds5001_fpgapci_module.c - Platform-specific sysfs loader for DS5001 FPGA
 *
...
/* This kobject will be created under /sys/bus/i2c/devices/ */
static struct kobject *ds5001_fpgapci_kobj;

FPGA_OPS_DATA tmp_pddf_fpga_ops_data={0};
/* Declaration for the function in our platform-specific driver */
extern int ds5001_fpgapci_register(FPGA_OPS_DATA *ptr);
...
static struct attribute* attrs_fpgapci[]={
...
static struct attribute_group attr_group_fpgapci={
	.attrs = attrs_fpgapci,
};
...
static ssize_t dev_operation(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
	if(strncmp(buf, "fpgapci_init", strlen("fpgapci_init"))==0 ) {
		...
		/* Call our platform-specific register function */
		ds5001_fpgapci_register(pddf_fpga_ops_data);
	}
	else {
...
static int __init ds5001_fpga_data_init(void)
{
	int ret = 0;
    ...
    /* Create a kobject with a unique name for our platform */
	ds5001_fpgapci_kobj = kobject_create_and_add("fpgapci", device_kobj);
	if(!ds5001_fpgapci_kobj) {
		pddf_dbg(FPGA, KERN_ERR "%s create fpgapci_cfg kobj failed ..\n", __FUNCTION__);
		return -ENOMEM;
	}

	ret = sysfs_create_group(ds5001_fpgapci_kobj, &attr_group_fpgapci);
	if (ret)
	{
		pddf_dbg(FPGA, KERN_ERR "%s create fpga sysfs attributes failed ..\n", __FUNCTION__);
        kobject_put(ds5001_fpgapci_kobj);
		return ret;
	}
...
static void  __exit ds5001_fpga_data_exit(void)
{
	pddf_dbg(FPGA, KERN_INFO "%s ..\n", __FUNCTION__);
	KOBJ_FREE(ds5001_fpgapci_kobj)
	return;
}

module_init(ds5001_fpga_data_init);
module_exit(ds5001_fpga_data_exit);

MODULE_AUTHOR("Celestica");
MODULE_DESCRIPTION("DS5001 FPGA platform data");
MODULE_LICENSE("GPL");
