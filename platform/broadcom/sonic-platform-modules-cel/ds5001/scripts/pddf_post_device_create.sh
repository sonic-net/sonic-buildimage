...
if [ "$bmc_present" = "False" ]; then
    echo mp2975 0x64 > /sys/bus/i2c/devices/i2c-110/new_device
    echo mp2975 0x66 > /sys/bus/i2c/devices/i2c-110/new_device
    echo mp2975 0x6B > /sys/bus/i2c/devices/i2c-110/new_device
    echo mp2975 0x6D > /sys/bus/i2c/devices/i2c-110/new_device

    #Set all FAN speed to 50%
    i2cset -y -f 109 0x0d 0x44 0x7f
