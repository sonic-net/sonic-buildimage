...
static ssize_t optoe_port_enable(struct i2c_client *client)
{
	int bus;
	int port;
	int present, enable, reset;
	int tmp;

	bus = i2c_adapter_id(client->adapter);
	port = QSFP_PORT_ID(bus);
	present = inb(QSFP28_CTRL_REG1) & QSFP28_MODULE_PRESENT_MASK;
	enable = inb(QSFP28_CTRL_REG3) & QSFP28_MODULE_ENABLED_MASK;
	reset = inb(QSFP28_RESET_REG) & QSFP28_MODULE_RESET_MASK;

	dev_dbg(&client->dev,
		"%s: bus: %d  port: %d",
		__func__, bus, port);

	/* enable module atached to the port */
	if (enable & (QSFP28_MODULE_NOT_ENABLED << (port - 1))) {
		tmp = enable & ~(1 << (port - 1));
		outb(tmp, QSFP28_CTRL_REG3);
	}

	/* make sure module attached to the port is not in reset mode */
	if (!(reset & (QSFP28_MODULE_NOT_RESET << (port - 1)))) {
		tmp = reset | (1 << (port - 1));
		outb(tmp, QSFP28_RESET_REG);
	}
		...
		char *buf, loff_t off, size_t len, int opcode)
{
	...
	/*
	 * Validate and enable the QSFP port
	 */
	retval = optoe_port_enable(client);
	if (retval) {
