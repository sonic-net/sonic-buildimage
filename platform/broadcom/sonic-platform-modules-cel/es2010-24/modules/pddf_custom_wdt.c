...
#define WDT_CONTROL_BASE		 0xA200
#define TEST_SCRATCH_REG		 0xA201
#define REBOOT_CAUSE_REG		 0xA205
#define WDT_SET_TIMER_H_BIT_REG		 0xA261
#define WDT_SET_TIMER_M_BIT_REG		 0xA262
#define WDT_SET_TIMER_L_BIT_REG		 0xA263
#define WDT_TIMER_H_BIT_REG		 0xA264
#define WDT_TIMER_M_BIT_REG		 0xA265
#define WDT_TIMER_L_BIT_REG		 0xA266
#define WDT_ENABLE_REG			 0xA267
#define WDT_FEED_REG			 0xA268
#define WDT_PUNCH_REG			 0xA269
...
static int watchdog_notify_sys(struct notifier_block *this, unsigned long code,
	void *unused)
{
	struct watchdog_noti_ctx *ctx = container_of(this, struct watchdog_noti_ctx, nb);
	if (likely(ctx)) {
		if (likely(ctx->p))
		{
			pr_info("Stopping CPLD watchdog. code %lu\n", code);
			watchdog_stop(ctx->p);
		}
		else
		{
			pr_err("CPLD watchdog is not Stopped. code %lu\n", code);
		}
	}
	else {
		pr_err("CPLD watchdog is not Stopped. code %lu\n", code);
...
static struct watchdog_noti_ctx watchdog_notifier = {
	.nb.notifier_call = watchdog_notify_sys,
	.p = NULL,
};
...
static int cpld_wdt_probe(struct platform_device *pdev)
{
	...
	if (timeout) {
		...
		pr_info("watchdog initial timeout is set to %u Second(s)\n", timeout/1000);
	}
    ...
    watchdog_notifier.p	= p;
	err = register_reboot_notifier(&watchdog_notifier.nb);
	if (err)
		return err;
...
static void cpld_wdt_remove(struct platform_device *pdev)
{
	...
	unregister_reboot_notifier(&watchdog_notifier.nb);
