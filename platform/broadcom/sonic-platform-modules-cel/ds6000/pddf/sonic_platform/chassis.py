...
class Chassis(PddfChassis):
    ...
    def __initialize_components(self):
        ...
        self.NUM_COMPONENT = len(Component.COMPONENT_LIST)
        '''
    ...
    def set_system_led(self, device_name, color):
        ...
        if device_name == "SYS_LED":
            val = nine_color_dict.get(color, 0xf)
            if val == 0xf: return False                
            return self._api_helper.cpld_lpc_write(0x0A62, val)
        if device_name == "ALARM_LED":
            val = nine_color_dict.get(color, 0xf)
            if val == 0xf: return False                
            return self._api_helper.cpld_lpc_write(0x0A63, val)
        if device_name == "OOB_LED":
            val = nine_color_dict.get(color, 0xf)
            if val == 0xf: return False                
            return self._api_helper.cpld_lpc_write(0x0A66, val)
        if device_name == "PSU_Front_LED":
            val = four_color_dict.get(color, 0x0)
            if val == 0x0: return False
            return self._api_helper.cpld_lpc_write(0x0A61, val)
        if device_name == "FAN_Front_LED":
            val = nine_color_dict.get(color, 0x0)
            if val == 0x0: return False
            return self._api_helper.cpld_lpc_write(0x0A65, val)
    ...
    def get_system_led(self, device_name):
        ...
        if device_name == "SYS_LED":
            status, result = self._api_helper.cpld_lpc_read(0x0A62)
            if status == True:
                result = int(result, 16) & 0x33
            else:
                return self.STATUS_LED_COLOR_UNKNOWN
            return ten_status_led.get(result, self.STATUS_LED_COLOR_UNKNOWN)
        if device_name == "ALARM_LED":
            status, result = self._api_helper.cpld_lpc_read(0x0A63)
            if status == True:
                result = int(result, 16) & 0x33
            else:
                return self.STATUS_LED_COLOR_UNKNOWN
            return ten_status_led.get(result, self.STATUS_LED_COLOR_UNKNOWN)
        if device_name == "OOB_LED":
            status, result = self._api_helper.cpld_lpc_read(0x0A66)
            if status == True:
                result = int(result, 16) & 0x33
            else:
                return self.STATUS_LED_COLOR_UNKNOWN            
            return ten_status_led.get(result, self.STATUS_LED_COLOR_UNKNOWN)
        if device_name == "PSU_Front_LED":
            status, result = self._api_helper.cpld_lpc_read(0x0A61)
            if status == True:
                result = int(result, 16) & 0x3
            else:
                return self.STATUS_LED_COLOR_UNKNOWN
            return four_status_led.get(result)
        if device_name == "FAN_Front_LED":
            status, result = self._api_helper.cpld_lpc_read(0x0A65)
            if status == True:
                result = int(result, 16) & 0x33
            else:
                return self.STATUS_LED_COLOR_UNKNOWN
            return ten_status_led.get(result)
        if device_name == "FANTRAY1_LED":
            return self._fan_drawer_list[0].get_status_led()
        if device_name == "FANTRAY2_LED":
            return self._fan_drawer_list[1].get_status_led()
        if device_name == "FANTRAY3_LED":
            return self._fan_drawer_list[2].get_status_led()
        if device_name == "FANTRAY4_LED":
            return self._fan_drawer_list[3].get_status_led()
        if device_name == "FANTRAY5_LED":
            return self._fan_drawer_list[4].get_status_led()
        if device_name == "PSU1_LED":
            return self._psu_list[0].get_status_led()
        if device_name == "PSU2_LED":
            return self._psu_list[1].get_status_led()
        if device_name == "PSU3_LED":
            return self._psu_list[2].get_status_led()
        if device_name == "PSU4_LED":
