...
class NoFantrayFailureCondition(FanCondition):
    def is_match(self, thermal_info_dict):
        fan_info_obj = self.get_fan_info(thermal_info_dict)
        return len(fan_info_obj.get_absence_fantrays()) == 0 if fan_info_obj else True
...
class NoFanRotorFailureCondition(FanCondition):
    def is_match(self, thermal_info_dict):
        fan_info_obj = self.get_fan_info(thermal_info_dict)
...
@thermal_json_object('psu.fan.any.failure')
class AnyPsuFanFailureCondition(PsuCondition):
    def is_match(self, thermal_info_dict):
        psu_info_obj = self.get_psu_info(thermal_info_dict)
        return len(psu_info_obj.get_failure_psu_fans()) > 0 if psu_info_obj else False

@thermal_json_object('psu.fan.no.failure')
class NoPsuFanFailureCondition(PsuCondition):
    def is_match(self, thermal_info_dict):
        psu_info_obj = self.get_psu_info(thermal_info_dict)
        return len(psu_info_obj.get_failure_psu_fans()) == 0 if psu_info_obj else True
...
class ThermalOverHighCriticalThresholdCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
        thermal_info_obj = self.get_thermal_info(thermal_info_dict)
        if thermal_info_obj:
...
@thermal_json_object('thermal.inlet.over.high_threshold')
class ThermalInletOverHighThresholdCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
        thermal_info_obj = self.get_thermal_info(thermal_info_dict)
        if thermal_info_obj:
            return thermal_info_obj.is_any_inlet_over_high_threshold()
        else:
            return False

@thermal_json_object('thermal.inlet.below.high_threshold')
class ThermalInletBelowHighThresholdCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
        thermal_info_obj = self.get_thermal_info(thermal_info_dict)
        if thermal_info_obj:
            return thermal_info_obj.is_any_inlet_below_high_threshold()
        else:
            return False

@thermal_json_object('thermal.inlet.over.high_critical_threshold')
class ThermalInletOverHighCriticalThresholdCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
        thermal_info_obj = self.get_thermal_info(thermal_info_dict)
        if thermal_info_obj:
...
class ThermalInletBelowHighCriticalThresholdCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
        thermal_info_obj = self.get_thermal_info(thermal_info_dict)
        if thermal_info_obj:
            return thermal_info_obj.is_any_inlet_below_high_critical_threshold()
        else:
            return False
