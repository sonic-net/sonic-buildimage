...
class FanInfo(ThermalPolicyInfoBase):
    ...
    INFO_NAME = 'fan_info'
    FAN_LOW_WARNING_SPEED = 1800
    FAN_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld1/fan_led"
    ...
    def _update_led_status(self):
        if not os.path.exists(self.FAN_LED_SYSFS_PATH):
            return

        try:
            is_any_absent = len(self._absence_fantrays) > 0 or len(self._absence_fans) > 0
            target_color = "amber" if is_any_absent else "green"

            if self._current_led_color != target_color:
                with open(self.FAN_LED_SYSFS_PATH, "w") as fd:
                    fd.write(target_color)

                self._current_led_color = target_color

        except Exception as e:
            self._current_led_color = None
            sonic_logger.log_warning("Failed to set fan status LED via sysfs due to: {}".format(e))
...
class PsuInfo(ThermalPolicyInfoBase):
    ...
    """
    INFO_NAME = 'psu_info'
    PSU_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld2/pwr_led"


    def __init__(self):
        self._absence_psus = set()
        self._presence_psus = set()
        self._failure_psu_fans = set()
        self._status_changed = False
        self._psus_status = True
        self._current_led_color = None
        ...
        try:
            for psu in chassis.get_all_psus():
                ...
                for fan in psu.get_all_fans():
                    psu_fan_rpm = fan.get_speed_rpm()
                    if psu_fan_rpm is not None:
                        is_fan_bad = psu_fan_rpm < 1
                    else:
                        is_fan_bad = True

                    if is_fan_bad:
                        if fan not in self._failure_psu_fans:
                            self._failure_psu_fans.add(fan)
                    else:
                        if fan in self._failure_psu_fans:
                            self._failure_psu_fans.remove(fan)

            for psu in chassis.get_all_psus():
                if not psu.get_status():
                    self._psus_status = False
                    break
    ...
    def _update_led_status(self):
        if not os.path.exists(self.PSU_LED_SYSFS_PATH):
            return

        try:
            target_color = "amber" if not self._psus_status else "green"

            if self._current_led_color != target_color:
                with open(self.PSU_LED_SYSFS_PATH, "w") as fd:
                    fd.write(target_color)
            ...
            self._current_led_color = None
            sonic_logger.log_warning("Failed to set psu status LED via sysfs due to: {}".format(e))
    ...
    def get_failure_psu_fans(self):
        """
        Retrieves failure psu fans
        :return: A set of failure psu fans
        """
        return self._failure_psu_fans
...
class ThermalInfo(ThermalPolicyInfoBase):
    ...
    F2B_INLET_SENSORS = ["F2B INLET TEMP1", "F2B INLET TEMP2"]
    B2F_INLET_SENSORS = ["B2F INLET TEMP1", "B2F INLET TEMP2"]
    THERMAL_SHUTDOWN_TEMP_MAP = {"CPU TEMP": 93}
    THERMAL_INLET_SENSORS = F2B_INLET_SENSORS
    THERMAL_INLET_HIGH_TEMP = 50
    THERMAL_INLET_CRITICAL_TEMP = 55
    THERMAL_CPU_HIGH_TEMP = 88
    THERMAL_CPU_CRITICAL_TEMP = 93
    THERMAL_TEMP_WARN_TOL = 2
    ALARM_LED_SYSFS_PATH = "/sys/bus/platform/devices/sys_cpld1/alarm_led"
    ...
    def collect(self, chassis):
        ...
        """
        try:
            sys_airflow = chassis.get_system_airflow()
            if sys_airflow == "INTAKE":
                self.THERMAL_INLET_SENSORS = self.B2F_INLET_SENSORS
                self.THERMAL_SHUTDOWN_TEMP_MAP.update({"B2F INLET TEMP1": 55, "B2F INLET TEMP2": 55})
            else:
                self.THERMAL_INLET_SENSORS = self.F2B_INLET_SENSORS
                self.THERMAL_SHUTDOWN_TEMP_MAP.update({"F2B INLET TEMP1": 55, "F2B INLET TEMP2": 55})
            ...
            for thermal in chassis.get_all_thermals():
                ...
                if name in self.THERMAL_INLET_SENSORS or "CPU" in name:
                ...
                if high_threshold != None and high_threshold != 'N/A':
                    ...
                    elif temp < (high_threshold - self.THERMAL_TEMP_WARN_TOL) and thermal in self._high_warning_thermals:
                ...
                if high_critical_threshold != None and high_critical_threshold != 'N/A':
                    ...
                    elif temp < (high_critical_threshold - self.THERMAL_TEMP_WARN_TOL) and thermal in self._high_critical_thermals:
        ...
        self._update_led_status(chassis)

    def _update_led_status(self, chassis):
        if not os.path.exists(self.ALARM_LED_SYSFS_PATH):
            return

        try:
            thermal_manager = chassis.get_thermal_manager()
            info_dict = thermal_manager._thermal_info_dict

            fan_info = info_dict.get('fan_info')
            psu_info = info_dict.get('psu_info')

            fan_color = fan_info.get_led_color() if fan_info else "green"
            psu_color = psu_info.get_led_color() if psu_info else "green"

            target_color = "amber" if (fan_color == "amber" or psu_color == "amber") else "green"

            if self._current_led_color != target_color:
                with open(self.ALARM_LED_SYSFS_PATH, "w") as fd:
                    fd.write(target_color)
                self._current_led_color = target_color

        except Exception as e:
            self._current_led_color = None
            sonic_logger.log_warning("Failed to set alarm status LED via sysfs due to: {}".format(e))
    ...
    def is_any_over_high_critical_threshold(self):
        """
        Retrieves if the temperature is warm up and over high critical threshold
        :return: True if the temperature is warm up and over high threshold else False
        """
        return len(self._high_critical_thermals) > 0

    def is_any_inlet_over_high_threshold(self):
        """
        Retrieves if the temperature is warm up and over high threshold
        :return: True if the temperature is warm up and over high threshold else False
        """
        for thermal_info in list(self.get_thermals_data().values()):
            if thermal_info.name in self.THERMAL_INLET_SENSORS:
                if thermal_info.curr_temp > self.THERMAL_INLET_HIGH_TEMP:
                    return True
            if "CPU" in thermal_info.name:
                if thermal_info.curr_temp > self.THERMAL_CPU_HIGH_TEMP:
                    return True
        return False

    def is_any_inlet_below_high_threshold(self):
        """
        Retrieves if the temperature is below high threshold
        :return: True if the temperature is below high threshold else False
        """
        for thermal_info in list(self.get_thermals_data().values()):
            if thermal_info.name in self.THERMAL_INLET_SENSORS:
                if thermal_info.curr_temp < self.THERMAL_INLET_HIGH_TEMP - self.THERMAL_TEMP_WARN_TOL:
                    return True
            if "CPU" in thermal_info.name:
                if thermal_info.curr_temp < self.THERMAL_CPU_HIGH_TEMP - self.THERMAL_TEMP_WARN_TOL:
                    return True
        return False
    ...
    def is_any_inlet_over_high_critical_threshold(self):
        """
        Retrieves if the temperature is warm up and over high critical threshold
        :return: True if the temperature is warm up and over high critical threshold else False
        """
        for thermal_info in list(self.get_thermals_data().values()):
            if thermal_info.name in self.THERMAL_INLET_SENSORS:
                if thermal_info.curr_temp > self.THERMAL_INLET_CRITICAL_TEMP:
                    return True
            if "CPU" in thermal_info.name:
                if thermal_info.curr_temp > self.THERMAL_CPU_CRITICAL_TEMP:
                    return True
        return False

    def is_any_inlet_below_high_critical_threshold(self):
        """
        Retrieves if the temperature is below high critical threshold
        :return: True if the temperature is below high critical threshold else False
        """
        for thermal_info in list(self.get_thermals_data().values()):
            if thermal_info.name in self.THERMAL_INLET_SENSORS:
                if thermal_info.curr_temp < self.THERMAL_INLET_CRITICAL_TEMP - self.THERMAL_TEMP_WARN_TOL:
                    return True
            if "CPU" in thermal_info.name:
                if thermal_info.curr_temp < self.THERMAL_CPU_CRITICAL_TEMP - self.THERMAL_TEMP_WARN_TOL:
                    return True
        return False
