...
#ifdef CONFIG_CIENA_INTEL_SPI_MTD_PARTITIONS
#define FREG_EX(n)			(0xe0 + ((n) * 4))
#endif
...
#ifdef CONFIG_CIENA_INTEL_SPI_MTD_PARTITIONS
module_param(writeable, bool, S_IRUGO);
#else
...
static void intel_spi_dump_regs(struct intel_spi *ispi)
{
...
#ifdef CONFIG_CIENA_INTEL_SPI_MTD_PARTITIONS
	for (i = 0; i < BXT_FREG_EX_NUM; i++)
		dev_dbg(ispi->dev, "FREG(%ld)=0x%08x\n", ispi->nregions + i,
			readl(ispi->base + FREG_EX(i)));
#endif
	...
	for (i = 0; i < BXT_FREG_EX_NUM; i++) {
		...
		region = readl(ispi->base + FREG_EX(i));
		base   = region & FREG_BASE_MASK;
		limit  = (region & FREG_LIMIT_MASK) >> FREG_LIMIT_SHIFT;

		if (base >= limit || (i > 0 && limit == 0))
			dev_dbg(ispi->dev, " %02ld disabled\n", ispi->nregions + i);
		else
			dev_dbg(ispi->dev, " %02ld base: 0x%08x limit: 0x%08x\n",
				ispi->nregions + i, base << 12, (limit << 12) | 0xfff);
	}
#endif
...
#ifdef CONFIG_CIENA_INTEL_SPI_MTD_PARTITIONS
	intel_spi_fill_partition(ispi, &pdata->parts[idx]);
	pdata->parts[idx].offset = 0x03000000;
	pdata->parts[idx].size   = 0x00050000;
	pdata->parts[idx].name   = "EFIVAR Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x03050000;
	pdata->parts[idx].size   = 0x00FB0000;
	pdata->parts[idx].name   = "BIOS Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x00041000;
	pdata->parts[idx].size   = 0x02597000;
	pdata->parts[idx].name   = "ME Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x025d8000;
	pdata->parts[idx].size   = 0x00010000;
	pdata->parts[idx].name   = "PDR Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x025e8000;
	pdata->parts[idx].size   = 0x00A00000;
	pdata->parts[idx].name   = "NAC NIS FW Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x00001000;
	pdata->parts[idx].size   = 0x00040000;
	pdata->parts[idx].name   = "NAC iRC FW Region";

	intel_spi_fill_partition(ispi, &pdata->parts[++idx]);
	pdata->parts[idx].offset = 0x02fe8000;
	pdata->parts[idx].size   = 0x00018000;
	pdata->parts[idx].name   = "Intel(R) PTT Region";
#else
