...
class LedControl(LedControlBase):
    ...
    def __init__(self):
        ...
        # Clear the front-panel all-on override (front::all / enable_all_leds).
        # brightness=1 on that node is a lamp-test override that forces every LED
        # on; it must be 0 for the individual per-colour LEDs to be visible.
        try:
            get_chassis().initialize_system_leds()
        except Exception as e:
            sonic_logger.log_warning(f"Failed to clear front-panel all-on LED override: {e}")
    ...
    def port_link_state_change(self, port, port_state=None):
        """
        Called when port link state changes.

        ledd v2.0 calls this with (port_name, port_state) where
        port_state is "up" or "down".  We accept port_state but
        always re-read the authoritative state from the DB so the
        LED colour reflects the full picture (all subports, xcvr
        presence, admin vs oper).

        :param port: A string, SONiC port name (e.g., "Ethernet0")
        :param port_state: Optional "up"/"down" hint from ledd (unused).
        """
