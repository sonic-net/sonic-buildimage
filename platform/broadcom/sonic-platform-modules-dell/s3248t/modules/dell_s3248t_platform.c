 ...
 * dell_s3248t_platform.c - Driver for S3248T switches
 *
 ...
 */
    ...
    static struct cpld_platform_data s3248t_cpld_platform_data[] = {
        [cpu_cpld] = {
            .reg_addr = 0x31,
        },
    ...
    static struct platform_device s3248t_cpld = {
        .name               = "dell-s3248t-cpld",
        .id                 = 0,
        .dev                = {
                    .platform_data   = s3248t_cpld_platform_data,
    ...
    static struct mux_platform_data s3248t_mux_platform_data[] = {
        {
    ...
    static struct platform_device s3248t_mux[] = {
        {
            .name           = "dell-s3248t-mux",
            .id             = 0,
            .dev            = {
                    .platform_data   = &s3248t_mux_platform_data[0],
                    .release         = device_release,
            },
        },
        {
            ...
            .id             = 1,
            .dev            = {
                    .platform_data   = &s3248t_mux_platform_data[1],
                    .release         = device_release,
            },
        },
        {
            ...
            .id             = 2,
            .dev            = {
                    .platform_data   = &s3248t_mux_platform_data[2],
                    .release         = device_release,
            },
        },
    };
...
static struct attribute *s3248t_cpld_attrs[] = {
...
static struct attribute_group s3248t_cpld_attr_grp = {
    .attrs = s3248t_cpld_attrs,
};
...
static int __init cpld_probe(struct platform_device *pdev)
{
    ...
    ret = sysfs_create_group(&pdev->dev.kobj, &s3248t_cpld_attr_grp);
    if (ret)
        goto error;
...
static int __exit cpld_remove(struct platform_device *pdev)
{
    ...
    sysfs_remove_group(&pdev->dev.kobj, &s3248t_cpld_attr_grp);
...
static int __init dell_s3248t_platform_init(void)
{
    int ret = 0;
    ...
    printk("dell_s3248t_platform module initialization\n");
    ...
    ret = platform_device_register(&s3248t_cpld);
    if (ret) {
        printk(KERN_WARNING "Fail to create cpld device\n");
        goto error_cpld;
    }

    cpld_pdata = s3248t_cpld.dev.platform_data;

    for (i = 0; i < PF_MUX_DEVICES; i++) {
        pdata = s3248t_mux[i].dev.platform_data;
        pdata->cpld = cpld_pdata[sys_cpld].client;
        ret = platform_device_register(&s3248t_mux[i]);
        if (ret) {
...
error_mux:
    i--;
    for (; i >= 0; i--) {
        platform_device_unregister(&s3248t_mux[i]);
    }
    platform_device_unregister(&s3248t_cpld);
error_cpld:
...
static void __exit dell_s3248t_platform_exit(void)
{
    int i;
...
module_init(dell_s3248t_platform_init);
module_exit(dell_s3248t_platform_exit);

MODULE_DESCRIPTION("DELL S3248T Platform Support");
