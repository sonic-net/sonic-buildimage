#!/bin/bash
# Pre-driver-install hook for PDDF mode on Dell S5296F.
#
...
modprobe -r dell_s5296f_fpga_ocores 2>/dev/null
...
if lsmod | grep -q '^dell_s5296f_fpga_ocores '; then
    sleep 0.5
    modprobe -r dell_s5296f_fpga_ocores 2>/dev/null
    if lsmod | grep -q '^dell_s5296f_fpga_ocores '; then
        echo "WARNING: dell_s5296f_fpga_ocores still loaded - PDDF may have I/O conflicts" >&2
    fi
fi
