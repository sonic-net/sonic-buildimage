#!/usr/bin/env python3
"""Dell S5296F hooks for switching between PDDF and Non-PDDF mode.
...
DEVICE_DIR = "/usr/share/sonic/device/x86_64-dellemc_s5296f_c3538-r0"
...
BSP_SERVICE = "platform-modules-s5296f.service"
...
def unload_ipmi_modules():
    ...
    s5296f_platform.sh init loads it with kipmid_max_busy_us=1000, so kipmid must
...
def unload_bsp_fpga_modules():
    ...
    systemd-modules-load loads dell_s5296f_fpga_ocores at boot (from
    ...
    """
    for mod in ("dell_s5296f_fpga_ocores", "i2c_ocores"):
...
def reload_bsp_fpga_modules():
    ...
    dell_s5296f_fpga_ocores and i2c_ocores are removed by
    ...
    """
    for mod in ("i2c_ocores", "dell_s5296f_fpga_ocores"):
...
def restore_bsp_i2c_providers():
    ...
    pddf_pre_driver_install.sh unloads dell_s5296f_fpga_ocores, i2c-i801 and
    i2c_ismt before PDDF drivers are installed. s5296f_platform.sh 'init' only
    reloads i2c_ocores/dell_s5296f_fpga_ocores, so i2c-i801 and i2c_ismt never
...
def stop_platform_svc():
    ...
    # s5296f_platform.sh deinit cannot remove ipmi_si while kipmid is busy.
...
def start_platform_pddf():
    ...
    # systemd-modules-load loads dell_s5296f_fpga_ocores at every boot from
