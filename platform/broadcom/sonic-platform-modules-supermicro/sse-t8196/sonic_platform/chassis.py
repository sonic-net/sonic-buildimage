...
class Chassis(PddfChassis):
    ...
    def ipmitool_set_cmd(self, cmd_str):
        try:
            # Split the command string into a list
            cmd_list = shlex.split(cmd_str)
            output = subprocess.check_output(cmd_list, universal_newlines=True).strip()
        except Exception as err:
            print(f'Unexpected {err}, {type(err)}')
            return False
        return True

    def set_system_led(self, *args):
        # args[0]: LED, args[1]: color
        if args[0]:
            color = args[1]
            if color == 'green':
                # Turn off blinking
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x02 0x01 0x01"):
                    return False
                # Turn off red
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x03 0x01 0x00 0x01"):
                    return False
                # Turn on green
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x03 0x01 0x01 0x00"):
                    return False
                ...
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x03 0x01 0x01 0x00"):
                    return False
                # Turn on blinking
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x02 0x01 0x00"):
                    return False
            elif color == 'red':
                # Turn off blinking
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x02 0x01 0x01"):
                    return False
                # Turn off green
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x03 0x01 0x01 0x01"):
                    return False
                # Turn on red
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x03 0x01 0x00 0x00"):
                    return False
            elif color == 'off':
                # Turn off blinking
                if not self.ipmitool_set_cmd("ipmitool raw 0x30 0x89 0x02 0x01 0x01"):
