...
void RebootThread::do_reboot(void) {
  ...
  std::cout << " do_reboot: Entering" << std::endl;
  ...
  if (m_request.method() == RebootMethod::COLD) {
    do_cold_reboot(s);
  	...
  	std::cout << " do_reboot: Calling Wam_reboot" << std::endl;
    do_warm_reboot(s);
  } else {
...
RebootThread::Progress RebootThread::send_dbus_reboot_request() {
  ...
  std::cout << " send_dbus_reboot_request: Entering" << std::endl;
  if (!status.ok()) {
  ...
  std::cout << " send_dbus_reboot_request: sending" << std::endl;
...
void RebootThread::do_warm_reboot(swss::Select &s) {
  ...
  std::cout << " do_warm_reboot: Calling dbus" << std::endl;
  if (send_dbus_reboot_request() == Progress::EXIT_EARLY) {
    return;
  }
