...
#
# Get an ipv4 or ipv6 address from a given string.
# Return None if the given string is not a valid ipv4 address.
#

def get_ip_address(value, af=4):
    if not value:
        return None
...
#
# Extract Loopback0 ipv4 or ipv6 address
#

def get_loopback_ip(af):
    loopback0_ip = None
    ...
    try:
        ...
        for loopback in loopback_interface.getKeys():
            if loopback.startswith("Loopback0|"):
                ...
                ip_address = get_ip_address(loopback0_prefix_str, af)
                if ip_address is not None:
                    loopback0_ip = ip_address
                    break
