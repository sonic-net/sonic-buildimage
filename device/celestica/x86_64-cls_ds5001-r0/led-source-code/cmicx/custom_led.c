...
uint16 phy_port[] = {0, 393,397, 385,389, 409, 413, 401, 405, 425, 429, 417, 421, 441, 445, 433, 437,
                       457, 461, 449, 453,  473, 477, 465, 469, 481, 485, 489, 493, 497, 501,505, 509,
                       9, 13, 1, 5, 25, 29, 17, 21, 41, 45, 33, 37, 57, 61, 49, 53,
                       73, 77, 65, 69, 89, 93, 81, 85,105, 109, 97, 101,121, 125, 113, 117,
                       513,515};

void
customer_led_handler(soc_led_custom_handler_ctrl_t *ctrl, uint32 cnt)
{
    uint16 front_port, physical_port,led_location = 0;
    uint16 led_uc_port;
    ...
    for(front_port = 1; front_port < (FRONT_PORT_MAX*2-MGMT_PORT-1); front_port++) {
        ...
        led_location = front_port + (front_port % 2 ? 1 : -1); // exchange the adjacent ports, the sequence of the led is not the same as the port sequence
        LED_HW_RAM_WRITE16(ctrl->pat_ram_base, (led_location -1), pattern);

    }
    /* Process all front SFP+ ports. */
    for(front_port = (FRONT_PORT_MAX*2-MGMT_PORT-1); front_port < (FRONT_PORT_MAX*2-1); front_port++) {
        physical_port = phy_port[front_port];
