...
#define RJ45_PORT_MAX         24
#define QSFP28_PORT_MAX         2
...
customer_led_handler(soc_led_custom_handler_ctrl_t *ctrl, uint32 cnt)
{
    ...
    for (front_port = 1; front_port <= RJ45_PORT_MAX ; front_port++) {
        pattern = RJ45_ALL_LED_OFF; /* Initialize pattern - macros will set bits (0 for ON, 1 for OFF) */
        ...
        if (led_control_data & LED_SW_LINK_UP) { // link up
            ...
            has_activity = (accu_val & LED_HW_RX) || (accu_val & LED_HW_TX);
            is_blink_tick = (cnt & ACT_TICKS);
