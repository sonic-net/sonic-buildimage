...
DEVICE_CX864E_N_PATH = '/usr/share/sonic/device/x86_64-asterfusion_cx864e_n-r0/CX864E-N/'
CONFIG_DB_TMP_PATH = '/tmp/config_db.json'
CONFIG_DB_PATH = '/etc/sonic/config_db.json'
YAML_PATH = DEVICE_CX864E_N_PATH + 'config_64x800G_asterfusion-cx864e-n.yaml'
YAML_TMP_PATH = '/tmp/config_64x800G_asterfusion-cx864e-n.yaml'
MOD_FILE_ERR = -1
...
def mod_config_db(dict_target, port_id, flag, speed='100000'):
    ...
    try:
        if dict_target.has_key('VLAN'):
            for vlan in dict_target['VLAN'].keys():
                plist = dict_target['VLAN'][vlan]['members'] if dict_target['VLAN'][vlan].has_key('members') else []
                for interface in breakout_port_name_list:
    ...
    try:
        if dict_target.has_key('ACL_TABLE'):
            for acl_table in dict_target['ACL_TABLE'].keys():
            ...
            #         if dict_target.has_key('ACL_RULE'):
            #             for acl_rule in dict_target['ACL_RULE'].keys():
...
#CONFIG_32X100G_ASTERFUSION-CX864E-N.YAML
