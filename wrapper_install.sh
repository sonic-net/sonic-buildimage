...
chmod +x "$SONIE_BIN"
# Install SONIE using sonic-installer.
# Since no bootloader is found yet, sonic-installer will automatically fallback to NullBootloader
# and run the installer binary directly.
sonic-installer install -y --skip_migration --skip-platform-check --skip-package-migration --skip-setup-swap "$SONIE_BIN"
...
sonic-installer install -y --skip-platform-check --skip-package-migration --skip-setup-swap "$SONIC_BIN"
