...
$(addprefix $(FILES_PATH)/, $(SONIC_MAKE_FILES)) : $(FILES_PATH)/% : .platform $$(addsuffix -install,$$(addprefix $(DEBS_PATH)/,$$($$*_DEPENDS))) $$(addprefix $(PHONY_PATH)/,$$($$*_PHONIES)) \
	...
	$(call LOAD_CACHE,$*,$@)
		...
		# Apply series of patches if exist. See resolve_patch_path (rules/functions) for
		# the default "<SRC_PATH>.patch" layout vs. an explicit $($*_PATCH_PATH) override.
		if [ -f $(call resolve_patch_abspath,$*)/series ]; then pushd $($*_SRC_PATH) && ( quilt pop -a -f 1>/dev/null 2>&1 || true ) && QUILT_PATCHES=$(call resolve_patch_abspath,$*) quilt push -a; popd; fi $(LOG)
		...
		# Clean up
		if [ -f $(call resolve_patch_abspath,$*)/series ]; then pushd $($*_SRC_PATH) && quilt pop -a -f; [ -d .pc ] && rm -rf .pc; popd; fi $(LOG)
...
$(addprefix $(DEBS_PATH)/, $(SONIC_DPKG_DEBS)) : $(DEBS_PATH)/% : .platform $$(addsuffix -install,$$(addprefix $(PYTHON_WHEELS_PATH)/,$$($$*_WHEEL_DEPENDS))) $$(addsuffix -install,$$(addprefix $(DEBS_PATH)/,$$($$*_DEPENDS))) \
	...
	if [ -z '$($*_CACHE_LOADED)' ] ; then
		...
		$(call ARCHIVE_PATCHED_SOURCE,$*)
		# Clean up
		if [ -f $(call resolve_patch_abspath,$*)/series ]; then pushd $($*_SRC_PATH) && quilt pop -a -f; [ -d .pc ] && rm -rf .pc; popd; fi
...
$(addprefix $(PYTHON_WHEELS_PATH)/, $(SONIC_PYTHON_WHEELS)) : $(PYTHON_WHEELS_PATH)/% : .platform $$(addsuffix -install,$$(addprefix $(PYTHON_WHEELS_PATH)/,$$($$*_DEPENDS))) $$(addprefix $(PHONY_PATH)/,$$($$*_PHONIES)) \
	...
	if [ -z '$($*_CACHE_LOADED)' ] ; then
		...
		# apply series of patches if exist. See resolve_patch_path (rules/functions) for
		# the default "<SRC_PATH>.patch" layout vs. an explicit $($*_PATCH_PATH) override.
		if [ -f $(call resolve_patch_abspath,$*)/series ]; then ( quilt pop -a -f 1>/dev/null 2>&1 || true ) && QUILT_PATCHES=$(call resolve_patch_abspath,$*) quilt push -a; fi $(LOG)
ifneq ($(CROSS_BUILD_ENVIRON),y)
...
ifneq ($(filter bookworm trixie,$(BLDENV)),)
		...
		pushd $($*_SRC_PATH) $(LOG_SIMPLE)
		# clean up
		if [ -f $(call resolve_patch_abspath,$*)/series ]; then quilt pop -a -f; [ -d .pc ] && rm -rf .pc; fi $(LOG)
		popd $(LOG_SIMPLE)
