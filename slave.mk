...
$(addprefix $(TARGET_PATH)/, $(SONIC_RFS_TARGETS)) : $(TARGET_PATH)/% : \
	...
	if [ -z '$($*_CACHE_LOADED)' ] ; then
		...
		MASTER_KUBERNETES_CNI_VERSION=$(MASTER_KUBERNETES_CNI_VERSION) \
		MASTER_KUBERNETES_CRI_TOOLS_VERSION=$(MASTER_KUBERNETES_CRI_TOOLS_VERSION) \
		MASTER_KUBERNETES_PACKAGE_URL=$(MASTER_KUBERNETES_PACKAGE_URL) \
		MASTER_CRI_DOCKERD=$(MASTER_CRI_DOCKERD) \
		MASTER_CRI_DOCKERD_DISTRO=$(MASTER_CRI_DOCKERD_DISTRO) \
		ENABLE_SBOM="$(ENABLE_SBOM)" \
			./build_debian.sh $(LOG)
