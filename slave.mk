...
$(foreach DOCKER_IMAGE,$(SONIC_TRIXIE_DOCKERS), $(eval $(DOCKER_IMAGE)_DEBS_PATH := $(TRIXIE_DEBS_PATH)))
$(foreach DOCKER_IMAGE,$(SONIC_TRIXIE_DOCKERS), $(eval $(DOCKER_IMAGE)_FILES_PATH := $(TRIXIE_FILES_PATH)))
$(foreach DOCKER_IMAGE,$(SONIC_TRIXIE_DBG_DOCKERS), $(eval $(DOCKER_IMAGE)_DEBS_PATH := $(TRIXIE_DEBS_PATH)))
$(foreach DOCKER_IMAGE,$(SONIC_TRIXIE_DBG_DOCKERS), $(eval $(DOCKER_IMAGE)_FILES_PATH := $(TRIXIE_FILES_PATH)))
...
ifeq ($(BLDENV),trixie)
	DOCKER_IMAGES := $(SONIC_TRIXIE_DOCKERS)
	DOCKER_DBG_IMAGES := $(SONIC_TRIXIE_DBG_DOCKERS)
	TRIXIE_DOCKER_IMAGES = $(filter $(SONIC_TRIXIE_DOCKERS),$(DOCKER_IMAGES_FOR_INSTALLERS) $(EXTRA_DOCKER_TARGETS) $(SONIC_PACKAGES_LOCAL))
	TRIXIE_DBG_DOCKER_IMAGES = $(filter $(SONIC_TRIXIE_DBG_DOCKERS),$(DOCKER_IMAGES_FOR_INSTALLERS) $(EXTRA_DOCKER_TARGETS) $(SONIC_PACKAGES_LOCAL))
else
	DOCKER_IMAGES = $(filter-out $(SONIC_JESSIE_DOCKERS) $(SONIC_STRETCH_DOCKERS) $(SONIC_BUSTER_DOCKERS) $(SONIC_BULLSEYE_DOCKERS) $(SONIC_BOOKWORM_DOCKERS) $(SONIC_TRIXIE_DOCKERS),$(SONIC_DOCKER_IMAGES))
	DOCKER_DBG_IMAGES = $(filter-out $(SONIC_JESSIE_DBG_DOCKERS) $(SONIC_STRETCH_DBG_DOCKERS) $(SONIC_BUSTER_DBG_DOCKERS) $(SONIC_BULLSEYE_DBG_DOCKERS) $(SONIC_BOOKWORM_DBG_DOCKERS) $(SONIC_TRIXIE_DBG_DOCKERS), $(SONIC_DOCKER_DBG_IMAGES))
endif
endif
endif
endif
...
trixie : $$(addprefix $(TARGET_PATH)/,$$(TRIXIE_DOCKER_IMAGES)) \
          $$(addprefix $(TARGET_PATH)/,$$(TRIXIE_DBG_DOCKER_IMAGES))
