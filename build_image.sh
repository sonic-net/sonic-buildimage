...
generate_onie_installer_image()
{
    ...
    local efi_part_size="${EFI_PART_SIZE}"
    ...
    declare -a onie_mk_demo_args=(
        ...
        "$platform_arg"                    #  3
        "$installer_dir"                   #  4
        "$platform_conf_file"              #  5
        "$output_file"                     #  6
        "$demo_type_arg"                   #  7
        "$IMAGE_VERSION"                   #  8
        "$efi_part_size"                   #  9
        "$part_size"                       # 10
        "$INSTALLER_PAYLOAD"               # 11
        "$TARGET_BOOTLOADER"               # 12
        "$SECURE_UPGRADE_SIGNING_CERT"     # 13
        "$SECURE_UPGRADE_DEV_SIGNING_KEY"  # 14
        "$ONIE_INSTALLER_PAYLOAD"          # 15
    )
...
generate_device_list()
{
    ...
    # Add kvm to the list
    if [ "$TARGET_MACHINE" = "alpinevs" ] || [ "$TARGET_MACHINE" = "vs" ]; then
      echo "x86_64-kvm_x86_64-r0" >> "$platforms_asic"
    fi

    # Explicitly add vs if Target Machine is vs
    if [ "$TARGET_MACHINE" = "vs" ]; then
      echo "vs" >> "$platforms_asic"
      echo "x86_64-vs-amd64-r0" >> "$platforms_asic"
    fi
}
...
build_onie_image() {
    ...
    if [ -n "$ONIE_RECOVERY_IMAGE" ] && [ -f "files/$ONIE_RECOVERY_IMAGE" ]; then
        echo "Creating SONiC KVM image"
        KVM_IMAGE_DISK=${OUTPUT_KVM_IMAGE%.gz}
        sudo rm -f "$KVM_IMAGE_DISK" "$KVM_IMAGE_DISK.gz"

        SONIC_USERNAME=$USERNAME PASSWD=$PASSWORD sudo -E ./scripts/build_kvm_image.sh "$KVM_IMAGE_DISK" "files/$ONIE_RECOVERY_IMAGE" "$OUTPUT_ONIE_IMAGE" "$KVM_IMAGE_DISK_SIZE" || {
            echo "Error : build kvm image failed"
            exit 1
        }
        [ -r "$KVM_IMAGE_DISK" ] || {
        ...
        echo "Skipping SONiC KVM image creation (no valid recovery ISO provided for $TARGET_MACHINE)"
    fi
}
...
build_recovery_image() {
    ...
    # Extract kernel if vmlinuz is missing (likely due to exclusion in squashfs)
    if ! sudo find "${FILESYSTEM_ROOT}/boot" -name "vmlinuz-*" -print -quit | grep -q .; then
        echo "Extracting kernel from ${linux_kernel} to ${FILESYSTEM_ROOT}..."
        sudo dpkg-deb -x "${linux_kernel}" "${FILESYSTEM_ROOT}"
    fi
...
build_sonie_image() {
     ...
     if [ -n "$PAYLOAD_UKI" ] && [ -n "$FILESYSTEM_ROOT" ]; then
         ...
         # Handle /grub content
         echo "Adding requisite /grub content to payload..."
