...
main() {
  ...
  local efi_part_size="$9"
  local xbootldr_part_size="${10}"
  local onie_installer_payload="${11}"
  local target_bootloader="${12}"
  local acpi_conf="${13:-}"
  ...
  # Exclude tests/ directory and leftover artifacts from the payload
  rm -rf "${tmp_installdir}/tests"
  find "${tmp_installdir}" -name "*.orig" -delete
  find "${tmp_installdir}" -name "*.zip" -delete
  find "${tmp_installdir}" -name "tmp.*" -delete
  ...
  sed -i -e "s/%%NOS_TYPE%%/${nos_type}/g" \
         ...
         -e "s/%%EFI_PART_SIZE%%/${efi_part_size}/" \
         -e "s/%%TARGET_BOOTLOADER%%/${target_bootloader}/" \
         -e "s/%%XBOOTLDR_PART_SIZE%%/${xbootldr_part_size}/" \
         -e "s/%%EXTRA_CMDLINE_LINUX%%/${extra_cmdline}/" \
         -e "s/%%CONFIGURED_PLATFORM%%/${platform}/g" \
         -e "s@%%OUTPUT_RAW_IMAGE%%@${output_raw_image}@" \
    "${tmp_installdir}/install.sh"

  if grep -q "%%IMAGE_VERSION%%" "${tmp_installdir}/install.sh"; then
      echo "Error: Failed to replace %%IMAGE_VERSION%% in install.sh during SONIE image creation." >&2
      exit 1
  fi
