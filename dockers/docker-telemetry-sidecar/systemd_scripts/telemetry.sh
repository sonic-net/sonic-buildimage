...
start() {
    ...
    docker create -t -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro \
        ...
        --name=$K8S_DOCKERNAME \
        --label raw_container_name=telemetry \
        ksdatatest.azurecr.io/sonic/docker-sonic-telemetry:20250910 \
        || {
    ...
    preStartAction
    /usr/local/bin/container start $K8S_DOCKERNAME
    postStartAction
}

wait() {
    /usr/local/bin/container wait $K8S_DOCKERNAME
}

stop() {
    container_id=$(docker ps --filter "label=raw_container_name=telemetry" --format '{{.ID}}' --quiet)

    if [ -z "$container_id" ]; then
        echo "container stop - No such container with label raw_container_name=telemetry"
    else
...
kill() {
    /usr/local/bin/container kill $K8S_DOCKERNAME
}

DOCKERNAME=telemetry
OP=$1
