...
def file_bytes_local(path: str) -> Optional[bytes]:
    try:
...
def sync_items(items: List[SyncItem]) -> bool:
    all_ok = True
    for item in items:
        src_bytes = file_bytes_local(item.src_in_container)
        if src_bytes is None:
            logger.log_error(f"Cannot read {item.src_in_container} in this container")
            all_ok = False
            continue

        cont_sha = sha256_bytes(src_bytes)
        ...
        if new_sha != cont_sha:
            logger.log_error(
                f"Post-copy SHA mismatch for {item.dst_on_host}: host {new_sha or 'read-failed'} vs container {cont_sha}"
            )
            all_ok = False
        else:
