...
generate_disk() {
  if [[ "${INSTALL_DISK:-0}" -eq 1 ]]; then
    ...
    mmd -i "${esp_part_image}" ::/EFI ::/EFI/BOOT

    # Locate Grub
    local platform
    platform=$([[ -f .platform ]] && cat .platform || echo vs)
    local arch
    arch=$([[ -f .arch ]] && cat .arch || echo amd64)
    local grub_efi_file="fsroot-${platform}-${arch}-recovery/usr/lib/grub/x86_64-efi/monolithic/grubx64.efi"

    if [[ -n "${GRUB_FILE:-}" && -f "${GRUB_FILE}" ]]; then
      echo "Using GRUB_FILE from environment: ${GRUB_FILE}"
      grub_efi_file="${GRUB_FILE}"
    fi

    if [[ "${DIRECT_BOOT}" -eq 1 ]]; then
      echo "Direct Boot enabled: Skipping GRUB installation."
    elif [[ -f "${grub_efi_file}" ]]; then
      echo "Using Grub: ${grub_efi_file}"
      mcopy -i "${esp_part_image}" "${grub_efi_file}" ::/EFI/BOOT/BOOTX64.EFI
    else
      echo "Warning: Grub not found at ${grub_efi_file} (tried path and GRUB_FILE), but Direct Boot not requested."
    fi
    ...
    local mcopy_target="::/EFI/BOOT/linux.efi"
    if [[ "${DIRECT_BOOT}" -eq 1 ]]; then
        mcopy_target="::/EFI/BOOT/BOOTX64.EFI"
    fi
    ...
    # Create Grub Config
    # Add serial console for debugging (if monolithic grub supports it)
    # Create Grub Config only if NOT direct boot
    if [[ "${DIRECT_BOOT}" -eq 0 ]]; then
        if [[ -f "${SCRIPT_DIR}/grub.cfg" ]]; then
            echo "Using existing grub.cfg from ${SCRIPT_DIR}/grub.cfg"
            cp "${SCRIPT_DIR}/grub.cfg" "${grub_config_file}"
        else
            echo "Generating default grub.cfg..."
...
set timeout=10
set default=0
set debug=fs,file,image,relocator,efi,chain,loader,mmap
set pager=0
EOF

            if [ "${ENABLE_RECOVERY_MENU:-0}" -eq 1 ]; then
                if [[ -n "${RECOVERY_ADDON:-}" && -f "${RECOVERY_ADDON}" ]]; then
                    echo "Injecting PE Addon for recovery mode into ESP..."
                    mmd -i "${esp_part_image}" ::/EFI/BOOT/linux.efi.extra.d 2>/dev/null || true
                    mcopy -i "${esp_part_image}" "${RECOVERY_ADDON}" ::/EFI/BOOT/linux.efi.extra.d/recovery.addon.efi
                fi
                cat <<EOF >> "${grub_config_file}"
menuentry "SONIE Recovery" {
    search --file --set=root /EFI/BOOT/linux.efi
    chainloader /EFI/BOOT/linux.efi boot_reason=recovery
}
EOF
            fi
...
EOF
        ...
        # Monolithic grub expects config at /EFI/SONIE-OS/grub.cfg (see grub-standalone.cfg)
        mmd -i "${esp_part_image}" ::/EFI/SONIE-OS
        mcopy -i "${esp_part_image}" "${grub_config_file}" ::/EFI/SONIE-OS/grub.cfg
        # Also keep it at standard location just in case
...
launch_qemu() {
  ...
  local qemu_args=(
    -nographic -m "${QEMU_MEMORY:-16G}" "${cpu_opts[@]}"
    ...
    -k en-us
    -debugcon file:${LOG_DIR}/ovmf_debug.log -global isa-debugcon.iobase=0x402
  )

  if [[ -n "${KERNEL_CMDLINE:-}" ]]; then
      qemu_args+=(-smbios "type=11,value=io.systemd.stub.kernel-cmdline.extra=${KERNEL_CMDLINE}")
  fi
