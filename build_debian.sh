...
sudo cp files/initramfs-tools/zstd $FILESYSTEM_ROOT/etc/initramfs-tools/hooks/zstd
sudo chmod +x $FILESYSTEM_ROOT/etc/initramfs-tools/hooks/zstd
...
pushd $FILESYSTEM_ROOT && sudo tar -I zstd -cf $OLDPWD/$FILESYSTEM_DOCKERFS -C ${DOCKERFS_PATH}var/lib/docker .; popd
